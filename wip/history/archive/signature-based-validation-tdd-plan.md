...
> **Superseded as the live tracker.** The current-state reference and reviewer verification map now live at [`../signature-based-validation/signature-based-validation.md`](../signature-based-validation/signature-based-validation.md). This file is kept as the phase-by-phase TDD build record (historical); where it disagrees with the live tracker on present behavior, the live tracker wins. Internal references to "TODOS.md" below mean *this* file under its original name.
...
Current state: A `PipeSignature` can be authored as a spec (`pipelex/builder/pipe/pipe_signature_spec.py`), compiled to a blueprint (`pipelex/pipe_signature/pipe_signature_blueprint.py`), instantiated as a runtime (`pipelex/pipe_signature/pipe_signature.py`) via the registered `PipeSignatureFactory`, dry-run (yields a mock `Stuff` from the declared output `StuffSpec`), and rejected on live-run (`PipeSignatureNotExecutableError`). Signatures slot into `PipeBlueprintUnion` and `PipeSpecUnion`. Strict-mode gating is live: `collect_signature_refs(pipe=...)` walks the dependency graph (see the post-archive update note below — it is a free function in `pipelex/pipe_signature/signature_walk.py`, not a `PipeAbstract` method); `dry_run_pipe(..., allow_signatures=False)` (the default) raises `SignaturesNotAllowedError` carrying both `signature_refs` and the qualified `dep_paths` chain. `dry_run_pipes` does a pre-pass that aggregates signature refs across all pipes in the batch so the user sees every offender in one error (post-review fix). `validate_bundle` and `validate_bundles_from_directory` catch the error and surface it via `ValidateBundleError.signature_check_error`. CLI surface: `pipelex validate pipe`/`bundle` now take `--allow-signatures` (default off); `pipelex validate --all` filters signature pipes out of the strict iteration; the agent CLI's `validate_*_core` functions default to strict as well (same as the main CLI — an earlier draft of this plan said lenient; the shipped default is strict, pinned by `test_agent_validate_defaults_strict.py`). Signatures still raise `PipeSignatureNotExecutableError` on live run. End-to-end `.mthds` parsing works: the interpreter happily round-trips `type = "PipeSignature"` through `PipelexBundleBlueprint` → library → runtime → dry-run, in both strict and lenient mode. The schema generator was updated to strip `pipe_category` from `PipeSignatureBlueprint`, but the JSON Schema file lives in `vscode-pipelex/` (not this repo) so no schema artifact needed regeneration here. Surprise: `pipe_abstract.py` cannot import from `pipelex.hub` — the cycle pipe_abstract → hub → libraries.library → libraries.pipe.pipe_library → pipe_abstract is real and breaks at runtime, not just under pyright's `reportImportCycles`. The dependency-graph walk therefore lives as free functions in `pipelex/pipe_signature/signature_walk.py` (a module downstream of `hub`), not as methods on `PipeAbstract` — see the post-archive update note below. The walk iterates `sorted(pipe_dependencies())` so the dep chain rendered in `SignaturesNotAllowedError` is deterministic (post-review fix). Phase 6 surfaced one design gap: `dry_run_pipes` aggregated dep paths with a "first wins" rule, which lost the informative controller chain when the signature pipe was iterated first; the aggregator now prefers the longest known dep chain so the error message always shows the most useful path.

**Post-archive update (2026-05-18) — signature graph walk moved off `PipeAbstract`.** After this plan was archived, the `pipe_lookup`-callable workaround was removed. `collect_signature_refs` / `collect_signature_paths` are no longer methods on `PipeAbstract` taking a `pipe_lookup` parameter — they are now free functions in a new module `pipelex/pipe_signature/signature_walk.py`, which sits downstream of `pipelex.hub` and imports `get_optional_pipe` directly. `PipeAbstract` keeps only `pipe_dependencies()` and `is_signature`. Call sites read `collect_signature_refs(pipe=...)` / `collect_signature_paths(pipe=...)`. The recursion, visited-set cycle protection, sorted iteration, and longest-dep-chain aggregation are unchanged. The phase-by-phase notes below (notably the Phase 4.2 deviations) predate this refactor and are kept verbatim as the historical implementation record — wherever they say `PipeAbstract.collect_signature_refs(pipe_lookup=get_optional_pipe)`, read `signature_walk.collect_signature_refs(pipe=...)`. The refactor landed in commit `66a584dc`.
...
5. `pipelex/core/pipes/pipe_abstract.py` — `PipeAbstract` (gains `is_signature`, promoted `pipe_dependencies`).
...
signature_walk.py:   collect_signature_refs() / collect_signature_paths() — free-function graph walk
                     (see post-archive note: originally PipeAbstract methods)

Validation:
  strict (default):   dry_run_pipe(... allow_signatures=False) → SignaturesNotAllowedError
                      if collect_signature_refs(pipe) is non-empty
...
- `pipelex/core/pipes/pipe_abstract.py` — `is_signature`, promoted `pipe_dependencies`.
- `pipelex/pipe_signature/signature_walk.py` — free-function dependency-graph walk: `collect_signature_refs` / `collect_signature_paths` (see post-archive update note).
