...
- [workflow-nondeterminism-audit.md](workflow-nondeterminism-audit.md) — **OPEN diagnosis:** verified audit of worker-local inputs leaking into the workflow command stream (post-PR-#984). Two HIGH findings can still reproduce the TMPRL1100 silent hang (flush activity gated on an activity-fed buffer; library/fingerprint leak on eviction); plus config-derived dispatch options and inline randomness drifting command arguments. Carries the suggested fix order.
