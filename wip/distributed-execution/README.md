...
- [nondeterminism-follow-ups-decisions-needed.md](nondeterminism-follow-ups-decisions-needed.md) — **DECISION NEEDED:** the one surfaced judgment call from that burn-down — the graph-event path has no H1-style guard against in-activity tracer mutation (latent, silent-data-corruption class). Options + recommendation inside; ties into T3 (request-scoped tracing state).
