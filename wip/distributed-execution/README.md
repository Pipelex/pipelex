...
- [nondeterminism-fix-review-follow-ups.md](nondeterminism-fix-review-follow-ups.md) — **OPEN follow-ups:** verified findings from the code reviews of the H1/M1 and H2 fixes. **PRIORITY item 0:** key per-run worker-local state by `run_id` not `workflow_id` — the H2 fix's now-reliable eviction cleanup can destroy a live successor run's library under a reused workflow id (retry/reset/resubmission). Plus: a cancellation test pinning the finally-ordering invariant, stale runner-fallback warning, `set_event_log` overwrite documentation, test-helper dedup, `finally` simplification, scoped-library helper consolidation, and a deferred design note on the unguarded graph-event path.
