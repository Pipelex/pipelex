...
- [validate-sweep-temporal-leak-repro.md](validate-sweep-temporal-leak-repro.md) — **bug brief + e2e repro:** the `/validate` dry-run sweep leaked nested controller sub-pipes to Temporal (HTTP 422 on a standalone `PipeBatch`). Fixed (commit `3377babb`) + in-process regression test; this doc is the not-yet-added deployment-faithful RED scenario for the `temporal-e2e-validate` skill (uses the new `pipelex validate bundle --temporal` flag).
