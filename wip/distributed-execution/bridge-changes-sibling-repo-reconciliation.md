...
**The break is deferred, not live.** Both consumers are pinned to / based on a *pre-rename* pipelex, so nothing is broken at this instant. The break materializes the moment either consumer is moved onto post-#966 pipelex. (Item 7 below — added 2026-06-11 — has a different trigger: it's a behavioral break from PR #987's reporting-path change, and it lands via the package's `pipelex>=0.27.0` floor on the next pipelex release containing #987, no rev bump required.) The fix is mechanical: the Mistral-native graph/usage path is a mirror of the Temporal path, and it needs the **exact same** `act_assemble_graph → act_assemble_tracing` rewiring the Temporal path already received in #967 — plus it gains cost reporting it never had.
...
### Behavioral break added after this diagnosis (PR #987, 2026-06-11): activity-side usage emissions bypass the plugin's registered buffer

7. **`primitives/wf_pipe_router.py` (usage-buffer contract) + deployment config.** PR #987 (`fix/Config`, the workflow-determinism fixes) added an H1 routing gate to `pipelex/reporting/reporting_manager.py::_emit_usage_event` (~line 210): any emission made **inside a temporalio activity** unconditionally skips the registered-context fast path and goes to the per-process activity event log (`act_{pid}_{uuid8}` writer). Mistral Workflows **is temporalio underneath** — its `@activity` decorator wraps `temporalio.activity.defn` (`mistralai/workflows/core/activity.py:181/:187`) — so every usage report from a plugin leaf activity (`act_pipelex_leaf.py`) has `in_activity() == True`. Consequence for the plugin's pre-H1 co-located contract (`wf_pipe_router.py:175-180` registers a `BufferingEventLog` via `set_event_log`; `:208-212` flushes gated on `if buffered_events:`): the buffer **silently stops receiving usage events**. Not a compile break, and not dead code — graph events are tracer-emitted from the workflow body and still land in the buffer — but the flush stops carrying usage. In default configs this is a **diversion, not a loss**: the fallback stamps `workflow_id = tracer_key or graph_id` (same partition) and writes through the backend built from `tracing_config`, which the multi-writer assembly tolerates. It degrades to **actual loss** when the activity-side process's `tracing_config` is disabled or unbuildable (the fallback bare-returns). The diversion incidentally gives the plugin the same H1 replay-determinism protection pipelex's own `WfPipeRouter` adopted — but the plugin never opted in, and its writer_id/sequence expectations change. Trigger condition: any pipelex containing #987 — the package's `pipelex>=0.27.0` floor means a fresh non-editable install picks this up the moment such a release ships, independent of the #966 rename breaks above. Fix during step 3: mirror post-#987 pipelex `wf_pipe_router.py` — drop the usage half of the buffer contract (usage rides the per-process activity log; make the flush schedule payload-pure) or consciously keep the buffer for graph events only and document the usage diversion; either way, ensure the deployment enables `tracing_config` where the activities run. Cross-ref: `nondeterminism-follow-ups-decisions-needed.md` §6 (the generic mechanism — live here, not latent).
...
- In `wf_pipe_router.py`: resolve item 7 (post-#987 usage routing) — mirror pipelex's post-#987 `pipelex/temporal/tprl_pipe/wf_pipe_router.py` (payload-pure flush gate; usage events ride the per-process activity log instead of the registered buffer), or keep the buffer for graph events only and document the usage diversion.
