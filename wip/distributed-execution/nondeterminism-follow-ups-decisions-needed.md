...
Two latent edges of the H1 routing check: (a) a host app running a DIRECT pipelex pipeline inside its **own** temporalio activity (legit runtime-bridge embedding) gets its registered direct-mode context bypassed — emissions divert to the per-process fallback with a restamped `workflow_id`, ignoring a custom event-log backend; (b) an emission from a thread spawned inside an activity without contextvar propagation (the `run_in_executor(None, ...)` pattern that exists in `bedrock_client_boto3.py`) would report `in_activity()=False` and resurrect the H1 cross-thread buffer write. All current `report_inference_job` call sites run in the activity's own context, so both are latent **within this repo** — same class as item 1; fold into the same T3-adjacent decision. **Update 2026-06-11:** edge (a) is NOT latent for `pipelex-mistralai-workflows` — Mistral Workflows wraps temporalio activities, so the plugin's registered `BufferingEventLog` usage path is bypassed by this gate on any pipelex release containing PR #987. Recorded as breakage item 7 in [bridge-changes-sibling-repo-reconciliation.md](bridge-changes-sibling-repo-reconciliation.md); fix it there, not here.

## 7. `PipelineManager` keying/lifecycle and the `/pipeline/start` submitter-state gap

Deferred from the pre-merge resubmission fix ([nondeterminism-pre-merge-fixes.md](nondeterminism-pre-merge-fixes.md), fix 1). What landed: a tolerant `remove_pipeline` called from `execute_pipeline`'s `finally` and from `pipeline_run_setup`'s own failure path, plus a 409 mapping in `pipelex-api`. What was deliberately NOT decided:

- **Should `PipelineManager` adopt run-unique keying / self-heal like the three patched siblings (library, tracer, event-log context)?** Today its collision raise is load-bearing: it fires at the top of `pipeline_run_setup`, before `open_tracer`, and is the only thing preventing two concurrent same-id direct-mode runs from cross-popping each other's LIVE tracer via `open_tracer`'s stale-key pop-and-replace healing (the tracer key in direct mode is the caller-suppliable `pipeline_run_id`, not a run-unique Temporal run id). Any move to self-heal/overwrite must simultaneously gate that healing to run-scoped keys or make direct-mode tracer keys run-unique. Note the registry is otherwise write-only — `get_pipeline`/`get_optional_pipeline` have no production callers — so the long-term question is really "what is this registry for".
- **The fire-and-forget `/pipeline/start` path has no submitter-side terminal hook.** `ApiRunner.start_pipeline` (pipelex-api) calls `pipeline_run_setup` and hands off to Temporal; nothing in the submitter process runs when the workflow later completes, so the manager entry (and any submitter-side state keyed by the run id) stays registered for the process lifetime. Consequence after the pre-merge fix: serial resubmission of a completed run's id works on the `execute` path, but on the `start` path it still collides — now a clean 409 instead of a 500, but still process-permanent for that id. Deciding where the start path's terminal cleanup belongs (post-handoff removal? completion webhook? TTL sweep alongside item 4's leak mitigation?) interacts with the tracer-shield coupling above and with what submitter-side state the start path actually needs once the workflow owns delivery.
