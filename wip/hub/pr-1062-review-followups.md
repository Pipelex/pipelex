...
**Status: [Phase A](#phase-a--in-pr-fixes) is APPLIED** (A1's doc-honesty fix + A2–A8), recorded at [Checkpoint A](../../TODOS.md#checkpoint-a-record--pr-1062-review-follow-ups) in the tracker. [Phase B](#phase-b--test-hardening), [Phase C](#phase-c--release-wave-additions) and the [F1](#f1--the-layer-rule-is-only-enforced-one-hop-deep) remedy (the A1-PR) are **not** started. PR [#1062](https://github.com/Pipelex/pipelex/pull/1062) is open against `dev`; it was green at `aef0da638` before Phase A. This document is the executable follow-up plan from a full `/review` pass, written so it can be picked up cold in a new session.
...
3. [Phase A](#phase-a--in-pr-fixes) — the code changes, in dependency order. A1 is now split per D-R4: only its doc-honesty fix stays in #1062; the remedy itself is its own follow-up PR. Everything else in the phase is free to proceed.
...
- [ ] **B2 — give the closure detector a negative control.** `_CLOSURE_SCRIPT` in `test_runtime_layer_import_closure.py` is a `textwrap.dedent` string, so no linter sees `INTERPRETER_PACKAGES` / `is_interpreter`; a typo makes all entry points pass vacuously, forever. The detector does work today (`pipelex.interpreter_hub` → exit 1, 51 modules; `pipelex.pipe_operators.func.pipe_func` → exit 1, 54), but nothing asserts that. Parametrize `(entry_point, expect_offender)` and include a known-dirty entry point asserting `returncode == 1` (e.g. `pipelex.interpreter_hub` itself — dirty by definition, stable across the D-R4 remedy). Per [D-R4](#decisions-taken) no new green entry points land in #1062, so this item stands alone here; the `pipelex.plugins` entry point arrives with the A1-PR.
...
> **CHECKPOINT B** — run `make agent-check` and the full `make agent-test`. Nothing here re-opens a drift contract, so this is a plain commit. B2's negative control lands on a green tree and simply pins the detector; per [D-R4](#decisions-taken) the property-side fix is in the A1-PR, not here.
