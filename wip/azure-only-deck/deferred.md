...
## An offline boot can fail on a remote-config cache that predates the 2026-09-20 publish

Raised by the Codex adversarial reviewer in round 3 and confirmed by the round's verifier. The cache at `~/.pipelex/cache/remote_config.json` carries `CACHE_SCHEMA_VERSION = 1` (`pipelex/system/pipelex_service/remote_config_cache.py:31`) with no TTL and no keying on the remote-config URL version, so a cache primed before the 2026-09-20 publish stays valid and is served verbatim. **This widened on 2026-09-21**: the follow-up deck change put every tier, the `for_text` and `for_object` choice defaults included, on handles that publish first served, so such a cache now fails the boot outright rather than only on the premium path. `pipelex update` copies clean-behind deck files with `shutil.copy2` (`pipelex/cli/commands/update_cmd.py:156-159`) and never touches the cache. When the next boot cannot reach the network, `fetch_remote_config` falls back to that cache and returns `source=CACHED` rather than `None`, so `ModelManager._enforce_gateway_model_membership` runs and raises `GatewayUnknownModelError` at `model_manager.py:186` on the promoted handle.
...
## Naming a premium alias directly in a pipe warns on every call — resolved at the deck layer
...
**Resolved, for this deck**: the follow-up deck change set `[llm.choice_defaults].default_temperature` to 1, so the alias, waterfall and bare-handle paths all pair their target with the value every model in the ladder fixes, and none of them warns. That is a deck fix rather than the resolution-layer one argued for here, and it holds only while every handle the deck names shares one fixed temperature — a deck mixing constrained and unconstrained models would bring the warning straight back, on whichever side of 1 the default falls. **Still open at the resolution layer**: `get_llm_setting` pairs a target with a deck-wide constant and never reads the target's own `valued_constraints` (`pipelex/cogt/models/model_deck.py:325, :337, :350`), and the warning is still emitted per call with no memoisation. Whoever owns that path decides whether to resolve the constraint at deck-build time or dedupe the warning per model.
...
## Whether large-codebase analysis belongs on the premium tier — resolved
