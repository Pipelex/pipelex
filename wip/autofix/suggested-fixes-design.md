...
### Step-2 exit — abstraction verdict (CHECKPOINT 1)

This is the gate the human-CLI step (master-plan step 5) reads. It consolidates the per-phase verdicts above into one statement.

**Verdict: `SuggestedFix`/`FixOp` survived every wave-1 fix shape with no structural change.** Step 2 deliberately added rules of four different *shapes* to stress the wire model before any human sees it; each rode the existing abstraction:

- **Multi-op, in-place table sync** (`sync-controller-inputs`, Phase A) — the first fix emitting more than one op (a `set_key` per added/changed variable + a `delete_key` per extraneous one). Rode `FixOp` unchanged. The one wire-level change in the *entire* step happened here and is a *value-type* widening, not a shape change: `TomlValue` (the type of `FixOp.value`) went from `TomlScalar` to `TomlScalar | dict[str, TomlScalar]`, so a single op can carry a whole inline-table mapping for the no-`inputs`-table case. `FixOp`'s fields, `SuggestedFix`'s fields, and the `table_path: list[str]` addressing were untouched.
- **Delete-shaped, on a new channel** (`strip-native-concept-redecl`, Phase B) — the first blueprint-channel fix and the first with a populated `SuggestedFix.source`. Rode the abstraction unchanged; the only new wiring was one planner function plus one call in the blueprint loop of `build_validation_error_items`. `DELETE_KEY` on `["concept"]` covered every authoring form with no new op kind.
- **Rename-shaped, position-preserving** (`strip-namespace`, Phase C) — exercised the last unexercised op kind, `RENAME_TABLE_KEY` (on the enum since the spike). It gained its applier implementation + `new_key` joined the loop fingerprint; no wire-model change. **The array-of-tables addressing extension the plan feared was never needed:** same-domain qualified refs (`steps[].pipe = "domain.code"`) keep resolving, and only declaration keys + `main_pipe` enforce snake_case, so the fix touches only those two sites — no `table_path` index segments, no new op kind.

So the human-CLI gate (master-plan step 5, bar 1 — "the abstraction has been stress-tested by rules of *different shapes* … so the wire shape and the `description` wording are known-stable before humans see them") **is met.** The wire shape is frozen for wave 1 modulo the additive `TomlValue` widening above; `description` stays a one-line human/agent string across all four rules.

**Warts carried forward** (not abstraction failures — honest caveats for the next steps):

- **`RENAME_TABLE_KEY` relies on tomlkit's private `Container._replace`** (the public API only offers `del`+re-add, which appends to the bottom and reintroduces the old branch's reordering bug). Mitigated by the golden byte-compare test as a CI tripwire on a tomlkit bump; pin is `tomlkit>=0.13.2`.
- **Canonical whole-file output** (Phase A′): the applier no longer preserves untouched bytes surgically — `serialize_and_format` reflows the whole file to canonical MTHDS style via `pipelex_tools.format_mthds`. Correct for a fix tool, a no-op on already-formatted files. The `pipelex_tools` stub/runtime `__all__` drift is worked around with a `TYPE_CHECKING` guard (upstream fix pending — [deferred-checkpoint-a-prime-review-items.md](deferred-checkpoint-a-prime-review-items.md) item 1).
- **Comment-reflow on `DELETE_KEY`**: a standalone comment sitting on the *deleted* element's leading line reflows onto the successor (tomlkit trivia). Comments on untouched content are preserved. Acceptable for author-reviewed, CI-reformatted output.
- **Loop maturity**: the source/file guard is exercised live (Phase B), but real multi-file targeting (populate `source` at every raise site; derive `is_single_file` from resolved dirs) is still step 3 (deferred items 0, 1). Two Phase-C behaviors — a one-rename-per-iteration convergence cap and the `main_pipe`-to-nonexistent-pipe strip — are deferred design tradeoffs, not bugs ([deferred-checkpoint-c-review-items.md](deferred-checkpoint-c-review-items.md)).

**strip-namespace decision: GO — shipped.** The stretch rule landed and `RENAME_TABLE_KEY` is proven end-to-end; recorded as the step-2 exit criterion in the master plan.

**Step-2 final review (2026-07-08).** An unanchored `/code-review` sub-agent (no plan/design context, diff pointer `5e62bd43c..HEAD` only) swept the whole step-2 diff and surfaced **one confirmed correctness bug**, fixed immediately (silent data loss, not a design tradeoff — so fixed, not deferred): `_rename_key_in_place` renamed only the **first** out-of-order sub-table holding the dotted key. A pipe whose header `[pipe."d.x"]` and whose block `[pipe."d.x".inputs]` straddle an intervening `[concept]` section carries the same dotted key `d.x` in *two* of the `OutOfOrderTableProxy`'s sub-tables, so the rename orphaned the pipe's `.inputs` under a phantom, still-invalid dotted key **while reporting the op APPLIED** — the file was mutated and left unfixable (the bare code now collides, so a re-strip is refused). Fix: rename in *every* chunk holding the key (the `DELETE_KEY` path was already safe — it uses tomlkit's public `__delitem__`, which removes from all chunks). Pinned by `test_rename_carries_nested_subtable_across_interleaved_chunks` (offending pipe with its own nested block sub-table split across chunks; the pre-existing interleaved fixtures never gave the offending pipe a nested sub-table, so this shape was uncovered). Everything else the review checked (categorizer `main_pipe` gating, the `StuffSpec.to_bundle_representation` refactor, `_expected_inputs_for_fix` flexible-type carve-out, the cross-file collision guard, enum match/case exhaustiveness, `_as_tomlkit_value`) it verified sound or already-documented. `make agent-check` + full `make agent-test` green after the fix.
