...
- **Applier finding — inline-table canonicalization**: tomlkit's incremental edits inside inline tables leave non-canonical whitespace (`{ a = "x",  b = "y"}`), which `plxt format` would churn and goldens can't pin. The applier now re-emits a *mutated* inline table with canonical `{ key = value, ... }` spacing, transplanting the line's trailing comment/indent via trivia (safe: TOML forbids comments inside inline tables). Block tables remain strictly in-place. This keeps fix output stable under the formatter — goldens are the plxt-canonical bytes.
