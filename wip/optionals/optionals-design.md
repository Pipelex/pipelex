...
New error types flow automatically through the existing machinery (`build_validation_error_items` → agent CLI JSON/markdown, API 422, our cross-repo spec suite) — the plumbing is already unified; we add enum values, fixtures, and spec-suite rows, not new channels.
...
| Protocol spec + cross-repo spec suite | `PipeIOContract.optional` in the protocol spec, new validation-error categories in the locked contract, valid/invalid fixtures (`optional_not_handled.mthds` etc.), error-QA corpus entries | M |
