...
- **Strict header-name and header-value validation** (raised by greptile on PR #1109 against `is_header_shaped`). A quoted TOML key with an illegal field-name character — `"x-foo bar" = "v"` — passes the shape rule, and so does a value carrying a CR/LF; both reach the provider factory and fail on the first inference call rather than at boot. Verified in the venv what that failure looks like: httpx builds the request, then h11 raises `LocalProtocolError: Illegal header name b'x-foo bar'` (and `Illegal header value` for the CR/LF case) at send time — the `http.client`/urllib3 stacks reject the same way — so nothing malformed reaches the wire; the gap is *when* the author learns of it, not *whether*. Closing it means growing "header-shaped" from "contains a hyphen" into an RFC 7230 token check on the name plus a no-CR/LF check on the value: small, but a design extension with a "where does it stop" edge, for a mistake that takes a deliberately quoted key. Deferred; revisit if a user actually hits the runtime error, or the next time `is_header_shaped` is touched for another reason.
...
- **Deferred — strict header-name/value validation** (greptile, `is_header_shaped`). See the new bullet under "Considered and deferred": h11 rejects an illegal name or value at send time, so it is a boot-vs-first-call timing gap for a deliberately quoted key, not a wire risk. Thread left open.
