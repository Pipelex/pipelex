...
| `pipelex-temporal/` (private) | `pipe_run.pipe_run_mode` (36 files), `pipeline.pipeline_models` | **yes — by far the largest consumer**, and the only importer of the deleted `pipeline_models` |

⚠ **This table is the 2026-07-29 draft snapshot and is superseded.** It was assembled from a hand-written repo list and undercounts: the authoritative, re-measured work list is [`hub/hub-split-tracker.md` → Third wave](hub/hub-split-tracker.md#third-wave--the-layer-placement-track-refactorlayer-boundary), which adds `test-bed`, `pipelex-workshop`, `pipelex-demos` and the three `pipelex-demo-*` repos. Enumerate the workspace directory when re-measuring; do not hand-list it.
...
**Round 3 (the whole PR, gstack `/review`).** Verdict was *ship, no blockers*, and the findings were all P2/P3 — but one of them is the most interesting thing this track produced, because it is a live counter-example to the claim the track exists to strengthen.

- **Five built-in vendor adapters were pulling the entire interpreter into `pipelex.providers` — a package the declaration already claimed was clean.** `providers/{anthropic,bedrock,google,mistral}/*_list.py` imported `MissingDependencyError` from `pipelex.exceptions`, the *public all-errors aggregate*, instead of from its definition site `pipelex.system.exceptions`. That aggregate re-exports every interpreter package's exceptions, so each of the five loaded 15 interpreter modules. It survived because it needed two independent blind spots and had both: the guard's rules only chase `interpreter_hub`, which an aggregate of exception classes never reaches; and the closure test's `providers.builtins` entry point does not cover these modules, because the vendor plugins import them *inside a function* — a deferred import is invisible to the static graph and to the aggregator's runtime closure at once. Fixed at the import site (the repo's "direct full-path imports everywhere" rule already forbade it), plus `providers.anthropic.anthropic_list` as a tenth entry point, with a negative control. The generalization now recorded in "Known inversions": **a module that re-exports across layers is a layer boundary with the sign filed off.**
- **`pipelex.test_extras` was measured clean, shipped, imported at boot — and undeclared**, while the guard's exclusion sentence named two packages and read as exhaustive. Declared, and the sentence now accounts for every top-level package. (`config`, `base_exceptions` and `urls` are also clean and undeclared; they were considered and left alone, because the transitive rule already covers them through their 30/32/5 declared runtime-layer importers. `test_extras` had no such cover.)
- **The cross-repo sweep list was incomplete, and the way it was incomplete is the lesson.** The first pass iterated a *hardcoded* repo list and missed six repos with real breaking sites — `test-bed`, `pipelex-workshop`, `pipelex-demos` and three `pipelex-demo-*`. A repo absent from both the affected and the cleared column reads exactly like one that was checked and cleared. The tracker now says: enumerate the workspace directory, do not hand-list it.
- Three bookkeeping fixes: a provenance comment in `pipelex/exceptions.py` that this PR itself made stale, the matched-set measurement snippet in `refactoring/modularity-refactors.md` (which a prescribed script check should have caught), and `refactoring/deferred-placement-follow-ups.md` §3, now marked resolved — its estimate of four caveat sites was also low, which is that section's own point about repeated caveats proving themselves.
