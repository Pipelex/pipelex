...
- `_plugins` (core): branch `feature/orchestration-mode-delivery-split`, tip **`919c09348`**, clean. (Phase 1 = `bd4e52827`; docs = `9c7a7785d`; this tracker = `919c09348`.)
...
**Then Checkpoint 3** (§4): as-built here + clean-context `/code-review` on the `pipelex-api` diff. **Phase 4** (bookkeeping) is unchanged and still pending — note that the core-doc Mistral prose + `pipe_classification.py` + the Mistral `execution-modes.md` were already moved to `"mistralai-workflows"` during Phase 1, so Phase 4's Mistral-rename scope shrank to wherever the actual token *registration* lives (the external `pipelex-mistralai-workflows` plugin, out of these repos) plus the trackers/superseded-plan markers + `make check-spec-links`.
