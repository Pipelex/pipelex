...
**Status:** **Phases 1 (core) + 2 (Temporal) DONE; Phase 3 (`pipelex-api`) NOT STARTED — paused for a fresh session.** Phase 1: `_plugins` commit `bd4e52827` (+ docs `9c7a7785d`, + this tracker `919c09348`) on `feature/orchestration-mode-delivery-split`. Phase 2: `pipelex-temporal` commit `01d7dc3` on `feature/orchestration-mode-delivery-split`. Both with all gates green + clean-context `/code-review` at their checkpoints. The §8 `/start`-honesty conformance risk is **resolved** (honest 400 is safe). The `pipelex-api` branch `feature/orchestration-mode-delivery-split` is **cut off `06cd5e6` with a clean tree — no Phase 3 code yet** (two partial edits were reverted to keep the handoff clean). **NEXT = Phase 3 (THE gate)** — see **"⏸️ COLD-START RESUME — paused before Phase 3"** at the head of the Checkpoint log / as-built section (§10) for the exact resume state, the locked design decisions, and the ready-to-execute checklist. Design **locked** in discussion (2026-06-22). This is **Direction A** — the real two-axis split across all three repos (`pipelex` core → `pipelex-temporal` → `pipelex-api`), not the API-edge-only reshape (Direction B, rejected: it relocates the smell to the boundary instead of removing it). It **supersedes** the dual-mode framing in the two follow-on plans `wip/plugins/execute-per-request-execution-mode.md` and `wip/plugins/orchestrator-dispatched-validate.md` — both shipped a working surface on top of the *flat* `PipelexExecutionMode`; this plan removes the flatness those plans worked around (`_async_start_mode`, the `/execute` fire-and-forget 400, the `ApiConfig` f&f field-validator, the `/validate` double-registration). Tracker: `_plugins/TODOS.md`. Parent effort: `wip/plugins/orchestrator-agnostic-runner-and-flavors.md`.
...
### ⏸️ COLD-START RESUME — paused before Phase 3 (2026-06-22)
