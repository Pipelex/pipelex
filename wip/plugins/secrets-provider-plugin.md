...
Status: **NOT STARTED.** Plan only. **Land [storage-provider-plugin.md](storage-provider-plugin.md) first** — it defines the shared "keyed registry + config-selected singleton" mechanism this plan reuses wholesale. This plan is the second, simpler application of that mechanism; most of it is "mirror storage, swap the nouns."
