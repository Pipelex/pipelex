...
### Original task — DONE: `pipelex-mistralai-workflows` is a discoverable `pipelex.plugins` entry-point plugin

Landed on `feature/Mistral-native` (worktree `pipelex-mistralai-workflows`), commit `272c72d`, editable-pinned to the corrected `../_workflows` base. Mirrors the `pipelex-temporal` reference pattern:

- **Entry point** `[project.entry-points."pipelex.plugins"]` → `mistral_native = pipelex_mistralai_workflows.mistral_plugin:MistralWorkflowsPlugin`.
- **`MistralWorkflowsPlugin`** (`name="mistral_native"`, `targets_api=PLUGIN_API_VERSION`): `register()` contributes the `MISTRAL_NATIVE` orchestrator. **No hub-slot claiming** (unlike `pipelex-temporal`) — the Mistral-native router is installed per workflow invocation by the workflow body itself (`wf_pipe_router.py` calls `set_pipe_router` in the controller path), so there is no process-global boot slot to claim. Simpler than the Temporal plugin by design.
- **`MistralNativeOrchestrator`** = the bridge's former `_run_mistral_native` arm (removed by Phase 3 of the plugin effort), now owned by the package: import-light (lazy-imports the SDK-pulling pipe-run factory inside `run`), wraps `PIPE_DISPATCH_ERRORS` → `PipelexBridgeDispatchError`, serializes via `serialize_completed_output(workflow_id=pipe_output.pipeline_run_id)`.

**Reconciliation forced by the externalization** (the merge made the plugin's pinned core the post-temporal-cut tree): dropped the `temporal` extra (`pipelex[temporal]` gone) + the two `run_pipe_via_bridge` Temporal-mode tests (`test_bridge_temporal_*` — redundant, that coverage already lives in `pipelex-temporal`'s `test_bridge_temporal_dispatch.py`; and broken, they imported externalized `pipelex.temporal.*`); repointed `PipelexPipeRunInput`/`Output` imports to `pipelex.runtime_bridge.payloads` (canonical home; the bridge only re-exports them, which pyright's strict re-export rule rejects). Added `test_plugin_discovery.py` (entry point declared + booting Pipelex registers the orchestrator).

**Gotcha worth remembering — stale `.egg-info` shadows the dist-info entry point.** After adding the entry point, `importlib.metadata.entry_points(group="pipelex.plugins")` still returned `[]` even though the `.dist-info/entry_points.txt` was correct. Cause: a leftover `pipelex_mistralai_workflows.egg-info/` at the repo root (gitignored, from an old setuptools `pip install -e`) sat on `sys.path` via the editable `.pth` and `importlib.metadata` resolved **it** (no entry points) instead of the venv dist-info. Fix: `rm -rf` the stale egg-info. Any editable-installed package whose entry points "aren't discovered" should check for a shadowing root egg-info first.

Gates green: plugin `make agent-check` (pyright 0 / mypy clean) + `make agent-test` (all passed).

### Deferred follow-ups

- **Bridge→MISTRAL_NATIVE execution test.** `test_native_recursion` drives the Mistral workflow directly (`workflow_env.client.execute_workflow`), and the activities tests use DIRECT mode, so no automated test drives `run_pipe_via_bridge(execution_mode=MISTRAL_NATIVE)` through the discovered orchestrator end-to-end. The wiring is covered by `test_plugin_discovery` (registration) and the orchestrator is thin, proven glue (verbatim extraction of the old arm; serialization shared with DIRECT/TEMPORAL and covered there), so this is a coverage nicety, not a gap in correctness. Adding it needs a Mistral-worker-context test like `test_activities_direct` but with MISTRAL_NATIVE.
