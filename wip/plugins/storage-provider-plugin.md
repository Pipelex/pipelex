...
Status: **NOT STARTED.** Plan only. Land this before the secrets plan — it defines the shared "keyed registry + config-selected singleton" mechanism (§ The new mechanism) that the secrets plan reuses.
...
- `pipelex-temporal` and `pipelex-mistralai-workflows` must bump `targets_api` to 3 (DX-1). They register no storage provider; the bump is the only change. Do it when the pipelex version carrying this lands, not before.
