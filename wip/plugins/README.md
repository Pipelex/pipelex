...
13. **Version the core↔plugin contract.** A plugin should declare which core API it targets so an incompatible pairing fails loud at load time, not mysteriously at runtime. "No backward compatibility" is the repo policy — make the contract break *visible*.
