> **ARCHIVED — describes an abandoned branch, NOT the current code.**
> This is the implementation that landed on the standalone `fix/dry-run` branch (commit `7a01854f`). That branch forked from `main` at v0.29.1 and was **never merged**. The live branch (`feature/Validate-with-signatures-4-fix-dry-run`) evolved ~55 commits independently — most importantly the signature-validation feature, which re-touched the exact same dry-run code paths. **None of the deletions / migrations / config removals below exist in the current tree.** Kept only as a reference for the consolidation that the new `D-plan.md` re-attempts on top of signature-validation. Do not treat any line number, "deleted", or "shipped" claim here as current.

# F. Implementation — What Shipped on the abandoned `fix/dry-run` branch

Maps the (now superseded) plan to the actual diff (commit `7a01854f` on `fix/dry-run`). Calls out divergences and one design addition (`keep_library_loaded`) that wasn't in the plan.
