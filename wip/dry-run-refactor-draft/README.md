...
| File | Purpose | Status |
|---|---|---|
| **[`/TODOS.md`](../../TODOS.md)** | **The execution tracker** (repo root). Checkboxes, status-at-a-glance table, mandatory ⛔ checkpoints with per-session handoff blocks, pre-flight human-input gate, cold-start protocol. The *do-this-in-order* spine; cold-starts a new session. | **Start execution here.** |
| **[`D-plan.md`](./D-plan.md)** | **The live plan.** The reframe (two operations + the run-mode⟂backend axis), the design (`BundleValidator` over a shared `acquire_library` / `prepare_pipe_job` seam; leaf-level run-mode mock; Temporal validation activity), decisions **D1–D5**, and a three-part phased migration (A in-process / B leaf-mock / C Temporal) with checkpoints. | **Read this first.** 2026-06-01, expanded with the backend dimension. |
