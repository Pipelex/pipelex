> **⚠️ Conclusion superseded (2026-06-01) — read this first.** This doc was written to justify routing DRY to a **local in-process** run even under a Temporal hub. That conclusion is **retired** by decision **D4** in [`D-plan.md`](./D-plan.md): run mode (LIVE/DRY) is now orthogonal to execution backend, so DRY honors the configured backend (DRY-on-Temporal really runs on the workers, mocking at the activity leaf — req 1). **The parity *evidence* below is still correct and still useful** — it now supports a *different* conclusion (decision **D5**): because the API process and the Temporal worker load the same MTHDS payload per request and register identical class registries, hosting the validation sweep **in-process (API) vs inside a worker activity is correctness-equivalent**. Read the rest of this doc as the parity proof, not as a routing recommendation.
>
> **Background — parity evidence still holds.** The boot-parity argument (API process and Temporal worker register the same class registry, user concept classes are loaded per-request not at boot) was re-checked against the live branch and is unchanged. Signature-validation does not alter this: signature pipes are loaded from the same per-request MTHDS payload as any other pipe. The `PARITY_HOLDS` finding stands; only the conclusion drawn from it changed (see banner above).
