...
## Status — APPLIED on `feature/Validate-with-signatures-4-fix-dry-run`

All four fixes are implemented and verified (TDD: failing test first, then the fix). `make agent-check` is clean (ruff / plxt / pyright `0 errors` / mypy); the targeted suite and the full `make agent-test` are green.

- **Fix 1** — `acquire_library` adopts `open_library`'s returned id. Added to `test_execution_seams.py`: `test_acquire_library_with_empty_id_adopts_generated_id` and `test_acquire_library_with_empty_id_tears_down_only_the_opened_library` (the catastrophe regression — verified RED first: the buggy `teardown("")` nuked the unrelated library along with all others).
- **Fix 2** — `registry_opened` flag set right after `open_registry`, guarded `close_registry` in the wrapper finally. Added to `test_pipeline_run_setup_characterization.py`: `test_failure_after_open_registry_closes_registry` and `test_failure_before_open_registry_does_not_close_registry` (the guard).
- **Fix 3** — the wrapper now captures `prev_library_id` before `acquire_library` and, on failure, restores-current-first-then-tears-down the library. Added `test_failure_after_acquire_restores_outer_current_library`. **Deviation from "keep it" in Fix 3 below:** the existing `test_load_failure_tears_down_library` was hardened to capture the `prev` baseline and assert *restoration* rather than a hardcoded `is None`. Fix 3 changes that test's semantics from "always clear" to "restore prev", and the hardcoded form is exactly the xdist fragility the Verification note flags.
- **Fix 4** — added the discriminating `test_prepare_pipe_job_skips_normalize_for_empty_inputs` to `test_execution_seams.py` **and** relabeled the over-claiming characterization test (did both options, not one). Discrimination was confirmed by experiment: flipping `if inputs:` → `if inputs is not None:` turns the new test RED (normalize fires on the empty-but-truthy `WorkingMemory`) while the old `root == {}` test stays GREEN — exactly the gap Fix 4 closes.

### Corrections to this brief (verified against the code)

- **Fix 2's "called nowhere" was inaccurate.** `close_registry` *is* called — in test fixtures (`tests/conftest.py:189`, `tests/integration/pipelex/fixtures/pipe_job_helpers.py:98`, the temporal conftests). The accurate statement is **no _production_ code closes the registry**. The fix is unaffected; only the wording was wrong.
- **Bonus pre-existing leak (out of scope — flagged, not fixed).** Production never closes the registry on the *success* path either: `runner.execute_pipeline`'s finally (`runner.py:196-211`) tears down tracer / event-log / library but not the registry, and nothing downstream pops it — so every *completed* run also leaks one `UsageRegistry`. This predates the seam extraction (it is not a reorder artifact), is a registry-ownership question spanning the setup→run→report boundary, and would risk breaking report generation if closed naively. Fix 2 deliberately plugs only the setup-failure window the reorder made reachable; the success-path leak belongs to a separate change.
...
- Cleanup/altitude (reuse): `main_pipe`-derivation duplicated across `execution_seams` / `dry_run_pipeline.py` / `inputs_ops.py` → extract `find_main_pipe_ref(blueprints)`; the restore-outer-current-then-teardown `finally` is now duplicated across `validate_bundle`'s entry points, `acquire_library`, **and** (after Fix 3) `pipeline_run_setup` → extract a shared helper/contextmanager (Fix 3 added one more copy, so the case is stronger now); `convert_to_working_memory_format` / `convert_stuff_spec_to_typed_named` share a registry-lookup+fallback core → delegate. Consider folding into the Phase-2 work since it touches the same seams.
