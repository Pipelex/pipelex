...
> **Campaign closed 2026-07-12 — this file is the live handoff.** The per-section findings files and prescreen artifacts referenced below (`suspects.md`, the findings files) were pruned at close-out; they live in git history under the pushed `drift-hunt-archive` tag: `git show drift-hunt-archive:wip/drift-hunt/<path>`. The campaign record is `findings/SUMMARY.md` + `inventory.md`.
