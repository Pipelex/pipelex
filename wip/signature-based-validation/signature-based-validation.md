...
| Strict pre-check: `collect_signature_refs` / `collect_signature_paths`, `SignaturesNotAllowedError`, `allow_signatures` threading + cross-batch aggregation, `ValidateBundleError.signature_check_error` | `pipelex/pipe_signature/signature_walk.py`, `pipelex/pipeline/bundle_validator.py` (`_signature_pre_pass`, run from `validate_pipes`), `pipelex/pipeline/validate_bundle.py`, `pipelex/pipe_signature/exceptions.py` (`SignaturesNotAllowedError`), `pipelex/pipeline/exceptions.py` (`ValidateBundleError`) | `tests/integration/pipelex/pipe_signature/test_dry_run_strict_mode.py`, `tests/e2e/test_signature_validation_mthds.py` |
...
- **Strict whole-bundle docstrings clarified** (on the then-`dry_run_pipes`, now `BundleValidator.validate_pipes` — see the Post-#956 note below — and on `PipeSignatureSpec`): whole-bundle strict rejects any bundle containing a signature; `--pipe` / `--allow-signatures` are the escape hatches.
...
## Review fixes (additive-library completeness, 2026-06-07)

A second review pass found three paths that still did per-file / pre-merge work, breaking the same-domain authoring the merged crate is meant to resolve. The concept side had moved same-domain resolution to the merged level (`validate_concept_references_in_blueprints` in the loader); these three completed the same move for the pipe side and the remaining loaders. All confirmed against the code and fixed with regression tests:

- **Qualified same-domain pipe refs now resolve across sibling files.** The per-file `validate_local_pipe_references` `@model_validator` on `PipelexBundleBlueprint` eagerly rejected a qualified same-domain step ref (`research.find_key_findings`) unless the target pipe was declared in the *same* file — while the bare form was already deferred (the exact asymmetry the concept side had already removed). In an additive library the concrete can live in a sibling file, reconciled by `LibraryCrateFactory`. Every other branch of the validator was already a no-op, so it is removed entirely; a genuinely missing ref is still caught post-merge by `validate_pipe_library_with_libraries` (`get_required_pipe` → `PipeNotFoundError` → `LibraryError`). The shared `collect_pipe_references` helper stays (used by `visibility_utils` and the dependency loader). — `tests/integration/pipelex/libraries/test_additive_multi_file_library.py` (`test_qualified_same_domain_sibling_pipe_ref_resolves`, `test_undeclared_qualified_pipe_ref_surfaces_clean_error`); the now-obsolete `tests/unit/pipelex/core/bundles/test_pipelex_bundle_blueprint_pipe_validation.py` was removed (it only exercised the deleted validator).
- **Concepts-only loading now validates concept references.** `LibraryManager.load_concepts_only_from_blueprints` skipped `validate_concept_references_in_blueprints` (only the full `load_from_blueprints` ran it), so the `pipelex structures` / `load_concepts_only` path accepted a dangling `concept_ref` / `item_concept_ref` and left an unrebuildable schema in the library. It now runs the same merged validator (with `already_loaded_concept_refs` for cross-batch resolution) before adding the batch's concepts. — `tests/integration/pipelex/pipeline/test_load_concepts_only.py` (`test_load_concepts_only_rejects_dangling_concept_ref`, `..._item_concept_ref`).
- **Dependency packages now load through the crate merge.** `LibraryManager._load_single_dependency` built pipes per-blueprint and called `add_new_pipe` directly, so a multi-file dependency (a `PipeSignature` header + concrete sibling) collided on the duplicate pipe ref — swallowed by the per-pipe `except (PipeLibraryError, ValidationError)` as a warning, silently dropping one declaration (load-order dependent: a signature-only survivor is non-executable). It now builds a `LibraryCrate` from the dependency's blueprints first (signature/concrete reconciliation + normalized contract check + duplicate detection) and runs `validate_concept_references_in_blueprints`, then loads the reconciled `crate.pipes` (export filter and synthetic-helper handling preserved). The per-pipe handler now catches only `ValidationError` (a single malformed blueprint stays lenient); a genuine duplicate definition raises from the crate instead of warning — aligning dependencies with the main load path. — `tests/unit/pipelex/libraries/test_dependency_multi_file_reconciliation.py`.
...
> **Post-#956 note.** The dry-run/validation consolidation (#956, commit `7b2ae132`) removed the standalone `pipelex/pipe_run/dry_run.py` and its `dry_run_pipe` / `dry_run_pipes` functions. The strict signature pre-check now runs inside `BundleValidator.validate_pipes` (`_signature_pre_pass`) in `pipelex/pipeline/bundle_validator.py`, which is what `validate_bundle` (and the `validate --all` standalone path via `acquire_and_validate`) call with `allow_signatures`. The behavior — one aggregated `SignaturesNotAllowedError` over the full pipe set, keeping the longest dep chain per signature — is unchanged; only the home moved. The `_signature_pre_pass` docstring spells out the wiring-before-signature ordering it preserves.
