...
- 2026-09-03, pre-landing review of the PR: the sequence's base now comes from the `backends_file_path` / `routing_profiles_file_path` property rather than a second `resolve_config_file` call, because four integration fixtures (and `pipelex-server`'s temporal one) pin a routing profile by patching that property and the sequence read past the patch — every such test booted on the real file, green. The in-repo fixtures now patch `routing_profiles_file_paths` whole, so a developer's personal override cannot layer over the pinned document either; the server fixture is noted on `L-260903-36002a`. Four review decisions taken: the loaders log the override files they merged (the one trace a machine-wide override leaves); a file that does not parse is the library's refusal, raised by the library, the routing loader and `is_pipelex_gateway_enabled` alike, so the boot names the file instead of a raw `TomlError` escaping before any clause; the doctor's Models row catches `RoutingProfileLibraryError`, the class the loader now raises for the likeliest override typo; and `pipelex init` gates the terms prompt on the merged document and warns when declining the terms disables the gateway in a base an override still pins on. A backend whose value is not a table (`acme = false`) is refused by name in both modes rather than crashing on `.copy()`.
