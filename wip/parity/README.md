...
- **Phase 2: ✅ done 2026-08-04, folded into the kernel stack.** 2.1 fixed in #1081; 2.3 fixed in #1082, with the boot-contract arm it turned out to owe in the same PR and the doc review on #1083 — commits named by subject in [`parity-gaps-plan.md`](parity-gaps-plan.md), because the stack is rebased and hashes do not survive it. 2.2 **withdrawn** on re-verification — the façade's single `model` wins the object resolution's first rung, so the style it derives already matches an interpreted run for every call the façade can express; the surviving narrowness is deferred as KF-16, because the whole model-derived-style mechanism is slated to become an explicit user choice (design in [`../prompting-style/README.md`](../prompting-style/README.md), which landed with the kernel stack). D-3 settled as recommended, (a), and its predicted layering cost did not materialise.
