...
- **Phase 2: ✅ done 2026-08-04, folded into the kernel stack.** 2.1 fixed in #1081; 2.3 fixed in #1082, with the boot-contract arm it turned out to owe in the same PR and the doc review on #1083 — commits named by subject in [`parity-gaps-plan.md`](parity-gaps-plan.md), because the stack is rebased and hashes do not survive it. 2.2 **withdrawn** on re-verification — the façade's single `model` wins the object resolution's first rung, so the style it derives already matches an interpreted run for every call the façade can express; the surviving narrowness was deferred as KF-16. **KF-16 is now closed, by dissolution rather than by fix** (2026-08-14, branch `fix/Keyless-dry-run`): the model-derived-style mechanism is deleted outright, the style is an authoring decision declared on the pipe, and both `llm_text` and `llm_object` take it as an argument. The façade and the interpreter agree because neither derives anything. Design in [`../prompting-style/prompt-style-as-an-authoring-decision.md`](../prompting-style/prompt-style-as-an-authoring-decision.md), build in [`../prompting-style/templating-style-implementation-plan.md`](../prompting-style/templating-style-implementation-plan.md). D-3 settled as recommended, (a), and its predicted layering cost did not materialise.
