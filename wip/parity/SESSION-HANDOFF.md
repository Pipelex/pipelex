...
- **⚠ Crate-wide resolution is a *runtime-parity* choice, not the spec's rule — and the MTHDS spec says the opposite.** `mthds/docs/spec/namespace-resolution.md` states outright that bare pipe references "do NOT fall through to other domains or other packages"; the runtime's `get_optional_pipe` searches across domains, and 1.1 mirrored it on purpose. So the two readers now agree **and both are more permissive than the standard**. That is the right stopping point for a parity change and the wrong place to leave the story untold — written up in [`bare-pipe-ref-spec-divergence.md`](bare-pipe-ref-spec-divergence.md), which also explains why it subsumes D-1. Do not "fix" either reader alone.
