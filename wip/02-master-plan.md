...
- `ReportingManager.inject_tokens_usages(pipeline_run_id, tokens_usages)` (`reporting_manager.py:224`) — docstring: *"Used after assembling usage data from distributed trace events, so that generate_report() can produce a complete cost report across all workers."*
- `ReportingManager.generate_report(pipeline_run_id)` (`reporting_manager.py:366`).
...
- [ ] Revisit the local-only registry fallback (`_get_registry_strict` at `reporting_manager.py:116`, `_try_add_to_registry` at `:138`) once the distributed read-back lands. (The old `_get_registry` TODO that previously sat here is already gone — the accessor was renamed to `_get_registry_strict` — but the local-only behaviour it flagged remains until the cross-worker path is wired.)
