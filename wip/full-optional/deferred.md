...
That asymmetry is the hint lint's own pre-existing scope decision, unchanged here. What changed is who sees it: the lint used to reach only the protocol validation report, and now rides every advisory-bearing validate channel — the whole-bundle and library-wide ones; the single-pipe surfaces and the builder's `validate_all` still carry none. So `pipelex validate bundle mine.mthds -L shared/` can report hint findings that belong to `shared/`, which an author cannot act on from where they are standing.
