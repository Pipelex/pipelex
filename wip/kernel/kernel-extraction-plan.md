...
- **Current phase:** **Phase 1 COMPLETE — at CHECKPOINT A.** Phase 0 (`23276cecd`) and Phase 1 tasks 1.1–1.6 (`3a11d113a`) are committed; task 1.7 and the checkpoint chores are the working tree. Branch `refactor/Kernel`, off `dev` with `dev` merged in at `221b8ee0b`. Nothing pushed, no PR open.
- **Gates:** `make agent-check` **green**, `make drift-check` **green** (`hub-layering-convention` acked), full `make agent-test` **green**. Two gate failures surfaced at this checkpoint and both are fixed — see "Two gates the Phase-1 commit left red" below. Note for the record: this doc previously claimed `make agent-check` was green at `3a11d113a`. It was not; the keyword-only gate was red there.
- **Cold review:** done on the full diff, with no inherited context. No correctness bug, no layering violation, no stale reference, and the three past bug fixes in this code (templating-style mutation, the multi-image/document dict-key collision, LLM-error cause unwrapping) were confirmed preserved verbatim. Three quality findings: one fixed (the boot contract covered only `llm_object` — it now covers both façade calls, which matters because the two arms share almost nothing below the façade), two deferred as KF-3 and KF-4 in `deferred-follow-ups.md`.
- **Next action:** commit → push → open PR #1 (numbered, targeting `dev`, **not merged**). Phase 2 is untouched.
- **Open decisions:** none. The kernel construction shape (0.2), the structure-prompt provenance and the deferred runtime-boot orchestrator question are all settled under Decisions; six deviations from the plan's letter are recorded there too.

### Two gates the Phase-1 commit left red

Both were consequences of the move that only a full run could surface, and both are worth remembering because the same shapes will recur in Phase 2:

1. **Five dead subject grants.** `subject_grants.toml` still granted positional subjects for `pipe_operators/llm/helpers.py::get_output_structure_prompt` (deleted) and four `LLMPromptBlueprint._extract_direct_*` methods (moved into `llm_prompt_content.py`). The convention's staleness check is *symmetric* — a grant whose def was moved or deleted hard-fails until the registry is cleaned. Removed, not re-granted: the kernel's replacements are fully keyword-only, which is the package's stated zero-grants doctrine. **Every operator moved in Phase 2 must have its grants swept the same way.**
2. **Two dead paths in `.test_durations`.** The reference-type test modules moved to `tests/unit/pipelex/kernel/`, and `tests/unit/repo/test_test_durations_paths.py` caught the committed shard-balancing artifact still pointing at their old home. Fixed by rewriting the 25 node ids' path prefix rather than regenerating the whole file: the tests moved verbatim, so their recorded durations are still accurate, and a full `make store-test-durations` would have churned every timing in the file for no gain. New modules simply have no entry, which pytest-split treats as average duration.
