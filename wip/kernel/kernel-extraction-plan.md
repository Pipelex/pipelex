...
- **One real bug was found by review and fixed — the PR's only behavior change.** `MethodKernel.make` minted a LIVE `CogtRunParams` on a keyless boot (`needs_inference=False`), bypassing the forced-DRY coercion `PipeRunParamsFactory` applies. Reachable, not hypothetical: `run_mode` alone gates mocking, so the call reached a real provider, and `needs_inference=False` means backends load *leniently* rather than credential-free. The hazard was the **default** — `MethodKernel.make(user_id=…)` names no mode. Root cause is this branch's own doing: the factory's docstring called itself "the single writer" of `run_mode`, which stopped being true when the kernel became a second run-params factory. The rule now lives once in `runtime_hub.resolve_run_mode_for_boot`, called by both factories, pinned by a third arm on the D4 contract module. **Phase 2 lesson: any new kernel-side factory of run-scoped state owes the same audit — ask which process-wide invariants the pipe tier applies that the kernel tier now skips.**
