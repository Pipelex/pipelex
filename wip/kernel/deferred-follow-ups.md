...
## KF-13 — RESOLVED — a DRY run's usage records carry no `pipe_code`, so a `--mock-usage` cost report has no pipe attribution
...
**Resolution.** Closed on the topology branch, by exactly the caller this entry predicted: `dry_run_pipe` now stamps the running pipe onto the `JobMetadata` it hands down, mirroring `live_run_pipe`. The fix arrived through a merge and landed as the predicted diff — the parity test went red on `assert interpreter_records[0].pipe_code is None` rather than drifting silently, and now asserts the difference the plan originally expected: the interpreter names the pipe it ran, the kernel has no pipe to name.
...
## KF-18 — A kernel-layer module reaches a named interpreter package through a function-local import, and no gate sees it

**Surfaced by** the B0 footprint measurement (see [`kernel-distribution-footprint.md`](kernel-distribution-footprint.md)), which walked the static import graph of the whole tree rather than the closures the existing tests pin.

`pipelex/core/memory/working_memory_factory.py` — kernel-layer, and one of the entry points listed in `KERNEL_LAYER_ENTRY_POINTS` — imports `pipelex.builder.pipe.pipe_spec` and `pipelex.builder.pipe.pipe_batch_spec` inside `_get_concrete_class_for_mocking`, under a `# Import here to avoid circular imports` comment. `builder` is a named interpreter package. Importing `pipelex.builder.pipe.pipe_spec` in a fresh interpreter pulls 6 interpreter modules.

**Why both gates stay green, and why that is not a bug in either.** `test_kernel_layer_import_closure.py` imports the entry point and sweeps `sys.modules`, so it sees module-level imports only — a function-local one is invisible to it by construction. `pipelex-dev check-hub-layering` asks a different question (does this reach `interpreter_hub`?) and the answer here is genuinely no: the pulled modules are Pipe-*spec* models, and none of them reaches the hub. So each gate is answering its own question correctly. This is the exact blind spot `test_kernel_layer_import_closure.py`'s own docstring names — "a function-local import inside a kernel module is invisible to it *and* to the static graph at once" — and the boot-contract test's post-call sweep is the mechanism that exists to close it. It does not close *this* one, because the sweep only covers the ops it calls and this mocking helper is on none of them.

**Why it is deferred rather than fixed.** B0 is a read-only measurement, and the fix is a design decision rather than a measurement: the import exists to break a circular import in dry-run mock generation, so removing it means deciding where the "pick a concrete subclass to mock" knowledge belongs — plausibly not in a kernel-layer memory factory at all. Doing that inside a measurement task would be exactly the reflex the standing constraint warns against.

**What settling it would take.** Two independent halves, and the second is worth more than the first.

- *The instance:* move the concrete-subclass choice out of `working_memory_factory`, or invert it so the builder registers its own mocking preference rather than the kernel naming builder types.
- *The class of bug:* Part B's D-B5 ban closes it structurally — `nothing under pipelex_kernel/ may import pipelex` is stated over a package boundary, so it catches function-local imports and unnamed-bucket destinations alike, which is what neither current gate can do. Until then, the cheap partial is to extend the closure test's detector to AST-scan each entry point's *own* module set for function-local interpreter imports, which would have caught this one.
