...
**Scope of this document:** the contracts and the decisions. The implementation tracker is the `plan.md` beside it, and the campaign's items are filed on the ledger under the epic. Ratification is still outstanding — the open questions at the end are what it has to settle — and phase 3's gate waits on it; phases 1 and 2 depend on nothing it leaves open, which is why phase 1 has already run.
