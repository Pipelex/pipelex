...
**Scope of this document:** the contracts and the decisions. The implementation tracker is the `plan.md` beside it, and the campaign's items are filed on the ledger under the epic. Ratified by Louis on 2026-09-20, and what the ratification settled is the last section, `Ratified decisions`. Phases 1 and 2 depended on nothing it left open, which is why they ran before it; what remains of phase 3's gate is the standard change in `mthds`.
...
- `question: str` — required. A template, preprocessed and parsed exactly as `PipeSearchBlueprint.prompt` is, so `$name` and `@name` behave as they do everywhere else and `validate_inputs` diffs required variables against declared inputs with the same code. **`prompt` is accepted as a synonym of `question`**, resolved at parse time: exactly one of the two may be set, both set is a blueprint validation error naming `question`, and `question` is what the blueprint carries, what the spec layer round-trips and what the builder and the agent CLI emit. Nothing downstream of parsing ever sees `prompt`.
...
**Worker.** `JudgmentWorkerAbstract(InferenceWorkerAbstract)` exposes one template method, `async def judge(self, judgment_job: JudgmentJob) -> dict[str, JudgmentAnswer]`, and one abstract hook, `_judge`. The template method copies `SearchWorkerAbstract.search_sourced_answer` step for step — validate, stamp `UnitJobId.JUDGMENT_ANSWER`, `judgment_job_before_start`, call the hook, `fill_model_and_provider` on a `CogtError`, and complete and report in `finally` — including the commented reason the reporting sits in `finally` (`search_worker_abstract.py:47`). After the hook returns, the template method checks that the answers' keys are exactly the questions' keys, that each answer's kind matches its question's, and that every option and level an answer names — its verdict and the keys of its distribution — is one its question offered, so no backend can return a verdict for a question nobody asked or a verdict the question never offered.

**Setting and deck.** `JudgmentSetting(ConfigModel)` has `model: str` and `description: str | None` and nothing else; `JudgmentModelChoice` is the usual union with a parsed model reference. `JudgmentDeckBlueprint` has `aliases`, `waterfalls`, `presets` and `choice_default`, flattened onto `ModelDeck` with `get_judgment_setting`, `validate_judgment_presets` and `check_judgment_choice_with_deck` as for search. The deck file is `pipelex/kit/configs/inference/deck/5_judgment_deck.toml` — the numeric prefix is what makes it kit-managed (`pipelex/cogt/models/deck_manifest.py:98`) — and an alias in it points at a versioned handle, never at a `latest` alias: a verdict model that changes under a method silently changes what every threshold in that method means.

**`choice_default` is optional for this family alone, and phase 2 discovered why.** Every other family's default names a handle the Pipelex Gateway carries, so it always resolves. A judgment backend is brought by the user, and `ModelManager._enforce_gateway_model_membership` walks every handle a deck's presets and choice defaults name: a default resolving to `jev-1.13.0`, which the active routing profile sends to the gateway by default and which the gateway does not serve, raises `GatewayUnknownModelError` at **every boot**, for every user, whether or not they ever ask for a judgment. So `JudgmentDeckBlueprint.choice_default` is `JudgmentModelChoice | None = None`, the kit deck ships no default, and a judgment names its own model. Ratification made that permanent rather than temporary: the hosted plane does not serve this family, so no default the kit ships can ever resolve, `@default-judgment` is not a usable default for `PipeJudge`, and phase 3's operator names its model or fails with a message saying no judgment model is configured. A user holding their own key remains free to name a default in their own deck, which the optional `"jev-*"` route resolves.
...
- **Backend records.** `[typesafe]` in `backends.toml` with `api_key = "${TYPESAFE_API_KEY}"`, and `backends/typesafe.toml` with `model_type = "judgment"`, `sdk = "typesafe"`, one table per versioned model (`inputs = ["text"]`, `outputs = ["judgments"]`, `costs = { input = 0.042, output = 0 }` per million tokens, from the vendor's models page at the time of writing, output being genuinely free), and a routing profile. **The managed routing profiles also carry an optional route, `"jev-*" = "typesafe"`**, which phase 2 found necessary: under `all_pipelex_gateway` every handle routes to the gateway by default, the gateway does not serve Jev, and a handle nobody serves is silently dropped from the deck — so without it a user holding a TypeSafe key still could not reach the model. An optional route applies only while its backend is enabled. The kit deck carries the alias and still no preset, since a preset would reach the gateway membership check whenever the backend is disabled. The default alias points at `jev-1.13.0`, with a comment saying that a newer versioned id is found on the vendor's models page and not through `GET /v1/models`, which lists only aliases. All mirrored into `.pipelex/inference/`.
...
- **Errors.** `TypesafeJudgmentResponseError` for an answer that is missing or of the wrong kind, and `TypesafeQuestionUnsupportedError` for a question this backend's bounds refuse, both under a `TypesafeError` base. Classification branches on `error_type` where the body carries one and falls back to the status, because a single `400` covers an illegal question, an unknown model and an oversized state: `authentication_error` classifies as credentials, `max_tokens_exceeded` as content, `api_usage_error` naming an unknown model as configuration, rate limit and overload as transient, and a `400` with a bare-string `detail` as a Pipelex defect, since static validation should have caught a malformed question. A bare `TypeSafeError`, raised before the call, is always a Pipelex defect. As built, the extract step lifts the body's `error_type` — or a code of its own for a bare-string `detail` — into `provider_error_code`, and the branching lives in the provider package (`classify_typesafe_error`), which hands everything it does not recognise to the shared classifier; that keeps the shared classifier free of vendor names. An `api_usage_error` whose message does not name an unknown model is classified as configuration with a contact-support action, since nothing observed says what else it covers. The worker also warns when the response names a model other than the pinned one, the one case where the pin could fail silently. The family classes are `JudgmentJobFailureError`, `JudgmentModelNotFoundError` and `JudgmentHandleNotFoundError`, and the operator's are `PipeJudgeError` and `PipeJudgeFactoryError`. Error pages and the identity snapshot are regenerated (`gep`, `gei`).
...
4. The hosted plane — `pipelex-server/temporal/` registers an activity for the new leaf and a queue for it, and the pin moves across `pipelex-server` in one commit. Nothing else: ratification decided the plane does not serve this family on a Pipelex-held key, so there is no `pipelex-manifold` route and no `pipelex-remote-config` entry to write, and a hosted run reaches a judgment model on the customer's own key or not at all.

## Ratified decisions

Ratified by Louis on 2026-09-20. The vendor-dependency question that stood here was closed earlier, by the phase 1 spike, in favour of the SDK behind an extra, for the reasons in Part 5 and in `spike-findings.md`.

- **The natives are `Choice` and `Rating`**, and `YesNo` gains its optional `probability`. `Rating` keeps the word the language already owns, against the vendor's "Score", which reads as an exact magnitude — the reading the vendor's own limitations page warns against.
- **The whole optional surface is pinned on day one**: `probability` on `YesNo`; `confidence` and `probabilities` on `Choice`; `confidence`, `probabilities` and `position` on `Rating`. The smaller alternative above — the verdict and one scalar apiece — was the fallback against resistance from the standard, and this workspace owns the standard, so there is none to trade against. A method can read a distribution, and fall back to the second most probable option, from the day the natives exist.
- **The operator's template field is `question`, with `prompt` accepted as a synonym**, as Part 1 now specifies. `question` is canonical and is what every layer below the parser speaks; `prompt` exists because every other inference operator calls its template that, so an author reaching for it is right about the language and wrong only about this operator.
- **The family is bring-your-own-key, permanently.** The hosted plane does not serve judgments: no Pipelex-held vendor key, no Manifold route, no remote-config entry, and therefore no default the kit can ship. `PipeJudge` names its model or says that none is configured, and the docs teach that a judgment names its own model. This is the settled design rather than a state waiting on the plane, which is what Part 3's `choice_default` paragraph and route step 4 now record.
