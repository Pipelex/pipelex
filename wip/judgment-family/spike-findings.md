...
`usage.input_tokens` and `usage.output_tokens` were reported on every successful call, and no answer carries a usage field of its own: usage is per request, never per question. Both grow with the number of questions — 1, 3 and 50 questions over the same state gave 323/22, 506/78 and 1147/904 input/output tokens.
...
**A rating is capped at ten levels**, `400 "Too many score levels. Must have at most 10 levels."` — a hard limit the blueprint must enforce, and one the design did not know about. A choice took 61 options without complaint.
...
| Eleven rating levels | 400 | `TypeSafeBadRequestError` | `{"detail": "Too many score levels. Must have at most 10 levels."}` |
...
`request_id` is present on **success** too, as `response.request_id`, read from the `x-typesafe-request-id` header. Every log line the worker writes should carry it, not only the failures.
...
- **A rating is capped at ten levels.** The design recorded "a bounded number" without the number.
