...
| ~~**`__init__` content-generator injection seam**~~ | `bundle_validator.BundleValidator.__init__` | **Resolved by Part C** — no constructor seam was needed: `validate_pipes` wraps its sweep loop in `scoped_content_generator(ContentGeneratorDry())` (ContextVar scope alongside `scoped_pipe_router`), forcing the inline generator under any hub. |
