...
| **`__init__` content-generator injection seam** | `bundle_validator.BundleValidator.__init__` | Hardcodes `PipeRun(PipeRouter(ObserverNoOp()))` (correct layer, deliberately not `get_pipe_run()`). Part C's `scoped_content_generator` will need a seam to force the inline generator — revisit when Part C lands. |
