...
|---|---|---|---|
| **Part B — leaf-level run-mode mock** | Move the LIVE/DRY decision down to the cogt leaf so DRY *honors the backend* (DRY-on-Temporal dispatches `act_llm_gen_*` and mocks inside the activity). Retires the "DRY → local" shortcut. (req 1) | **COMPLETE (2026-06-10)** on `feature/Mock-activities` (PR #983) — all phases done, Checkpoint E reached, Tier 17 GREEN+RED. | [`followup-leaf-run-mode-mock.md`](./followup-leaf-run-mode-mock.md) |
| **Unified dry run — retire `is_mock_inference` as a mode** | One non-live mode: **dry run** ("no effects": every leaf mocked, no PipeFunc execution, no storage IO). `is_mock_inference` becomes `CogtRunParams.is_mock_usage`, an **internal** sub-flag of DRY (no public CLI) that flips usage reporting from zero-token/suppressed to non-zero/rendered — what the cost-assembly tests (incl. `temporal-e2e-validate` Tier 8b) consume. **Supersedes eng-review decision D8.** | **IMPLEMENTED (2026-06-10)** on `feature/Mock-activities` — plan + as-built: [`plan-unified-dry-run.md`](./plan-unified-dry-run.md). | [`design-unified-dry-run.md`](./design-unified-dry-run.md) |
