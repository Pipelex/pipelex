> **Background — still structurally accurate; line numbers are as of ~v0.29.1.** The four-class taxonomy, the call chains, the hub swap, and the `DryPipeRouter` dead-code proof were re-verified against the live branch and still hold (the hub swap is now at `pipelex.py:446–461`; `PipelexRunner(pipe_run=…)` injection is unchanged). What this doc predates is the **signature-validation** feature — see the new [§7 Signature-validation deltas](#section-7-signature-validation-deltas-added-since-this-doc) at the end for the surface it added to the dry-run path.
...
```
pipelex/pipe_run/dry_pipe_router.py:class DryPipeRouter(PipeRouterProtocol):
```

**No production instantiation found.** Only appears in its own definition. **Dead code confirmed** (re-verified on the live branch — `grep -rn DryPipeRouter` still finds only the class definition).
...
## Section 7: Signature-validation deltas (added since this doc)

The original taxonomy predates the **signature-validation** feature, which landed on the live branch and re-wired the dry-run path. None of it changes the four-class taxonomy above (routers/runs are untouched), but it heavily changes `pipelex/pipe_run/dry_run.py` — the module the consolidation plan wants to delete. What's new:

- **A third pipe category, `PipeSignature`** (`pipelex/pipe_signature/`). A contract-only pipe (`type = "PipeSignature"`) declaring `inputs` / `output` / `description` with no body. Its `_dry_run_pipe` **mints a mock `Stuff`** from the declared output concept via `WorkingMemoryFactory.make_mock_content`; its `_live_run_pipe` raises `PipeSignatureNotExecutableError`.
- **`dry_run.py` is now the home of the strict signature pre-check.** `dry_run_pipe` and `dry_run_pipes` gained an `allow_signatures` parameter. When `allow_signatures=False` (the default — both CLIs are strict by default), `dry_run_pipes` walks every pipe with `collect_signature_refs` / `collect_signature_paths` (free functions in `pipe_signature/signature_walk.py`, kept off `PipeAbstract` to avoid an import cycle) and raises a **single aggregated `SignaturesNotAllowedError`** listing every reachable signature plus the controller dep-chain. This aggregation-into-one-error is a deliberate UX property the consolidation must preserve.
- **`dry_run.py` now exports a helper that `pipe_signature.py` imports.** `convert_stuff_spec_to_typed_named` (used to resolve a signature's declared output to a typed mock) lives in `dry_run.py` and is imported by `pipelex/pipe_signature/pipe_signature.py`. So `dry_run.py` is no longer a leaf the plan can simply delete — there is a hard dependency from the signature runtime into it.
- **`validate_bundle` threads `allow_signatures` and `dry_run_pipe_codes`.** The latter (with the `_pipes_to_dry_run` selector) implements the `--pipe` single-slice path: load the whole bundle so deps resolve, but dry-run only the selected pipe. `ValidateBundleError` gained `signature_check_error` to carry the strict refusal up to the CLI for friendly rendering.
- **Callsites that now thread `allow_signatures`:** `cli/commands/validate/_validate_core.py`, `cli/agent_cli/commands/validate/_validate_core.py`, `builder/operations/validate_ops.py`, `builder/operations/runner_code_ops.py`, plus `validate_bundle.py` itself. These are the same callsites the consolidation wants to migrate to `PipelexRunner` — they now carry an extra parameter each.

**Implication for the plan:** routing dry-run through `PipelexRunner` means `run_mode == DRY` is no longer the only mode-shaped concern on that path — `allow_signatures` (strict vs lenient) and `dry_run_pipe_codes` (which pipes to dry-run) ride alongside it, and the batch-level strict pre-check needs a home that runs *before* per-pipe execution. See `D-plan.md` for how this reshapes the design.
