...
> **Status: DESIGN AGREED with the user (2026-06-10). No plan yet — write the implementation plan in a fresh session from this document alone.** This design **supersedes eng-review decision D8** of [`followup-leaf-run-mode-mock.md`](./followup-leaf-run-mode-mock.md) ("is_mock_inference KEPT as thin reportable-mock"): the *mode* is retired; the *capability* (non-zero synthetic usage) survives as a secondary flag on dry run. It also resolves the "fate of `is_mock_inference`" item tracked in `wip/registry/deferred-followups.md`.
>
