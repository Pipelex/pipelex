> **Background — still valid.** The CPU-only / no-network / no-disk characterization of the dry-run path holds. One addition since this was written: signature pipes now mint their declared mock output via `WorkingMemoryFactory.make_mock_content` (polyfactory, same cheap cost class as the mock-input generation profiled in Stage 3). No new I/O, no new locks. The verdict — dry-run is safe in-process even under high concurrency — is unchanged.
