...
Root-caused 2026-06-10 (CI py3.11–3.14 hang on the three GraphSpec-crossing tests): an exception raised by the data converter while decoding an activity result fails the *workflow task*, which Temporal retries indefinitely — the submitter's `execute_workflow` never returns. The trigger was environmental (`ZoneInfoNotFoundError`: no IANA tzdata on the runner for py3.11+), and kajson 0.7.0 structurally eliminated that trigger class: its self-sufficient timezone wire format carries the UTC offset, so decoding degrades gracefully to a fixed offset instead of raising, and kajson declares `tzdata` itself. But the failure *shape* is general: any deterministic decode bug (kajson registry miss, schema drift, future enum rename) will present as an infinite hang on every host, not a failed run. `dispatch_dry_validate`'s 12-minute `workflow_execution_timeout` bounds the API path; direct `execute_workflow` callers (tests, scripts) have no bound.
...
## 9. The library-window teardown dance is hand-mirrored across two files (and already drifts)

Surfaced during the review of the validation-api extraction (`feature/Tweaks-for-validation-api`): the in-process validate orchestration moved out of `PipelexMTHDSProtocol.validate` into the shared `validate_bundles_in_process` (`pipelex/pipeline/validate_in_process.py`), so the local runtime and the upcoming `ApiRunner.validate` share one teardown implementation. But `act_dry_validate` (`pipelex/temporal/tprl_pipe/act_dry_validate.py`) still carries its own copy of the same restore-current-library + teardown-in-`try` + suppress-teardown-error-on-body-failure block — the comments literally say "mirrors act_dry_validate" / "Mirrors act_dry_validate's finally". So the load-bearing teardown semantics now live in *two* places (the new module deliberately unified runner + API but left the Temporal activity as a by-hand mirror).

The two copies have already diverged at the edges — none documented as intentional:

- `validate_in_process` gates the whole block on `validation_library_id is not None and validation_library_id != prev_library_id`, then tears down unconditionally inside.
- `act_dry_validate` gates the restore on `prev_library_id is not None and prev_library_id != library_id` (with an `else: clear_current_library()` branch the other site never executes), and gates only the teardown on `if library_id is not None`.

The behavior difference is benign today and the divergence is **pre-existing** (the extracted block is byte-identical to the old `runner.py` code — the extraction introduced no new drift), but "mirrors act_dry_validate" is now aspirational, which is exactly the failure mode the docstring's "lives once, so the two backends cannot drift" claim is meant to prevent.

**Direction (design tradeoff, do not reflexively patch):** collapse both sites onto one implementation — a `teardown_validation_library(*, validation_library_id, prev_library_id, body_succeeded, log_context)` helper, or a `validation_library_window(...)` context manager that owns the restore-first / teardown / suppress-on-error contract. The activity keeps its own `body_succeeded` flag; only the finally body is shared. Worth doing when `ApiRunner.validate` lands (third consumer), so the unification is proven against all three call shapes at once rather than guessed at now.
