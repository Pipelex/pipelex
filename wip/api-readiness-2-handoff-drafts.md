...
**5. `docs/errors/` site is currently incomplete.** This branch ships with 23 fewer pages under `docs/errors/` than what PR #931 originally added. Plugin/factory error modules use deferred imports and never reach the new `__subclasses__()`-based discovery, so their pages were physically deleted by an orphan-cleanup commit. The dropped class names are listed in the "Phase 6 follow-up: `/code-review` pass at xhigh effort" session log entry in `_for_api/TODOS.md`. Their `PipelexError.type_uri()` values still resolve and point at 404s on docs.pipelex.com until Phase 7 closes the discovery contract. **Awareness only — no code change needed in pipelex-api.**
