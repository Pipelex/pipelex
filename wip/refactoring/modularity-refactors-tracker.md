...
> **Moved here from the repo-root `TODOS.md` on 2026-07-29**, so that slot is free for whatever is actively being built — the same treatment the hub-split tracker got in [`wip/hub/`](../hub/hub-split-tracker.md). The track is **parked, not archived**: everything below is unedited except relative links, which now resolve from `wip/refactoring/`.
>
> ⚠ **The branch/PR line below is a historical record — the work shipped under different numbers.** Verified 2026-08-01: M3 + M1 + M2 + F1 all reached `dev` inside the squash `8c0b99b3a` *"fix(hub): move the interpreter-touching built-in plugins out of the runtime layer, and make the layering guard transitive (#1064)"* — it is the commit that created `pipelex/pipe_machinery/`, `pipelex/mthds_parsing/`, `pipelex/providers/` and `pipelex/interpreter_plugins/`. PR **#1067 therefore has no commit of its own on `dev`**, and the branches (`refactor/Modularity-3`, `refactor/Modularity-4`, `refactor/Hub-2`) and the `_hub` worktree are all deleted. #1070 and #1071 landed as their own commits. Read the body as of 2026-07-28; re-verify any ref before acting on it.
>
> **What is actually left: [Phase 5](#phase-5--close-out) — the release-gated cross-repo sweep**, shared with the hub split, the Phase 3 type moves and the `interpreter_plugins` relocation. It must land as one breaking wave. v0.41.0 and v0.42.0 have since shipped, so the release gate that deferred it is open.

**Design doc:** [`modularity-refactors.md`](modularity-refactors.md) — the *why*, the rulings, the measurement snippets. This file is the *how*: ordered work, checkboxes, hard checkpoints.

**Worktree:** `_hub/` · **Branch:** `refactor/Modularity-3` (M3 + M1 + M2 + F1, pushed at `6e74c6f26`; `refactor/Modularity-4` merged in via PR #1065 and deleted) · **Base:** `refactor/Hub-2` (PR #1064, still open — see [Gating](#gating)) · **PR:** [#1067](https://github.com/Pipelex/pipelex/pull/1067), open **against `refactor/Hub-2`** so the diff carries the modularity work only; GitHub retargets it to `dev` when #1064 lands · **Status:** **F1 complete and reviewed three times — CHECKPOINT F1 cleared.** All four tracks reviewed and their fixes committed. Only Phase 5 remains.
...
- [x] Re-run the [core classification snippet](modularity-refactors.md#measurement) and paste the result into [Measurements](#measurements) below with today's date. — reproduced the [Measurements](#measurements) table exactly (97 / 51×3 / 33 / 30×4, same nine modules).
...
#### Review round — M1, done 2026-07-28, fixes applied
...
**Deferred, not fixed** — placement and naming accuracy, no silent bug, recorded in [`deferred-placement-follow-ups.md`](deferred-placement-follow-ups.md): three modules in `mthds_parsing/` that are not parsing (`pipe_sorter`, the generic half of `helpers`, `handle_pipe_errors`); the parser fixture corpus still filed under `tests/unit/pipelex/core/`; and the `pipeline`/`pipe_run` leaf leak. `pipe_machinery/` was checked module by module and is accurately named throughout.
...
#### Review round — M2, done 2026-07-28, fixes applied
...
#### Review round — F1, done 2026-07-28, fixes applied
...
- [x] **One PR is open — [#1067](https://github.com/Pipelex/pipelex/pull/1067), based on `refactor/Hub-2` rather than `dev`.** [Gating](#gating) said no track may open against `dev` until #1064 lands, or the diff carries #1064's changes. Stacking on the base branch satisfies that without waiting: the diff is the modularity work only, and GitHub retargets the PR to `dev` automatically when #1064 merges. **Still verify the retarget happened** once #1064 lands — an auto-retarget is not a merge, and the base must read `dev` before this one goes in.
- [ ] Write the **cross-repo sweep handoff** into `wip/refactoring/` — the sweep is release-gated and shared with several other tracks, each of which keeps its own consumer table in [`../hub/hub-split-tracker.md`](../hub/hub-split-tracker.md): the [hub split](../hub/hub-split-tracker.md#cross-repo-sweep), the [Phase 3 type moves](../hub/hub-split-tracker.md#cross-repo-impact-added-by-phase-3), the [Phase 4 `concept_provider=` signature break](../hub/hub-split-tracker.md#cross-repo-impact-added-by-phase-4), the [`interpreter_plugins` relocation](../hub/hub-split-tracker.md#cross-repo-impact-added-by-the-f1-remedy), and the [layer-placement third wave](../hub/hub-split-tracker.md#third-wave--the-layer-placement-track-refactorlayer-boundary). It must land as **one breaking wave**, not one per track. Known external hits **for the M1 module set**:
  ...
  **Re-measured 2026-07-27 across every repo in the workspace.** The previous table was materially incomplete — it named one repo; there are eight. It also called `pipelex-temporal` "private and unchecked" when it is checked out at the workspace root and carries the most hits of any consumer.
  ...
  ⚠ **This table is the M1 slice — it is not the sweep inventory.** The inventory is the union of it and the hub tracker's tables linked above. Repos appear there that appear nowhere below — the [layer-placement third wave](../hub/hub-split-tracker.md#third-wave--the-layer-placement-track-refactorlayer-boundary) alone adds `test-bed`, `pipelex-workshop`, `pipelex-demos`, `pipelex-demo-frostbeam`, `pipelex-demo-mistral` and `pipelex-demo-vibe`. Read a repo's absence from both columns below as *"not measured for this wave"*, never as *"clear"* — that misreading is what the hub tracker records as the way the first pass silently lost six real breaking sites.
  ...
  **The four wire-string consumers are the dangerous ones**: they branch on the error class name as an `error_type` value, so the rename does not break their build — they silently fall through to a generic error branch. No Python grep or type checker finds them.
  ...
  Verified clean **for the M1 module set only** — this is not a clearance for the sweep: `pipelex-cookbook`, `cocode`, `pipelex-mistralai-workflows`, `pipelex-worker`, `pipelex-starter-python`, `pipelex-relay`, `sandbox`, `pipelex-daytona-sandbox` (it imports `pipelex.plugins.contract` / `registrar`, which do not move). ⚠ The first three of those **do** carry hits in the other waves — `cocode` and `pipelex-cookbook` in the hub split and the layer-placement third wave, `pipelex-mistralai-workflows` in the hub split, Phase 3 and the third wave — so they must still be visited. **M2 and F1 contribute nothing to this sweep** — zero external consumers import `pipelex.plugins.<vendor>`, and zero import either img-gen mapping under any name (re-measured across every repo at F1 time; the only hit anywhere is the released `pipelex/` checkout itself). No kajson-registered class moves, so serialized payloads are untouched.

  **Partly executed in the field, re-measured 2026-08-01** (`for repo in $(ls -d */ | tr -d '/'); do grep -rln --include='*.py' -E 'pipe_run\.pipe_run_mode|pipeline\.pipeline_models|from pipelex\.hub import|pipelex\.graph\.graph_rendering' "$repo"; done`, skipping the `_*` worktrees). That pattern covers the **hub-split and layer-placement** paths only — the M1 renames above are a different set and are not measured by it. On those paths most consumers were already swept as they bumped: `pipelex-temporal`, `pipelex-api`, `pipelex-transport`, `pipelex-mistralai-workflows`, `pipelex-cookbook`, `cocode`, `test-bed`, `pipelex-workshop`, `pipelex-demos` and `pipelex-demo-frostbeam` all measure zero. **The only consumers still on removed paths are `pipelex-demo-mistral` and `pipelex-demo-vibe`, 10 files each** — and both pin `pipelex[...]>=0.35.0` *unbounded*, so they resolve to the current release and are broken on a fresh install right now, not at sweep time. They are also exactly the two repos that appear nowhere in the table above, which is the concrete cost of reading this slice as the inventory.
...
> **Assessed and written up as an implementation handoff: [`modularity-review-follow-ups.md`](modularity-review-follow-ups.md).** Each of the three open items below was re-verified against the tree on 2026-07-28 at `7ddb77f87` and carries a stable id there — **FU-1**, **FU-2**, **FU-3** — plus a concrete recommendation, reproduction commands and sizing. Cite the ids. The bullets below remain the record of *what was noticed*; the handoff is the record of *what to do*. Target branch: `refactor/Hub-2`, after #1067 merges into it.

- [ ] **FU-1 — Decouple `error_type` from the Python class name.** Give each `PipelexError` an explicit stable wire identifier instead of deriving `error_type` and the docs `type_uri` from the class name. Every error-class rename is currently a silent break for any consumer switching on the string, and nothing in the repo flags it. Evidence: `mthds-starter-js/src/lib/errors.ts:176`, `pipelex-starter-js/src/lib/errors.ts:257`, `vscode-pipelex/.../cliValidationBackend.test.ts:99`, `playroom/src/app/api/graph/route.ts:119` — **all four re-confirmed live 2026-07-28, still branching on the retired `PipelexInterpreterError`**. Would also remove the need for the `redirect_maps` workaround. Independent of all three tracks; touches the error model and the agent-CLI JSON envelope, so it needs a matching spec/conformance change. → The assessment **splits this in two**: a golden identity snapshot closes the "nothing flags it" half cheaply and with no spec change, and is what should land before the sweep; the `_declared_error_type` refactor is deferred, since the no-backward-compat rule means it cannot save the four consumers anyway. Note `title` / `type_uri` already have the `_declared_*` hatch — `error_type`, the only one machines branch on, is the one that lacks it. See [FU-1](modularity-review-follow-ups.md#fu-1--nothing-flags-an-error-class-rename-and-it-is-a-wire-break).
- [ ] **FU-2 — Close the two holes in the img-gen neutrality guard, or downgrade its comment to what it proves.** Raised by the [third F1 review round](#third-review-round--review-done-2026-07-28); recorded rather than fixed because both are cheap and neither is worth machinery on a track whose whole argument was "placement, not indirection". (a) **Transitive vendor SDKs are unguarded.** `_import_roots` reads only the mapping modules' own import statements and `_CLOSURE_SCRIPT` counts only `pipelex.providers`, so an `openai` import reached through some other `pipelex.cogt` module passes both checks. ~~Measured clean today — **zero** vendor-SDK imports anywhere in `pipelex/` outside `providers/` — and *that* is the stronger invariant, unguarded and worth a repo-wide test of its own rather than a denylist bolted onto this one.~~ (b) **The glob under-claims its own coverage.** `_MAPPING_DIR.glob("img_gen_*_mapping.py")` is non-recursive and suffix-keyed, so "a third taxonomy family is covered the day it lands" holds only for a family landing in that exact directory under that exact name. Either bind the convention with a test or soften the comment.

    ⚠ **Correction, 2026-07-28 — the struck sentence is false, and it was the stated justification for a test that would be red on day one.** Struck rather than silently rewritten, because the defect is instructive: five modules outside `providers/` do import a vendor SDK — `tools/pdf/pypdfium2_renderer.py` (module-level), `tools/storage/s3_storage_provider.py`, `tools/storage/gcp_storage_provider.py`, `tracing/dynamodb_event_log.py`, `reporting/reporting_manager.py`. All legitimate: they are *infrastructure* SDKs (pypdfium2, boto3/botocore, google.cloud), which is exactly the shim-vs-implementation split M2's own review recorded for `providers/storage/`. The invariant that **is** true and worth pinning is narrower — `pipelex/cogt/**` imports zero vendor SDKs, and `cogt → pipelex.providers` is exactly the documented five statements. [FU-2](modularity-review-follow-ups.md#fu-2--the-img-gen-neutrality-guard-is-weaker-than-its-comment-and-its-recorded-remedy-is-wrong) carries the scan that produced this and the corrected remedy. **This is the fourth time on this branch that a check's justification, rather than the check, was the defect** — and the first where the bad justification was written into a *deferred* item, where nothing would have re-tested it before someone acted on it.
- [ ] **FU-3 — Gate the bookkeeping files a bulk rewrite silently breaks.** ⚠ Re-measured 2026-07-28: **both artifacts are clean today** (`subject_grants.toml` 0 out-of-order pairs; `.test_durations` 0 dead file paths, 2 node-id orphans that are the known profile-dependent `pipe_img_gen` parametrizations) — so this is about the *missing gate*, not a live defect. [FU-3](modularity-review-follow-ups.md#fu-3--the-bookkeeping-files-a-bulk-rewrite-breaks-are-still-ungated) recommends doing the sort check inside the existing `keyword_only_guard.py` and gating durations on the **file path** rather than the node id — a path check is unambiguous, needs no collection run, and needs no tolerance policy for the two benign orphans. Three instances on this branch, each found by hand well after the fact: `subject_grants.toml` losing its sort (fixed in `bc22ba934`), the matched triple disagreeing on `pipe_signature` (fixed in M3), and `.test_durations` carrying 112 orphan node ids across three commits (fixed in the third F1 round). All three share a shape — a generated or bookkeeping artifact that no gate validates, invalidated by a bulk path rewrite, invisible until someone measures it. A cheap `make check` addition covers two of them: assert `subject_grants.toml` keys are sorted, and assert every `.test_durations` node id resolves against a full unfiltered collection. ⚠ The collection must run with `-m ""` — the marker filter in `addopts` hides ~312 items, so a naive check would report every `e2e` entry as an orphan.
...
- **Where — updated 2026-08-01: everything below shipped; start a fresh branch off `dev`.** The code is on `dev` and only [Phase 5](#phase-5--close-out) is left. The historical record: the work was built in worktree `_hub/` on one branch `refactor/Modularity-3` (`6e74c6f26`, carrying M3 + M1 + M2 + F1), based on `refactor/Hub-2` (PR #1064), with F1 developed on `refactor/Modularity-4` and merged in through PR #1065 — the separate-PR topology [Gating](#gating) prescribed, honoured and closed. All of it reached `dev` inside the squash `8c0b99b3a` (#1064), so **PR #1067 has no commit of its own on `dev`**; the worktree and all three branches are deleted. Related memory: `project_modularity_refactors.md`, `project_hub_split_refactor.md`.
...
- **Blocking:** ~~PR #1064 must merge before #1067 can retarget to `dev` and go in.~~ **Nothing — #1064 merged, and v0.41.0 / v0.42.0 have shipped, so the release gate on the Phase 5 sweep is open too.** **No open decisions.**
- **✅ The two cross-repo edits survived and are committed** — `conformance/tests/pipelex_transport/test_data.py` (`ALLOWED_SURFACE` now reads `pipelex.pipe_machinery.pipe_abstract`) and workspace-root `docs/specs/pipelex-transport-boundary.md`, both verified clean in their own repos on 2026-08-01. They were flagged here as uncommitted and at risk; they are neither. Nothing to recover before Phase 5.
