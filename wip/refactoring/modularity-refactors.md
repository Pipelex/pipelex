...
Core module classification, run from the repo root on a synced venv. Reports, per `pipelex.core.*` module, how many interpreter modules it loads and whether it drags in `interpreter_hub`. After M1, extend `I` with `"mthds_parsing"` and `"pipe_machinery"` (see M1 move 4):
...
I = {"libraries", "pipe_operators", "pipe_controllers", "codegen", "builder", "interpreter_plugins", "pipe_signature"}
