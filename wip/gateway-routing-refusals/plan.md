...
- **Checkpoint 2 — implementation landed on this branch (2026-09-02).** Steps 1–6 are done. `GatewayRoutingRefusal`, `_GATEWAY_ROUTING_REFUSAL_BY_CODE` and the `gateway_routing_refusal` property sit beside the two existing families; the Classify step gains the flag field, `_classify_gateway_routing_refusal` and a third branch ahead of the status ladder; the Render step gains `_render_gateway_routing_refusal_detail` and a third branch ahead of the action-kind match; `tests/unit/pipelex/cogt/inference/test_gateway_routing_refusals.py` covers the chain; the error-model page gained a third `####` half and the closing paragraphs were corrected; the changelog entry sits beside its two siblings. Nothing in the design section changed while building it — the four arms, the `CONTACT_SUPPORT` ruling for `pig-02` and the flag on `pig-01` alone all landed as ratified. What the build added beyond the design:

  - **`pig-05` is not reachable from today's runtime at all**, which is stronger than the design's "the runtime's `GoogleLLMWorker` pointed at the gateway". No worker speaks Google's native protocol to a Pipelex-operated gateway: both `gateway_plugin.py` and `manifold_plugin.py` build their LLM workers on the OpenAI substrate, and `ManifoldNativeClient` serves only the `/v1/pipelex/*` extract and search routes. The map entry stays — it is right the day such a path is wired, and it costs nothing — but no Extract hop is pinned for it, and the module docstring and the docs page both say why.
  - **The Google Extract hop would not carry a gateway code even then, and returns a wrong one.** `_google_provider_error_code_from_details` reads `error.status` and then a top-level `status`; the gateway's envelope has no `error.status` and a top-level `status: "failure"`, so the hop returns the string `"failure"` as `provider_error_code` — a value that looks like a real code. Probed, pinned as the trap it is (`test_the_google_hop_would_not_carry_a_gateway_code_and_nothing_reaches_it_today`), stated in the docs, and filed as L-260902-a8f50b to be fixed *before* any native Google path is pointed at a gateway.
  - **The three-way disjointness is asserted as sets, not only per code.** The unresolvable-references module's two-way pins stayed where they are; this module adds a set-level case over all three private maps, which is the direction a per-code table cannot see.
  - **A local environment repair, unrelated to the change**: this worktree's `.venv` carried shebangs from its former path (`_two`), so `pyright`, `pytest` and 97 other console scripts failed with `bad interpreter`. Rewritten in place; nothing in the repo changed.

  Next: open the stacked PR for review with `Closes L-260831-9963b5` in its body.

- **Checkpoint 3 — reviewed on PR #1185 (2026-09-02).** A `/review` pass over the diff, with an adversarial second read, produced five changes and one correction. Nothing about the four arms, the `CONTACT_SUPPORT` ruling or the flag placement moved; what moved is the honesty of three claims and the reach of the tests.

  - **`pig-01` is hedged, because the code covers more than the design assumed.** The gateway's body reader returns "no model" from a bare `catch` around both the JSON and the multipart read (`pipelex-manifold/src/pig/modelResolver.ts:255-270`), so a body it cannot parse earns `pig-01` alongside "this gateway serves no such model" and "the body names none" — three facts under one code. `is_model_not_found` still fires for all of them, which is right for the common case and cannot be narrowed from the code alone, so the detail now sends the reader to the gateway's own message before it sends them to the deck rather than asserting a deck-versus-gateway disagreement outright. The real fix is gateway-side and is filed as [L-260902-701614](http://localhost:4747/i/L-260902-701614), on the precedent of `pig-12` being drawn out of `pig-09`.
  - **The `pig-03` scope claim is narrowed to what its evidence supports.** "No client the runtime ships produces any of those" was too strong twice over: `test_manifold_clients.py` pins four steering headers *by name* while the gateway refuses on an allow-list, so a `portkey_ai` release sending some other `x-portkey-*` header would earn `pig-03` with that test still green; and `gateway_img_gen_worker.py` does send `x-portkey-config` on every image call, which Portkey's cloud reads and a Pipelex-operated gateway refuses — harmless while `pipelex_gateway` keeps its default endpoint (`gateway_factory.py`: `backend.endpoint or PORTKEY_GATEWAY_URL`), a client bug the day that backend names one of ours. The code stays out of the family; the sentence supporting that decision no longer overstates.
  - **The family's HTTP answer moved from 422 to 500, and now says so.** `CONFIGURATION` implies `ErrorDomain.CONFIG`, which renders as 500, where the `CONTENT` these four used to receive implies `INPUT` and renders as 422. That is the right answer in kind and the same one the status ladder's 404 arm has always given — but it puts these in server-error alerting and hands outer clients a status they retry, which is worth stating beside "never retried" (a claim about `InferenceErrorCategory.is_retryable` inside this process, and about nothing else). Documented in the error-model page and the changelog, and pinned by `TestWhatTheFamilyAnswersOverHTTP`.
