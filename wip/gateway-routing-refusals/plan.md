...
### A ride-along the inventory turned up: `pig-12`

Since the unresolvable-references work landed, the manifold split "no bucket configured" out of `pig-09` on the LLM routes into its own code, `pig-12` at 400 (manifold commit "An unserved storage scheme is the operator's fact, so it stopped reading as the caller's", on the manifold's `dev`). The runtime does not map it, so `pig-12` today falls through to the 400 arm and tells a caller to revise their prompt for a deployment that serves no storage at all — the exact mis-reading the native routes' `pipelex_storage_uri_unsupported` was given `STORAGE_NOT_SERVED` / CONTACT_SUPPORT to avoid. And the runtime's own doc still says `pig-09` "folds in causes that are not the caller's to repair — no bucket configured", which the gateway no longer does.

This is the same file, the same mechanism and one map entry, so it rides along rather than becoming a fourth ledger item: `pig-12` → `GatewayUnresolvedReference.STORAGE_NOT_SERVED`, one row in the unresolvable-references table, the stale `pig-09` paragraph rewritten, one row in that family's test table. It is listed as its own step below so a reviewer can see it apart from the routing family.
...
| `DISABLED_INTEGRATION` | `pig-02` | `CONFIGURATION` / `CHANGE_MODEL` | **set** | the model is served by an integration this deployment has not enabled — pick a model on an enabled integration; the message names the integration and, for whoever operates the gateway, the variables to set |
...
- **`CHANGE_MODEL` throughout, and `is_model_not_found` set only for `pig-01` and `pig-02`.** The flag is not a category: it selects the `*ModelNotFoundError` class, which `pipe_operator.py` catches and re-raises as `PipeOperatorModelAvailabilityError` carrying the model handle — the pipe-level error a caller already gets when the deck itself cannot find a model. `pig-01` is literally that case seen from the gateway. `pig-02` is a model that is *effectively* not servable: the handle resolves, but nothing behind it can answer, and the caller's move (pick another model) and the error class that names the handle are both the same as for `pig-01`. `pig-05` and `pig-06` leave the flag unset because the model *does* exist and *is* served — it cannot do what was asked, or was asked over the wrong protocol — and the render detail carries that distinction; the error class stays the family's generic failure class with the CHANGE_MODEL advice.
- **`pig-02` is not `CHECK_CREDENTIALS`.** The missing credential is the gateway operator's, not the caller's. In the hosted plane `CHECK_CREDENTIALS` would send a caller to rotate their own perfectly valid key; in a self-hosted deployment the caller is the operator and the gateway's message already names the integration and the variable. `CHANGE_MODEL` is the move that is right for both. This is the one arm a reviewer might want as `CONTACT_SUPPORT` instead — the deployment's gap rather than the caller's — and the reason not to is that, unlike `STORAGE_NOT_SERVED`, another model *does* avoid it, so the caller holds a repair the operator does not. If review prefers `CONTACT_SUPPORT`, it is a one-line change with the flag then unset too.
...
- Ride-along: add `"pig-12": GatewayUnresolvedReference.STORAGE_NOT_SERVED` to `_GATEWAY_UNRESOLVED_REFERENCE_BY_CODE`, and amend the `REFERENCE_UNRESOLVED` and `STORAGE_NOT_SERVED` docstrings so `pig-09` no longer claims the no-bucket cause and `STORAGE_NOT_SERVED` names both its wire codes.
...
- Add `_classify_gateway_routing_refusal(*, refusal: GatewayRoutingRefusal)` returning `CONFIGURATION` / `CHANGE_MODEL` for every member, with `is_model_not_found=True` for `UNKNOWN_MODEL` and `DISABLED_INTEGRATION` only.
...
- **Classification**: every member is `CONFIGURATION` / `CHANGE_MODEL` and never retryable; `is_model_not_found` is set for exactly `UNKNOWN_MODEL` and `DISABLED_INTEGRATION`; a bare 400 without a recognized code still takes the status ladder.
...
- **Rendered advice**: no member renders the generic "review the prompt, parameters, and inputs" line; `pig-01` and `pig-05` are told apart; `pig-02` does not mention the caller's credentials.
...
- Ride-along: add `("pig-12", GatewayUnresolvedReference.STORAGE_NOT_SERVED)` to the unresolvable-references module's table; its map-coverage case fails until it is there, which is the point of that case.
...
- Ride-along: add the `pig-12` row to the reference table and rewrite the paragraph claiming `pig-09` folds in the no-bucket cause.
- `CHANGELOG.md` under `[Unreleased]` → Added: one condensed entry in the style of the two existing gateway entries, with a sentence for `pig-12`.
...
- **Checkpoint 1 — plan ratified.** This document reviewed; the arms confirmed or amended (the `pig-02` question above is the one most likely to move). Flip `status` to `active` in the same change that records the ruling.
