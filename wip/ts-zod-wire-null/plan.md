...
**Follow-up: the structural one is now closed.** [L-260901-26da36](http://localhost:4747/i/L-260901-26da36) was completed by `pipelex#1184`, squashed onto `dev` as `753737d459ddcdebbdaafd8eb53ecd7aa314ece7`. `make test-ts-gates` provisions a pinned prettier and zod into a gitignored `.ts-toolchain/` and runs the codegen suite with `PIPELEX_REQUIRE_TS_GATES=1`, under which the two toolchain-dependent gates fail instead of skipping; the `Tests (ts emission gates)` CI job runs that target behind `actions/setup-node`, and `Tests (all)` requires it. The `_ts_object_key` deferral recorded in round 2 was picked up in the same PR — the new gate reddened on it before the fix was written. [L-260901-47759d](http://localhost:4747/i/L-260901-47759d), the recursive-concept print-width overflow, is still open, and the deliberate absence of a `tsc --strict` gate is recorded with it in `docs/contribute/typescript-emission-gates.md`.
