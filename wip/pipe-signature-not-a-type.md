...
**Spec-layer `inputs` is required, unlike the optional blueprint `inputs` (Phase-3 finding).** A no-inputs signature (`{description, output}` only) is valid at the blueprint/language layer (`PipeBlueprint.inputs` defaults to `None`), but the spec/authoring layer requires `inputs` (`PipeSpec.inputs: dict[str, str]` has no default), so a no-inputs signature fails at the spec layer with a raw pydantic `inputs Field required`. This asymmetry **predates this feature** — `PipeSpec.inputs` has always been required — and is orthogonal to the type-tag removal; the single-pipe `pipelex-agent pipe` command and bundle-spec authoring both inherit it (a no-inputs signature can only be authored at the blueprint level, e.g. hand-written `.mthds`). Fixing it means making `PipeSpec`/`PipeSignatureSpec` `inputs` optional to match the blueprint, a small change touching every spec — deferred, out of scope for the type-tag removal.
