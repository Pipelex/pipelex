...
| Note | What it defers |
|---|---|
| [`runtime-boot-external-interpreter-orchestrator.md`](runtime-boot-external-interpreter-orchestrator.md) | **RESOLVED.** A runtime-only boot used to accept an interpreter-layer orchestrator contributed by an *external* entry-point plugin, because `build_registrar` discovered externals unconditionally. The plugin entry-point group split supplied the missing layer signal and scoped discovery, so the name is no longer registered and the existing gate rejects it. Neither proposed remedy was built. |
