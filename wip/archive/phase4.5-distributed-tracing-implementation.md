...
- [x] Test: deduplication handles Temporal replay (emit same events twice, assembler produces correct graph) — `test_ndjson_dedup_assembly.py`
- [x] Test: pipeline failure produces partial graph with FAILED/CANCELED nodes — `test_partial_failure` in `test_graphspec_assembler.py`
...
- [x] Test: graph output files (Mermaid, ReactFlow) generated correctly from assembled GraphSpec — `test_graph_output_generation.py`
- [x] Test: two-pass assembly handles cross-workflow producer/consumer in arbitrary workflow_id order — `test_cross_workflow_producer_map` in `test_graphspec_assembler.py`
...
**Next steps**: See [wip/01-master-plan.md](wip/01-master-plan.md) for Step 6 (Standalone Activity Tracing), Phase 6 (Cross-Package Dependencies), and Future Phases.

**Deferred items**: See [wip/deferred-items.md](wip/deferred-items.md).
