...
> **Superseded as the live tracker.** The current-state reference and reviewer verification map now live at [`../signature-based-validation.md`](../signature-based-validation.md). This file is kept as the phase-by-phase TDD build record (historical); where it disagrees with the live tracker on present behavior, the live tracker wins. Internal references to "TODOS.md" below mean *this* file under its original name.
