...
Implemented design: `COMPOSITE = "Composite"` on `NativeConceptCode` with a `CompositeContent(StuffContent)` structure class that holds the named branch contents as top-level fields (pydantic `extra="allow"` so the branch names surface at the top level of the serialized content, exactly like the typed path — not nested under a wrapper key). It supports the full content surface: `smart_dump`, kajson round-trip (`__json_encode__` hands kajson the live components), transport round-trip (`dump_for_transport` stamps each component with `__pipelex_class__`/`__pipelex_module__` markers and hydration rebuilds typed contents), `rendered_markdown`/`rendered_html` (iterate named sub-contents), so `main_stuff.json/md/html/viewer` delivery works unchanged.
