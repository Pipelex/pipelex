...
## Gemini-taxonomy models routed through an args-dict img-gen worker would silently drop the resolved size

The GEMINI branch of `ImgGenArgsFactory.make_args_from_aspect_ratio` resolves and validates the `(aspect_ratio, size)` pair through the taxonomy grids, then returns only the ratio literal — by design, because the two real gemini wire paths build their own `image_config` (the native `GoogleImgGenWorker` directly, the gateway via `GatewayFactory.make_extras` on the `gateway_completions` path). But the args-dict workers (`GatewayImgGenWorker`/`gateway_img_gen`, fal, HuggingFace, azure_rest, openai) also consume this branch: if a gemini-taxonomy model were ever routed through one of them, a requested `2k`/`4k` would validate client-side and then silently fall back to the provider's 1K default. Today that routing doesn't exist — the gateway remote catalog defaults gemini img-gen models to `gateway_completions` (no per-model `sdk` override), the kit deck uses the native `google` sdk, and the Phase 4 live e2e proved exact 2K pixels through `all_pipelex_gateway`. Hardening would require splitting "validate the pair" from "build wire args" (the same branch backs `img_gen_param_support` checks, which must not raise on valid sized pairs). Revisit if the remote catalog ever re-routes a gemini image model to `gateway_img_gen`.
