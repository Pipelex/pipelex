...
## Phase-5 triage (Smart Inputs, 2026-07-08) — RE-DEFERRED

Smart Inputs landed the shaper, but it did **not** collapse this split — and the reason is instructive, not an oversight. The loader (`_convert_temporal_inputs`) runs **before** the pipeline-inputs seam, at the CLI-format layer: it turns a top-level TOML date/datetime *literal* into a `DateContent` while the file is being parsed, with no view of the pipe or its concepts. The shaper runs **at** the seam, top-down against the declared concept. They are genuinely different steps at different layers — the loader's early conversion is what lets a bare TOML date reach the seam as a `StuffContent` (so it takes the shaper's already-built-`StuffContent` path rather than a bare-date arm). Collapsing them would mean either moving concept-aware shaping up into the format loader (wrong layer — the loader has no signature) or deferring the loader's literal conversion down into the shaper (which would still leave the no-signature bottom-up path needing its own conversion). The two-line `date=v.date(), time=v.timetz()` split is fixed by `datetime`'s own API (near-zero drift risk), so the original deferral rationale — different responsibilities, cross-layer-coupling cost, low drift — stands unchanged. Re-deferred to a dedicated `tools`/loader pass if the loader is ever restructured; not worth cross-layer coupling now.
