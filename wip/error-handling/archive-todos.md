...
- **Authored pages list:** _(none yet — initial run emits 215 generated pages, all of which can be claimed for hand-editing by adding `<!-- gstack:authored -->` as a standalone line at the top of the page; the generator then preserves it on every subsequent run)._
- **Files touched:**
    - Production:
...
- **Surprises / deviations (record in `api-companion-revisions.md` §E):**
    ...
    7. **`errors/index.md` landing page added to the generator and to the docs nav.** Listed once under "Reference > Error Reference", links every per-class page grouped by top-level `PipelexError` branch (CogtError / PipelineExecutionError / TemporalFlowError / etc.). Best-practice take on the user's nav question: 215 leaf pages in the sidebar would be unreadable, but a single landing page in nav makes the section discoverable while the per-class pages stay reachable via direct `type_uri` dereference. Generator emits the index alongside the per-class pages and honors the same `<!-- gstack:authored -->` preservation rule; a new test pin (`test_index_page_links_every_class`) asserts the index contains a link to each loaded class.
