...
- **`recover_error_report(exc: BaseException) -> ErrorReport`** (`pipelex/temporal/tprl/temporal_error.py`) — total. When the failure carries no embedded report, or `from_dict` fails on a malformed / schema-drifted payload, the function synthesizes `UnrecoverableWorkflowFailureError(_message_from_exc(exc)).to_error_report()`. `UnrecoverableWorkflowFailureError` lives in `pipelex/temporal/exceptions.py` next to `WorkflowExecutionError` etc., with `error_domain=RUNTIME` and `_declared_title="Workflow failed without recoverable error details"`. Callers in `workflow_caller.py` (the three former `if error_report is not None` branches at `execute_workflow` / `execute_child_workflow` / `start_child_workflow`) now treat the return as always-usable.
