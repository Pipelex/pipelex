...
> **ARCHIVED 2026-05-28 — superseded.** Execution ledger for the `feature/API-readiness-2` branch (PRs #931 + #933 — error-handling overhaul finalization, structural refactor of `*_exceptions.py` modules, post-#931 `/review` follow-ups). All in-repo phases (0–7) landed. The only remaining track from this plan is webhook signing (Stage 5), tracked independently at [`../security/webhook-signing.md`](../security/webhook-signing.md). The cross-repo `pipelex-api` adapt PR is handed off — see [`../api-readiness-2-handoff.md`](../api-readiness-2-handoff.md). Moved here from the repo root for history. Relative links below were written relative to the repo root, not this `wip/error-handling/` location.
...
- **2026-05-27 — API-side adapt plan written at workspace level.** Cross-repo coordination scaffolding now in place. The pipelex-api agent in `../pipelex-api/` independently audited their TODOS + the pipelex-side `wip/api-readiness-2-handoff-drafts.md` and produced a recap of the four surface-touching deltas (Phase 6 import moves + `MthdsDecodeError` deletion; STRICT provenance keying; `WebhookTarget` reserved-key validator; native `JobMetadata.request_id`) plus their own deferred work (Phase 4 Temporal webhook recovery, Phase 5 docs/archive). Their plan: run pipelex-api's test suite against the editable pipelex first to scope the delta concretely, then walk the handoff list, then decide on Phase 4 fold-in. Cross-checked against the pipelex side — diagnosis is correct.

  **Workspace-level plan landed at `../wip/adapt-pipelex-api-to-feature-API-readiness-2.md`** — cold-start-safe agent prompt for a fresh session opened at `/Users/lchoquel/repos/Pipelex/` (workspace root, one level above both repos). Structure: workspace map → read-first list → delta summary → Phases A–H (validate baseline → fix imports → fix WebhookTarget → STRICT audit → wire native `request_id` → fold-or-split decisions on F/G → verify + ship) → verification matrix → don'ts → handoff checkpoint. Four `⛔ CHECKPOINT` hard-stops at the natural handoff points (A after baseline failure-bucketing; BC after mechanical fixes; DE when the adapt slice is shippable; H after PR is open) — each requires recording state in both `pipelex-api/TODOS.md` (new Phase A0 section) and this Session log, closing the cross-repo loop. Webhook signing (Phase 5) explicitly out of scope per the original handoff doc.

  **Decisions deferred to the API-side session** (surface at Checkpoint A, not pre-baked): (F) fold the deferred Temporal webhook recovery into the adapt PR or split — default fold-in since Phase E already does the `request_id` plumbing it depends on; (G) fold the deferred docs/archive — default split, docs deserve their own review cycle.

  **No code changes on the pipelex side in this session.** Branch `feature/post-pr933-followups` unchanged; the API-team feedback loop is the work that's been done. **Next action:** hand the workspace plan to a fresh Claude Code session at `/Users/lchoquel/repos/Pipelex/`, or pick up Phase 5 (webhook signing) on the pipelex side independently per [`wip/security/webhook-signing.md`](wip/security/webhook-signing.md) — the two tracks no longer block each other.
