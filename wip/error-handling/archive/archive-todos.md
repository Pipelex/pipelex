...
This worktree (`feature/API-readiness`) carries the **pipelex-side** companion work for the `pipelex-api` error-handling design. The original per-item spec is in [`wip/error-handling/changes-for-api-early-draft.md`](changes-for-api-early-draft.md); the deviations we are taking from that spec — and *why* — are documented in [`wip/error-handling/api-companion-revisions.md`](../api-companion-revisions.md). The cross-repo consumer (the API) lives in the side-by-side worktree [`../pipelex-api/wip/error-handling/`](../../pipelex-api/wip/error-handling/).
...
### [x] Item E — Per-class `type` URI doc pages *(spec item 7)*
...
- **Authors override per-class content where it adds value** by hand-editing the generated page; the generator detects an `<!-- pipelex:authored -->` marker and skips regeneration of that page.
...
| E | Per-class doc pages *(spec 7)* | 4 | [x] |
...
- A few high-traffic classes have authored overrides (marked with `<!-- pipelex:authored -->`); the rest use the generated default and look sensible.
- mkdocs build clean; `make agent-check` + `make agent-test` clean.

**Fill in at checkpoint time:**

- **Generator location + invocation:**
    - Library: `pipelex/errors/error_pages_generator.py::generate_error_pages(output_dir: Path)` — renders pages, returns an `ErrorPagesReport(written / unchanged / preserved)`.
    - Class walker: `pipelex/errors/error_module_registry.py::iter_pipelex_error_subclasses()` — force-imports every `*/exceptions.py` and known non-standard error module under `pipelex/`, then walks `PipelexError.__subclasses__()` recursively (filtering out `tests.*` modules so synthetic subclasses can never leak into the docs output).
    - CLI: `.venv/bin/pipelex-dev generate-error-pages [--output DIR] [--quiet]` — defaults to `docs/errors/`, bootstraps Pipelex so `PipelexError.type_uri()` resolves, then teardown.
- **Authored pages list:** _(none yet — initial run emits 215 generated pages, all of which can be claimed for hand-editing by adding `<!-- pipelex:authored -->` as a standalone line at the top of the page; the generator then preserves it on every subsequent run)._
- **Files touched:**
    - Production:
        - `pipelex/errors/error_module_registry.py` (new) — explicit + filesystem-scan-based registry of modules that declare `PipelexError` subclasses. Exposes `load_all_error_modules()` and `iter_pipelex_error_subclasses()`.
        - `pipelex/errors/error_pages_generator.py` (new) — `ErrorPagesReport` dataclass, `generate_error_pages()`, `render_error_page()`, `page_slug()`. Authored-marker detection is line-anchored (`_has_authored_marker`) so the generator's own instructional text never falsely flags a generated page.
        - `pipelex/cli/dev_cli/commands/generate_error_pages_cmd.py` (new) — Typer-friendly wrapper that bootstraps + tears down Pipelex around the generator call and prints a Rich status panel.
        - `pipelex/cli/dev_cli/_dev_cli.py` — registered `generate-error-pages` in `PipelexDevCLI.list_commands` and added the matching `@app.command(...)` declaration.
        - `pipelex/core/pipes/inputs/input_stuff_specs_factory.py` — **pre-existing bug fix** (flagged per repo CLAUDE.md). A duplicate `class InputStuffSpecsFactoryError(PipelexError)` shadowed the canonical one in `pipelex/core/pipes/inputs/exceptions.py`, producing two distinct class objects with the same name. The shadow caused `iter_pipelex_error_subclasses()` to enumerate 216 classes against only 215 unique kebab slugs (1 collision). Removed the local class definition, imported the canonical one (which inherits from `InputStuffSpecsError`, the correct hierarchy), and exposed it via `__all__` so the existing test import path (`from pipelex.core.pipes.inputs.input_stuff_specs_factory import InputStuffSpecsFactoryError`) still resolves.
        - `docs/errors/*.md` (215 generated pages, new) — one page per non-test `PipelexError` subclass. Each page carries: class identity table (`error_type`, `title`, `type_uri`, `error_domain`, defining module, parent-class link, class-level `user_action` when declared), a class docstring fragment when present, and a back-link to `under-the-hood/error-model.md`. Pages are stamped with `<!-- pipelex:generated -->` and instruct maintainers to add `<!-- pipelex:authored -->` on its own line to claim a page for hand-editing.
    - Tests:
        - New: `tests/unit/pipelex/errors/test_error_pages_generator.py` (`TestErrorPagesGenerator`) — emits-one-page-per-subclass pin, idempotency pin (rerunning over a generated tree writes nothing), authored-marker preservation pin, marker-detection precision pin (a marker embedded inside other text on the same line is NOT a claim), and the `ErrorPagesReport.total` arithmetic pin.
        - Updated: `tests/unit/pipelex/test_pipelex_error_type_uri_uniqueness.py` — refactored to use the shared `iter_pipelex_error_subclasses()` registry rather than its own ad-hoc import block, so the uniqueness guard and the docs generator walk the SAME class set (eliminating the prior risk where the test missed a hierarchy a new generator would still pick up).
- **Surprises / deviations (record in `api-companion-revisions.md` §E):**
    1. **Pre-existing duplicate class `InputStuffSpecsFactoryError` discovered and fixed.** Two distinct class objects with the same name lived in `pipelex/core/pipes/inputs/exceptions.py` (inherits from `InputStuffSpecsError`) and `pipelex/core/pipes/inputs/input_stuff_specs_factory.py` (inherited from `PipelexError`). The existing type-URI uniqueness test missed it because its hard-coded import block loaded only one of the two modules. The shadow only surfaced once `iter_pipelex_error_subclasses()` walked the post-bootstrap class set. Fix in scope per the repo CLAUDE.md "flag and fix pre-existing bugs" rule. The factory module now re-exports the canonical class via `__all__` so test imports (`from .input_stuff_specs_factory import InputStuffSpecsFactoryError`) keep working without rewrite.
    2. **Filesystem-discovery + non-standard-module list, not a hard-coded module list.** The plan was silent on registry strategy. I chose a `pipelex/`-rooted filesystem scan for `exceptions.py` / `*_errors.py` plus an explicit short list of non-standard modules (`system/environment.py`, `tools/misc/toml_utils.py`, `tools/secrets/secrets_utils.py`, `pipe_operators/shared/template_image_analyzer.py`) so new error modules under the standard naming convention are picked up automatically — no manual registry maintenance for the common case. The non-standard list is small and stable; adding to it is a one-line change.
    3. **Authored-marker detection is line-anchored, not substring-anchored.** The plan called for `<!-- pipelex:authored -->` as the claim marker but did not specify how to detect it. A naive `in`-substring check would have produced a self-trap: the generator's own instructional text references the marker, so every generated page would have been flagged as authored on the second run (verified — initial implementation reproduced the bug; idempotency test caught it before landing). Detection now requires the marker to appear as its own stripped line, which is the natural form a maintainer would write and not what the generator's prose contains.
    4. **`base_uri` retargeted to `https://docs.pipelex.com/latest/errors` — the actual docs host.** Original plan defaulted `base_uri` to `https://pipelex.dev/errors`, a domain that does not serve these pages. Per follow-up discussion, retargeted the default to the live MkDocs + mike docs site so `type_uri` dereferences directly to a real page. The `/latest/` alias is mike's canonical pointer at the current stable release (forced via `docs/overrides/main.html` already), so a `type` URI emitted by any deployed version always lands on up-to-date docs. Both `pipelex/pipelex.toml` and `.pipelex/pipelex.toml` updated; the `ErrorsConfig` docstring rewritten to match.
    5. **`type_uri()` now emits a trailing slash (`<base>/<slug>/`).** Verified MkDocs (with its default `use_directory_urls: true`) builds each `docs/errors/<slug>.md` as `errors/<slug>/index.html`, served at `errors/<slug>/`. The built page's `<link rel="canonical">` matches: `https://docs.pipelex.com/latest/errors/llm-completion-error/`. Without the trailing slash, clients dereferencing the URI would hit a 301 — minor cost but avoidable. Implementation: one-character change inside `PipelexError.type_uri()` (`f"{base}/{slug}/"`). All hardcoded test fixtures rewritten via a one-shot regex sweep (24 replacements across 13 files); the `_DEFAULT_TYPE_URI` constant in `tests/helpers/error_report.py` updated to the same shape.
    6. **`not_in_nav` directive added to `mkdocs.yml`.** Without it, `mkdocs build --strict` (= `make docs-check`) emits an INFO line listing the 215 generated per-class pages as "not in nav" — noisy and obscures real misses. The directive whitelists `errors/*.md` (excluding `errors/index.md`, which IS in nav) and `CLAUDE.md` (pre-existing docs-authoring guide that was producing the same noise). `make docs-check` now finishes with zero INFO/WARNING/ERROR lines.
    7. **`errors/index.md` landing page added to the generator and to the docs nav.** Listed once under "Reference > Error Reference", links every per-class page grouped by top-level `PipelexError` branch (CogtError / PipelineExecutionError / TemporalFlowError / etc.). Best-practice take on the user's nav question: 215 leaf pages in the sidebar would be unreadable, but a single landing page in nav makes the section discoverable while the per-class pages stay reachable via direct `type_uri` dereference. Generator emits the index alongside the per-class pages and honors the same `<!-- pipelex:authored -->` preservation rule; a new test pin (`test_index_page_links_every_class`) asserts the index contains a link to each loaded class.
    5. **Pre-existing baseline test failure persists.** `tests/e2e/agent_cli/test_offline_run_dry.py::TestOfflineDryRun::test_gateway_no_cache_no_network_fails_with_unavailable` still fails on `feature/API-readiness` for unrelated reasons (CLI subprocess returns an empty JSON payload instead of an `error_type`). Identical failure documented at Checkpoint 2 and Checkpoint 3 — NOT introduced by Stage 4 and not in scope for this checkpoint.
