...
**Status:** 🔎 review follow-up, **NOT implemented.** These are the highest-value findings from the code review of the fail-safe floor that landed on `fix/Temporal-failsafe`. None blocks the merge — the silent-hang hole *is* closed — but all four concern the single load-bearing mechanism the fix relies on, and they are best done together as one small refactor.
