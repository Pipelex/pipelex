...
| Temporal integration | Landed — `TemporalError.from_message_exception()` derives retryability from `InferenceErrorCategory.is_retryable` for category-carrying `CogtError`s (the `non_retryable_error_types` list is the fallback) and packs the full `ErrorReport` into `ApplicationError.details`. Every in-scope activity is decorated with `@convert_pipelex_errors`, so the bridge runs in production. **Open gap:** the report is packed into `ApplicationError.details` but not recovered on the submitter side, so a pipe failing on a Temporal worker reaches the CLI and HTTP adapters stripped of its classification — see the track's Open gaps. | [track-temporal-integration.md](track-temporal-integration.md) |
...
1. **Extract / Classify / Render decomposition** — proposed, not started. The per-worker classification pipeline is now uniform but duplicated across 18+ workers; this refactor consolidates it into one per-provider Extract function plus a shared Classify + Render. Its prerequisites are all met. See [track-extract-classify-render.md](track-extract-classify-render.md).
...
3. **Temporal submitter-side error recovery** — the error bridge packs the full `ErrorReport` into `ApplicationError.details` on the activity → workflow hop, but the symmetric workflow → submitter recovery is missing, so a pipe failing on a Temporal worker reaches delivery stripped of its classification. See the Open gaps in [track-temporal-integration.md](track-temporal-integration.md); full brief in `TODOS.md` at the repo root.
