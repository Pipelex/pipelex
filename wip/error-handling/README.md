...
| Retry & resilience | The application-level `PipeRouter` transient-retry loop has been removed — direct (non-Temporal) execution is a single pipeline-level attempt, and resilience is the Temporal track's job (activity `RetryPolicy` keyed off `InferenceErrorCategory`). `PipeBatch` still bounds fan-out via `gather_bounded` (chunked, `max_concurrency`, default 8) — admission control, not retry. Tier 1 (transport retry) is the provider SDKs' own retry of brief blips; making it an explicit, uniform policy is the open Workstream 2 in the track doc. | [track-retry-and-resilience.md](track-retry-and-resilience.md) |
