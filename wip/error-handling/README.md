...
| Retry & resilience | Landed — the application-level `PipeRouter` transient-retry loop has been removed (direct, non-Temporal execution is a single pipeline-level attempt; resilience is the Temporal track's job — activity `RetryPolicy` keyed off `InferenceErrorCategory`). `PipeBatch` still bounds fan-out via `gather_bounded` (chunked, `max_concurrency`, default 8) — admission control, not retry. Tier 1 (transport retry) is now an explicit, uniform policy: `cogt.transport_max_retries` is wired into every inference SDK client factory, the SDK-less `azure_rest` image-gen path gets a `tenacity` transport-retry floor, and `instructor`'s structured-output retry is confined to schema re-ask (no longer a transport-retry loop nested on Tier 1). | [track-retry-and-resilience.md](track-retry-and-resilience.md) |
