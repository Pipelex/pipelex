...
| Track | Status | Doc |
|---|---|---|
| Error metadata model | Mostly landed for LLM workers — inference errors self-describe via `to_error_report()`, and every LLM worker raise now carries structured `ProviderErrorMetadata` (status_code, request_id, retry_after_seconds, provider_error_code, body) plus `UserAction(kind, detail)` with semantic `UserActionKind` values. `InferenceErrorCategory.UNKNOWN` distinguishes unrecognized fallback paths from genuine `CONTENT`. Img-gen / extract / search workers still need migration; non-inference still depends on string-keyed dicts in `agent_output.py`. | [track-metadata-model.md](track-metadata-model.md) |
| Worker classification | Landed for LLM workers — every LLM worker (Anthropic, OpenAI Completions, OpenAI Responses, Mistral, Google) maps SDK exceptions to a `CogtError` subclass with `error_category`, unwraps `InstructorRetryException` correctly on structured-gen paths, and carries the metadata + user-action upgrades described above. Img-gen, extract, search workers and AWS Bedrock LLM still at the original "category-only" standard; beyond-reference upgrades A–C scheduled in `_tprl/TODOS.md` Phases 10–12. | [track-worker-classification.md](track-worker-classification.md) |
| Extract / Classify / Render | Proposed (not started) — decomposes the per-worker pipeline into one per-provider Extract function + shared Classify + shared Render. Cuts duplication across 18+ workers. Sequenced after the current `TODOS.md` sweep lands. | [track-extract-classify-render.md](track-extract-classify-render.md) |
...
4. [track-extract-classify-render.md](track-extract-classify-render.md) — proposed refactor for after the worker-classification sweep lands; decomposes the per-worker pipeline.
