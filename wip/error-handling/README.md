...
1. **Webhook payload reserved-key collision** — `WebhookTarget.payload` accepts caller keys that `_notify_webhook` later overwrites with Pipelex-owned keys (`pipeline_run_id` / `status` / `result_url` / `error`), so a static payload key shifts meaning with delivery status. Analysis and options: [track-webhook-payload-collision.md](track-webhook-payload-collision.md).
2. **Test-coverage backfill** — the `/review` pass found gaps where the refactor removed or never added test coverage. This phase backfills it.
3. **Webhook signing (Stage 5 / Item F)** — the last unshipped stage of the API-readiness companion plan. Cross-repo: the pipelex side and the API side land in lockstep. Authoritative plan: [../security/webhook-signing.md](../security/webhook-signing.md).
4. **Metadata-model long tail** — a handful of `CogtError` subclasses still carry no class-level `error_category`, and several non-inference `PipelexError` subclasses still depend on the `agent_output.py` fallback dicts for `hint` / `error_domain` rather than carrying class-level metadata. See the Followups in [track-metadata-model.md](track-metadata-model.md).

Items 1-2 are the remaining post-#931 `/review` follow-ups — an in-repo unit sequenced with checkpoints as Phases 3-4 in [../../TODOS.md](../../TODOS.md) (Phase 1, the STRICT-disclosure INPUT-domain gap, and Phase 2, the `request_id` log wiring, both landed 2026-05-22 — see [track-strict-disclosure-input-domain-gap.md](track-strict-disclosure-input-domain-gap.md)). Item 3, webhook signing, is the original plan's final stage — a separate cross-repo track that lands on its own schedule (TODOS Phase 5). Item 4 is an independent long tail. Everything else is landed and described in current-state terms in the track docs below. Optional, non-blocking review notes — observations that were deliberately not acted on, each with a conditional trigger for when to revisit — are collected in [review-notes/](review-notes/).
