...
| Retry & resilience | Landed — `PipeRouter` retries `TRANSIENT` `CogtError`s with config-driven exponential backoff (`max_transient_retries`, default 3); `PipeBatch` bounds fan-out via `gather_bounded` (chunked, `max_concurrency`, default 8). `tenacity` retry removed from the gateway workers. The router's retry decision unifies the raw-`CogtError` and `PipeRunError`-wrapping-a-`CogtError` cases, so transient LLM failures (wrapped by `PipeLLM` / `PipeStructure`) are retried in-process too — see the archived [todos-llm-retry-loop-bypass.md](todos-llm-retry-loop-bypass.md). | [track-retry-and-resilience.md](track-retry-and-resilience.md) |
