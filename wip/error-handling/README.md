...
1. **Webhook signing** — the last unshipped piece of the API-readiness companion work. Cross-repo: the pipelex side and the API side land in lockstep. Authoritative plan: [../webhook-signing.md](../webhook-signing.md).
...
4. **Temporal fail-safe floor — review backlog (parked, gated on Temporal productionization).** The silent-hang hole is *closed* (workflow-level catch-alls + `PipelexError` worker floor, landed on `fix/Temporal-failsafe`), but the code review left follow-ups open: a `SecurityError`-contract decision (Finding 3), small cleanups (5, 8, 10), a product question (12), and two findings from a later `/review` pass — the `_carries_temporal_failure` predicate's third fragility mode (Finding 13) and a pre-existing `build_search_attributes` replay-determinism hazard (Finding 14). **Live backlog:** [temporal-failsafe-review-followups.md](temporal-failsafe-review-followups.md). Fix rationale (✅ resolved, read-only): [temporal-error-handling-failsafe-gap.md](temporal-error-handling-failsafe-gap.md) (the "why it was the real defect" brief; also tracks deferred hardening C/D/F — submitter deadline, CI dispatch-guard, stuck-workflow observability) and [temporal-failsafe-guard-hardening.md](temporal-failsafe-guard-hardening.md) (the guard/consolidation findings 1/6/7/9).
...
7. [track-temporal-integration.md](track-temporal-integration.md) — extends the same model across the activity → workflow boundary. For the fail-safe floor that backstops it, the deep dive is the three `temporal-failsafe-*.md` docs (start at [temporal-failsafe-review-followups.md](temporal-failsafe-review-followups.md) for live work; the other two are resolved rationale).
