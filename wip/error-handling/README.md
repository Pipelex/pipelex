...
4. **`request_id` log wiring** — `JobMetadata.request_id` and the `request_id` kwarg on every `WorkflowLog` / `ActivityLog` method shipped, but no activity or workflow threads the value into a log call yet, so the kwarg is dead surface. See [api-companion-revisions.md](api-companion-revisions.md) §B.
