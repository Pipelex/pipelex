...
| Retry & resilience | **Superseded — see [track-retry-and-resilience.md](track-retry-and-resilience.md): the target architecture removes the application-level retry loop (direct execution = single attempt; resilience = Temporal).** Landed in current code — `PipeRouter` retries `TRANSIENT` `CogtError`s with config-driven exponential backoff (`max_transient_retries`, default 3); `PipeBatch` bounds fan-out via `gather_bounded` (chunked, `max_concurrency`, default 8). `tenacity` retry removed from the gateway workers. The router's retry decision unifies the raw-`CogtError` and `PipeRunError`-wrapping-a-`CogtError` cases, so transient LLM failures (wrapped by `PipeLLM` / `PipeStructure`) are retried in-process too — see the archived [todos-llm-retry-loop-bypass.md](todos-llm-retry-loop-bypass.md). | [track-retry-and-resilience.md](track-retry-and-resilience.md) |
...
| Temporal integration | Landed — `TemporalError.from_message_exception()` derives retryability from `InferenceErrorCategory.is_retryable` for category-carrying `CogtError`s (the `non_retryable_error_types` list is the fallback) and packs the full `ErrorReport` into `ApplicationError.details`. Every in-scope activity is decorated with `@convert_pipelex_errors`, so the bridge now runs in production — see Followup 5 in the track doc. | [track-temporal-integration.md](track-temporal-integration.md) |
...
Completed sweeps and plans are archived here, kept for their running notes and checkpoint history:
...
- [archive-temporal-activity-boundary.md](archive-temporal-activity-boundary.md) — wiring `from_message_exception` into every in-scope Temporal activity (Followup 5 of the Temporal-integration track).
