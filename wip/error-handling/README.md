...
| API readiness (error-handling companion) | Stages 1-4 landed (PR #931) — `PipelexError.title()` / `type_uri()`, `request_id` on `JobMetadata`, `DisclosureMode` with `to_dict(disclosure_mode=)` / `to_problem_document()`, total `recover_error_report`, `ErrorReport` as a `BaseModel` threaded to the webhook, and per-class error doc pages. Stage 5 (webhook signing) plus a set of post-#931 `/review` follow-ups are still open — see [What's still open](#whats-still-open). | [api-companion-revisions.md](api-companion-revisions.md) |
...
1. **STRICT-disclosure INPUT-domain gap** — `DisclosureMode.STRICT` keys its redaction passthrough on an inherited `error_domain == INPUT`, so a domain-less wrapper raised `from` an INPUT cause can leak its own `message`, and `to_problem_document(STRICT)` can still echo provider metadata for INPUT reports. Analysis and options: [track-strict-disclosure-input-domain-gap.md](track-strict-disclosure-input-domain-gap.md).
2. **`request_id` log wiring** — `JobMetadata.request_id` and the `request_id` kwarg on every `WorkflowLog` / `ActivityLog` method shipped, but no activity or workflow threads the value into a log call yet, so the kwarg is dead surface. See [api-companion-revisions.md](api-companion-revisions.md) §B.
3. **Webhook payload reserved-key collision** — `WebhookTarget.payload` accepts caller keys that `_notify_webhook` later overwrites with Pipelex-owned keys (`pipeline_run_id` / `status` / `result_url` / `error`), so a static payload key shifts meaning with delivery status. Analysis and options: [track-webhook-payload-collision.md](track-webhook-payload-collision.md).
4. **Test-coverage backfill** — the `/review` pass found gaps where the refactor removed or never added test coverage (notably the `request_id` logging path, which lands with item 2). This phase backfills it.
5. **Webhook signing (Stage 5 / Item F)** — the last unshipped stage of the API-readiness companion plan. Cross-repo: the pipelex side and the API side land in lockstep. Authoritative plan: [../security/webhook-signing.md](../security/webhook-signing.md).
6. **Metadata-model long tail** — a handful of `CogtError` subclasses still carry no class-level `error_category`, and several non-inference `PipelexError` subclasses still depend on the `agent_output.py` fallback dicts for `hint` / `error_domain` rather than carrying class-level metadata. See the Followups in [track-metadata-model.md](track-metadata-model.md).

Items 1-4 are the post-#931 `/review` follow-ups — an in-repo unit sequenced with checkpoints as Phases 1-4 in [../../TODOS.md](../../TODOS.md). Item 5, webhook signing, is the original plan's final stage — a separate cross-repo track that lands on its own schedule (TODOS Phase 5). Item 6 is an independent long tail. Everything else is landed and described in current-state terms in the track docs below. Optional, non-blocking review notes — observations that were deliberately not acted on, each with a conditional trigger for when to revisit — are collected in [review-notes/](review-notes/).
...
- [archive-retry-and-resilience.md](archive-retry-and-resilience.md) — the retry-and-resilience plan: remove the `PipeRouter` transient-retry loop and make Tier 1 transport retry explicit and uniform.
...
- [archive-retry-graph-trace.md](archive-retry-graph-trace.md) — a phantom-error-node graph bug; resolved by removing the retry loop that caused it.
