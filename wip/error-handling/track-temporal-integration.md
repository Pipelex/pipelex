...
- `recover_error_report` (`pipelex/temporal/tprl/temporal_error.py`) walks the failure's `__cause__` chain for an `ApplicationError`, pulls the details-packed report dict via the now-public `error_report_dict_from_details`, and rebuilds an `ErrorReport` through `ErrorReport.from_dict` (the strict inverse of `to_dict` in VERBOSE mode). It is **total** — it always returns an `ErrorReport`, never `None`. When the chain carries no report payload (a non-Pipelex exception, a worker crash, a heartbeat timeout) it synthesizes an `UnrecoverableWorkflowFailureError` report (`error_domain=RUNTIME`) carrying the most informative message recoverable from the chain. `from_dict` is `extra="forbid"`, so a found-but-malformed payload is an internal contract bug within one deploy (the activity bridge and submitter share the schema); rather than raise — which would abort the caller before it delivers the failure webhook — it synthesizes the same `UnrecoverableWorkflowFailureError` fallback, tagged with an `[error report failed schema validation]` marker so the contract bug stays visible on the wire. The error path therefore never crashes and the receiver is always notified.
...
### Workflow-level fail-safe floor (landed)

The bridge above converts every error that crosses an *activity* boundary. The remaining gap was an error raised **inline in workflow code** that never went through an activity — neither an `ActivityError` nor an `ApplicationError`, so Temporal treated it as a *workflow-task* failure and retried it indefinitely (a silent hang surfaced only as a generic timeout after the execution timeout). The floor closes that:

- `WfPipeRouter.run()` and `WfPipeRun.run()` each end with an `except PipelexError` boundary. `WfPipeRouter` converts a genuine inline error to a terminal `TemporalError` via `from_message_exception`; `WfPipeRun` routes it through deferred delivery (FAILED webhook still fires) and chains a details-carrying `TemporalError` so the submitter stays classified. Both convert only *genuine* inline errors — an escaping `PipelexError` already carrying a Temporal failure in its `__cause__` chain (e.g. a nested `TemporalPipeRouter` child-dispatch wrapping a sub-pipe failure as `WorkflowExecutionError`) is already terminal and recoverable, so it propagates untouched (`_carries_temporal_failure` guard) rather than being flattened.
- The worker now registers `workflow_failure_exception_types=[WorkflowExecutionError, PipelexError]` (`pipelex/temporal/temporal_task_manager.py`). Anything that slips past the catch-alls still fails terminally (degrading to a synthesized `UnrecoverableWorkflowFailureError`) instead of hanging. Scoped to `PipelexError`, not `Exception`: transient Temporal/infra errors keep Temporal's default task-retry.

**Coverage:** `tests/integration/pipelex/temporal/test_workflow_inline_error_failsafe.py` pins the escape path across the router catch-all, the `WfPipeRun` inline-delivery path, and the worker-level floor, each bounded by a short execution timeout so a regression fails fast instead of hanging. The systemic analysis and deferred hardening (sync-API deadline, activity-dispatch CI guard, stuck-workflow observability) live in [`temporal-error-handling-failsafe-gap.md`](./temporal-error-handling-failsafe-gap.md).

### Deferred review followups

Two optional refinements from the activity-boundary review (tracked in the deferred backlog) remain deliberate scope choices: the integration test verifies the converted payload rather than Temporal's retry-engine behavior, and `error_report_dict_from_details` identifies the report by dict shape. Optional to revisit.
