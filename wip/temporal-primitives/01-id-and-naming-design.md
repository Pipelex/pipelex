...
Reads cleanly with the primitives catalogued in `00-temporal-id-primitives.md` (moved to workspace `docs/history/temporal-primitives/`). The original problem statement (`workflow-and-activity-ids.md`) and the pre-checkpoint plan (`id-and-naming-plan-pre-checkpoints.md`) are archived in workspace `docs/history/temporal-primitives/`.
