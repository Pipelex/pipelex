...
- [ ] **`search_domain_codes` consumer sweep beyond the transport bridge.** Found by enumerating the workspace (the transport item above covers only `pipelex-server/transport/`): `pipelex-api/api/routes/pipelex/pipeline.py` still passes the deleted `search_domain_codes=` kwarg to `pipeline_run_setup` — a `TypeError` on every execution request the moment the pin moves (and remember: its openapi-check runs under `make check` only, so an agent-gate-green bump still fails CI); `pipelex-js` mirrors the lookup semantics in `packages/runtime/src/library/live-library.ts` and drives it from `scripts/python-live-library-oracle.py` (semantic parity, not just a kwarg); `pipelex-mistralai-workflows` names it in integration tests; the demo repos (`pipelex-demo-mistral`, `pipelex-demo-vibe`) use it in example workflows. Each needs the entry-affordance/`search_scope` migration or the kwarg dropped.
- [ ] **Aliased-entry `search_scope` wiring test.** The package-fixture conformance work above must also pin the run-setup wiring: invoke a dependency pipe as the entry via `alias->domain.pipe`, supply a bare envelope concept declared only in the dependency, and assert it resolves. Today no test invokes an entry pipe through an alias end-to-end, so deleting the `has_cross_package_prefix` branch in `pipeline_run_setup` (or capturing `raw_entry_ref` after `pipe_code` rebinds) reddens nothing.
