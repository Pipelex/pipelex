...
> **Status: ✅ DONE — implemented 2026-06-12.** `pending_signatures` + `is_runnable` added to `PipelexValidationReport` and populated in `PipelexMTHDSProtocol.validate`; first test coverage for the protocol `validate` landed in `tests/integration/pipelex/pipeline/test_protocol_validate.py` (verdict on both paths, strict-mode raise, library-lifecycle contract); changelog updated under `[Unreleased]`.
...
- **Downstream — CORRECTED 2026-06-12:** the original assumption here ("pipelex-api serializes whatever `validate` returns, fields appear automatically") is **wrong**. `pipelex-api`'s `/validate` route does NOT call `PipelexMTHDSProtocol.validate` — it builds its own strict `ValidateResponse` pydantic envelope (`api/routes/pipelex/validate.py`), calling `validate_bundle` directly (direct mode) or `act_dry_validate` (Temporal mode), and neither that envelope nor `DryValidateResult` carries `pending_signatures`. Surfacing the verdict on the hosted `/validate` therefore needs explicit changes — now part of a broader protocol-surface alignment (the full consistency pass also found `/models` divergence, `pipe_structures` shape conflicts, and a duplicated protocol-version constant). Live plan: workspace root `wip/mthds-protocol-surface-alignment.md` (supersedes `../pipelex-api/wip/validate-pending-signatures-envelope.md`).
