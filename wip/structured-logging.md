...
- **Event-name emission on delivery paths.** Add an `event` field to the delivery log lines — `event=webhook_delivery` / `event=webhook_failure` on the webhook path, `event=storage_delivery` / `event=storage_failure` on the storage path. This is folded into this refactor rather than done standalone because the lines it touches (`delivery_executor.py` around 239 / 243 / 280 / 282 / 285) are exactly the `request_id_suffix` interpolation / kwarg-threading sites listed as deletion targets in "Reality of the current code" below — a separate event-name pass would touch them, then this refactor would touch them again. Emit the four `event=...` fields once the structured-field surface lands.
