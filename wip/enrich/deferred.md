...
## 2b. The input-spec factory accepts `Concept[0]` where the io-ref parser refuses it

Surfaced while ruling §2. `InputStuffSpecsFactory.make_from_string` and `PipeBlueprint.generic_validate_inputs` both match a `(\d*)` bracket group without the parser's `N >= 1` check, so an input declared `Text[0]` is silently read as a single item, while the same suffix on an `output` is rejected by `parse_concept_with_multiplicity` with "multiplicity must be at least 1". Both sites now share the suffix→multiplicity projection, so the two halves agree on `[1]`; the range rule is the one thing still stated in only one of them. Fixing it means deciding which error a bad input count raises (`InputStuffSpecsFactoryError`, or the parser's typed `PipeVariableMultiplicityError` promoted to the blueprint validator) — a small error-surface choice, not a mechanical fix, which is why it was not folded into the `[1]` ruling.
