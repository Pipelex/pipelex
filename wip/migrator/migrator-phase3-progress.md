...
✅ **S6 part 3 is complete and up for review — PR [#1115](https://github.com/Pipelex/pipelex/pull/1115), `feature/Migrator-3c` → `dev`, head `b1f3102ae` after the review pass.** Items 6–9 of [What is actually left](#what-is-actually-left) are Milestones 11–14; the two parked #1114 review items that were routed to part 3 are Milestones 15 (§2, *a remedy is named only where it would write*) and 16 (§4, the doctor's unescaped Rich fields). `agent-check`, `agent-test`, `cmig`, `docs-check` and `check-spec-links` all green. One companion change lands in a **different repo** — Milestone 13 is pushed on `feature/Migrator` in the workspace-root repo, with no PR of its own. **The bot review pass on #1115 is executed** (Codex, Greptile, Cubic — one round, ten threads, all answered and resolved; the fixes are the fourth commit, *"answer the PR #1115 review"*, and the one item deferred is in [`migrator-pr-1115-review-followups.md`](./migrator-pr-1115-review-followups.md), a repo-wide test-layout decision rather than a migrator item). Every required check is green on the head and the merge state is clean. **⏸ C is not reached**: it waits on #1115 being merged, since its cold-start checks read the files off `dev`.

**S6's three parts are all built.** The pre-S7 bucket is on `dev` (part 1, #1113); the two `migrate` commands (milestone 5), the downgrade diagnosis (milestone 6), boot tolerance (milestone 7), the validation-error report (milestone 8), the telemetry-remedy retirement (milestone 9) and the `doctor` pending-migrations row with `--fix` (milestone 10) are on `dev` (part 2, #1114); the drift-contract review list, the two skills, the spec section, the published contract and the two parked #1114 items (milestones 11–16) are part 3, #1115, reviewed and waiting for the merge. What follows the merge is not on this branch: ⏸ C's cold-start check in the runbook, then S7. See [What is actually left](#what-is-actually-left) at the bottom for the small items that ride with whoever next touches their module.
...
6. ~~**The `config-docs` drift contract's `review` list**~~ — done, see Milestone 11.
7. ~~**The skills**~~ — done, see Milestone 12.
...
**Milestone 11 — the `config-docs` drift contract's review list (item 6).** `drift.toml`'s `config-docs` contract now reviews three kinds of target, not one: `docs/configuration/**/*.md` as before, plus `pipelex/migration/ledgers/` and the two modules holding R8's four validator sites (`pipelex/system/configuration/configs.py`, `pipelex/cogt/config_cogt.py`). The `description` names the four qualnames, because `drift plan` prints the description and the review paths and prints no comments — a site named only in a TOML comment reaches nobody at review time. **Two rulings taken here.** *The goldens are deliberately NOT in the review list*, against the tracker's own parenthetical: `docs/contribute/drift-contracts.md` bars generated artifacts from a review list, `make umig` generates them, and their staleness is already the derived check `check-migration-schemas` — which is the tier the drift doc says such a relationship belongs to. `plan.md` says "the ledger files", and the ledgers are the hand-authored half. *No verify command was added* — `check-ledger` and `check-migration-schemas` already gate `make check` and CI, so re-running them under the ack would add the unstaged-file hard error without adding a guarantee (the same reasoning the `hub-layering-convention` contract records). The contract was reviewed and acked for real: `make cl` and `make cmig` both green, no trigger file moved, nothing owed. Two docs moved with it — `docs/migration-ledger.md` now says where the "author's responsibility" for validator-expressed narrowing is *declared*, and `docs/contribute/drift-contracts.md` gained a paragraph saying a review target does not have to be prose, with the bar (a person reads and judges it; never something a command regenerates).

**Milestone 12 — the two skills (item 7).** `.claude/skills/add-migration/SKILL.md` is written. Its spine is one rule — *derive the entry from the gate's refusal, never from memory* — then: read what `cl`/`cmig` say (with a table mapping the issue kinds that mean **an entry is owed** to what it owes, explicitly not the whole kind list), decide `safe` vs `unsafe`, write the two edits to the ledger, handle the reserved-path rule, `make umig` and read its diff, prove with `cl`/`cmig`, add the `**Migration:**` changelog bullet. It also states what it must never do (hand-edit a golden, widen a check, reuse a retired name, put a value in an operation) and carries the `pre_history` special case. `/release` gained **step 3b** between the quality checks and the branch cut: `make check-migration-schemas` blocks the release, a failure routes to `add-migration`, and on a pass it diffs `pipelex/migration/ledgers/` against `origin/main` to confirm a `**Migration:**` bullet and a correct `introduced_in` for every new breaking entry — plus the note that a breaking entry makes the release a minor bump. The Files-touched list and the Important-details list were updated to match.

**Milestone 13 — `docs/specs/command-surface-map.md` (item 8), in the workspace-root repo.** A new `## Configuration migration surface` section rather than rows in the master table, following the precedent the codegen family set — the master table is indexed by hop in the *validate* chain and migrate has no hook, no `mthds-agent` command and no HTTP route. The section carries a chain block for `[M] pipelex migrate`, `[Ma] pipelex-agent migrate` and `[Md] pipelex doctor --fix`, a two-row table with the same columns as the master one, the `needs_attention`-is-the-verdict sentence, the no-user-values rule, and an `<!-- unverified -->` marker explaining why this family is not in `conformance/` (it acts on the invoking machine's own configuration directories). One correction the spec now states rather than overclaiming: `mthds-agent migrate` is **not** a registered command, but under the pipelex runner it still reaches `pipelex-agent migrate` through hop 3's unknown-command passthrough, and under `--runner api` it is an `ArgumentError` — a fallback, not a surface. The front matter's `sources:` gained both command modules. `make check-spec-links` in `conformance/` is **green** (the CLAUDE.md note about 35 issues is stale — the pair is passing today). ⚠ **This change is uncommitted in the workspace-root repo on branch `feature/Migrator`**, a different repo from this branch.

**Milestone 14 — publishing the contract (item 9).** `docs/migration-ledger.md` lost its Status banner, `mkdocs.yml` lost the `migration-ledger.md` line from `not_in_nav` and the comment block explaining when to move it, and the page gained a `Migration Ledger` row in the nav's Project section beside `Drift Contracts`. A trap worth recording: `mkdocs.yml` contains **two** structures that look like a nav — the real `nav:` at the bottom, and the `llmstxt-md` plugin's `sections:` map several hundred lines above it, whose entries have the same `- path.md: "Title"` shape. The row was added to both (the llms.txt listing wants it too), and `make docs-check` is what caught that the first one was not the nav. Docs build clean under `--strict`.

### Milestone 15 — the §2 ruling, built

**The ruling.** *A remedy is named only where it would write.* The presence of a `migration` block means the migration engine has something to say about these files — **not** that the remedy would rewrite them. What separates the two is a field, not a sentence: `MigrationErrorBlock` carries **`would_write: bool`** (`any(plan.did_change for plan in plans)`), and every surface that used to name `pipelex migrate` on the block's mere presence now branches on it, pointing at the **dry run** where nothing would be written, because the dry run is what actually carries the answer. This is Milestone 10's ruling one step along — *a fix is offered only where a command would actually write* — and Milestone 9's — *a probe's verdict is a field its callers branch on, never a sentence they search*. The two flags are independent and never both false: a file with nothing to write and nothing for a person is clean, and clean files are not in `plans`.

**Where it landed — five surfaces, not the four the follow-up named.** (1) `pipelex/base_exceptions.py` — the field with its docstring. (2) `pipelex/core/validation.py` — `_pending_migration` computes it, and `_migration_prose` swaps both its opening and its closing sentence on it. When `would_write` is false the "what it would carry forward" sentence is absent by construction, since `carried` reads `plan.steps` and a plan with steps changes. (3) `pipelex/cli/commands/doctor_cmd.py` — `_telemetry_is_out_of_date_or_wrong` branches on `own_plan.did_change` rather than `not own_plan.is_clean`, so a file the migration would only report on falls to `INVALID` (which routes to Manual Fixes and is what `--fix` declines to offer a command for), and the `INVALID` message gains the `--dry-run` pointer **only when a scan actually found something** — a genuinely wrong file gets no pointer, which is what keeps `test_check_telemetry_config_invalid_new_format_names_the_field`'s `MIGRATE_COMMAND not in message` true. (4) `pipelex/cli/agent_cli/commands/agent_output.py` — the hint is now three-way and tells the agent to read `migration.would_write` off the payload it already holds rather than to run a command to learn it. (5) **`pipelex/cli/error_handlers.py`, which the follow-up did not name** — the human telemetry handler had exactly the same `if exc.migration is not None:` shape and is the surface a person actually reads; it now has three arms, and the middle one keeps "never offer `init telemetry`" while sending the reader to the dry run.

**The docs that would otherwise have disagreed with the CLI.** `docs/migration-ledger.md` — the structured-block bullet lists the field, a new blockquote states the rule, the three per-surface bullets under "Every surface reports it the same way" are rewritten three-way, and the `doctor`-row bullet now says `out_of_date` means the plan would rewrite that file. `docs/tools/cli/agent-cli.md` and `pipelex/cli/agent_cli/CLAUDE.md` — both said presence was "the whole signal" and sent an agent to `migrate --yes` from it. The `cli-docs` drift contract opened on the three `pipelex/cli/**` edits and was reviewed and acked; `docs/tools/cli/migrate.md` needed nothing, since it describes the command and the `pending_migrations` row and neither moved.

**Changelog.** Folded into the two existing `[Unreleased]` bullets rather than added as a new one — the block has never shipped without `would_write`, so a bullet describing a fix to it would be describing a fix to something no user has seen.

**Tests, written first and red before the code.** `TestARemedyIsNamedOnlyWhereItWouldWrite` in `tests/unit/pipelex/core/test_validation_report.py` (both sides plus the never-both-false invariant on a specimen that is both at once), `TestTheRemedyIsNamedOnlyWhereItWouldWrite` in `tests/unit/pipelex/system/telemetry/test_telemetry_stale_remedy.py` (the same pair through the real loader), `TestTheTelemetryRowNamesMigrateOnlyWhereItWouldWrite` in `tests/unit/pipelex/cli/test_doctor_config_checks.py` (including the regression guard that a genuinely stale file is *still* `out_of_date`), and the would-not-write arm of the human handler in `test_error_handlers_more.py`.

### Milestone 16 — §4, the doctor's unescaped Rich fields

The last parked #1114 item. `display_health_report` printed nearly every dynamic value straight through Rich's markup parser: a `[dev]` path segment was silently dropped and an unbalanced `[/b]` raised `MarkupError` and took the whole report down — and the fields most likely to carry brackets are the ones quoting what went wrong, so the report failed hardest exactly when it mattered. One pass now escapes the four row messages (`config_message` embeds `report_validation_error(...).message`, which quotes the user's own keys), every path off `ConfigLocationInfo`, backend names, environment-variable names, deck filenames, the first line of a backend file's error, and `str(exc)` at each site the `--fix` pass reports one. Two things deliberately stay raw: `status_rich_label(status)`, which returns markup on purpose, and the module constant `MIGRATE_COMMAND`.

**One residual, recorded rather than half-fixed.** `console.print(error_msg)` prints `BackendCredentialsErrorMsgFactory.make_comprehensive_error_msg(...)`, a plain-string builder that already hardcodes a Rich `\[` in one of its lines — escaping it at the print site would render that as a literal backslash. It is the same layer question §4 already ruled on for `stale_configuration_warning`: a builder whose consumers include non-Rich sinks cannot escape for one of them.

`TestEveryDynamicFieldSurvivesRichMarkup` covers it with the recorded-console pattern, and each assertion was mutation-tested — the escape reverted, the test watched go red, the escape restored.
...
✅ **Part 3 is done, over two sessions.** Session 1 built items 6 through 9 (Milestones 11–14), committed as *"the review obligation, the two skills, and publishing the contract"*. Session 2 built the two parked #1114 review items routed here — Milestone 15 (§2) and Milestone 16 (§4) — committed as *"a remedy is named only where it would write, and the doctor stops eating brackets"*. Both are pushed as [#1115](https://github.com/Pipelex/pipelex/pull/1115), and a third, shorter session answered its bot review — the last unescaped doctor line, the closing sentence of the migration paragraph, the `add-migration` closing rule, the `/release` ledger-diff baseline, and the two test modules folded back to one class each — committed as *"answer the PR #1115 review"* (head `b1f3102ae`). One item is deferred, in [`migrator-pr-1115-review-followups.md`](./migrator-pr-1115-review-followups.md): whether the one-TestClass-per-module rule holds against the narrative multi-class layout that predates this PR in two of the touched modules and dozens more — a repo-wide decision, not this branch's. Nothing is left on this branch; ⏸ C waits on the merge.
