...
### The §2 ruling — taken, not built

**The ruling.** *A remedy is named only where it would write.* The presence of a `migration` block means the migration engine has something to say about these files — **not** that the remedy would rewrite them. What separates the two is a field, not a sentence: `MigrationErrorBlock` gains **`would_write: bool`** (`any(plan.did_change for plan in plans)`), and every surface that today names `pipelex migrate` unconditionally branches on it, pointing at the **dry run** where nothing would be written, because the dry run is what actually carries the answer. This is Milestone 10's ruling one step along — *a fix is offered only where a command would actually write* — and Milestone 9's — *a probe's verdict is a field its callers branch on, never a sentence they search*. The two flags are independent and never both false: a file with nothing to write and nothing for a person is clean, and clean files are not in `plans`.

**What it changes, on the four surfaces the follow-up names.** (1) `pipelex/base_exceptions.py` — the new field with its docstring. (2) `pipelex/core/validation.py` — `_pending_migration` computes it; `_migration_prose` swaps its opening ("Your configuration may be out of date rather than wrong" → "The migration history has something to say about these files") and its closing sentence ("Run `…` to bring these files up to date" → "`…` would rewrite nothing here — run `… --dry-run` to read what it found, and fix these files by hand"). (3) `pipelex/cli/commands/doctor_cmd.py` — `_telemetry_is_out_of_date_or_wrong` branches on `own_plan.did_change` rather than `not own_plan.is_clean`, so an unexplained-only file falls to `INVALID`, which already routes to the manual-fixes section and does not offer `migrate`; its message should carry the `--dry-run` pointer so the unexplained diagnosis is not lost. (4) `pipelex/cli/agent_cli/commands/agent_output.py` — the `TelemetryConfigValidationError` hint stops sending the agent to `migrate --yes` unconditionally. Plus the two contract sentences that would otherwise disagree with the CLI: `docs/migration-ledger.md` → "Reporting a stale configuration on a validation error" and `docs/tools/cli/agent-cli.md` ("Its presence is the whole signal: the configuration is *out of date* rather than wrong").

**Why it is not built.** It was started this session — the field and `_migration_prose` were written — and **reverted deliberately at the pause**, because `would_write` is a required field and leaving one construction site in a tree with tests that build the block would have been a half-built change across a session boundary. Nothing of it is in the working tree. The next session implements the four surfaces above plus tests, in one commit; the design decision is done and needs no re-litigation.
