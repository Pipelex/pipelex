...
make reuse                    - Add license headers to all files and check them
...
reuse: env
	...
	@. $(VIRTUAL_ENV)/bin/activate && \
	find pipelex tests -type f \( -name "*.py" -o -name "*.toml" -o -name "*.env.example" \) -print0 | xargs -0 \
		reuse annotate --license Elastic-2.0 --copyright-prefix spdx-symbol --year 2025 --copyright "Evotis S.A.S." --template pipelex  --skip-unrecognised --skip-existing >/dev/null && \
		reuse lint --quiet || { \
...
check: cc check-unused-imports reuse-check 
	@echo "> done: check"
