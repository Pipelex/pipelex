...
- **IN PROGRESS (Phases 1+2 done; Phase 3 paused for a fresh session) — re-cut `execution_mode` into `orchestration_mode` (open string token) + `DeliveryMode` (closed enum)** — plan [`wip/plugins/orchestration-mode-and-delivery-split.md`](wip/plugins/orchestration-mode-and-delivery-split.md). **Phase 1 (core, `_plugins` `bd4e52827`) + Phase 2 (`pipelex-temporal` `01d7dc3`) landed on `feature/orchestration-mode-delivery-split` in each repo — gates green + clean-context `/code-review` each. §8 `/start`-honesty risk resolved (honest 400 safe). NEXT = Phase 3 (`pipelex-api`, THE gate): branch cut off `06cd5e6`, tree clean, no code yet. Cold-start resume state + locked decisions + checklist are in the plan's §10 "⏸️ COLD-START RESUME" block.** **Supersedes the flat-`PipelexExecutionMode` framing of the two bullets above:** it removes the conflation those plans worked around — `_async_start_mode`, the `/execute` fire-and-forget 400, the `ApiConfig` f&f field-validator, and the `/validate` double-registration. Two orthogonal axes: an **open** orchestration token (core owns only `"direct"`; plugins own `"temporal"` / `"mistralai-workflows"` — realigns with "the base names no orchestrator") + a **closed** `DeliveryMode {BLOCKING, FIRE_AND_FORGET}` that endpoints set, never receive. `/start` becomes **honest** (4xx when the resolved mode can't do async) instead of degrading. Design **locked 2026-06-22**; Direction A (core → `pipelex-temporal` → `pipelex-api`, one repo/branch/commit per phase). **Top risk to settle before the API phase:** does the MTHDS-Protocol/conformance suite require `/start` to return `202` on a non-async (`direct`) backend? See the plan's §8.
