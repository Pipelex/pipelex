...
- `search_gen_structured_object(search_object_assignment, *, output_class) -> BaseModelTypeVar` — the in-process arm. The class goes down to the provider; an instance of it comes back, validated once at the leaf.
...
The resolution helper was generalized rather than copied: `resolve_search_output_class` sits beside `resolve_object_class` and both delegate to one private `_resolve_class`, so the `SchemaToModelFactory` call keeps a single site.
