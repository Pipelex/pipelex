...
- **Current phase:** Phase 1 — **1.1, 1.2, 1.3 DONE + COMMITTED; CHECKPOINT B1 ✅ PASSED 2026-07-09; 1.4 (types emitters) DONE + COMMITTED `002ec98a4`.** Resolver + normalized-crate + encodings + `pipelex resolve` CLI landed (B1); now the three **types emitters** (`python-structures`, `python-pydantic`, `ts-zod`) over a neutral resolved-concept layer, with imprecision surfacing + extension-file story. `make agent-check` + full `make agent-test` PASS.
- **Next action:** Phase 1 continues at **1.5 (`pipelex codegen` CLI family)** — wire `codegen types --target …` to `emit_types`, add the **per-pipe projections deferred from 1.4** (parse/serialize helpers, the ts binder file, the runnable compact-input example from `input_renderer`/`pipe_io_contracts` — all coupled to pipe selection), and re-point `build inputs`/`build structures` as thin aliases; then **1.6** (both fixtures), **1.7** (quality gates: pyright-on-emitted-Python in pipelex, tsc in conformance per D7). **Before Checkpoint B, resolve the deferred items in `wip/devx/deferred-checkpoint-b1-items.md`:** B1-1 (Python-backed `structure="<ClassName>"` — **emitter floor DONE** (surfaced opaque, never silently emitted); the normalization-layer policy [error vs disable] + **Louis discussion still pending**), B1-2 (stop flattening native refinement bases — round-trip identity, still pending), B1-5 (cross-package fold-in / fixture 1). Cold-start reads that doc.
- **Last checkpoint passed:** **Checkpoint B1** — resolver landed (mid-phase); cold review triaged, decisions D1–D7 settled, gates green, deferrals documented. (1.4 is mid-phase, not a checkpoint — no `/code-review` fan-out; next checkpoint is **B** after 1.7.)
- **Last commit(s):** mthds `feature/Codegen` `9104ea6` · workspace-root `feature/Follow-ups` — Checkpoint-A `50772e6`, then **B1 docs commit `5fd7ed1`** (north-star + deferred-checkpoint-{a,b1} docs) · conformance `feature/Codegen` `d42cadb` · pipelex `feature/Devex-codegen` — Checkpoint-A `1a7f9fe42`, **Phase-1 `64c7c155c`** (1.1–1.3), tracker-pin `dd26ad8b4`, **B1 closeout `a63bd19`** (F4/F5 fixes + test + tracker), SHA-pin `60bb0495e`, then **1.4 emitters `002ec98a4`**. **Review diff for the NEXT checkpoint (B) = `a63bd19..HEAD`.** **None pushed.**
...
- [ ] `codegen types --target ts-zod|python-pydantic|python-structures` (concept set) — thin wiring over the landed `emit_types` dispatcher (`pipelex/codegen/emitters/types_emitter.py`), writing `EmittedFile`s to disk; `codegen inputs [--pipe <ref>]` re-surfacing `input_renderer`/`InputShaper` over the selected pipe (compact default, explicit opt-in — Smart Inputs policy unchanged); output-side artifacts fed by `pipe_io_contracts` + `output_renderer`.
- [ ] **Per-pipe emitter helpers (moved from 1.4, need pipe selection):** `python-pydantic` + `ts-zod` **parse/serialize helpers per pipe**; the **ts binder file** (the snake↔camel wire mapping applied over the pure `types.ts`, completing the purity split); the **runnable compact-input example** embedded in emitter docstrings/JSDoc (from `input_renderer`).
