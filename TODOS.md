...
- **Where:** worktree `_hub/`, based on `refactor/Hub-2` (PR #1064). **One branch**: `refactor/Modularity-3`, pushed at `6e74c6f26`, carrying M3 + M1 + M2 + F1. F1 was developed on `refactor/Modularity-4` and merged in through PR #1065 — the separate-PR topology [Gating](#gating) prescribed for it, honoured and now closed; that branch is deleted. The track's own PR is **[#1067](https://github.com/Pipelex/pipelex/pull/1067), against `refactor/Hub-2`**. Related memory: `project_modularity_refactors.md`, `project_hub_split_refactor.md`.
...
- **Blocking:** PR #1064 must merge before #1067 can retarget to `dev` and go in. **No open decisions.**
