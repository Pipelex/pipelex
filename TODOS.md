...
- **Status:** **PHASE 4 COMPLETE + CHECKPOINT 3 DONE & TRIAGED (2026-07-11). PUSHED. NEXT = absorb the upstream `dev` merge (new "Phase M" below), NOT Phase 5.** Phase 4 ground out all 4 remaining packages (batches 8–11): plugins (150 kept, 8 demoted), cli (168 kept, 47 demoted), cogt (274 kept, 7 demoted), tools (283 kept, 1 demoted). ZERO `seeded = true` entries remain across the registry (`--report`: `seeded remaining: 0`); 1,513 grants total. Gates green after every batch + full `make agent-test` green at the checkpoint (twice: before + after the review fixes). CHECKPOINT 3 review DONE + triaged (3 fresh Sonnet agents over `a1649f269..ff8bfbdc6`): correctness + mechanical-safety clean, grant-judgment found 3 real misses all fixed in `c60a448b8` (record: `wip/subject-grants/checkpoint-3-review.md`). Commits **PUSHED** through `c60a448b8` (batches `ea0e642e6` plugins → `c3aa0ee3c` cli → `f542ba941` cogt → `ff8bfbdc6` tools → tracker `b751a1654` → review-fix `c60a448b8`). **⚠ A big update is coming in from `dev`. Sequencing decision (settled with Louis): MERGE FIRST, Phase 5 LAST — and CHECK WITH LOUIS before starting Phase 5.** See **Phase M — Absorb the upstream `dev` merge** below (it explains why, the one destructive misstep to avoid, and the cold-start-safe workflow).
- **Resume point:** `feature/Signatures` in the `_sig` worktree (treat `_sig` as repo root), **pushed** at `c60a448b8`. Working tree clean. **NEXT WORK = Phase M (the `dev` merge), not Phase 5.** Start by reading Phase M below, then confirm the merge target + source ref with Louis. Do NOT jump to Phase 5.
