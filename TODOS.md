...
- **Not overengineered.** The two surfaces are justified separately, and neither is speculative. The **primitives** were extracted only once two paths shared them — `pipelex.temporal` consumes them in-tree today, and the landing-soon `pipelex-mistralai-workflows` is the second consumer. The **entry point** (`run_pipe_via_bridge`) has no in-tree caller yet; it is a boundary pre-positioned in core for that same imminent consumer, and it must live with the producer so the sibling package can import it — putting it in the consumer would force non-Mistral hosts through a Mistral dependency for DIRECT/Temporal dispatch. `__init__.py` is empty (no re-export indirection, per project convention); the mode set is a flat enum, not a plugin registry. No speculative interfaces or config.
