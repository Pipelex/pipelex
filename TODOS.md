...
- **Plan state:** ✅ finalized, **Phase 1 (Part A) + Phase 2 (Part B) complete, committed & pushed — at Checkpoint B**
- **Current phase:** Phase 2 done & committed (`d91701f4`). **Phase 3 (new session — OK to proceed).** End-to-end multi-file proof + full `make agent-test`, plus the two folded-in review fixes. **Decisions recorded** (see Decisions section): #1 → move the concept-ref check out of `make_from_blueprints` into the loader; #2/#3 → reparent `ConceptLibraryError`/`PipeLibraryError` under `LibraryError` and catch once.
...
- **Part B (committed in `d91701f4` "Refactor concept validation and introduce cross-file reference resolution"; pushed, origin in sync. The earlier "uncommitted/no authorization" note was stale — verified clean tree + matching file list). Files:**
...
- **Finding #1 (batch-local check) → move to loader.** `make_from_blueprints` becomes a pure structural merge + fingerprint; the concept-ref check moves to `load_from_blueprints`, validating against `batch ∪ already-loaded library concepts ∪ native`. Rejected the "thread known refs into the factory" alternative — the check belongs where library state lives, and this keeps `make_from_blueprints` world-agnostic. Phase 3 integration test must cover the cross-batch case.
- **Finding #2/#3 (raw error) → reparent under `LibraryError`, catch once.** `ConceptLibraryError`/`PipeLibraryError` reparented from `PipelexError` to `LibraryError`; a single `except LibraryError` arm in `_translate_to_validate_bundle_error` + widened `_load_mthds_files_into_library` catch. Fixes the new undeclared-ref error and the pre-existing raw duplicate-concept/duplicate-pipe errors uniformly.
