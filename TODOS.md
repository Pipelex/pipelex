...
- [x] Gates checklist clean (all five items). Grants for the two new entry-concept defs recorded before the first check; `.test_durations` reconciled against a full `-m ""` collection (three renames + seven param-id changes; the remaining orphans predate this branch).
