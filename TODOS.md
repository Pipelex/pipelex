...
- **Next action:** **STOP at Checkpoint C for Louis' review**, then **Phase 3 — every surface, one authority** (firm the task list at reconciliation): API routes on `pipelex-api` per the 0.2 envelope spec, agent-CLI passthrough (`pipelex-agent` / `mthds-agent`), MCP tool projection, webapp form + n8n re-point. **For Louis at this stop:** confirm **D8** (formal defer of both JS starters), **D9** (`build structures` re-point deferral), **D10** (wire-native ts keys), **D11** (nested-model → dict-with-imprecision), B1-1 policy. **RELEASE-GATED tails (ride the next release train, unchanged):** starter-CI wiring of `codegen check`, conformance check/stamp/lock de-skeleton (B1-6), JS starter conversions. See `wip/devx/deferred-checkpoint-b1-items.md` + `wip/devx/deferred-checkpoint-c-items.md`.
...
| D8 | `mthds-starter-js` conversion in Phase 2 or deferred | OPEN (flagged for Louis at the Checkpoint C stop), leaning DEFER | Louis chose "convert one starter as a proof" (`pipelex-starter-python`) at Checkpoint C1, so both JS starters (`pipelex-starter-js`, `mthds-starter-js`) are deferred to post-release by default; the single-starter proof passed at Checkpoint C — awaiting Louis' formal confirmation of the defer. |
