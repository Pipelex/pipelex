...
- **Current phase:** Phase 2 — **COMPLETE (2.1 trust chain + 2.2 single-starter proof); CHECKPOINT C ✅ PASSED 2026-07-10.** `pipelex-starter-python` (branch `feature/Codegen`) now builds and runs on generated code only: hand-written models in `piper/examples/*.py` deleted, a `python-pydantic` client per method under `piper/generated/<method>/` (stamped `models.py` + `codegen.lock`, both shipped as package data), deterministic `inputs.template.json` scaffolds, `make codegen` / `make codegen-check` targets (`PIPELEX ?= pipelex` variable), offline smoke tests. Trust chain verified live (hand-edit → exit 1 → regen heals; bundle field add → typed field appears → revert → byte-identical; full regen over a current tree = all `Unchanged`). The proof caught 3 real engine bugs, fixed in this worktree: model-spec-less boot rejected any bundle pinning a model; native `Image` materialized structureless (→ **D11** nested-model → dict-with-imprecision, PROPOSED); uuid-random template placeholders. Cold reviews (one per repo) triaged: `codegen inputs` now write-if-changed, direct DICT-path emitter tests added, `codegen.lock` in the wheel; 2 follow-ups deferred to `wip/devx/deferred-checkpoint-c-items.md`. Gates green in both repos (pipelex `make agent-check` + full `make agent-test`; starter `make agent-check` + `make agent-test`).
- **Next action:** **STOP at Checkpoint C for Louis' review**, then **Phase 3 — every surface, one authority** (firm the task list at reconciliation): API routes on `pipelex-api` per the 0.2 envelope spec, agent-CLI passthrough (`pipelex-agent` / `mthds-agent`), MCP tool projection, webapp form + n8n re-point. **Louis' answers at this stop (2026-07-10):** **D8 SETTLED** (JS starters deferred), **D10 SETTLED** (stay wire-native snake_case; ⚠ reconcile the camelCase wording in `docs/specs/pipelex-codegen.md` name-derivation at the next workspace-root touch), **D9** → hold for a dedicated discussion with Louis at the next cold-start. Still awaiting: **D11** (nested-model → dict-with-imprecision), **B1-1** policy — both explained to Louis, answers pending. **RELEASE-GATED tails (ride the next release train, unchanged):** starter-CI wiring of `codegen check`, conformance check/stamp/lock de-skeleton (B1-6), JS starter conversions. See `wip/devx/deferred-checkpoint-b1-items.md` + `wip/devx/deferred-checkpoint-c-items.md`.
- **Last checkpoint passed:** **Checkpoint C** (Phase 2 complete — starter proof landed, reviews triaged, gates green; D8/D11 awaiting Louis). Prior: **C1** (trust chain), **B** (engine + emitters + CLI), **B1**, **A**.
- **Last commit(s):** mthds `feature/Codegen` — …**B review `7b5573f`** (untouched since B) · workspace-root `feature/Follow-ups` — …**C north-star + deferred-doc `e64053e`** · conformance `feature/Codegen` `d42cadb` (untouched) · **starter `pipelex-starter-python` `feature/Codegen` (NEW, off `dev`) — conversion `5f54039`, C review-fix `c06f4e7`** · pipelex `feature/Devex-codegen` — …**C1 review-fix `9d4fc59d6`**, C1 tracker `e05150f2d`, **2.2 starter-proof fixes `062b97992`**, **C review-fix `ef6a9e793`**. **Review diffs already run for C: pipelex `9d4fc59d6..062b97992`, starter `65f8521..5f54039`; review diff for the NEXT checkpoint (D) = pipelex `ef6a9e793..HEAD`, starter `c06f4e7..HEAD`.** **None pushed.**
