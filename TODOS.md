...
- **Status:** **STAGE 1 PART 2 DONE (2026-07-12)** — under-the-hood (19) + advanced (7) = 26 pages swept via the wave Workflow (`wf_28527b4e-470`): 435 claims checked, **42 confirmed / 0 rejected** by adversarial verify (D6), **0 reclassified** (D11 carve-out doesn't apply — internals/injection pages have no bundle-teaching examples). Severity: 10 breaks-a-user / 23 wrong / 4 misleading / 5 stale-detail; density 1.62 (14/26 dirty), or 2.0 against the 21 substantive pages (5 `advanced/*-injection.md` are 8-line "Under Construction" stubs = legit clean). Findings in `wip/drift-hunt/findings/under-the-hood-advanced.md`; scoreboard updated in `SUMMARY.md`. Load-bearing spot-checks hand-verified (all held). Carry-in F10 (`execute_pipeline`→`execute`) confirmed. No fixes applied (fixes = Stage 2). **Running Stage-1 total: 887 claims, 86 drift, 3 convention, 0 rejected.** ⟶ Part 1 (building-methods, 44 drift) recap below. Earlier: CHECKPOINT 0 CLOSED (D7–D10), Phase H DONE, Phase B DONE, Stage 0 DONE, gates green.
...
- **Next:** **Part 3 NOT started — awaits Louis' explicit go** ("resume the Drift Hunt from TODOS.md — run Stage 1 Part 3"). Part 3 = features (21) + reliability (4) = 25 pages. Wave Workflow cost for Part 2: 68 agents, ~4.8M tokens, ~6.5 min wall. Reusable Part-2 script: `/private/tmp/claude-501/-Users-lchoquel-repos-Pipelex--drift/397c0ff4-0b63-4662-bf6d-23d360735918/scratchpad/drift-hunt-uth-advanced-part2.js` (adapt PAGES + seeds for Part 3 — pull Part 3 seed rows from `suspects.md`; note the CTX has a D11 carve-out block baked in that Part 1's script lacked). ⚠ **No outgoing carry-ins from Part 2** — all Part-2 findings are self-contained.
