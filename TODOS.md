...
**Status:** **Phase A + B + C DONE (through MAJOR GATE 2 / Checkpoint C — THE gate).** Phase A/B core seams on `refactor/Plugins-4` (`4b5449f88` Phase A; `75196e2e3` seam refinement; `7378afdd8` Phase B docs); `pipelex-temporal` on `refactor/own-temporal-config` (`6d9adff`). **Phase C** = `pipelex-api` made orchestrator-agnostic on `refactor/orchestrator-agnostic-base` (cut from `dev`): F1 execution via the orchestrator-registry hub seam (byte-equivalent — see the F1 deviation note in the as-built), F2 validate always DIRECT, F3 exception-handler mappers consumed at app construction, `[api]` config = Option C `api.toml` via `load_plugin_config`, `temporal` extra dropped. Two clean-context `/code-review`s: correct, byte-equivalence verified, no silent bugs; 5 findings applied + 2 pre-existing deferred. All gates green (308 tests, pyright 0, mypy 0, pylint 10, openapi-check). **Commit pending** (pipelex-api code + this tracker update); nothing pushed. **NEXT = Phase D** (`pipelex-api-hosted` Temporal flavor — MAJOR GATE 3). The in-`pipelex` plugin-system foundation (seams inverted, Temporal externalized, `plugins.boot_orchestrator` gate) was done on `refactor/Plugins-3`.
