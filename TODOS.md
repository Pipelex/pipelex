...
- **Current phase:** Phase 2 COMPLETE + **Phase-2 code review applied** → **CHECKPOINT 2 cleared (2026-06-01).** The tabular codec is fully implemented and hardened against the review's fix-now findings; all 51 codec unit tests GREEN (44 + 7 new); `make agent-check` green repo-wide; no tools-scope regression (1269 passed / 1 pre-existing skip; the 2 csv-integration RED tests are the un-hooked Phase-3/4 ones, by design).
- **Last checkpoint cleared:** CHECKPOINT 2 (codec landed, unit-green, lint-clean).
- **Git state (2026-06-01):** Phase-1 work is in `acde0561` + `7ef68ac9` (R1–R7 fixes). **Phase-2 changes + the review fixes are UNCOMMITTED in the `_csv` worktree at session end** — files touched: `pipelex/tools/tabular/csv_codec.py` (bodies filled + review fixes #2/#3/#4/#5/#7), `tests/unit/pipelex/tools/tabular/test_csv_codec.py` (R13/R14 + 7 new review-fix tests), `wip/csv-support/phase2-review-followups.md` (new — deferred finding #1). **Next session: commit these before starting Phase 3.** Nothing pushed.
- **Phase-2 code review (xhigh, 2026-06-01) — fix-now set APPLIED; #1 DEFERRED:**
  - **Applied (codec hardening, each with a red-first test):** **#2** skip blank physical lines (`[]`) in `_read_table`, matching `csv.DictReader` — fixes the phantom all-`None` row / spurious coercion error (the two empty-cell tests now feed a quoted-empty cell via `write_value_csv(..., header=...)`, not a blank line). **#3** reject a data row wider than the header (`CsvColumnError`) instead of silently dropping surplus cells. **#4** `_assert_single_char_delimiter` + `except LookupError` close the A2 boundary leak (bad delimiter `TypeError` / unknown-encoding `LookupError` now wrapped, read & write). **#5** `csv_from_list_content` guards `isinstance(item, row_model)` → `CsvError` instead of silently writing empty cells on a type mismatch. **#7** coercion-error label falls back to the pydantic error `type` for model-level (empty-`loc`) failures, so no more `field(s) ?`.
  - **#1 DEFERRED → [`wip/csv-support/phase2-review-followups.md`](wip/csv-support/phase2-review-followups.md):** non-string `Literal`/`Enum` row fields are accepted as flat but can't round-trip. NOT reachable from `.mthds` (generator always emits string `Literal`s; `choices` is `list[str]`), so it's a latent gap behind an unreachable path. Reject-vs-coerce is a real design call — left for a deliberate decision, not patched here.
  - **Still deferred (review, not actioned):** #6 NaN round-trip (niche, documented), #8 whitespace-padded header diagnostic, #9 `Decimal` over-strict (debatable), #10 e2e wiring test only checks param name (Phase-4 concern); cleanup #11 dup typing-introspection vs `tools/typing/`, #12 double `model_fields` pass, #13 `value or None` idiom.
- **Green so far:** the **codec is implemented & unit-green**. `read_rows`/`write_rows`/`flat_field_names`/`assert_supported_table_suffix`/`list_content_from_csv`/`csv_from_list_content` all behave per the locked contract. NOT yet wired into the runtime: the input hook (Phase 3) and `--save-csv` (Phase 4) are untouched, so the integration + e2e tests remain RED (by design).
- **Next concrete action:** start **Phase 3 — input integration**. Hook `stuff_factory.make_stuff_from_stuff_content_or_data` **Case 2.5** (`stuff_factory.py:399-413`, pinned by A1): when the resolved `url` ends `.csv` and the concept is a flat structured concept, resolve the row concept's `StructuredContent` subclass (via the class registry / `concept.get_structure_class()`) and call **`list_content_from_csv(path, row_model)`** to produce `ListContent[row-concept]`. **Reject remote URLs** (`http(s)://`/`s3://`/signed) with a clear "local paths only in v1" error (A1). Then make `tests/integration/pipelex/csv/test_csv_roundtrip.py` part (a) GREEN (input → `ListContent[Person]`, `death_year is None` for Vint Cerf). The codec entry points are DONE and LOCKED — Phase 3 only adds the call site + concept→class resolution + remote-URL guard.
  - **Phase-2 test-bug fixes (R13/R14, 2026-06-01)** — both surfaced by the now-correct implementation, same class as R1–R3; contract unchanged, only fixtures corrected:
    - **R13** `test_empty_cell_on_required_field_raises` used header `value` with `RequiredTextRow` (whose field is `text`) → that's a column mismatch (`CsvColumnError`), not the intended empty-required path. Swapped the model to **`IntRow`** (field `value` matches header) so the blank cell → `None` → required-int coercion fails → `CsvCoercionError`. Stays distinct from the sibling `test_required_text_empty_cell_rejected` (text path).
    - **R14** `test_coercion_reject_table[FloatRow-1,5]` wrote `value\n1,5\n` via an f-string — the comma is the delimiter, so `csv.reader` split `1,5` into two cells and `value` read as `1` (valid → no raise). Added a `write_value_csv` helper that builds the single data cell through `csv.writer` (correct quoting), used by BOTH the accept- and reject-coercion tables. Now `1,5` is one cell → float coercion rejects it → `CsvCoercionError`.
  - **R8 RESOLVED:** the `CsvCoercionError` message format is now `"Could not coerce CSV row {n} of {path} for concept {Concept!r}: field(s) {names} — {ValidationError}"`. The loose `"2" in message` assertion still holds (row number is in the message); tightening it further is optional.
  - **R9 RESOLVED:** `QUOTE_MINIMAL` (stdlib default) is documented as the codec's quoting contract in the module docstring. The exact-line write assertions rely on it.
  - **R5 decided (unchanged): keep `CsvError` for the `.xlsx` seam in v1** — no `pipelex[tabular]` extra exists yet, so `MissingDependencyError`'s install hint would be a false promise. Revisit when the xlsx codec + `tabular` extra ship.
  - **Still-open deferred review items:** **R10** — hermetic `conftest.py` for the e2e subprocess tests (Phase 4). **R11** — tighten the integration country check to an exact multiset (optional). **R12** — opt the caller-fixable input errors (`CsvColumnError`/`CsvCoercionError`/`CsvFlatnessError`) into `_authors_caller_facing_message = True` for STRICT disclosure; keep OFF on `CsvReadError` (revisit during Phase 3 when these errors first surface through the runner boundary).
- **Open questions blocking:** none. Dry-mode runtime semantics already RESOLVED in Phase 1 (PipeBatch cardinality + PipeCompose field extraction both preserved).
- **Notes for next session — IMPLEMENTED codec API (Phase 3/4 call these; signatures FINAL):**
  - **Module:** `pipelex/tools/tabular/csv_codec.py` (stdlib `csv`; no `from __future__ import annotations`). Public:
    - `read_rows(path: Path, *, delimiter=",", encoding="utf-8") -> list[dict[str, str]]` — pure primitive, no suffix gate.
    - `write_rows(path: Path, headers: list[str], rows: list[dict[str, str]], *, delimiter, encoding) -> None`.
    - `flat_field_names(row_model: type[StuffContent]) -> list[str]` — SHARED flatness classifier (raises `CsvFlatnessError`); returns declared order. Accepts `str/int/float/bool/date/datetime/Literal/Enum`, optionals; rejects unions/list/dict/nested/`Any`.
    - `assert_supported_table_suffix(path: Path) -> None` — `.csv` ok; `.xlsx` → `CsvError` naming `pipelex[tabular]`; else generic `CsvError`. **Called by both binding entry points** (NOT by the raw `read_rows`/`write_rows`).
    - `list_content_from_csv(path, row_model: type[T], *, delimiter, encoding) -> ListContent[T]` ← **Phase 3 input hook calls this.** Empty cell → `None` before validate (`value or None`); extra/missing-required column → `CsvColumnError`; missing-optional → field default; per-row `ValidationError` → `CsvCoercionError` naming row+field.
    - `csv_from_list_content(list_content: ListContent[T], row_model: type[T], path, *, delimiter, encoding) -> None` ← **Phase 4 `--save-csv` calls this.** Headers from declared `row_model` (empty list → header-only); `model_dump(mode="json")` then `_to_cell` maps `None`→`""`, bool→`true/false`.
    - Internal helpers (don't call from outside): `_read_table`, `_row_to_dict`, `_validate_header`, `_is_flat_annotation`, `_to_cell`.
  - **Error taxonomy (LOCKED):** `pipelex/tools/tabular/exceptions.py` → `CsvError(ToolError)` (`error_domain = ErrorDomain.INPUT`) + `CsvReadError`, `CsvFlatnessError`, `CsvColumnError`, `CsvCoercionError`.
  - **Fixtures:** `tests/integration/pipelex/csv/csv_demo/{csv_demo.mthds, people.csv, inputs.json}`. Inline `[concept.X.structure]` blocks auto-generate registered `StructuredContent` subclasses (fetch via `concept.get_structure_class()`).
  - **RED tests awaiting Phase 3/4:**
    - `tests/integration/pipelex/csv/test_csv_roundtrip.py` (`TestCsvRoundtrip`): (a) `test_csv_input_builds_typed_list` → still RED until the Phase-3 hook lands; (b) `test_list_content_writes_to_csv` → **already GREEN** (verified 2026-06-01 — `csv_from_list_content` resolves `PersonSummary` via the real library + writes the exact `name,country,summary` lines); (c) `test_full_pipeline_round_trip` → RED until Phase 3+4. Run (c) under `dry_runnable or not (inference or ...)`.
    - `tests/e2e/pipelex/cli/test_csv_run.py` (`TestCsvRun`): wiring/`--save-csv` checks → RED until Phase 4. Subprocess tests are `gha_disabled`, inherit ambient env, `cwd=tmp`. The forwarding test still has a `# type: ignore[arg-type]` to simplify once `save_csv` exists.
