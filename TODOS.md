...
- **2026-05-15 — Phase 13 landed. Sweep DONE.** Final integration check complete. Decisions:
  ...
  - **Category policy for response-shape failures.** Two buckets: (a) "provider returned a payload but it is empty/malformed" → `error_category=CONTENT` + `UserActionKind.CHANGE_INPUT` + `provider_metadata=None` (mirrors Phase 9's LLM treatment of "empty choices" / "content is None"); (b) "provider returned no response at all" (`response is None` after retries exhausted) → `error_category=UNKNOWN` + `UserActionKind.CONTACT_SUPPORT` + `provider_metadata=None`. `provider_metadata=None` is correct on all of these — they are non-SDK paths with no provider error code, status, or request id to extract.
