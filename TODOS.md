...
- Code state: **clearly-described + uncommitted**, pending the user's commit/push go-ahead (this ledger's HARD-STOP-1 precedent is to not commit without an explicit ask). Dirty: `pipelex/temporal/config_temporal.py`, new `tests/unit/pipelex/temporal/test_dispatch_options_no_temporalio.py`, and this `TODOS.md`. `uv.lock` untouched. (Previous phases are on `refactor/Dataclasses`; this lands as the Phase-5/6 commit on top of `87cb9541`.)
