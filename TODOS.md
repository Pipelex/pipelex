...
- **Status:** **STAGE 1 PART 1 DONE (2026-07-12)** — building-methods (35 pages) swept via the wave Workflow: 452 claims checked, 47 confirmed / 0 rejected by adversarial verify (D6), then **3 reclassified as authoring convention → 44 drift findings** (25 breaks-a-user / 11 wrong / 6 misleading / 2 stale-detail; density 1.26, 26/35 pages dirty). Findings + defect-class analysis + dogfood observations in `wip/drift-hunt/findings/building-methods.md`; running scoreboard in `wip/drift-hunt/findings/SUMMARY.md`. No fixes applied (fixes = Stage 2). **New scope ruling (Louis, 2026-07-12): omitting scaffolding — a top-level `domain`, a pipe `description`, or the `prompt` body — in a focused example is authoring CONVENTION, not drift; a reader fills it in. A written prompt that ignores its own declared input is still drift. The `validate-doc-bundle-examples` derived check must inject placeholder `description`/`domain` (and tolerate omitted prompts) before validating.** Load-bearing rules hand-spot-checked (all held). Earlier: CHECKPOINT 0 CLOSED (D7–D10), Phase H DONE, Phase B DONE, Stage 0 DONE, gates green.
- **Next:** **Part 2 NOT started — awaits Louis' explicit go** ("resume the Drift Hunt from TODOS.md — run Stage 1 Part 2"). Part 2 = under-the-hood + advanced (26 pages). ⚠ **Carry into Part 2:** `docs/under-the-hood/execution-graph-tracing.md:269` has the same stale `execute_pipeline()` symbol as building-methods F44 (a Part-2 page; verifier spotted it while checking F44). Wave Workflow cost for Part 1: 82 agents, ~6.1M tokens, ~11 min wall. Reusable script: `.../workflows/scripts/drift-hunt-building-methods-wf_d707ca81-495.js` (adapt PAGES + seeds for Part 2).
