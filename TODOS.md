...
- **Current phase:** Phase 1 — **1.1–1.4 DONE; 1.5 + 1.6 + 1.7 DONE; CHECKPOINT B ✅ PASSED 2026-07-10.** The `pipelex codegen` CLI family (`types`, `inputs`) landed over `emit_types`/`input_renderer`; the **ts binder** (`binder.ts`) landed; **B1-2 resolved** (native-backed refinement bases keep their `refines` link, round-trip preserved). Quality gates: strict-pyright-on-emitted-Python (D7) + serialize→parse round-trip + resolve→emit e2e over fixture 2. Cold review triaged (critical ts-binder key-corruption fixed via D10 wire-native keys; B1-2 coverage gap closed). `make agent-check` + full `make agent-test` PASS; mthds `mkdocs --strict` builds.
- **Next action:** **Phase 2 — trust chain** (stamps · `codegen.lock` · offline `codegen check` · write-if-changed) in a fresh session, then 2.2 starter conversion. **Deferred to a later phase (documented, flagged for Louis at/ before Phase 2):** B1-1 normalization-layer policy for `structure="<ClassName>"` (emitter floor DONE; hard-error-vs-disable + single-source-of-truth discussion pending); B1-4 explicit defaults/multiplicity (Phase-1-later, not a B blocker); **B1-5 cross-package fold-in + fixture 1** (reframed — Phase-1 types/inputs projections don't need it; Phase-3 pipe-graph concern needing a canonical cross-package ref-naming/collision decision); `build structures` full re-point (D9); **D10** ts-zod wire-native-vs-camelCase — confirm before the starter conversion consumes the ts output. See `wip/devx/deferred-checkpoint-b1-items.md`.
- **Last checkpoint passed:** **Checkpoint B** — engine + emitters + CLI landed; cold review triaged (1 critical fixed, 1 coverage gap closed, 2 mthds doc fixes); decisions D9/D10 PROPOSED + flagged; gates green. (Next checkpoint is **C** after 2.2 starter conversion.)
- **Last commit(s):** mthds `feature/Codegen` — …`9104ea6`, **B step-3 `c077af9`**, **B review `7b5573f`** · workspace-root `feature/Follow-ups` — …**B1 `5fd7ed1`**, **B deferred-doc `e603a3a`** · conformance `feature/Codegen` `d42cadb` (untouched this phase) · pipelex `feature/Devex-codegen` — …**1.4 `002ec98a4`**, tracker `db8458ea5`, **CHECKPOINT-B work `a1f73cc46`**, **B review-fix `7e1eb01a5`**. **Review diff for the NEXT checkpoint (C) = `7e1eb01a5..HEAD`.** **None pushed.**
- **Open decisions:** D1–D7 **SETTLED**; **D9 PROPOSED** (defer `build structures` engine re-point to Phase 2 — behavior-preservation impossible against the divergent bare-when-unique engine). D8 OPEN (settle at C — `mthds-starter-js` conversion).
