...
**CHECKPOINT 3 — in progress (2026-07-03).** Gates green (`make agent-check`, `make agent-test`). Committed as `767baa5` on platform `feature/inference-profiles` (branched off `dev` @ `08f22bf`). Fresh-context Sonnet `/code-review` dispatched on the commit — triage before Phase 1d. **Phase 1 exit criteria after triage:** profiles can be created/listed/bound; the ref field exists in core; nothing consumes it yet.
...
**Last updated:** 2026-07-03 — Checkpoints 1–2 CLEARED; Checkpoint 3 review in flight.

- **Where we are:** Phase 1a (core ref) DONE — `4921a031d` on core `feature/BYOK-per-request`. Phase 1b (pipelex_shared foundation) DONE — `725875c` on infra-python-tools `feature/inference-profiles` (rebased onto `origin/main`, its real mainline). Phase 1c (platform CRUD + X-Org-Id forwarding) IMPLEMENTED — `767baa5` on platform `feature/inference-profiles`; gates green; Checkpoint 3 code-review fan-out dispatched, triage pending. No branch pushed anywhere.
- **Deviations from plan so far:** ID prefix `iprof` (not `ip`); ciphertext read is opt-in `get_profile_with_ciphertext` (public-by-default reads); default binding on its own path `/organizations/{org_id}/default-inference-profile`; scrubber hardening + `save_method` timestamp fix rode along as flag-and-fix in pipelex_shared.
- **Branches:** core `_byok` = `feature/BYOK-per-request` @ `4921a031d` (+tracker commits). `infra-python-tools` = `feature/inference-profiles` @ `725875c` (off `origin/main`). `pipelex-platform` = `feature/inference-profiles` @ `767baa5` (off `dev` @ `08f22bf`); **uncommitted editable-pin override in `pyproject.toml`/`uv.lock`** (deliberate — see the Phase 1c pin task).
- **Pending sign-offs:** Q6 (hosted adapter location) — before Phase 2b; Q2 launch posture — Checkpoint 5; infra task-role split + CMK key policy — Checkpoint 4; **push approval** for the feature branches (blocks the platform pin-bump landing).
- **Next:** triage Checkpoint 3 review → Phase 1d (infra KMS CMK + platform encrypt grant) or pause for sign-offs.
