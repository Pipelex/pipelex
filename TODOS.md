...
- **Status:** **STAGE 1 PART 3 DONE (2026-07-12)** — features (21) + reliability (4) = 25 pages swept via the wave Workflow (`wf_1fd524ac-dbd`): 308 claims checked, **10 confirmed / 0 rejected** by adversarial verify (D6), **0 reclassified** (D11 doesn't apply — capability pages carry no bundle examples). Severity: 2 breaks-a-user / 7 wrong / 1 misleading / 0 stale; **density 0.40 (8/25 dirty) — the cleanest section of the campaign by ~4×**, and **all four reliability pages are clean**. Findings in `wip/drift-hunt/findings/features-reliability.md`; scoreboard updated in `SUMMARY.md`. No fixes applied (fixes = Stage 2). **Running Stage-1 total: 1,195 claims, 96 drift, 3 convention, 0 rejected.** ⟶ Parts 1–2 recaps below. Earlier: CHECKPOINT 0 CLOSED (D7–D10), Phase H DONE, Phase B DONE, Stage 0 DONE, gates green.
- **Part 3 shape notes (matter for Part 4):** these pages are SHORT capability/landing pages (20–130 lines, almost no code fences), and Stage 0's pre-screen had found **zero** suspect rows in both sections — so every agent started seedless, and seeds were hand-built from the Part 1/2 defect classes. Two NEW review rules had to be invented and are now baked into the Part-3 script's CTX (**reuse them in Part 4**): (a) **exhaustive vs illustrative lists** — for a *closed* list a missing shipped member is drift; for a hedged list ("such as"/"and more") only a *named-but-nonexistent* member is drift, and an omission is an out-of-scope completeness wish (ambiguous ⇒ treat as illustrative); (b) **floor counts** — "60+ models" is false only if the real number is *below* it. Also added: cross-repo guardrails (`plxt` and the skills plugin ship from `../vscode-pipelex/` + `../mthds-plugins/`, both reachable — run/read them, never call them phantom) and the distributed-execution product rulings (private plugins, not on PyPI ⇒ "no install instructions" is not drift; no Temporal wire-format flagging). **Dominant Part-3 defect class: stale CLOSED enumerations** (native concepts 11-of-15; operator roster 6-of-7, `PipeStructure` missing on 2 pages; retired `flux-2-pro` still listed) → new drift-contract candidate `doc-enumerations-vs-registry`.
- **⚠ False-negative discipline (new, learned in Part 3):** a *clean* result on a page you seeded as high-risk is the campaign's most likely place to fool itself. Part 3's high-risk clean pages were therefore hand-verified after the run (retry config keys + defaults, telemetry enums, cost-tracking snippet imports, `deepseek-ocr` handle) — **all held, so the low density is real**. Do the same in Part 4.
...
- **Next:** **Part 4 NOT started — awaits Louis' explicit go** ("resume the Drift Hunt from TODOS.md — run Stage 1 Part 4"). Part 4 = the tail (18 pages): get-started, contribute, tools non-CLI, setup, viewpoint, root index/license + the D9 additions (`docs/agents/debugging-hanging-pytest-runs.md`, `docs/analytics/data-extraction.md`, `docs/distributed-execution/index.md`, `docs/CLAUDE.md`, plus root `CONTRIBUTING.md` + `CODE_OF_CONDUCT.md` — the two root files got NO Stage 0 pre-screen, so they start seedless). Part 4 completes Stage 1 ⟶ **CHECKPOINT 1 (STOP for Louis)**. Wave cost for Part 3: 35 agents, ~2.7M tokens, ~6 min wall. **Reusable script (start from this one, not Part 2's — it carries the new rules): `/private/tmp/claude-501/-Users-lchoquel-repos-Pipelex--drift/517bfc93-228b-4e3c-a108-4e99c4a8a75c/scratchpad/drift-hunt-features-reliability-part3.js`** (swap PAGES + seeds; the CTX's PART3 rule block generalizes as-is). ⚠ **One outgoing carry-over from Part 3:** `docs/tools/cli/agent-cli.md:38` omits `--mock-inputs`'s `(requires --dry-run)` qualifier — out of Stage-1 scope per D2 but reopened by F1; fold into Stage 2, don't re-review it in Part 4.
...
- **Open questions:** none blocking — only the explicit go for Stage 1 Part 4 (the last part; it closes Stage 1 and opens Checkpoint 1).
...
## Stage 1 — semantic review fan-out (shape settled per D10; Parts 1–3 DONE, Part 4 pending its go)

Four parts, one cold-start session each (D10), 104 pages total. Cookbook pages are excluded (Stage C, per D7). **Each part starts only on Louis' go.** Actual burn so far: Part 1 + Part 2 + Part 3 ≈ 4.5M + 4.8M + 2.7M tokens.
...
- [ ] **Part 4 — the tail (18 pages).** get-started, contribute, tools non-CLI, setup, viewpoint, root index/license, plus the D9 additions (4 pages + root CONTRIBUTING.md/CODE_OF_CONDUCT.md — no Stage 0 seed rows for the two root files). **Completes Stage 1 ⟶ Checkpoint 1.**
