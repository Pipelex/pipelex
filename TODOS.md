...
- **DONE (Phases 1–4) — re-cut `execution_mode` into `orchestration_mode` (open string token) + `DeliveryMode` (closed enum)** — plan [`wip/plugins/orchestration-mode-and-delivery-split.md`](wip/plugins/orchestration-mode-and-delivery-split.md). **Phase 1 (core, `_plugins` `bd4e52827`) + Phase 2 (`pipelex-temporal` `01d7dc3`) + Phase 3 (`pipelex-api` `dd84a9a`, THE gate) landed on `feature/orchestration-mode-delivery-split` in each repo — gates green + clean-context `/code-review` each (Phase 3: no must-fix). Phase 4 = the external `pipelex-mistralai-workflows` plugin migrated to the new core surface (deleted-enum import + registration under the deleted member + own `execution_mode` wire field + old `run` signature → `orchestration_mode`/`delivery`/new protocol), commit `c2013fb` on `feature/Mistral-native`, green against the merged core (agent-check + full integration suite). §8 `/start`-honesty risk resolved (honest 400 safe). DEVIATION (Phase 4): the Mistral plugin pins core editable at `../_workflows` (branch `feature/mistralai-2x-bump`), NOT the orchestration-split worktree — so Phase 4 first required merging `feature/orchestration-mode-delivery-split` into `feature/mistralai-2x-bump` (`_workflows` merge commit `a1448ad8f`, conflict-free; core gates green) to give the plugin a core carrying BOTH the split and the mistralai-2x bump. Spec/conformance untouched (no `docs/specs/` heading or `conformance/` test asserts mode values → `make check-spec-links` not triggered). See the plan's §10 Phase 4 as-built.** **Supersedes the flat-`PipelexExecutionMode` framing of the two bullets above:** it removes the conflation those plans worked around — `_async_start_mode`, the `/execute` fire-and-forget 400, the `ApiConfig` f&f field-validator, and the `/validate` double-registration. Two orthogonal axes: an **open** orchestration token (core owns only `"direct"`; plugins own `"temporal"` / `"mistralai-workflows"` — realigns with "the base names no orchestrator") + a **closed** `DeliveryMode {BLOCKING, FIRE_AND_FORGET}` that endpoints set, never receive. `/start` becomes **honest** (4xx when the resolved mode can't do async) instead of degrading. Design **locked 2026-06-22**; Direction A (core → `pipelex-temporal` → `pipelex-api`, one repo/branch/commit per phase). **Top risk to settle before the API phase:** does the MTHDS-Protocol/conformance suite require `/start` to return `202` on a non-async (`direct`) backend? See the plan's §8.
