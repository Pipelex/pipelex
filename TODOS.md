...
- **Next concrete action:** start **Phase 2** — implement `pipelex/tools/tabular/csv_codec.py` (fill the bodies, keep signatures) until `tests/unit/pipelex/tools/tabular/test_csv_codec.py` goes GREEN. The public API + error taxonomy are LOCKED (see "Phase-1 deliverables" below); do NOT rename them. **FIRST read [`wip/csv-support/phase1-review-findings.md`](wip/csv-support/phase1-review-findings.md) and fix R1–R7** — a `/code-review` found three Phase-1 tests that are RED-for-the-wrong-reason (can never go green: e2e `splitlines()` count, NUL-byte premise false on Py3.13, single-column empty-cell write assertion) plus skeleton docstring/taxonomy inconsistencies (read_rows/write_rows error classes, `.xlsx` should raise `MissingDependencyError`, Csv* titles). Fixing them keeps the RED state honest before implementing.
...
- [x] Run the new tests; confirmed they fail **for the right reasons** (codec `NotImplementedError`; `.csv` input parsed as a record → `ValidationError`/`PipeExecutionError`; `save_csv` missing from CLI signatures; `--save-csv` → typer exit 2 "No such option"), **not collection errors**. 56 tests, all RED. `make agent-check` GREEN repo-wide.
