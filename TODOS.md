...
- [ ] **Tests first (red→green).** New `tests/unit/pipelex/core/memory/input_shaper/` (one TestClass per module + shared `data.py`): one arm per D5 row — Text-refining ← str; Number-refining ← int/float and **rejects bool** (the `bool ⊑ int` trap) and rejects `"42"` (no cross-type parse); YesNo-refining ← bool; Date-refining ← date/datetime obj + ISO str; Image/Document-refining ← bare str + `{"url":…}` dict; Structured ← dict (pydantic validates; missing-required-field → D4); Dynamic/Anything ← bottom-up passthrough. D2: list shapes element-wise into `ListContent[declared]`; single bare value auto-wraps; empty list → empty `ListContent[declared]`; `X[N]` count-checked (single satisfies `[1]`, not `[2]`); list-under-singular → D4. D6: envelope `{concept, content}` compat-checked, explicit-refining-concept wins, incompatible → D4; the exact-two-keys collision rule. D8: unknown provided name → error listing declared names. D9 guards: top-level `null`/`None` → D4 (absence = omit key); bool never leaks into the Number arm. Each failure asserts the rendered expected-shape template appears in the message.
- [ ] **New module `pipelex/core/memory/input_shaper.py`** — `InputShaper.shape(pipeline_inputs, *, input_specs: InputStuffSpecs, search_domain_codes) -> WorkingMemory`. Internal `resolve_input_kind(concept) -> InputKind` (new `StrEnum`: TEXT/NUMBER/YESNO/DATE/IMAGE/DOCUMENT/STRUCTURED/DYNAMIC) composed from ordered `is_compatible(concept, get_native_concept(X), strict=True)` checks + the structured/dynamic fallback (exhaustive `match/case`, no `case _`). Per-input algorithm: D8 name check (batch, first) → look up spec → envelope/explicit-object arm (D6 compat) → D2 multiplicity peel → D5 dispatch on kind → build `Stuff` typed as the declared (or compatible-more-specific) concept. Delegate Dynamic / out-of-matrix natives / already-built `StuffContent` to `StuffFactory.make_stuff_from_stuff_content_or_data`.
- [ ] **New error classes in `pipelex/core/memory/exceptions.py`** (D4) — an `InputShapingError` base + specific subclasses (wrong scalar kind; list-where-singular; count mismatch; structure validation failed; explicit concept incompatible; unknown input name; null-at-top-level). Follow the `InputsTimeOnlyNotSupportedError` precedent: `error_domain = ErrorDomain.INPUT`, class-level `user_action = UserAction(CHANGE_INPUT, …)`, `_authors_caller_facing_message = True`. Each message names the input, the declared concept ref, what was provided, and **renders the expected shape** via `stuff_spec.render_stuff_spec(...)` / `InputStuffSpecs.build_inputs_template()`. `make gep`.
- [ ] `make agent-check` green; targeted pytest on the new modules green; `make tb`.
