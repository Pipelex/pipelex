...
- **Status:** **STAGE 1 PART 1 DONE (2026-07-12)** — building-methods (35 pages) swept via the wave Workflow: 452 claims checked, **47 confirmed findings, 0 rejected** by adversarial verify (D6). Findings + defect-class analysis + dogfood observations in `wip/drift-hunt/findings/building-methods.md`; running scoreboard in `wip/drift-hunt/findings/SUMMARY.md`. No fixes applied (fixes are Stage 2). Severity: 28 breaks-a-user / 11 wrong / 6 misleading / 2 stale-detail; density 1.34 find/page, 26/35 pages dirty. Load-bearing rules driving the batch were hand-spot-checked against code (all held). Earlier: CHECKPOINT 0 CLOSED (D7–D10), Phase H DONE, Phase B DONE, Stage 0 DONE, gates green.
...
- [x] **Part 1 — building-methods (35 pages). DONE 2026-07-12.** 47 confirmed findings, 0 rejected. → `findings/building-methods.md`, scoreboard `findings/SUMMARY.md`. Dominant class: doc bundle examples that fail `pipelex validate bundle` (~18). Strongest drift-contract candidate: a `validate-doc-bundle-examples` derived check.
