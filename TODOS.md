...
**NEXT (resume here):** **Phase 3 — mthds-js (T5)** in `../mthds-js` (0.10.0 → 0.11.0). Type `PipelexValidationReport` + `ValidationErrorItem`; surface `validationErrors` on `ApiResponseError` (422); add named contents to `validate()` (send `mthds_names` per Issue 5); type `implementation_version` on `VersionInfo`; publish to npm. Then vscode (T6–T11). The whole train still releases LAST (pipelex 0.34.0 → re-pin pipelex-api `==0.34.0` → mthds-js → vscode).
