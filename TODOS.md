...
> - **Next entry point:** **FIRST apply the seam-extraction review fixes** — [`seam-extraction-review-fixes.md`](wip/dry-run-refactor-draft/seam-extraction-review-fixes.md) (xhigh code review found a confirmed `open_library`-id teardown-all landmine, a widened report-registry leak, and an asymmetric current-library-restore — all in or adjacent to the new seams; cheaper to fix before Phase 2 composes them). **THEN Phase 2 — Build `BundleValidator`** (`pipelex/pipeline/bundle_validator.py`), composing `acquire_library` + `prepare_pipe_job` against a direct in-process `PipeRun`. Re-verify the seam symbols still exist before editing.
