...
**Worktree:** `_hub/` · **Branch:** `refactor/Modularity-3` · **Base:** `refactor/Hub-2` (PR #1064, still open — see [Gating](#gating)) · **Status:** **M3 complete, at CHECKPOINT M3.** M1 and M2 not started.
...
| tests pinning pipe classes into the class registry | 0 | **1** | ✅ **1** module / 3 tests, both mutation checks red |
...
- **State:** **M3 done and committed, sitting at CHECKPOINT M3.** Phase 0 baseline captured and reproduced exactly; all M3 exit criteria met (see the actuals column); all gates green (`cko`, `agent-check`, full `agent-test`, `drift-check`, `chl`, `tb`, `gep` no-churn). M1 and M2 not started. All review findings folded in; see [Review findings](#review-findings).
