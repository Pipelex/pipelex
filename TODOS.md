...
- [ ] **(E1 — cheap half, do alongside the assembly rework)** Gate the per-pipe graph serialization in `pipe_abstract.py:487-498` on `parent_graph_context.emit_graph_events` (AND the existing `data_inclusion.*`), so costs-only mode (`--no-graph --costs`) stops paying `smart_dump()` / `rendered_pretty_text()` / `rendered_pretty_html()` / `model_dump(mode="json")` per pipe for a `GraphSpec` that is then discarded. `on_pipe_start` still mints node ids (usage-event `node_id` correlation is unaffected) — only the discarded data payloads are skipped. **The deep optimization (skip the in-memory tracer entirely in costs-only mode) stays deferred (§7);** this gate captures the dominant cost (documents, image data-URLs) for the common "cost tracking without graphs" run, which is now more expensive than the old `--no-graph` (which opened no tracer at all).
