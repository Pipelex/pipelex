...
- [x] `reporting_manager.py` `_emit_usage_event_runner_fallback`: added early-return `if not graph_context.emit_usage_events: return`. Fast path confirmed only reachable when costs on (`set_event_log` only called when costs on).
...
- **CP1 (DONE — transport decoupled):** Emission now keys off `is_generate_costs` independent of `--graph`, in BOTH modes. DIRECT (`pipeline_run_setup`): tracer opened when graph OR costs; `event_log` created when tracing enabled; passed to the graph tracer only when graph on (costs-only → `event_log=None`, tracer still mints node ids); `set_event_log` (usage transport) only when costs on; the run's `GraphContext` carries `emit_graph_events`/`emit_usage_events` (propagated via `copy_for_child`). TEMPORAL (`wf_pipe_router`): same gating, plus the emit flags are copied onto `wf_graph_context`. The runner-side usage fallback (`reporting_manager._emit_usage_event_runner_fallback`) early-returns on `not emit_usage_events`; the fast path is structurally only reachable when costs on.
  - **Gate sites touched (final inventory):** `pipeline_run_setup.py` (tracer-open gate + `event_log if is_generate_graph else None` + flags + `set_event_log` gate), `runner.py` finally (tracer-close gate), `reporting_manager.py` fallback (usage gate), `wf_pipe_router.py` (tracer event-log + `set_event_log` gates + flag carry), and **one new site beyond the original inventory:** `pipe_run.py` DIRECT finally gates `assemble_graph_on_output` on `emit_graph_events` (preserves `--no-graph` now that the event stream is shared — see the Phase-1 checklist note). No `match`/`case` exhaustiveness sites were involved (the gates are plain booleans).
