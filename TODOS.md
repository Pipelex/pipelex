...
> **⤳ SUPERSEDED by the validate/HTTP conformance plan (workspace `TODOS.md`, 2026-06): two T4-era divergences are now reversed.**
> 1. **Conformance HTTP arm is BUILT** — the "conformance HTTP arm stays deferred (D5)" divergence (T4 below) no longer holds. `conformance/` now boots `pipelex-api` as a subprocess and asserts the **real wire** for `/validate`, `/models`, `/version`+`/health` (D-G), with the conformance CI installing the sibling (D-H). The spec's HTTP-section `<!-- unverified -->` markers were retired/narrowed accordingly. The `wip/conformance-http-arm.md` brief is marked revived/built.
> 2. **`/validate` reframed to a 200-diagnostic endpoint** — the old "invalid bundle → 422 problem" contract this plan assumed (Decisions-locked "One call returns both" / "Per-pipe FAILURE as data") is replaced: **every produced verdict (valid OR invalid) rides a 200 discriminated on `is_valid`**; non-2xx is reserved for no-verdict conditions (request-shape 422, auth, 5xx). pipelex emits `is_valid` + the `dry_run` category + signatures-as-data; pipelex-api returns the `ValidReport | InvalidReport` union; the consumers (mthds-js typed `ValidationResult` union, the vscode backends) read `is_valid` off the body and the fabricated-`blueprint_validation` synthesis sites are deleted (vscode followup #10 closed). The structured-info invariant is now total (every invalid verdict carries a non-empty `validation_errors[]`). The `mthds_names`→`mthds_sources` rename also propagated to mthds-js. The 200-diagnostic contract landed in the MTHDS open standard (`mthds/`) too (D-F). **Release train unchanged:** still pipelex 0.34.0 LAST → pipelex-api re-pin `==0.34.0` + image → mthds-js publish → vscode re-pin + release.
...
- **Phase 4 / Checkpoint 4 (vscode-pipelex, T6–T11)** — **DONE (uncommitted)** on branch `feature/Use-pipelex-api` in `../vscode-pipelex`. Backend abstraction (`ValidationBackend` + `CliValidationBackend` single-spawn exit-code discriminator + `ApiValidationBackend` over `MthdsApiClient`), settings, SecretStorage key commands, multi-file disk gather + per-item `mthds_names`, cross-file diagnostics via `source`, `MIN_AGENT_VERSION`→0.34.0, lenient API version gate, transport-failure policy, privacy confirm. `mthds` pinned `portal:../../../mthds-js`. Full vitest suite GREEN (141 tests, 37 new), `tsc` clean, node+browser+full builds green, bundle verified (no telemetry leak; `mthds` isolated to the API-backend chunk). See Checkpoint 4 below for as-built.

**NEXT (resume here):** **Phase 5 — vscode-pipelex: docs polish, manual QA, release prep.** Core docs/CHANGELOG already landed with Phase 4 (see Checkpoint 4). Remaining: manual QA of both backends against a real `pipelex-api` (valid/invalid/multi-file/hosted-key), then the **release-LAST** train: cut pipelex 0.34.0 → re-pin pipelex-api `==0.34.0` + ship image → publish mthds-js 0.11.0 → re-pin vscode `mthds` to the published version + release the extension. The whole train still releases LAST.
...
> **CHECKPOINT 4 — ✅ REACHED (code + tests + docs), uncommitted.** The extension validates and renders graphs through either backend, selected by `pipelex.backend`, with full diagnostics parity on the API path. CLI remains the default and behaviorally unchanged.
>
> **As-built (2026-06-16, `../vscode-pipelex` branch `feature/Use-pipelex-api`, UNCOMMITTED):**
>
> - **Backend seam** (`editors/vscode/src/pipelex/validation/`): `backend.ts` (`ValidationBackend` iface, flat `ValidationOutcome {ok, errors}` — discriminated union dropped because the extension tsconfig is `strict:false` and boolean-literal discriminants don't narrow there — `BundleAnalysis`, `BundleFile`, `BundleRequest`, `BackendError` taxonomy `{not-found|too-old|unreachable|infra|declined}`, `AnalyzeAbortError`, `GraphAnalysisSink`). `cliValidationBackend.ts` (single spawn; `--format json` always so BOTH streams are machine-readable — the agent CLI defaults error output to markdown, which the OLD on-save path silently mis-parsed; exit 0 → ok [+`graphspec` when `--view`]; exit 1 → parse `validation_errors`, OR synthesize one diagnostic from `message` when `error_domain==='input'` with no list, else infra/too-old). `apiValidationBackend.ts` (one `client.validate(contents, true, names)`; 422-with-`validationErrors` → ok:false; 422 `ValidateBundleError` no-list → synthetic; auth/5xx/unreachable/non-`problem+json`/timeout → `BackendError` notify-no-fallback; abort+timeout race around the uncancellable client call). `backendFactory.ts` (reads `pipelex.backend`/`pipelex.api.baseUrl` per call; owns shared `ApiVersionGate` + per-host remote-consent in globalState). `apiKey.ts` (SecretStorage→env token, Set/Clear commands). `apiVersionGate.ts` (`parseCleanRelease` lenient — prerelease/dev/non-semver = capable; warn-once below floor; never throws/blocks). `bundleGather.ts` (flat non-recursive `*.mthds`/`*.plx` disk gather = CLI `--library-dir` parity; primary first; documents the nested-dir/installed-lib/symlink divergence). `crossFileDiagnostics.ts` (owner = `source` → declaration scan `[pipe.X]`/`[concept.X]` → primary fallback; places diagnostics across sibling URIs).
> - **Consumers refactored.** `pipelexValidator.ts` is the single on-save orchestrator: gathers files, `withGraph = panel.isShowingMthds(uri)`, ONE `analyze()`, cross-file diagnostics keyed per-directory (each save replaces that dir's diagnostic set), and feeds `graphSink.applyAnalysis()` (no 2nd call). `methodGraphPanel.ts` routes graph generation through the backend (`getBackend` injected; defaults to `CliValidationBackend` so its ~22 existing tests — which mock `spawnCli`/`resolveCli` — still pass); its `.mthds` save subscription self-refreshes ONLY when validation is disabled (else the validator drives). `pipelexExtension.ts` wires the factory, registers key commands, calls `validator.setGraphSink(panel)`.
> - **T8 floor bump** via the `bump-pipelex-version` skill: `MIN_AGENT_VERSION` 0.31.0 → **0.34.0** (reason: structured `validation_errors[]` `source`/`field_name`); too-old message reason text updated; CHANGELOG `### Changed` records the **compatibility-floor break**.
> - **Settings/commands** (`package.json`): `pipelex.backend` enum (default `cli`), `pipelex.api.baseUrl` (default `http://localhost:8081`), `pipelex.setApiKey`/`pipelex.clearApiKey`. **Dep:** `"mthds": "portal:../../../mthds-js"` (local pin; publish deferred).
> - **Tests (T11):** 37 new across `crossFileDiagnostics`, `apiValidationBackend`, `cliValidationBackend`, `apiVersionGate`, `bundleGather` (`.test.ts`). Existing 104 still green (updated 1 mock to add `setGraphSink`). `yarn test` 141/141, `tsc --noEmit` clean (only pre-existing webview/LSP DOM-lib errors remain), `yarn build` (node + browser + webview) green. Bundle verified: no `posthog`/CLI deps leaked; `MthdsApiClient` lands only in the dynamically-imported `backendFactory` chunk.
> - **Docs:** `docs/features/validation-backends.md` (CLI vs API, settings, self-hosting, key handling, privacy, cross-file, version gating); CHANGELOG `[Unreleased]` `### Added`/`### Changed`.
>
> **Deviations from the plan sketch (deliberate):** (1) `ValidationBackend` has NO `getBackendVersion()` — version gating folded into each backend (CLI throws `too-old`; API warns via the gate). (2) Diagnostic source label `pipelex-agent` → `pipelex` (either backend can produce them). (3) `ValidationOutcome` flat, not a discriminated union (`strict:false` narrowing). (4) The 200-`validated_pipes`-FAILURE channel stays unsynthesized (deferred per Out-of-scope). (5) Concept declaration-scan finds only `[concept.X]` table-style, not `[concept]` block-style — `source` is the primary mechanism, so this is a best-effort fallback only.
>
> **Checkpoint 4 boundary — UNCOMMITTED + release deferred.** Code is uncommitted on `feature/Use-pipelex-api`. The whole train releases LAST (pipelex 0.34.0 → pipelex-api `==0.34.0` → mthds-js 0.11.0 publish → re-pin vscode `mthds` to the published version → release extension). **Record the released extension version here once cut.**
>
> **Code review (2026-06-16, recall mode, 15 findings).** The three must-fix correctness items were **fixed** in `../vscode-pipelex` (still uncommitted on `feature/Use-pipelex-api`; 142 vitest GREEN incl. +1 new, `tsc` clean on changed files, node bundle builds): (1) open graph panel no longer goes stale on a non-success save — `GraphAnalysisSink` gained `applyBackendError`/`applySkipped`, driven from the validator's catch + LSP-skip paths; (2) the 0.34.0 CLI floor is now enforced **up front** in `CliValidationBackend.analyze` (a `[0.31.0,0.34.0)` CLI emits `source`-less errors and previously slipped through the lazy-only check); (3) the per-URI in-flight cancel now runs before the LSP-skip early return. The other 12 findings (loopback `0.0.0.0`/`[::1]` detection, `endsWith` cross-file mis-match, slow-server "unreachable" message, unbounded `/version` probe, runtime-toggle staleness, dir-keyed diagnostics race, two divergent `BackendError`→UX switches, `extractJson` brace-span, duplicated `blueprint_validation` synthesis, unconditional gather, dead `backendKind()`) are **deferred with justification** in `../vscode-pipelex/wip/api-backend-code-review-followups.md` — none are silent must-fixes; pick from there around Phase 5 QA. Brand boundary + docs conventions verified clean.
...
- **mthds-js agent/CLI surfaces don't yet consume the new typed fields** (code-review "noted-only" items — all in files Phase 3 `ad74bde` did NOT touch; capture, don't fix blind):
  - **Agent `validate` envelope drops `validationErrors`.** `runProtocolValidate` (`src/agent/commands/api-commands.ts`) catches the 422 and emits `agentError(err.serverMessage ?? err.message, "ValidationError", …)`, discarding the now-typed `err.validationErrors`. Deliberate for Phase 3 (the VS Code extension is the intended consumer, the agent CLI surface wasn't scoped) — but if agents are ever meant to map per-error diagnostics, this is the wiring point. Splitting the diagnostic shape (per-pipe FAILURE ≠ line-level) interacts with the existing "200-`validated_pipes`-FAILURE channel" follow-up above.
  - **Dead `success: true` literal.** `agentSuccess({ success: true, ...report })` (`api-commands.ts` `runProtocolValidate`) spreads a report that now carries its own `success`, so `...report` overrides the literal. Harmless today (server always sends `success: true` on 200) but misleading; drop the literal and rely on `report.success`, or drop `success` from the typed report. Same pattern lives near `cli/commands/validate.ts`.
  - **`install.ts` renders "pipelex-api undefined".** `src/cli/commands/install.ts:155` builds `` `${ver.implementation} ${ver.implementation_version}` `` unguarded; a runner that omits `implementation_version` (now formally optional) prints a literal `undefined`. Pre-existing (the read predates Phase 3 via the index signature), but the field's new optional typing makes the unguarded read a flagged mismatch — guard with `?? ""` / a conditional join when touched.
...
- [x] **T6 (P1)** vscode — `CliValidationBackend` single-spawn exit-code discriminator (D1/Q1) — DONE (`cliValidationBackend.ts`; `--format json` always; exit-code discriminator; input-domain no-list → synthetic diagnostic)
- [x] **T7 (P1)** vscode — `ApiValidationBackend` transport/non-problem+json policy: notify, clear stale, no fallback (Issue 4) — DONE (`apiValidationBackend.ts` + validator `handleBackendError` clears dir + `notifyOnce`)
- [x] **T8 (P2)** vscode — raise `MIN_AGENT_VERSION`→0.34.0 (compat-floor break) + lenient API version parse (Issue 3) — DONE (via `bump-pipelex-version` skill; `apiVersionGate.parseCleanRelease`)
- [x] **T9 (P2)** vscode — privacy confirm before first remote request, directory-wide wording — DONE (`backendFactory.confirmRemote` modal, per-host globalState, fired before first non-localhost send)
- [x] **T10 (P2)** vscode — multi-file disk gather + CLI-resolution parity verify + send per-item names (TODO 1) — DONE (`bundleGather.ts` flat = `--library-dir` non-recursive glob verified in pipelex `bundle_cmd.py:103`; divergence documented; names threaded to API)
- [x] **T11 (P1)** cross-repo — tests: STRICT both-branches, CLI==API parity, transport, cross-file mapping, conformance — DONE (vscode: 37 new tests across backends/gather/cross-file/version-gate; STRICT both-branches + real-wire 422 verified upstream in pipelex + pipelex-api suites; conformance HTTP arm deferred D5)
