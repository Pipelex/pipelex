...
**Branch:** `fix/For-API-update` · **Status:** Phase 1 COMPLETE (CHECKPOINT 1 reached — transport decoupled, tree green). Next: Phase 2.
...
- [ ] **(A2 — do FIRST, single-owner prerequisite)** Thread `emit_graph_events` / `emit_usage_events` as params into `GraphTracerManager.open_tracer` (and `graph_tracer.setup`) so the returned `GraphContext` is born with the correct flags. This **removes both post-hoc `model_copy(update={...})` footguns** (`pipeline_run_setup.py:210`, `wf_pipe_router.py:102`) — the default-`True` field values currently make the safe behavior opt-in, so every caller must remember the second copy. Building Phase 2 on the born-correct context means the F1 gate below falls out of one decision instead of being patched onto each arm. (Keeps the field defaults `True` for direct constructions / `copy_for_child`.)
...
- [ ] DIRECT: generalize `graph_assembly.py` so the single `read_events` feeds both `GraphSpecAssembler` (if context `emit_graph_events`) and `UsageAggregator.aggregate` → `pipe_output.tokens_usages` (if `emit_usage_events`); top gate becomes `is_enabled and (graph or costs)` (D3, D5). Best-effort try/except mirrors the graph path; failures set `usage_assembly_error`. (This subsumes the standalone `emit_graph_events` gate added on `pipe_run.py:75` in Phase 1.)
- [ ] **(E1 — cheap half, do alongside the assembly rework)** Gate the per-pipe graph serialization on `parent_graph_context.emit_graph_events` (AND the existing `data_inclusion.*`), so costs-only mode (`--no-graph --costs`) stops paying `smart_dump()` / `rendered_pretty_text()` / `rendered_pretty_html()` / `model_dump(mode="json")` per pipe for a `GraphSpec` that is then discarded. **The xhigh review CONFIRMED the waste is wider than first scoped — gate ALL of these sites, not just the input block:**
  - `pipe_abstract.py:487-489` — **input** IOSpec (`smart_dump`/`rendered_pretty_text`/`rendered_pretty_html` per input stuff).
  - `pipe_abstract.py:575-577` — **output** IOSpec (same three calls on `main_stuff`). *(Originally un-named; confirmed in review.)*
  - `pipe_abstract.py:496-498` — **pipe/concept registry** (`self.model_dump(mode="json")` + per-concept `model_json_schema()`).
  - `graph_tracer` `teardown`/`close_tracer` — builds the full `GraphSpec` (node→NodeSpec + data/batch/parallel edge correlation passes) at end of every costs-only run, then the return value is dropped (`pipe_run.py:60`, `runner.py:209`). Skip the spec-building work when `emit_graph_events` is False (close-as-cleanup without assembling).

  `on_pipe_start` still mints node ids (usage-event `node_id` correlation is unaffected) — only the discarded payloads/spec are skipped. **The deep optimization (skip the in-memory tracer entirely in costs-only mode) stays deferred (§7);** this gate captures the dominant cost (documents, image data-URLs) for the common "cost tracking without graphs" run, which is now more expensive than the old `--no-graph` (which opened no tracer at all).
- [ ] TEMPORAL: `act_assemble_graph` → returns `TracingAssemblyResult{graph_spec, tokens_usages}` (read once); `WfPipeRun.run` Step 2 sets both on `pipe_output` (+ `usage_assembly_error` on `ActivityError`). Update `AssembleGraphArg`/activity name + worker activity registration (note rename blast radius: skill refs, tests). **(F1 — gate BOTH sites on `emit_graph_events`, else costs-only TEMPORAL yields a non-None `graph_spec` while DIRECT yields `None`):** (1) the `act_assemble_graph` dispatch in `wf_pipe_run.py:78-91` (skip it / don't set `graph_spec` when graph off), AND (2) the in-memory assignment in `wf_pipe_router.py:155-157` (`close_tracer` populates a full `GraphSpec` regardless of `event_log`, then assigns it unconditionally — gate that assignment). The router site is a SEPARATE assignment the original plan text did not name; both must be gated. No prod impact today (Temporal unshipped) but it's the exact asymmetry F1 flags.
- [ ] **Tests (no spend):** extend `test_split_worker_usage.py` pattern → assert `pipe_output.tokens_usages` is populated and non-empty after a cross-worker (router+runner) run, and that the aggregated total matches the emitted events. DIRECT integration: `pipe_output.tokens_usages` populated and equals the live registry's contents (fidelity cross-check). **(F1 regression):** a costs-only (`--no-graph --costs`) run yields `pipe_output.graph_spec is None` in BOTH modes. **(E1):** assert the costs-only path does not serialize stuff data (e.g. spy that `smart_dump`/`rendered_pretty_*` are not called, or that captured IOSpec data fields are None).
