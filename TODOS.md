...
- **Fixtures — `tests/integration/pipelex/csv/csv_demo/`:** `csv_demo.mthds` (flat `Person`/`PersonSummary` + a flat `Link` for the detection tests), `people.csv`, `inputs.json`, `links.csv`. Inline `[concept.X.structure]` blocks auto-generate registered `StructuredContent` subclasses (no `test_structures.py`).
...
- **Input headers:** required; must match field names exactly (no implicit remap). **Extra column → error; missing required column → error; missing optional column → that field is `None` for all rows** (CT2 lenient). Empty cell → `None` before pydantic (field must be optional).
...
Also addressed in PR #955 review rounds (not deferred): error row numbers now point at the physical CSV line; CSV detection gated to the single-key `{"url": ...}` wrapper; `--save-csv` validates before `mkdir`; newline delimiters rejected with a typed `CsvError`; an omitted optional column is `None` for all rows regardless of the row-model default (CT2); `CsvError` opts into `_authors_caller_facing_message` so STRICT disclosure keeps its caller-facing message. The one item still deferred: local paths containing URL-reserved `?`/`#` (entangled with the `resolve_uri` scheme-classification gap, item A above; niche + graceful).
