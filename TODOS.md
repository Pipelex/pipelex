...
- **D3 — union catch.** `BundleValidator`'s per-pipe classify-catch is `except (PipelexError, ValidationError, FactoryException)` — **not** bare `PipelexError`. The current tuple (`dry_run.py:84`) catches pydantic `ValidationError` and polyfactory `FactoryException`; `PipelexError` covers neither (the `FactoryException` arm is how a `PipeSignature` mint surfaces cleanly, comment `dry_run.py:85-87`). Wrap **both** `prepare_pipe_job` (mock-input build) and the run in one `try`; keep the `format_pydantic_validation_error` branch and the recursive `__cause__` walk for SKIPPED.
...
- **Phase 3 `allowed_to_fail` — TWO sites + a model field:** today the match exists at **both** `dry_run.py:89` (per-pipe) and `:186` (aggregate). Migrate both. `DryRunOutput` carries only bare `pipe_code` — **add a `pipe_ref` field** (or namespaced matching silently regresses to bare-code).
...
- [ ] **Catch (D3):** `except (PipelexError, ValidationError, FactoryException)` wrapping both `prepare_pipe_job` and the run; keep the `format_pydantic_validation_error` branch; `SKIPPED` = recursive `_root_cause_is(exc, PipeNotFoundError)` walk (§4.2 step 5 / §8).
...
- [ ] Migrate `pipelex.toml` `allowed_to_fail_pipes` to namespaced refs: `infinite_loop_1` → `failing_pipelines.infinite_loop_1`; **delete the obsolete `pipe_builder` entry**. Match on `pipe.pipe_ref` at **both** former sites (C-7). Add a pos/neg matching test.
...
Eng review of this plan (`/plan-eng-review`), 2026-06-01, scope reduced to the in-process consolidation (D-plan Part A). Commit `4f90703f`.

| Review | Trigger | Why | Runs | Status | Findings |
|--------|---------|-----|------|--------|----------|
| Eng Review | `/plan-eng-review` | Architecture & tests (required) | 1 | CLEAR (PLAN) | scope→consolidation; 2 decisions (API migration, catch surface), 1 char-test, 5 test gaps, checkpoint split, 0 critical gaps |
| Outside Voice | `codex` | Independent 2nd opinion | 1 | issues_found | caught the teardown-on-success blind spot + 6 fold-in corrections |

**OUTSIDE VOICE (codex):** surfaced a real blind spot — `validate_bundle()` leaves the library loaded on success and ~6 callers depend on it (→ D6 two-lifecycle model) — plus an error-precedence flip (→ D7 preserve order) and concrete fold-ins (registry close-in-finally, two `allowed_to_fail` sites + `DryRunOutput.pipe_ref`, Phase-1 truthiness/mutation/bare-code traps, an agent-CLI bug reporting all-SUCCESS).

**CROSS-MODEL:** two tensions (teardown model, step order); both resolved toward preserving current behavior (D6, D7). No residual disagreement.

**Decisions locked:** D1 scope→consolidation alone · D2 migrate pipelex-api now (no shim) + public inner sweep · D3 union catch `(PipelexError, ValidationError, FactoryException)` · D4 characterization test before Phase 1 · D6 two explicit teardown lifecycles · D7 preserve `validate_with_libraries`→signature order · D8 checkpoint split (added A2, Phase-0/3 seams).

**UNRESOLVED:** none — all decisions answered.

**VERDICT:** ENG CLEARED (scope = in-process consolidation) — ready to implement, starting at Phase 0. The two backend follow-ups (leaf run-mode mock, Temporal validation activity) are deferred to their own branches and tracked in `wip/dry-run-refactor-draft/`.
