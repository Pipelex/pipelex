...
- **Decisions taken:** phase structure reworked so no checkpoint rests on a self-contradicting tree; OQ1 promoted to a Phase 0 go/no-go gate; breaking-change posture held (no deprecation period) but preceded by a bounded corpus measurement. **OQ1 ruled GO** — owner-domain qualification is neutral-to-better for dependency packages; the dependency lookup-scope defect it exposed is pre-existing and deferred to the packaging project (`wip/pipe-refs/dependency-subpipe-scope-deferred.md`). **Crate envelope ruled: the pass is envelope-preserving and does not recompute the fingerprint** — it cannot pick the right digest scheme, so each consumer makes the fingerprint correct at its own boundary (rationale in the design doc's Phase 1 outcome).
...
- **Deliberately left for Phase 2:** `_domain_of`'s message still says "the normalizer expects a merged, key-qualified crate", and the pass still raises `CrateNormalizationError`, from a module whose second consumer is not a normalizer. Both are wrong-sounding the moment `library_manager` calls the pass. Left verbatim so Phase 1 stays a true refactor with exactly one behavior delta; Phase 2's "break message" work owns the rewording, and a rename of the error class would be wire-visible (`error_type` is a wire string) and needs `docs/errors/` regenerated with it.
...
- [x] **Fix the batched sub-pipe code derivation.** `SubPipe.pipe_code` becomes `domain.foo`, and the runtime derives `f"{self.pipe_code}_batch"` (`pipelex/pipe_controllers/sub_pipe.py:161`), producing the invalid code `domain.foo_batch`. Use the resolved sub-pipe's local code for that derivation.
...
- [x] Conformance-shaped unit coverage for the resolution rows on the two-domain fixture: own-only resolves; sibling-only errors; both-declare resolves to own; nowhere errors. Plus the export-bypass closure case.
- [x] **Mutation-check the rows.** Put the old crate-wide rule back and run them: sibling-only and export-bypass must go red. If they stay green the fixture does not discriminate and the tests are decoration. Restore afterwards.
