...
- [x] Platform-side IAM: `api_inference_profiles_kms_policy` grants **`kms:GenerateDataKey` only** (deviation: the envelope's encrypt path never calls `kms:Encrypt`, so least-privilege drops it), scoped to the CMK ARN, attached to the shared `api_task_role`. **Decision taken: attach narrowly, keep the role split as the standing follow-up** (documented at `iam.tf` `api_ses_policy` note). Decrypt deliberately not granted (Phase 2d worker-fleet concern). ⚠️ still needs Checkpoint-4 sign-off.
...
**Last updated:** 2026-07-03 — Checkpoints 1–4 ALL CLEARED. Phase 1 (Foundation) CODE COMPLETE across core + shared + platform + infra. Stopped at Checkpoint 4 for user sign-off (CMK key policy + task-role split).
...
- **Pending sign-offs (the current critical path):** ① push approval for the four feature branches (unblocks the platform pin bump, CI, and PRs); ② **Checkpoint 4 (NOW): CMK key policy + task-role split** — the two ⚠️ decisions above belong to the user; ③ Q6 hosted-adapter location — before Phase 2b; ④ Q2 launch posture — Checkpoint 5.
- **Next:** Phase 1 (Foundation) is code-complete. Once Checkpoint-4 sign-off + push approval land: open the four PRs, let CI `terraform plan` review the infra change, then move to Phase 2 (Binding B end-to-end) — prereqs are Phase 1 merged/deployable + Q6 sign-off + the pipelex-temporal plugins track landing (repo hazard note).
