...
- `flat_field_names(row_model)` — the **single shared flatness classifier** used by BOTH read and write. `_is_flat_annotation` unwraps `Optional`/PEP-604 unions, ACCEPTS `str/int/float/bool/date/datetime` + `Literal` + `Enum`, REJECTS genuine multi-arm unions, list/dict/nested-model/`Any`. Raises `CsvFlatnessError` naming field + concept; returns declared field order.
