...
**Flagged, NOT fixed (pre-existing, out of scope):** the `(ValueError, PipelexError)` MRO means `ValueError.__init__` wins, so `.message` is never set and `to_error_report()` raises `AttributeError` — affects `TemporalConfigError`/`WorkerTaskQueueUnknownError` **and the untouched `ConfigModelError(ValueError, FatalError)`**. Identical before and after C6 (so not a regression). Proposed fix (reorder bases to `(PipelexError, ValueError)`, sweep all such classes, add a regression test) recorded in [`wip/plugins/valueerror-mixin-message-unset-latent-bug.md`](wip/plugins/valueerror-mixin-message-unset-latent-bug.md).
