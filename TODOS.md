...
**Worktree:** `_hub/` · **Branch:** `refactor/Modularity-3` · **Base:** `refactor/Hub-2` (PR #1064, still open — see [Gating](#gating)) · **Status:** not started, reviewed.
...
| tests pinning pipe classes into the class registry | 0 | **1** |
...
- **State:** *not started, reviewed.* Phase 0 baseline not yet captured beyond the figures in [Measurements](#measurements). All eight review findings are folded in; see [Review findings](#review-findings).
...
- [ ] **T1 (P1, human: ~3h / CC: ~20min)** — M3 — Add the regression test pinning pipe + stuff classes into the class registry after boot
  - Surfaced by: Test review — M3 splits `register_classes`; nothing asserts registry contents; failure is silent at boot
  - Files: `tests/unit/pipelex/test_hub_lifecycle.py`, `pipelex/pipelex.py`
  - Verify: `make tb`, then drop one `register_classes` line and confirm the test goes red
...
- [ ] **T6 (P2, human: ~1h / CC: ~10min)** — M3 + M2 — Add `drift-ack` steps for `hub-layering-convention`
  ...
  - Verify: `make drift-check` green at each checkpoint
