...
- **Status:** **PHASE M MERGE ABSORBED (2026-07-11). Merge committed + reseeded, `make agent-check` GREEN. NEXT = REDO THE GRIND (Phase M step 5 — the ~1,927 seeded wave), then checkpoint review, then STOP for Louis before Phase 5.** ⚠ **We did NOT merge dev into the full grind tip** (that gave 23 conflict files, ~15 tangled signature-demote files). Louis's safer plan (executed): **rewound `feature/Signatures` to the pre-grind mechanism commit `9f5bd54a3`** (CHECKPOINT-1 tooling + seed + docs, before batch 1) and merged `origin/dev` there — only **9 conflicts, all trivial** (4 generated model docs + 1 error page → took dev's; CHANGELOG → dev's + re-added the Drift-contracts bullet; TODOS → restored from archive; 2 tiny literal-sweep source files → kept dev's new siblings + our kw-only int-subjects). The full Phase-4 grind (batches 1–11, ~1,453 written rationales + ~180 demotes) is **preserved but NOT in the branch** — archived at tag **`subject-grants-grind-archive`** + branch **`subject-grants-grind-archive-branch`** (both at old tip `2ebf3f293`). Merge-skew fixes to boot: `drift/core.py` `from pipelex.types import StrEnum` → `from enum import StrEnum` (dev dropped py3.10 + the shim); `make install` for dev's new deps (`pipelex-tools`, mthds 0.8, transformers 5). Merge commit **`5bb8c2756`** (2nd parent = dev tip `048c87971`); reseed follows in a 2nd commit. **PUSH STATE: force-push after `make agent-test` green** (rewind ⇒ force-push, Louis-approved; push the archive tag too so the grind is preserved on origin).
- **Resume point:** `feature/Signatures` in the `_sig` worktree (treat `_sig` as repo root). Tip = merge `5bb8c2756` + the reseed commit. `make agent-check` GREEN (pyright 0, mypy 0 across 2317 files, cko PASSED); `make agent-test` was the final gate at handoff. **NEXT WORK = redo the grind (Phase M step 5), NOT Phase 5.** All the rubric case-law, the **Batch execution recipe**, and the checkpoint-2/3 learnings below still apply verbatim — the only change is the code being reviewed is now dev's post-merge tree.
- **Registry state (post-merge, post-reseed):** **1,935 grants total, 1,927 `seeded = true` remaining** (8 real = the tooling's own grants). Reseed added **285** new ungranted subjects from dev's wave (`subject-grant --seed`, additive/idempotent) and **25 dead-grants were deleted** (defs dev renamed/moved/removed: `load_concepts_only*`, `make_storage_provider_from_config`, `structures_cmd` helpers, `StructureGenerator._*`, `LLMWorkerInternalAbstract._*`, etc. — all matched changelog removals). 0 literal-subject / 0 grant-param-mismatch from the merge. **⚠ TREAT ALL 1,927 SEEDED AS UNREVIEWED — redo the whole grind.** BUT the archive's registry (`git show subject-grants-grind-archive:subject_grants.toml`) holds ~1,453 already-written rationales: for any def whose signature dev left unchanged, **port that rationale verbatim** (cheap keep); genuinely re-review only the dev-delta and anything the archive demoted. This "port-forward" shortcut makes the redo far cheaper than a cold grind.
...
> **PROGRESS (2026-07-11): steps 0–4 DONE via the REWIND variant — steps 5–7 REMAIN.** The written steps 0–1 assumed merging onto the full grind tip and hand-resolving the mechanism conflicts; instead Louis chose to **rewind to the pre-grind tooling commit `9f5bd54a3` first** (proven with `git merge-tree`: 23 conflicts at the grind tip → 9 at pre-grind), which sidestepped ALL the mechanism conflicts (dev has no guard/registry files, so they auto-merged) and left only 9 trivial ones. Steps 2–4 (survey `cko` → `--seed` → delete 25 dead-grants) ran exactly as written. **NEXT = step 5 (redo the grind).** The `--seed`/`fko`-order hazard is already safely past.
