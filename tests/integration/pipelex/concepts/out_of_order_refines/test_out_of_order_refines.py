...
class TestOutOfOrderRefines:
    ...
    async def test_simple_out_of_order_refines_single_file(self):
        ...
        1. VIPCustomer is defined BEFORE Customer in the PLX file
        ...
        """
        plx_file_path = Path(__file__).parent / "out_of_order_refines.plx"
        ...
        await validate_bundle(plx_file_path=plx_file_path)
    ...
    async def test_multi_level_out_of_order_refines_across_files(self):
        ...
        File 1 (base_domain.plx):
          - Person (root concept with structure)

        File 2 (middle_domain.plx) - concepts defined in REVERSE order:
        ...
        When loading middle_domain.plx:
        ...
        assert (multi_file_dir / "base_domain.plx").exists(), "base_domain.plx not found"
        assert (multi_file_dir / "middle_domain.plx").exists(), "middle_domain.plx not found"

        # validate_bundles_from_directory loads all PLX files in the directory
        # Files are loaded in order, but within middle_domain.plx concepts are out of order
        # with pytest.raises(ConceptFactoryError) as exc_info:
