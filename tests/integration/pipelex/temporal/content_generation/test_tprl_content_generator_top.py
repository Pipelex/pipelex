...
class TestTprlCrafterTop:
    ...
    @pytest.mark.inference
    @pytest.mark.dry_runnable
    async def test_tprl_make_object(self, tprl_job_metadata: JobMetadata, top_crafter: ContentGeneratorProtocol):
        llm_setting_for_object = get_model_deck().get_llm_setting(llm_choice="$testing-structured")

        person_direct: Person = await top_crafter.make_object(
            job_metadata=tprl_job_metadata,
            object_class=Person,
    ...
    @pytest.mark.inference
    @pytest.mark.dry_runnable
    async def test_tprl_make_object_list(self, tprl_job_metadata: JobMetadata, top_crafter: ContentGeneratorProtocol):
        llm_setting_for_object = get_model_deck().get_llm_setting(llm_choice="$testing-structured")

        person_list_direct: list[Person] = await top_crafter.make_object_list(
            job_metadata=tprl_job_metadata,
            object_class=Person,
