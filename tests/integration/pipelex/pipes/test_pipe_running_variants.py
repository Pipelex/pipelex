...
class TestPipeRunningVariants:
    ...
    async def test_pipe_no_input(
        ...
        pretty_print(await stuff.content.rendered_html(), title=f"{topic}: run pipe '{pipe_code}' in html")
        pretty_print(await stuff.content.rendered_markdown(), title=f"{topic}: run pipe '{pipe_code}' in markdown")
