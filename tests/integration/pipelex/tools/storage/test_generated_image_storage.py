...
class TestGeneratedImageStorage:
    ...
    async def test_generated_image_uri_format_applied(
        ...
        # The `generated/` leaf must be THERE. Asserting only the scope prefix
        # would pass just as well with the bytes loose at the root of the scope,
        # which is the state this leaf exists to end: a file under no leaf is
        # invisible to anything listing a run's content by category, and shares
        # the root with every leaf added later.
        assert storage_key.startswith("test/scope/generated/")
        assert storage_key.count("/") == 3  # test / scope / generated / <file>
