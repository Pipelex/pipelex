...
Without this guarantee, the entry points split into two shapes — one returning
inside the ``with`` (translated) and one outside (raw ``pydantic.ValidationError``
propagates) — and downstream handlers that only ``except ValidateBundleError``
