...
class TestAsyncCogtLLMGenText:
    ...
    async def test_gen_text_async_using_llm_preset(self, llm_preset_id: str, topic: str, prompt_text: str):
        llm_worker_async, llm_job = get_async_worker_and_job(llm_preset_id=llm_preset_id, user_text=prompt_text)
        generated_text = await llm_worker_async.gen_text(llm_job=llm_job)
        assert generated_text
        pretty_print(generated_text)
    ...
    async def test_gen_text_async_multiple_using_llm_preset(self, llm_preset_id: str, topic: str, prompt_text: str):
        ...
        for _ in range(4):
            ...
            if temperature > 1:
                break
            if llm_worker_async.llm_engine.llm_model.llm_family == LLMFamily.O_SERIES:
            ...
            task: asyncio.Task[str] = asyncio.create_task(llm_worker_async.gen_text(llm_job=llm_job))
            tasks.append(task)
