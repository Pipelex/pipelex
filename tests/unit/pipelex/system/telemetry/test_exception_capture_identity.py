...
def _crash_with(*, capture: ExceptionCapture, error: BaseException) -> None:
...
class TestExceptionCaptureIdentity:
    ...
    def test_a_crash_inside_a_run_is_attributed_to_the_runs_caller(self, mocker: MockerFixture) -> None:
        ...
        capture, custom_client = _make_capture(mocker=mocker, mode=PostHogMode.IDENTIFIED, configured_user_id="pipelex-worker")
    ...
    def test_an_anonymous_stream_identifies_nobody_even_for_a_known_caller(self, mocker: MockerFixture) -> None:
        capture, custom_client = _make_capture(mocker=mocker, mode=PostHogMode.ANONYMOUS, configured_user_id="configured-id")
