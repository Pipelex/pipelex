"""The ``otlp`` sink files each record under the span it was logged in.

That span is the Pipelex span active at the log call, and outside one OpenTelemetry's current span, the
host's own. The sink hands the SDK a context holding that span, from which the SDK takes the record's
trace id, span id and flags, so a collector files the record under it; a record logged outside any span
carries none. A boot line held until the sink arrives keeps the span it was logged in. The sink
...
class TestOtlpSinkTraceContext:
    ...
    def test_a_record_logged_in_a_pipelex_span_is_filed_under_it_rather_than_the_hosts(self, otlp_log: tuple[Log, InMemoryLogExporter]) -> None:
        fresh, exporter = otlp_log
        tracer = TracerProvider().get_tracer(__name__)
        pipelex_span = tracer.start_span("pipe")
        with tracer.start_as_current_span("host") as host_span:
        ...
        in_pipe, in_host = _own_records(exporter)
        assert in_pipe.trace_id == span_context.trace_id
        assert in_pipe.span_id == span_context.span_id
        assert in_pipe.trace_flags == span_context.trace_flags
        assert in_host.span_id == host_span.get_span_context().span_id
    ...
    def test_a_boot_line_held_until_the_sink_arrives_keeps_the_span_it_was_logged_in(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        assert inside_pipelex.span_id == pipelex_span.get_span_context().span_id
        assert outside.span_id == 0
