...
class TestMirrorDir:
    ...
    def test_excluded_file_target_only_is_preserved(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = mirror_dir(source, target, exclude_files=frozenset({"telemetry.project.toml"}))
    ...
    def test_excluded_file_is_not_copied(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = mirror_dir(source, target, exclude_files=frozenset({"telemetry.toml"}))
    ...
    def test_excluded_dir_target_only_is_preserved(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = mirror_dir(source, target, exclude_dirs=frozenset({"storage"}))
    ...
    def test_nested_excluded_dir_is_preserved(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = mirror_dir(source, target, exclude_dirs=frozenset({"traces"}))
    ...
    def test_idempotent_and_symmetric_with_has_diff_dirs(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            first = mirror_dir(source, target, exclude_files=exclude_files, exclude_dirs=exclude_dirs)
            assert first.has_changes is True

            second = mirror_dir(source, target, exclude_files=exclude_files, exclude_dirs=exclude_dirs)
            assert second.has_changes is False

            assert has_diff_dirs(source, target, exclude_files=exclude_files, exclude_dirs=exclude_dirs) is False
