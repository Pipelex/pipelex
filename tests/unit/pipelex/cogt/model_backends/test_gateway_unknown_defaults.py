...
The same skew tolerance applies per model: an unknown per-model key that is header-shaped is a
request header the back office started serving on purpose, and anything else is pruned rather than
sent to the provider. A local backend file gets the strict counterpart of that rule — see
`test_backend_library_leniency.py`.
"""
...
class TestGatewayUnknownDefaults:
    ...
    def test_per_model_unknown_keys_are_untouched(self) -> None:
        """This function is scoped to `defaults`; the per-model rule is the loader's, tested below."""
