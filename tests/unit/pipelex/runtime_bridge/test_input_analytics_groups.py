"""The bridge payload refuses malformed analytics groups at the WIRE.

A `PipelexPipeRunInput` crosses a process boundary, so its groups are whatever
...
from pipelex.system.analytics_groups import ANALYTICS_GROUPS_MAX_ENTRIES


def _payload(analytics_groups: dict[str, str]) -> PipelexPipeRunInput:
...
class TestTheWireRefusesMalformedAnalyticsGroups:
    ...
    def test_well_formed_groups_cross_the_wire(self, groups: dict[str, str]) -> None:
        assert _payload(groups).analytics_groups == groups
    ...
    def test_malformed_groups_are_refused_at_construction(self, bad_groups: dict[str, str]) -> None:
        with pytest.raises(ValidationError):
            _payload(bad_groups)

    def test_the_groups_survive_a_json_round_trip(self) -> None:
        ...
        assert round_tripped.analytics_groups == {"organization": "org_acme"}
