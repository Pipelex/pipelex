...
class GoogleErrorHandlingTestData:
    ...
    # CLIENT_ERROR_CASES minus the 404 row: the LLM worker specializes a 404 to
    # LLMModelNotFoundError (not a plain LLMCompletionError), so the 404 case is
    # covered by its own dedicated test instead of this parametrized set.
    LLM_CLIENT_ERROR_CASES: ClassVar[list[tuple[str, int, str, InferenceErrorCategory, str | None]]] = [
        (
            "auth_401",
            401,
            "Request had invalid authentication credentials",
