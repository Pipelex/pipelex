...
mode. Also pins that the built-in ``PipeFuncPlugin`` contributes ``direct`` and ``local_sandbox`` and
that ``direct`` resolves to the in-process executor.
"""
...
from pipelex.plugins.pipe_func.pipe_func_plugin import LOCAL_SANDBOX_PIPE_FUNC_EXECUTION_MODE, PipeFuncPlugin
...
class TestPipeFuncExecutorRegistry:
    ...
    def test_builtin_plugin_registers_direct_and_local_sandbox(self) -> None:
        """The core PipeFuncPlugin contributes both built-in modes, and ``direct`` resolves in-process."""
        registrar = _make_registrar()
        ...
        registry = PipeFuncExecutorRegistry(registrar.pipe_func_executors)
        assert set(registry.modes) == {DIRECT_PIPE_FUNC_EXECUTION_MODE, LOCAL_SANDBOX_PIPE_FUNC_EXECUTION_MODE}
        ...
        sandbox_executor = registry.get_required(mode=LOCAL_SANDBOX_PIPE_FUNC_EXECUTION_MODE)(pipe_func_config)
        assert isinstance(sandbox_executor, SandboxPipeFuncExecutor)
