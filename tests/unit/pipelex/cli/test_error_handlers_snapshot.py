...
EXPECTED_MODEL_AVAILABILITY_PANEL = (
    ...
    "\n"
    "💡 Tip: Your local model deck may be out of date: new aliases and presets are \n"
    "added to Pipelex over time, and existing '.pipelex/inference/deck/*.toml' files \n"
    "are not refreshed automatically. To pick up the latest definitions, delete your \n"
    "local deck files under '.pipelex/inference/deck/' (or the whole \n"
    "'.pipelex/inference/' directory) and run 'pipelex init inference' to regenerate \n"
    "them.\n"
    "If that does not resolve it, make sure the handle is defined in one of \n"
    "'.pipelex/inference/deck/*.toml', that the backend it routes to (see \n"
    "'.pipelex/inference/routing_profiles.toml') is enabled in \n"
    "'.pipelex/inference/backends.toml', and that you have the necessary credentials,\n"
    "or specify a different model in the 'my_pipe' pipe.\n"
    "Learn more about the inference backend system: \n"
    "https://docs.pipelex.com/latest/setup/configure-ai-providers/\n"
    "Join our Discord for help: https://go.pipelex.com/discord\n"
    "\n"
