...
class TestDataFlowMermaidWithData:
    ...
    def test_list_data_preserved(self) -> None:
        """Test that list data in IOSpec is preserved in stuff_data."""
        ...
        producer_node = {
            ...
            "node_io": NodeIOSpec(
                inputs=[],
                outputs=[IOSpec(name="output", concept="Text", digest="stuff_001", data=list_data)],
            ),
        }
        graph = self._make_graph(nodes=[producer_node])
        graph_config = make_graph_config(include_stuff_json=True)
