...
def _make_worker(mocker: MockerFixture, thinking_mode: ThinkingMode) -> OpenAIResponsesLLMWorker:
...
def _mock_config(mocker: MockerFixture) -> None:
    ...
    mocker.patch(
        "pipelex.providers.openai.openai_responses_llm_worker.get_config",
...
class TestOpenAIResponsesReasoning:
    """Tests for _resolve_reasoning on OpenAIResponsesLLMWorker."""
    ...
    def test_reasoning_effort_maps_correctly(
        ...
        result = worker._resolve_reasoning(job_params=job_params)  # ruff: ignore[private-member-access]  # pyright: ignore[reportPrivateUsage]
        assert result is not None
        assert result.get("effort") == expected_openai_effort
    ...
    def test_reasoning_budget_raises_capability_error(self, mocker: MockerFixture):
        ...
        with pytest.raises(LLMCapabilityError, match="reasoning_budget"):
            worker._resolve_reasoning(job_params=job_params)  # ruff: ignore[private-member-access]  # pyright: ignore[reportPrivateUsage]
