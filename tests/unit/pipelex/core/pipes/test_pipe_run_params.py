"""Unit tests for pipe_run_params module."""
...
class TestMakeOutputMultiplicity:
    """Test cases for make_output_multiplicity function."""
    ...
    def test_make_output_multiplicity_all_cases(
        self, nb_output: Optional[int], multiple_output: Optional[bool], expected_result: Optional[PipeOutputMultiplicity], description: str
    ):
...
class TestOutputMultiplicityToApply:
    ...
    def test_output_multiplicity_to_apply_all_cases(
        self,
        base: Optional[PipeOutputMultiplicity],
        override: Optional[PipeOutputMultiplicity],
        ...
        assert result.enable_multiple_outputs == expected_result.enable_multiple_outputs, f"Failed enable_multiple_outputs for case: {description}"
    ...
    def test_override_none_uses_base(self):
        ...
        assert result.enable_multiple_outputs is False
        ...
        assert result.enable_multiple_outputs is True
        assert result.specific_output_count is None
        ...
        result = output_multiplicity_to_apply(False, None)
    ...
    def test_override_false_forces_single_output(self):
        ...
        for base in test_bases:
            result = output_multiplicity_to_apply(base, False)
            assert result.resolved_multiplicity is False, f"Failed for base={base}: resolved_multiplicity should be False"
            assert result.enable_multiple_outputs is False, f"Failed for base={base}: enable_multiple_outputs should be False"
    ...
    def test_override_true_enables_multiple_outputs(self):
        ...
        result = output_multiplicity_to_apply(None, True)
        assert result.resolved_multiplicity is None
        assert result.enable_multiple_outputs is True
        assert result.specific_output_count is None

        # Base False -> enable multiple, LLM decides count
        result = output_multiplicity_to_apply(False, True)
        assert result.resolved_multiplicity is True
        assert result.enable_multiple_outputs is True
        assert result.specific_output_count is None

        # Base True -> keep True
        result = output_multiplicity_to_apply(True, True)
        assert result.resolved_multiplicity is True
        assert result.enable_multiple_outputs is True
        assert result.specific_output_count is None
        ...
        for base_int in [1, 3, 5, 10]:
            result = output_multiplicity_to_apply(base_int, True)
    ...
    def test_override_int_uses_override_count(self):
        ...
        for base in test_bases:
            result = output_multiplicity_to_apply(base, override_int)
            assert result.resolved_multiplicity == override_int, f"Failed for base={base}: resolved_multiplicity should be {override_int}"
            assert result.enable_multiple_outputs is True, f"Failed for base={base}: enable_multiple_outputs should be True"
    ...
    def test_override_precedence_logic(self):
        ...
        result = output_multiplicity_to_apply(True, False)
        assert result.resolved_multiplicity is False
        assert result.enable_multiple_outputs is False
        assert result.specific_output_count is None

        result = output_multiplicity_to_apply(5, False)
        assert result.resolved_multiplicity is False
        assert result.enable_multiple_outputs is False
        assert result.specific_output_count is None

        # Override int beats any base
        result = output_multiplicity_to_apply(True, 3)
        assert result.resolved_multiplicity == 3
        assert result.enable_multiple_outputs is True
        assert result.specific_output_count == 3

        result = output_multiplicity_to_apply(7, 3)
        assert result.resolved_multiplicity == 3
        assert result.enable_multiple_outputs is True
        assert result.specific_output_count == 3

        # Override True with int base preserves int
        result = output_multiplicity_to_apply(5, True)
