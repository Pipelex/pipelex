...
async def _save_graph_outputs(graph_spec: GraphSpec, output_dir: Path) -> dict[str, int]:
    ...
    pretty_print(dataflow_with_data.stuff_data, title="Dataflow stuff_data")
    ...
    if dataflow_with_data.stuff_data:
        ...
        dataflow_html = await render_mermaid_html_async(dataflow_with_data.mermaid_code, title="Dataflow")
    ...
    if combo_with_data.stuff_data:
        ...
        combo_html = await render_mermaid_html_async(combo_with_data.mermaid_code, title="Combo")
    (output_dir / "combo.html").write_text(combo_html, encoding="utf-8")

    log.info(f"✅ All graph outputs saved to: {output_dir}")

    return {
        "dataflow_stuff_data_count": len(dataflow_with_data.stuff_data),
        "combo_stuff_data_count": len(combo_with_data.stuff_data),
    }
