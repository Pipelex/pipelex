...
class TestMermaidFromJson:
    ...
    async def test_mermaid_diagrams_from_json_graph(self, topic: str, graph_json_path: str) -> None:
        ...
        assert dataflow_with_data.mermaid_code.startswith("flowchart TD")

        dataflow_mmd_path = output_dir / "dataflow.mmd"
        dataflow_mmd_path.write_text(dataflow_with_data.mermaid_code, encoding="utf-8")
        log.info(f"Saved dataflow.mmd: {dataflow_mmd_path}")
        log.info(f"Dataflow stuff_data count: {len(dataflow_with_data.stuff_data)}")
        ...
        assert combo_with_data.mermaid_code.startswith("flowchart TD")

        combo_mmd_path = output_dir / "combo.mmd"
        combo_mmd_path.write_text(combo_with_data.mermaid_code, encoding="utf-8")
        log.info(f"Saved combo.mmd: {combo_mmd_path}")
        log.info(f"Combo stuff_data count: {len(combo_with_data.stuff_data)}")
    ...
    async def test_dataflow_mermaid_structure(self, topic: str, graph_json_path: str) -> None:
        ...
        graph_spec = load_graphspec(Path(graph_json_path))
        dataflow_with_data = graphspec_to_dataflow_mermaid_with_data(graph_spec)

        mermaid_code = dataflow_with_data.mermaid_code
        ...
        # Verify stuff_data has expected format (s_xxx keys)
        for stuff_id in dataflow_with_data.stuff_data:
    ...
    async def test_combo_mermaid_structure(self, topic: str, graph_json_path: str) -> None:
        ...
        graph_spec = load_graphspec(Path(graph_json_path))
        combo_with_data = graphspec_to_combo_mermaid_with_data(graph_spec)

        mermaid_code = combo_with_data.mermaid_code
