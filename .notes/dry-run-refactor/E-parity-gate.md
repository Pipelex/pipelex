...
The plan in `D-plan-for-codex.md` routes DRY runs to a local in-process `PipeRun` **even when the hub default is the Temporal-backed runner** (i.e. in the hosted `pipelex-api-deploy` runner). The codex outside-voice review flagged one concrete risk:
