...
`native.Anything` belongs in that pipe and is not there yet. It no longer crashes the engine's renderer — the structureless arm renders it without resolving a class — but its template value is the empty mapping `{}`, which the input shaper refuses in both shapes, so adding it fails the round-trip gate this page describes. Its slot coverage waits on that being resolved.
