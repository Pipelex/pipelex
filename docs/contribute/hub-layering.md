...
- **The Pipe machinery — interpreter.** `core.pipes.pipe_abstract`, `pipe_blueprint`, `pipe_factory`, `core.pipes.rendering`, `core.bundles`, and `core.interpreter`. Every one of them names a `Pipe`, and a pipe is the interpreter's own object — they import `pipe_operators` / `pipe_controllers` / `libraries` **directly**, not through a hub. `core.bundles.pipelex_bundle_blueprint` is a discriminated union over every pipe blueprint. No amount of dependency injection makes those runtime.

  The pipe-kind registration manifest used to sit here too, and it was the fattest edge of the lot — a list of every pipe class and every pipe factory, so importing it loaded the whole interpreter. It now lives outside `core/` entirely, as `pipelex.pipe_machinery.registry_models.PipeRegistryModels`; `core.registry_models` keeps the value model only. See [Registration surface](registration-surface.md) for what adding a pipe kind touches.
