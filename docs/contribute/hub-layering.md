...
- **The data model — runtime.** `core.concepts`, `core.domains`, `core.stuffs`, `core.memory`, and the input/output *specs* under `core.pipes.inputs` and `core.pipes.stuff_spec`. These describe what a method's values *are*. Nothing in them needs a loaded method, and each measures **zero** interpreter modules.
- **The Pipe machinery — interpreter.** `core.pipes.pipe_abstract`, `pipe_blueprint`, `pipe_factory`, `pipe_output`, `core.pipes.rendering`, `core.bundles`, `core.interpreter`, and `core.registry_models`. Every one of them names a `Pipe`, and a pipe is the interpreter's own object — they import `pipe_operators` / `pipe_controllers` / `libraries` **directly**, not through a hub. `core.bundles.pipelex_bundle_blueprint` is a discriminated union over every pipe blueprint; `core.registry_models` is a registry of every pipe kind. No amount of dependency injection makes those runtime.
