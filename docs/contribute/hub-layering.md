...
Two different splits run through the built-in plugins, and they are easy to conflate. The first is **mechanism vs adapters**, and it does *not* cross a layer boundary: `pipelex/plugins/` is the plugin mechanism — the `PipelexPlugin` contract, the `PluginRegistrar`, and the capability registries the registrar accumulates into — while `pipelex/providers/` holds the built-in vendor adapters that register through it, one directory per vendor. Both packages are declared runtime-layer. What the split makes visible is the one-way dependency: adapters depend on the mechanism, the mechanism knows nothing of its adapters, and the engine depends only on the mechanism. It also stops `pipelex/plugins/` from reading as "the vendors", which is what made the second split hard to see.

The second split *is* the layer boundary, and the same rule of thumb divides it. Most built-ins adapt a runtime-layer port — an inference backend, a model lister, a storage or secrets provider — and they are the seventeen vendor packages under `pipelex/providers/`. Two adapt interpreter-layer ports, because their job is to *construct* interpreter-layer objects: `DirectOrchestratorPlugin` registers a `DirectOrchestrator` and a `DirectBundleValidator`, and `PipeFuncPlugin` registers a concrete `DirectPipeFuncExecutor`. Those two live in **`pipelex/interpreter_plugins/`**.
...
1. **The layer rule.** A module in the declared runtime layer — `pipelex.cogt`, `pipelex.core`, `pipelex.plugins`, `pipelex.providers`, `pipelex.reporting`, `pipelex.system`, `pipelex.tools`, plus the `pipelex.runtime_hub` module itself — may not import `pipelex.interpreter_hub`. The declaration is compliant with an empty exception list, so the guard hard-blocks on **any** violation. Entries are matched exact-or-dotted-prefix, which is why a bare *module* is a legal entry beside the packages: without `pipelex.runtime_hub` spelled out, the module at the centre of the rule would be exempt from it. `pipelex.core` was six entries until its interpreter tenants moved out — see [Where core splits](#where-core-splits) for what that enumeration was working around.
...
- **`cogt/img_gen/img_gen_args_factory.py` imports two vendor img-gen factories** (`GoogleImgGenFactory`, `OpenAIImgGenFactory`) at module level, for taxonomy and geometry helpers. Unlike the config imports this one *is* slated to go, and not through a registry: dispatch is keyed on `AspectRatioTaxonomy`, a `cogt`-owned enum, and the gateway worker is a single SDK spanning both taxonomies, so there is no provider key to dispatch on. The fix is to move the helpers inward into a neutral `cogt/img_gen/` mapping module and have both sides import down to it. A third such edge, `model_backends/backend_factory.py` reaching for `VertexAIFactory`, is already deferred inside a function and self-resolves when VertexAI support is removed.
