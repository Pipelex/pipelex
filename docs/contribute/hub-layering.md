...
1. **The layer rule.** A module in the declared runtime layer — `pipelex.cogt`, `pipelex.plugins`, `pipelex.reporting`, `pipelex.system`, `pipelex.tools`, the `pipelex.runtime_hub` module itself, plus core's data-model packages `pipelex.core.concepts`, `pipelex.core.domains`, `pipelex.core.memory`, `pipelex.core.pipes.inputs`, `pipelex.core.pipes.stuff_spec` and `pipelex.core.stuffs` — may not import `pipelex.interpreter_hub`. The declaration is compliant with an empty exception list, so the guard hard-blocks on **any** violation. Entries are matched exact-or-dotted-prefix, which is why a bare *module* is a legal entry beside the packages: without `pipelex.runtime_hub` spelled out, the module at the centre of the rule would be exempt from it.

    Read that compliance claim as exactly what it is. **The rule is about a *direct* import, and the guard does not follow the import graph.** A declared runtime-layer module can still reach `interpreter_hub` through an undeclared intermediary — `runtime_bridge`, `pipeline`, `pipe_run`, `config`, … — with both gates green. Four modules under `pipelex.plugins` do exactly that today; see [Known inversions](#known-inversions) for the measurement and the planned fix. Why `core/` is listed package by package rather than wholesale is [below](#where-core-splits).
...
Named so this document stays honest. Not one of them imports `interpreter_hub` *directly*, so the guard does not flag them — but four reach it transitively, which is why "`plugins` is a runtime layer" is not yet unconditionally true. This is measured, not suspected:

```
pipelex.plugins.direct.direct_plugin          interpreter_hub=True   interpreter modules= 57
pipelex.plugins.discovery                     interpreter_hub=True   interpreter modules= 67
pipelex.plugins.builtins                      interpreter_hub=True   interpreter modules= 67
pipelex.plugins.pipe_func.pipe_func_plugin    interpreter_hub=True   interpreter modules= 58
```

**The headline property is unaffected**, and that is why the closure test stays green: nothing on the inference-layer or `runtime_hub` path reaches these four. They load only on the full boot path, where the interpreter is loaded anyway. A static sweep of every declared runtime-layer module puts the blast radius at exactly these four of 476 — the other ten declared packages are clean across 345 modules.

- **`plugins/direct/direct_plugin.py` and `plugins/pipe_func/pipe_func_plugin.py` are the two leaves**, and they breach for the same reason: their job is to *construct interpreter-layer objects*. `direct_plugin` registers a `DirectOrchestrator` and a `DirectBundleValidator`, reaching `interpreter_hub` by two independent routes (`runtime_bridge/` and `pipeline/`), so cutting either one alone changes nothing. `pipe_func_plugin` imports the concrete `DirectPipeFuncExecutor` from `pipe_operators/` at module level — it needs the class at runtime, because it constructs it.
- **`plugins/builtins.py` and `plugins/discovery.py` breach purely as aggregators.** `builtins.py` imports every built-in plugin including those two; `discovery.py` imports `builtins`, and sits on the boot path. Neither carries an edge of its own, so fixing the two leaves clears all four.
- `plugins/pipe_func_executor_registry.py` is typed by a protocol from `pipe_operators/` and defers that import under `TYPE_CHECKING` precisely because `pipelex.config` imports it and the inference layer imports `pipelex.config` — a module-level import there would drag the interpreter back into every inference closure. Being type-only it loads nothing, so it is not one of the four above — the sanctioned deferral pattern. The placement inversion itself is unfixed.

**The fix is decided and scheduled as its own change**, because it is a relocation rather than a declaration edit: split `builtins.py` by layer, move the two interpreter-touching plugins to an interpreter-side home, invert `discovery`'s dependency so the plugin list arrives as a parameter, and let the boot entrypoint — which sits in no declared layer and is free to weld — compose the two halves. That is the seam external plugins already use: an out-of-tree orchestrator or PipeFunc executor arrives through the entry-point mechanism without welding anything, and the in-tree built-ins are the only ones that weld. Alongside it, the guard learns to follow the import graph, so this class of breach cannot recur silently.
