...
!!! note "Advisory warnings on validate"
    Whole-bundle and whole-library validate surfaces (`validate bundle`, `validate method`, `validate pipe --all`) also carry a `warnings` array — advisory optionality lints on a VALID bundle that never flip the verdict or the exit code. Each entry has the **same shape as a validation error item** (`category`, `error_type`, `pipe_code`, `domain_code`, `variable_names`, `message`) — this is a different shape from the `init`/`doctor` setup `warnings` (`{type, message}`) documented under Output Contract below. Two lint families occupy it. The useless-`!` lint (`optional_force_redundant`) reports a `!` (force) input whose slot is guaranteed present in every analyzed flow, so the assertion can never fire. The [intent-hint](../../building-methods/concepts/intent-hints.md) lints report authored hint content this version of the standard does not recognize — an unknown hint key (`hint_unknown_key`), an unknown `intent` word (`hint_unknown_intent`), or a known word on a site it does not apply to (`hint_inapplicable_intent`, e.g. `rating` on a text field). Hint lints are advisory by design: the content is preserved through the crate and onto the descriptor, so a method written against a newer vocabulary still validates. In markdown, warnings render as a "Warnings" section. The array is empty when there is nothing to report; single-pipe `validate pipe` omits it (no flow context to lint in).

!!! note "Per-pipe contracts on a valid report"
    A successful validate envelope carries two per-pipe artifacts, both keyed by the namespaced `pipe_ref` (`domain.code`) and both covering exactly the same set of pipes — `PipeSignature` placeholders and controller-inferred inputs included.

    **`pipe_io_contracts`** is the typed IO contract: for each pipe an `inputs` map and one `output`. An input entry carries the `concept_ref`, the JSON Schema of its content (`json_schema`), and two facts about the slot as declared:
    ...
    An output entry carries `concept_ref`, the same `multiplicity` / `item_count` pair, and a two-valued `optional` — `!` is rejected on outputs, so there is nothing three-valued to report.

    **`input_form`** is the renderer-facing view of the same inputs — the [input-form descriptor](../../under-the-hood/input-form-descriptor.md), an ordered list of field descriptors per pipe. Reach for `pipe_io_contracts` when you need the schema and the declared contract; reach for `input_form` when you are building a form and want field kinds, defaults, and choices without reading schemas.

    The JSON Schema on an input names its concept: its top-level `title` is the `concept_ref` and its top-level `description` is the concept's authored description. A fixed count also emits `minItems`/`maxItems` on the array wrap.
