---
title: "Content Generation Across Boundaries"
description: "How Pipelex serializes dynamic Pydantic models and large binary content across a worker boundary — JSON schema propagation, class source injection, and storage-based payload offloading — independent of the host runtime."
---
...
Both implementations of `ContentGeneratorProtocol` share that one helper — the in-process generator and a host runtime's workflow arm — so the conversion, the short-circuit and the typed dry-run fidelity error have a single home. `dump_mode` is the one thing they differ on: an object that crossed a payload boundary needs `mode="json"`, because some field types only round-trip cleanly through their json form.
