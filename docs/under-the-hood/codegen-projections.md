...
A crate from `load_crate_for_concept_projection` has **not** been proven runnable, and the lighter contract is scoped to this one projection: `resolve`, `codegen inputs`, `validate` and `run` all still reject a bundle whose `PipeFunc` implementation is missing. The two entry points sit side by side, each with its own documented contract, precisely so that taking the lighter one stays a deliberate decision at the call site.
