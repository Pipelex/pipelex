...
Both codegen commands load and normalize the crate through the same shared module the resolver uses (`pipelex/cli/commands/crate_loading.py`), and all three share the resolve/validate exit-code contract. They do **not** share one entry point, because they do not need the same thing loaded.

### What each projection loads

`codegen inputs` renders from the **live pipe**, so it loads the full validated library through `load_normalized_crate`, exactly like `resolve`.

`codegen types` does not. Its emitter input (`ResolvedLibrary`) carries concepts and no pipes, the resolved-concept layer reads only the normalized crate — never the loader, the class registry, or live pipes — and the crate is built from parsed bundle blueprints rather than from live pipe objects. So it goes through `load_crate_for_concept_projection`, which loads domains and concepts and **never instantiates the closure's pipes**. Nothing about the projection itself changes; what changes is that a whole layer of work it never used stops happening.

Two consequences fall out of that, both intentional:

- **A `PipeFunc` method can be bootstrapped.** Writing a `@pipe_func` requires the generated `structures.py` — the validator enforces that the function's return type *is* the output concept's structure class. If generating `structures.py` in turn required that function to already be importable, the only way to start a new PipeFunc method would be to hand-write a throwaway stub, generate, then restore it. Because the projection never builds a pipe, `pipelex build structures` now works with the implementation absent, half-written, or unimportable — it does not import your `@pipe_func` module at all, so an error inside it cannot break structure generation.
- **It needs no model deck, so it works offline.** Loading model specs exists to validate pipe `model = …` pins against the deck. With no pipe instantiated there is no pin to validate, so `codegen types` skips the deck — and with it a remote gateway-config fetch.

The safety property is that the projection is *genuinely* concept-only: for the same bundles, the emitted files and the crate `fingerprint` are byte-identical whether or not the implementation is present. If that ever stopped holding, the lighter load would be unsound — which is why the regression suite asserts that byte-identity directly, rather than merely asserting the command exits `0`.

Note the scope carefully: skipping pipe instantiation skips pipe validation for **every** pipe type, not just `PipeFunc`. `PipeFunc` is the motivating case, not the boundary — a `PipeSequence` naming a nonexistent step also sails through `codegen types`. That is coherent (a concept projection has no business ruling on pipes) but it does mean `codegen types` / `build structures` are not validation gates and must not be used as one.

A crate from `load_crate_for_concept_projection` has therefore **not** been proven runnable, and the lighter contract is scoped to this one projection: `resolve`, `codegen inputs`, `validate` and `run` all keep the strict load and still reject a bundle broken at pipe level. The two entry points sit side by side, each with its own documented contract, precisely so that taking the lighter one stays a deliberate decision at the call site.
