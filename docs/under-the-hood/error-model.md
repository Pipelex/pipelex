...
|---|---|---|
| `pig-03` | 400 | "the client tried to route" — a refused `x-portkey-*` header, the `?model=` query form, a `@<slug>/<model>` virtual-key model, or a path and body naming different models. Reaching it means a client bug, not a caller's or an operator's mistake; `tests/unit/pipelex/providers/manifold/test_manifold_clients.py` pins that the manifold clients send none of those forms. |
...
The gateway's remaining codes — the storage deadline outcomes (`pig_storage_timeout` at 504, `pig_storage_client_disconnected` at 499) — belong to no family and classify on their status like anything else, which is the right reading for a timeout.
