...
A working memory therefore has two shapes. In memory, a `Stuff` holds a resolved `Concept` — the runtime's own subclass of the protocol's `ConceptAbstract`, carrying the `description`, the `structure_class_name` and the `refines` the standard keeps in the library a method loads. On the wire, a stuff *names* its concept: every dump — `model_dump`, `smart_dump`, `dump_for_transport`, the `working_memory.json` a run saves, the runner's `PipeOutput` response — emits `{"stuff_code": …, "stuff_name": …, "concept": "<domain>.<Code>", "content": …}`, the form the MTHDS [CLI I/O contract](https://mthds.ai/latest/spec/cli-io-contract/) requires, because `StuffAbstract` serializes `concept` as the ref string. The definition never leaves the runtime, so a dump is one-way: a reader resolves each ref through the concept library the method loaded, the same way the input side resolves an input envelope's `concept`.

**This runtime emits and resolves `<domain>.<Code>` only.** The standard also defines `<package_address>::<domain>.<Code>`, the spelling for a concept a *dependency package* contributes, and nothing here writes it or accepts it — that form is not implemented yet. The gap is closed on the reading side rather than on the writing side, by the one rule both readers share, `ConceptProviderAbstract.resolve_wire_concept_ref`: a bare ref is matched against every key the concept library holds for that spelling — the host/native entry keyed by the ref itself, plus each `<alias>-><domain>.<Code>` entry a dependency landed under (`ConceptLibrary.add_dependency_concept`, where the alias is the whole package address on the live route). One candidate resolves, none raises naming the ref, and **more than one raises naming every key that matched**, because a bare ref genuinely cannot say whether the host's `scoring.WeightedScore` or a dependency's was meant, and binding one package's definition to another package's data is a silent data fault rather than a failure. Enumeration is why `list_concept_keys_for_ref` sits on the provider abstraction at all: resolution alone cannot distinguish "one match" from "two".
...
The worker-side hydration helper (`pipelex/runtime_bridge/primitives/hydration.py`) is open core: it iterates the raw dict, resolves each stuff's concept ref through the concept library of the current library — `load_from_crate` having run first, so the crate's concepts are declared and its dynamic classes registered — uses `StuffContentFactory` to rebuild typed content from the registered classes, and preserves aliases. A stuff whose `concept` is not a ref string, names a concept the library does not hold, or names a spelling the library holds more than once, fails the hydration with a `PipeJobError`. A current library is **not** a precondition, because a caller legitimately has none: `Pipelex.make()` sets no current library, and the transport boundary's `scoped_library_for_crate(None, …)` is a documented no-op that falls back to the active class registry. With no library, hydration resolves the native refs from the pinned native set and refuses anything a bundle declares, naming the stuff — the same answer the delivery executor's best-effort local hydration gives, which likewise resolves a native ref from the pinned native set with no library and falls back to the raw render for anything else, since a delivery worker never loads the crate. That reader treats an ambiguous ref as "not known here" too — a logged warning naming the colliding keys and a raw render — because rendering a result must never fail a delivery. It also guards against cross-exec class-identity mismatches (`_validate_as_known_class` round-trips items through `model_dump()`): the dynamic classes a worker re-execs from the crate are new Python identities sharing a name with whatever a data converter eagerly rebuilt, and a naive `model_validate` would reject the older instance.
