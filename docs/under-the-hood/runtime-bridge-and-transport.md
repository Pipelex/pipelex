...
description: "The backend-neutral seam that carries a pipe run across process boundaries — boundary DTOs, LibraryCrate propagation, deferred hydration, and per-call registry/library isolation — and where open core ends and the closed pipelex-transport layer begins."
---
...
The class-registry accessor and its library scoping (`pipelex/service_hub.py`, `pipelex/system/registries/class_registry_access.py`, `pipelex/method_hub.py`, `pipelex/libraries/library.py`, `pipelex/libraries/library_manager.py`) is open core — the same machinery serves direct execution, the open runner, and any host-runtime plugin.
