---
title: "Secrets Provider Plugins"
description: "How Pipelex selects a secrets backend by config, the keyed-registry-plus-config-selected-singleton seam a secrets plugin rides, and how to author an out-of-tree pipelex-secrets-<backend> package."
---
...
boot (Pipelex.setup)
  └─ build_registrar(config, builtin_plugins=BUILTIN_PLUGINS, …)   # pure, import-light
       ├─ for each built-in plugin                   (SecretsPlugin is one)
...
`pipelex/providers/secrets/secrets_plugin.py` is the reference secrets plugin. It is **core-unconditional** — a secrets provider is required infra, so it joins the kernel layer's `KERNEL_CORE_UNCONDITIONAL_PLUGIN_NAMES` — composed into `CORE_UNCONDITIONAL_PLUGIN_NAMES` at the boot entrypoint — and cannot be disabled into a boot with no secrets provider:
