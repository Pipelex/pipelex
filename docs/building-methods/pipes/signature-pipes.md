---
description: "Declare contract-only pipes with PipeSignature. Mock dependencies in dry-run, gate strict validation, and let agents stub a method end-to-end before implementing each step."
---
...
Because a signature is a contract rather than a way of running, it sits **outside** the executable pipe taxonomy: it is not a `PipeType` or `PipeCategory` like `PipeLLM` or `PipeSequence`. Its runtime form is a dry-run-only shim — it can verify inputs and mock its declared output, but it never executes live. Implementing a signature is not swapping one pipe *type* for another; it is fulfilling a contract with a concrete pipe that satisfies it.
