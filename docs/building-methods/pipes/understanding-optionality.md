---
description: "Optionality lets a Pipelex method declare that a value may legitimately be absent — with `?` and `!` presence markers, recorded absences, and static validation that proves every absence is handled."
---
...
- [Agent CLI](../../tools/cli/agent-cli.md#validate) — how the marker is reported on `pipe_io_contracts` as the three-valued `presence`
