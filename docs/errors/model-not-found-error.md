...
- **The caller's**, when the deck neither defines the reference nor names it in any of its own entries. Only the method being run can have named it, in an inline model setting such as `model = { model = "gpt-5.1", temperature = 0.2 }`, which the check at load does not look into. The error is then in the `input` domain, so an HTTP surface answers 422, and its message survives STRICT disclosure: it is the same mistake that `ModelChoiceNotFoundError` reports at load for a reference written as a string.
