...
> **Status.** This specification was written before its implementation, deliberately: the guarantees below are what the engine is built against. The data format, the operation vocabulary and the checks land first; `pipelex migrate` and boot tolerance land last. Of the checks, the coverage gate (`make check-migration-schemas`), its regenerator (`make up-migration-schemas`), the ledger check (`make check-ledger`) and the transform-golden verifier inside the coverage gate all exist today; the verification of a pre-history entry does not, and **an entry carrying `pre_history = true` is refused by `make check-ledger` until its verification lands** — the flag exempts an entry from the coverage gate, and an entry no gate verifies is exactly the escape hatch this contract refuses. Until the commands exist, this page is built but kept out of the documentation navigation.
