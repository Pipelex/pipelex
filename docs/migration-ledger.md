...
**A backup does not belong in the user's `git status`, and pipelex writes the rule that keeps it out.** A project's `.pipelex/` is normally inside a git repository, so one run would otherwise put a dozen untracked `.bak.<stamp>` files in front of the user and leave them to work out which are ours. Pipelex therefore keeps a `.gitignore` *inside* the configuration directory, carrying one line per personal override file — `pipelex_override.toml` and its siblings, `inference/backends_override.toml` and `inference/routing_profiles_override.toml`, files that hold only a developer's own choices and have no business in a project's commits either — and the backup rule — `*.bak.` followed by the exact stamp shape, spelled out digit class by digit class, built from the same infix the namer uses so renaming it cannot leave a rule that matches nothing. The stamp is spelled out in full rather than abbreviated to a `[0-9]*` run, because a run would also swallow a copy the user named themselves along the lines of `pipelex.toml.bak.1-notesZ` — one pruning explicitly refuses to touch, and hiding a file we decline to manage is the worse half of both answers. In-directory rather than in the repository root because the walk is `~/.pipelex/` and the project `.pipelex/` and nothing else, so a local file needs no idea where the root is and never edits a file the user maintains for their whole project.

One backup is deliberately out of reach of this rule. A configuration file that is a symlink is migrated through to the file it names, so its backup lands beside *that* file — in a dotfiles directory, which is the user's and not ours. Pipelex does not follow it there: writing into the `.gitignore` of somebody's whole dotfiles repository is a larger liberty than the single untracked copy it would hide.

The file is written by `pipelex init`, and by any real (non-`--dry-run`) `pipelex migrate` — the second so that a machine whose `.pipelex/` predates this gets the rule from the very run that would otherwise dirty it. **Including the run that finds every file already at the current schema**, which is not the edge case it sounds like: that is the state a machine is in from the moment it migrates once until the next schema change, so it is what most users' next `migrate` actually does, and tying the rule to the write pass alone would have delivered it to almost nobody. The one real run that does not write it is the one whose migration the user declined — "nothing was written" has to stay true of the whole directory, convenience file included, and the next run writes it. It is **create-if-absent and nothing more**: once a `.gitignore` is there it is a file in the user's repository, possibly theirs, possibly ours with a line taken out on purpose, and re-asserting a rule into it every run would be a tool arguing with its user. Two omissions are deliberate. A `.rescue.` copy is **not** ignored — it exists only because a write could not be vouched for and the report tells the user to go and get it, so turning up in `git status` is that reminder working. Neither is a copy the user named themselves: pruning already refuses to touch a `pipelex.toml.bak.notes` on the grounds that it is theirs, and hiding it from their own `git status` would be the same mistake pointed the other way.
