...
# a fresh worker can both pass the ``is None`` check and both call ``Pipelex.make``;
...
def ensure_pipelex_booted(
    ...
    """
    if Pipelex.get_optional_instance() is not None:
        return
    with _boot_lock:
        # Re-check inside the lock: another thread may have booted while we waited.
