...
from pipelex.service_hub import ServiceHub, get_console, set_service_hub
...
def setup_doctor_runtime(*, log_config_overrides: Mapping[str, Any] | None = None, config_dir: Path | None = None) -> None:
    """Spin up a fresh ServiceHub and configure logging for doctor checks.
    ...
    set_service_hub(service_hub)
    try:
        service_hub.setup_config(config_cls=PipelexConfig, config_dir=config_dir)
    except ValidationError as validation_error:
        validation_error_msg = report_validation_error(category="config", validation_error=validation_error)
    ...
    service_hub.set_console_print_target(target=log_config.console_print_target)
