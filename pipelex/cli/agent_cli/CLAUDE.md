...
| `codegen` | Agent mirror of the bare `pipelex codegen` family (types\|check subcommands). `types --target <flavor>` projects the closure's **concept set** into stamped typed artifacts + `codegen.lock` (write-if-changed) — it loads the normalized crate through `load_crate_for_concept_projection`, so it never instantiates a pipe, never imports the customer's `@pipe_func` module, and needs no model deck (offline, no gateway-config fetch); a bundle whose `PipeFunc` implementation is missing or unimportable still generates. That is the weaker contract: it is not a runnability verdict — `validate` and `run` still reject such a bundle. `check` is the offline drift check (pure hashing, no Pipelex boot — exit 0 current, 1 drift as a structured `CodegenDriftError` with `drifts[]`, 2 no/unreadable lock). Both: `--format markdown\|json` (success, default: markdown) + `--error-format markdown\|json` (errors, defaults to `--format`'s value). |
