...
# `operator.*` and `controller.*` are derived from the runtime's own registries, `feature.*` is
# hand-maintained in the generator because no registry describes a language feature, and every
# exclusion reason is declared in the generator's source alongside them.
#
...
def generate_corpus_vocabulary_content() -> str:
    ...
    the enum. `feature.*` has no registry to follow, so it renders alphabetically; it goes last
    because it is the one namespace a reader cannot predict from a registry.
    """
