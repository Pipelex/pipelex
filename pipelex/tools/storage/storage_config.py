...
class StorageMethodConfig(ConfigModel):
    ...
    #
    # `uri_format` renders the FILENAME ONLY. The prefix is not configurable:
    # `GeneratedContentFactory` composes `{storage_scope}/generated/` itself and
    # appends whatever this renders. So `{primary_id}`, `{secondary_id}` and
    # `{storage_scope}` are all gone from the supported set.
    #
    # The first two were always `user_id` then `pipeline_run_id` — "who ran it"
    # then "which run" — and the first half is the bug the storage change exists
    # to remove: keying by the uploader makes a run's output unreadable by
    # anyone else on the same team.
    #
    # `{storage_scope}` was the honest replacement for a while, but leaving the
    # prefix in an operator's hands means the layout is only a default: a config
    # that omits it puts every run's generated bytes in one namespace, and one
    # that omits the leaf puts them loose at the root where nothing listing a
    # run's content by category can find them. An invariant a config may switch
    # off is not an invariant, so the prefix moved into code and this string
    # kept the part that is genuinely a preference — the filename, including any
    # subdirectory the operator wants beneath the leaf.
    #
    # A config naming any retired placeholder is rejected at BOOT rather than
    ...
    URI_FORMAT_SUPPORTED_FIELDS: ClassVar[frozenset[str]] = frozenset({"hash", "extension"})
    ...
    def _collect_uri_format_error_msgs(self) -> list[str]:
        ...
        if self.uri_format.startswith("/"):
            # Appended to `{storage_scope}/generated/`, so a leading slash yields
            # a `//` in the key — a different object from the one the operator
            # meant on every backend that treats the key as an opaque string.
            return ["- uri_format must not start with '/': it renders the filename under the generated/ leaf, not an absolute key"]
        try:
