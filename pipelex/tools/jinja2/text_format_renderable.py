...
Jinja2 filter to work with any type that implements the `rendered_for_template_async` method.
...
class TextFormatRenderable(Protocol):
    ...
    (aliased as `format`). The filter will call `rendered_for_template_async()` to get
