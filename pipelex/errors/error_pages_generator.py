...
def _commit_page(target: Path, new_content: str, report: ErrorPagesReport) -> None:
    ...
    Rendered content is passed in eagerly rather than computed lazily here —
    avoids the loop-variable-in-closure pattern and its mypy ambiguity, and
    the per-page render cost is negligible compared to the I/O it gates.
    """
    if target.exists() and _has_authored_marker(target.read_text(encoding="utf-8")):
...
def _has_authored_marker(content: str) -> bool:
...
def _parent_link(cls: type[PipelexError]) -> str:
    """Return a markdown link to the parent class's docs page, or a plain label."""
    ...
    parent = parents[0]
    if parent is PipelexError and cls is PipelexError:
        return "`Exception` (Python builtin)"
    return f"[`{parent.__name__}`]({page_slug(parent)}.md)"
