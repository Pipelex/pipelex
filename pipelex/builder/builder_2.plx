...
steps = [
    ...
    { pipe = "polish_flow", result = "prepared_flow" },
    { pipe = "design_pipe_signatures_3", result = "pipe_signatures" },
...
IMPORTANT: define the main pipe of the pipeline, i.e. unless it's a single pipe workflow, the main pipe is the overall orchestrator sequence of the whole pipeline. Give it a clear name, make it clear it's the main pipe in a "# Comment", but don't write "main" in its pipe_code or the description itself.
"""
...
Is the flow consistent? Are there any missing variables or variable names that are not the expected names?
