...
async def imgg_gen_single_image(imgg_assignment: ImggAssignment) -> GeneratedImage:
    ...
    imgg_job = ImgGenJobFactory.make_img_gen_job_from_prompt(
        img_gen_prompt=imgg_assignment.imgg_prompt,
        img_gen_job_params=imgg_assignment.imgg_job_params,
        img_gen_job_config=imgg_assignment.imgg_job_config,
        job_metadata=imgg_assignment.job_metadata,
    )
    generated_image = await imgg_worker.gen_image(img_gen_job=imgg_job)
