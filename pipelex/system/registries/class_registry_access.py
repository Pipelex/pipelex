...
class registry and is itself inside ``service_hub``'s import closure
...
import ``service_hub`` at module level without forming a cycle. Hosting the accessor here is what
lets those modules use a plain top-level import instead of a lazy ``importlib`` shim.

``pipelex.service_hub.get_class_registry`` is the public accessor and delegates here; prefer it
...
process-global Kajson one. Resolving *which* library is current is method-layer knowledge, so the
low layer holds only a slot — ``pipelex.method_hub.set_method_hub`` installs the real resolver at
boot. Unresolved, or with no library pinned, callers get the process-global registry.
"""
...
def _no_library_scoping() -> ClassRegistryAbstract | None:
    """Core default resolver: with no MethodHub installed there is no per-library scoping."""
    return None
...
class ClassRegistryScoping:
    ...
    ServiceHub (the doctor path, most unit tests) degrades to the process-global registry instead
