...
from pipelex.cogt.model_backends.backend import resolve_model_specs_section
...
from pipelex.system.pipelex_service.pipelex_service_onboarding import (
...
def _service_config_the_ledger_can_explain(*, config_path: Path) -> PipelexServiceConfig | None:
    ...
    helper performs is a merge of one. This is what lets a file written by an earlier release —
    one still carrying a table the schema has since dropped — boot with a warning rather than a
    refusal, once the ledger carries the entry that drops it.
    """
...
def _backends_document(*, config_dir: Path | None) -> dict[str, Any] | None:
    ...
    ``config_manager.backends_file_paths``. The reader below goes through here so it cannot end
    up branching on a narrower view of the configuration than the loader it feeds.
...
def enabled_managed_gateway_sections(*, config_dir: Path | None = None) -> dict[str, str]:
    ...
    — see `resolve_model_specs_section`.

    Read here, off the raw document, because the boot needs the answer *before* it can load the
    backend library: what it fetches is the input to that load.
    ...
    **It must read `enabled` exactly as the library loader does** — the raw value's truthiness, and
    ...
    answer here: it fetches the managed backends' model specs only when this says one is enabled,
    then hands them to a loader that decides for itself which backends are enabled. Reading the
    literal `true` here while the loader read truthiness would leave `enabled = 1` in a state neither
    could name: no specs fetched, backend loaded — refused as *"model specs were not provided"* on a
    strict boot, silently dropped from the deck on a lenient one. An override the loader saw and this
    did not would be the same split.
    ...
    Returns:
        ``{backend_name: section_name}``, empty when no managed gateway backend is enabled — including
        when there is no base file.
