...
class PipelexPlugin(Protocol):
    ...
    **Invariant — a plugin belongs to exactly one layer.** Its adapters are either all runtime-layer
    ...
    ``pipelex.plugins`` for the runtime half, ``pipelex.interpreter_plugins`` for the interpreter half
    — and a plugin straddling the two would put the method interpreter back into every runtime import
    closure. External plugins are discovered through an entry point and so live in no declared layer,
    but the same rule keeps them honest about what they pull in.
