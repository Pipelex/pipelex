...
def build_registrar(*, config: "PipelexConfig") -> PluginRegistrar:
    ...
    plugin named in ``config.plugins.disabled``; and is fail-loud on every
    conflict (duplicate backend/mode/slot, version mismatch, broken plugin).
    """
    registrar = PluginRegistrar(config=config)
    disabled = set(config.plugins.disabled)
    for plugin, origin in _iter_discovered_plugins():
        if plugin.name in disabled:
            if plugin.name in CORE_UNCONDITIONAL_PLUGIN_NAMES:
                raise CoreUnconditionalPluginDisabledError(plugin_name=plugin.name)
            log.info(f"Plugin '{plugin.name}' is disabled via plugins.disabled; skipping.")
            ...
            continue
        _register_plugin(registrar, plugin=plugin, origin=origin)
    return registrar


def _iter_discovered_plugins() -> Iterator[tuple[PipelexPlugin, PluginOrigin]]:
    for plugin in BUILTIN_PLUGINS:
        yield plugin, PluginOrigin.BUILTIN
    for entry_point in _external_entry_points():
        yield _load_external_plugin(entry_point), PluginOrigin.EXTERNAL
