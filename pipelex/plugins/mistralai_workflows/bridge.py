"""Framework-agnostic core of the mistralai_workflows plugin.

This module contains the boundary types (``PipelexPipeRunInput`` /
``PipelexPipeRunOutput``) and the dispatch entry-point
(``run_pipe_via_bridge``) used by the Mistral Workflows activity wrapper. It
deliberately does NOT import ``mistralai.workflows`` at module top-level so
that callers can use the bridge directly (Tier 3 usage) and so that unit tests
can exercise it on a venv without the optional dep installed.
...
async def _scoped_library_for_crate(library_crate: LibraryCrate | None) -> AsyncGenerator[str | None, None]:  # noqa: RUF029
    ...
    library_id = f"mistralai_workflows_{uuid4().hex[:8]}"
...
def _require_pipelex_temporal_extra() -> None:
    try:
        ...
        msg = "TEMPORAL_* execution modes require the pipelex[temporal] extra. Install with: pip install 'pipelex[temporal,mistralai-workflows]'"
