...
from openai import APIConnectionError, BadRequestError, NotFoundError, omit
...
class GroqLLMWorker(LLMWorkerInternalAbstract):
    def __init__(
        self,
        sdk_instance: Any,
        ...
        self.groq_client_for_text: openai.AsyncOpenAI = sdk_instance
        if structure_method:
            instructor_mode = structure_method.as_instructor_mode()
            log.verbose(f"Groq structure mode: {structure_method} --> {instructor_mode}")
    ...
    async def _gen_text(
        ...
        messages = GroqFactory.make_simple_messages(llm_job=llm_job)

        try:
            temperature = llm_job.job_params.temperature
            response = await self.groq_client_for_text.chat.completions.create(
            ...
            msg = f"Groq model not found:\n{self.inference_model.desc}\nmodel: {self.inference_model.model_id}\n{not_found_error}"
            raise LLMModelNotFoundError(msg) from not_found_error
        except APIConnectionError as api_connection_error:
            msg = f"Groq API connection error: {api_connection_error}"
            raise LLMCompletionError(msg) from api_connection_error
        except BadRequestError as bad_request_error:
            msg = f"Groq bad request error with model: {self.inference_model.desc}:\n{bad_request_error}"
            raise LLMCompletionError(msg) from bad_request_error
        except openai.RateLimitError as rate_limit_error:
            msg = f"Groq rate limit exceeded: {rate_limit_error}"
            raise LLMCompletionError(msg) from rate_limit_error

        groq_message: ChatCompletionMessage = response.choices[0].message
        response_text = groq_message.content
        if response_text is None:
            msg = f"Groq response message content is None: {response}\nmodel: {self.inference_model.desc}"
            raise LLMCompletionError(msg)

        if (llm_tokens_usage := llm_job.job_report.llm_tokens_usage) and (usage := response.usage):
            llm_tokens_usage.nb_tokens_by_category = GroqFactory.make_nb_tokens_by_category(usage=usage)
    ...
    async def _gen_object(
        ...
        try:
            ...
            try:
                ...
                msg = f"Groq instructor failed with model: {self.inference_model.desc} trying to generate schema: {schema} with error: {exc}"
                raise LLMCompletionError(msg) from exc
        except NotFoundError as exc:
            msg = f"Groq model '{self.inference_model.model_id}' not found: {exc}"
            raise LLMCompletionError(msg) from exc
        except BadRequestError as bad_request_error:
            msg = f"Groq bad request error with model: {self.inference_model.desc}:\n{bad_request_error}"
            raise LLMCompletionError(msg) from bad_request_error
