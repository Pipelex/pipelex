...
def _make_openrouter_img_gen_worker(
    *,
    ...
    return OpenAICompletionsImgGenWorker(
        openai_completions_factory=OpenRouterCompletionsFactory(is_http_url_enabled=True),
...
class OpenRouterPlugin:
    """Built-in driver for OpenRouter image generation via the OpenAI-completions substrate."""

    name = "openrouter"
    targets_api = PLUGIN_API_VERSION

    def register(self, registrar: PluginRegistrar) -> None:
        registrar.add_inference_backend(family=InferenceFamily.IMG_GEN, sdk="openrouter_img_gen", make_worker=_make_openrouter_img_gen_worker)
