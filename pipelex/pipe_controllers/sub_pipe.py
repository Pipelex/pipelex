...
class SubPipe(BaseModel):
    ...
    async def run_pipe(
        ...
        log.debug(f"SubPipe {self.pipe.code} to generate {self.pipe.output_concept_code}")
        ...
        if batch_params := self.batch_params:
            try:
                ...
                raise PipeInputError(
                    f"Input list stuff named '{batch_params.input_list_stuff_name}' required by sub_pipe '{self.pipe.code}' "
                    f"of pipe '{calling_pipe_code}' not found in working memory: {exc}"
                ) from exc

            input_concept_code = input_list_stuff.concept_code
            output_concept_code = self.pipe.output_concept_code

            sub_pipe = get_required_pipe(pipe_code=self.pipe.code)
        ...
        elif isinstance(self.pipe, PipeCondition):
            # This is the only line that changes between run and dry_run
            if sub_pipe_run_params.run_mode == PipeRunMode.DRY:
                sub_pipe_run_params.run_mode = PipeRunMode.DRY
                pipe_output = await self.pipe.run_pipe(
                    job_metadata=job_metadata,
                    working_memory=working_memory,
                    pipe_run_params=sub_pipe_run_params,
                    output_name=self.output_name,
            ...
            required_variables = self.pipe.required_variables()
            log.debug(required_variables, title=f"Required variables for {self.pipe.code}")
            ...
            try:
                ...
                sub_pipe_path = sub_pipe_run_params.pipe_layers + [self.pipe.code]
            ...
            log.debug(required_stuffs, title=f"Required stuffs for {self.pipe.code}")
            # This is the only line that changes between run and dry_run
            if sub_pipe_run_params.run_mode == PipeRunMode.DRY:
                sub_pipe_run_params.run_mode = PipeRunMode.DRY
                pipe_output = await self.pipe.run_pipe(
