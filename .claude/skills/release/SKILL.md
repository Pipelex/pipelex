...
`gh release view vX.Y.Z` confirms the Release and its notes. A publish that failed *after* the merge is recovered by re-running that run's **failed jobs**, which keeps the successful ones, or by a `workflow_dispatch` — needed when the failure calls for a change to the workflow file, since a re-run replays the file the run started with. Both paths are safe once the dists have reached PyPI: `skip-existing` opens on a re-run and on a dispatch, so the publish job no longer dies on the files already there, and the build job recovers the bytes this version published rather than rebuilding them — the artifact an attempt of the same run stored, else one an earlier run stored for the same commit, else the files PyPI serves, checked against the digests PyPI publishes beside them. A dispatch from `main` after the back-merge names a different commit than the release run did, so it recovers from PyPI rather than from an artifact; the result is the same bytes. The Release step itself is idempotent — it edits an existing release rather than failing on one. A dispatch accepts `main`, `release/vX.Y.Z` and `pre-release/v*` and nothing else, and its run lists under the ref it was dispatched from rather than under the release branch.
