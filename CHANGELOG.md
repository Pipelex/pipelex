...
- **Advisory `warnings` on the validation report + the useless-`!` lint:** The validation report gains a general `warnings` array — same item shape as `validation_errors`, but advisory: it never flips `is_valid`. Its first occupant is `optional_force_redundant`: a `!` (force) input whose slot is guaranteed present in every analyzed flow — the assertion can never fire, so the marker is dead weight. The lint aggregates across flows before warning (a `!` that is meaningful in one flow is never flagged because another flow guarantees the slot). Whole-bundle agent-CLI validate surfaces carry the same `warnings` array, rendered as a "Warnings" section in markdown.
