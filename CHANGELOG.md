...
- **CSV input & output for pipelines.** A `.csv` file can now be read directly into a typed `ListContent[YourConcept]` and a list output can be written back out as CSV. On input, an `inputs.json` reference whose `url` ends in a tabular suffix under a flat structured concept (e.g. `{ "people": { "concept": "demo.Person", "content": { "url": "people.csv" } } }`) loads as one list whose items are instances of that concept — column headers must match the concept's field names, cells are coerced via Pydantic (`birth_year` → `int`, ISO dates → `date`), and an empty cell becomes `None` (so the field must be optional). On output, the new `--save-csv <path>` flag on `pipelex run pipe|bundle|method` writes the main stuff to a literal, cwd-relative CSV path when that output is a flat list. The row concept must be flat (scalar fields only — `text`/`integer`/`number`/`boolean`/`date`, plus optionals and choice-constrained scalars); a nested/list/dict/concept-typed field is rejected with a clear error naming the offending field. CSV reading uses the Python standard library; `.xlsx` is recognized but routed to a clear "needs `pipelex[tabular]`" message (the Excel backend is not built yet). v1 reads local file paths only — a remote `url` (`http(s)`/`s3`/`gs`/…) with a tabular suffix is actively rejected. See the [CSV Input & Output guide](https://docs.pipelex.com/latest/building-methods/pipes/csv-input-and-output/).
- **`PipeSignature` — contract-only pipes for top-down pipeline design.** A new pipe type (`type = "PipeSignature"`) declares a pipe's `inputs`, `output`, and `description` without an implementation, so an author or agent can sketch a complete pipeline before committing to the operator that will eventually do the work. Signatures slot into `PipeBlueprintUnion` and `PipeSpecUnion`, are produced by the new `PipeSignatureFactory`, and ship with their own runtime (`PipeSignature` in `pipelex/pipe_signature/`). At dry-run time a signature mints a mock `Stuff` matching its declared output (multiplicity included); at live-run time it raises `PipeSignatureNotExecutableError`. Multiplicity is supported on inputs (`Doc[]`, `Image[3]`) just like every other pipe. Optional `signature_for` field hints the downstream pipe type the signature stands in for (tooling-only; cannot itself be `PipeSignature`). New docs page: [Signature Pipes](https://docs.pipelex.com/latest/building-methods/pipes/signature-pipes/). Cross-link: `wip/signature-based-validation/signature-based-validation.md`.
- **`--allow-signatures` flag on `validate`.** `pipelex validate pipe`, `pipelex validate bundle`, and every `pipelex-agent validate` subcommand (`pipe`, `bundle`, `method`) accept `--allow-signatures` (default off) to dry-run pipelines that still contain `PipeSignature` placeholders. Without the flag, strict validation refuses any pipeline whose dependency graph reaches a signature, raising `SignaturesNotAllowedError` and reporting every reachable signature plus the controller chain (`signature_refs` and `dep_paths`) so the user knows exactly which pipes are still stubs. The main CLI and the agent CLI are both strict by default.
- **Graph walk for signature detection.** `collect_signature_refs()` and `collect_signature_paths()` (in `pipelex/pipe_signature/signature_walk.py`) walk a pipe's dependency graph (via the new default `pipe_dependencies()` method on `PipeAbstract` and per-controller overrides), aggregating signature pipe_refs and the controller chains that reach them. Used by the strict-mode pre-check in `BundleValidator` (`_signature_pre_pass`, run from `validate_pipes`) so `validate_bundle` surfaces a single `SignaturesNotAllowedError` listing every offender across the bundle.
...
- **Gemini deck: `gemini-3.0-pro` removed — Google shut down the underlying `gemini-3-pro-preview`.** Use `gemini-3.1-pro` (→ `gemini-3.1-pro-preview`) instead; it was already in the deck and is now also registered on the `portkey` backend for parity with `google`. The dead `gemini-3.0-pro` handle is dropped from the `google`, `portkey`, and `openrouter` backends and from the test-profile collections. `gemini-3.0-flash` and the `nano-banana-pro` image model (`gemini-3-pro-image-preview`) are unaffected. Anyone referencing `gemini-3.0-pro` must switch to `gemini-3.1-pro` (or the `gemini-pro-latest` alias). The Pipelex Gateway lists models from its own remote config — drop `gemini-3.0-pro` there separately.

- **`validate_bundle`, `validate_bundles_from_directory`, and `BundleValidator.validate_pipes` / `acquire_and_validate` now accept `allow_signatures: bool = False`.** Strict by default. The same flag is threaded through `pipelex/cli/commands/validate/_validate_core.py` (the CLI entry point) and `pipelex/cli/agent_cli/commands/validate/_validate_core.py` (the agent CLI), both strict by default. `ValidateBundleError` gained a `signature_check_error: SignaturesNotAllowedError | None` field so callers (and the CLI error renderer) can show the signature dep chain when validation fails on a signature.

- **Dry-run and validation consolidated into `BundleValidator`; the standalone `pipelex/pipe_run/dry_run.py` is removed.** The free functions `dry_run_pipe` / `dry_run_pipes` and the modules `pipelex/pipe_run/dry_run.py`, `dry_run_with_graph.py`, and `dry_pipe_router.py` are gone. Their work now lives in `BundleValidator` (`pipelex/pipeline/bundle_validator.py`) with two lifecycles — `acquire_and_validate` (opens a fresh library, sweeps every pipe, owns teardown; the standalone `validate --all` path) and `validate_pipes` (the inner sweep against an already-open library, called by `validate_bundle`) — and the library/job setup steps are extracted into `pipelex/pipeline/execution_seams.py` (`acquire_library`, `prepare_pipe_job`). The mock-minting and stuff-spec converters (`convert_to_working_memory_format`, `convert_stuff_spec_to_typed_named`) moved onto `WorkingMemoryFactory` as classmethods. No behavior change — validation ordering (wiring pass → signature pre-pass → per-pipe dry-run sweep) and outputs are preserved; only the import surface moved. Callers importing from `pipelex.pipe_run.dry_run` must switch to `BundleValidator` / `WorkingMemoryFactory`.

- **The `validated_pipes` JSON envelope now identifies every pipe by its namespaced `pipe_ref` (`domain.code`) on every validate surface.** Previously `pipelex-agent validate all` / `validate bundle` reported namespaced refs while `validate pipe` (and the builder validate ops) reported the bare code, so the same pipe could appear under two identifiers depending on which command produced it. All surfaces now emit the qualified ref, which cannot collide across domains. The shared projection helper `build_validated_pipes` dropped its `use_ref` parameter accordingly, and its entry type is now a named `ValidatedPipeEntry` `TypedDict`. Consumers that parsed `pipe_code` from `validated_pipes` expecting the bare form (e.g. the MTHDS skills) must match on the namespaced ref.
