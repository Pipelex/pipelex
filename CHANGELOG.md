...
- **Required custom search attributes on the Temporal namespace.** Pipelex now sets five custom Keyword search attributes on every workflow start — `PipeCode`, `PipelineRunId`, `SessionId`, `UserId`, `DomainCode` — so the Temporal dashboard can filter, group, and audit workflows by their actual semantic identity rather than by opaque workflow ids. The attributes must be registered once per namespace before workflows are accepted by the cluster; a soft-fail check at worker boot calls `ListSearchAttributes` and warns (with the exact `temporal operator search-attribute create` command) when any are missing. Documented at [under-the-hood/temporal-deployment.md](under-the-hood/temporal-deployment.md).
