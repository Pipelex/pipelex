...
- **The shipped model deck resolves only to models the Pipelex Gateway serves from Azure (Breaking)**: every default alias and preset keeps its name, but the premium tier and `best-gpt` now resolve to `gpt-6-astra`, the general and large-context-text tiers to `gpt-5.6-terra`, the large-context-code tier to `gpt-5.6-sol`, the small tiers to `gpt-5.6-luna`, and image generation's general and premium tiers to `gpt-image-2`; `$engineering-codebase-analysis` reaches its model through `@default-large-context-code`, the alias that names its job, instead of through the premium tier. Document extraction was already `azure-document-intelligence` and web search stays on Linkup, which Azure has no equivalent for. `pipelex update` refreshes the numbered deck files and backs up any you edited; a project that wants the previous models keeps them by naming them in `x_custom_llm_deck.toml`, which pipelex never touches.
- **The shipped deck declares `temperature = 1` throughout (Breaking)**: the whole GPT-5.6 range and `gpt-6-astra` fix their temperature at 1, so every preset states that value instead of one the worker would override while warning on every call, and `[llm.choice_defaults].default_temperature` is 1 for the same reason — it is the temperature paired with an alias, a waterfall or a bare handle named outside a preset, the `for_text` and `for_object` choices every pipe without an explicit model takes among them. Presets are therefore distinguished by their model tier and their reasoning effort alone: `$writing-factual` and `$writing-creative` issue the same call, and `$engineering-structured` runs at temperature 1. A method that needs a temperature of its own should name a model that accepts one directly, such as `gpt-4o`.
