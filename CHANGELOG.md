...
- **Every span and event a run produces is attributed to that run's caller**: `PostHogSpanExporter` resolves the identity per span and `track_event` per event, both through one `TelemetryIdentity` value, instead of stamping the single `distinct_id` bound when the telemetry manager was built. A run's `user_id` becomes the capture's `distinct_id` and its `analytics_groups` ride PostHog's groups facet; the span sites write both onto the span as `pipelex.run.user_id` and `pipelex.run.analytics_groups`, so every OpenTelemetry exporter sees the same fact and Langfuse finally receives `langfuse.user.id` — withheld for those placeholders, since that field is the attribution itself and has no later resolution behind it. The configured `user_id` and the Gateway-key hash stay as the fallback for everything that names no caller of its own: an event outside any run, and a run whose `user_id` is one of the placeholders — `local` for a run on your own machine, `dry-run-no-user` for a dry-run sweep, `single-tenant` for every run on a pipelex-api deployment configured with no users — which are the same strings everywhere and so are declined as identities rather than merging every deployment onto one person. The groups ride a fallback capture too, so a host may group its runs without naming their callers. `mode = "anonymous"` identifies nobody, a run's own caller and the configured `user_id` included. Pipelex's own Gateway stream is one shared project across every deployment, so it receives neither: a run's caller reaches it only as a one-way digest taken inside the Gateway-key hash, which keeps two callers of one deployment apart while making a collision between two deployments impossible, and the host's `analytics_groups` are not forwarded to it at all. The Gateway-key hash itself rides that stream's group facet, because the digest consumes it and a shared project would otherwise have no way to count a deployment's usage under the key that produced it. Neither the user id nor the groups ever become an event property or a span name. A host that serves many callers from one process therefore sees each caller's generations on their own timeline rather than everything under one identity per deployment.
...
- **`RunMetadata` is frozen**: a field can no longer be reassigned once the run exists, and an attempt raises a pydantic `ValidationError` at runtime rather than merely failing a type check. The span sites and both PostHog exporters read a run's identity off it while the run is in flight, so a value that changed underneath them would attribute one run's spans to two different callers. Construct a new `RunMetadata` wherever one used to be mutated.

### Fixed

- **A crash no longer sends a `PipelexError`'s message as it was written**: the privacy redaction replaced a `PipelexError`'s message only when the error was handed over bare, and the exception autocapture holds nothing but the `(type, value, traceback)` triple an interpreter excepthook is given — so the one capture nobody asks for was the one sending the message verbatim. Such a message may repeat whatever the caller passed in, a file path or a slice of the document that would not parse, and it travelled on the shipped default, where `custom_posthog.mode = "off"` still builds the Gateway client whose construction installs the hooks. Both forms are redacted now, to a same-class stand-in carrying the privacy notice, the class name and the traceback, and nothing else — and so is every `PipelexError` PostHog would reach from the error it is handed, since it sends the whole `__cause__` and `__context__` chain and every member of an exception group, not only the error at the top. The redaction works on a copy of that chain, so the exception the interpreter prints is left as it was. It goes by class: an error of another class keeps its own message, so one that copied a `PipelexError`'s text into it — `raise RuntimeError(f"failed: {error}") from error` — still sends that text.
- **Both telemetry streams record a crash, rather than whichever one went first**: PostHog stamps an error once a client has captured it and drops every later capture of that same object, a guard against one stream recording one error twice that read two destinations as a repeat. The stamp is cleared between the streams now, so Pipelex's stream — always the second — stops losing every unhandled exception on a machine that also reports to an operator's own project. An error the host had already captured itself before it escaped is still recognised as sent, redacted or not, and is not recorded a second time.
- **An unhandled exception is captured under the same identity as everything else on its stream**: the autocapture was built with the configured `user_id` and the Gateway-key hash and sent them raw, so `mode = "anonymous"` still named you on every crash, and a run's `$exception` landed on a different PostHog person from its own spans and events. Both streams now resolve the identity they use for a capture that belongs to no run — which is what an exception is, since the interpreter hooks are handed nothing but the exception itself.
- **An anonymous capture on your PostHog no longer marks the Gateway capture of the same event**: both streams were handed the same properties dictionary, and the anonymous branch stamped `$process_person_profile = false` onto it before the Gateway capture sent it with a `distinct_id`. Each capture now takes its own copy.
...
- **The Pipelex Gateway backend is gone from the runtime (Breaking)**: `pipelex_gateway` leaves the shipped `backends.toml`, its `backends/pipelex_gateway.toml` override file and the generated `pipelex_gateway_models*.md` references leave the kit, the `all_pipelex_gateway` routing profile is gone, and the `pipelex-gateway` provider plugin is deleted along with its `gateway_*` SDKs, its error classes and their docs pages. A `backends.toml` that still carries a `[pipelex_gateway]` table loads it as an unknown backend with no plugin behind it, so remove the table; a routing profile naming it must move to another backend. The shipped default routing profile is now `all_enabled_backends`, which needs no Pipelex account: it sends each model to the first enabled backend that serves it, in the order its `fallback_order` lists, and `all_openrouter` is the documented one-key path. `pipelex init` pre-selects OpenRouter on a first-time setup and no longer sets `all_pipelex_gateway` when several backends are chosen. `pipelex_manifold` and every bring-your-own-keys backend are untouched.
- **The Gateway terms of service are gone (Breaking)**: `GatewayTermsNotAcceptedError`, the `[agreement]` section of `pipelex_service.toml`, `pipelex init agreement` and the `pipelex-agent accept-gateway-terms` command are removed, and `pipelex-agent init` no longer reads `accept_gateway_terms` from its config. The `[onboarding]` section is kept, and the first-run check now fires on `inference_setup_completed` alone. A `pipelex_service.toml` written by an earlier release keeps booting: the migration ledger carries an entry that drops the retired section, and `pipelex migrate` rewrites the file.
- **The run-scoped telemetry identity applies to the one remaining stream**: `dev` grew `TelemetryIdentity` and `RunIdentityPolicy`, which resolve who a capture is attributed to from the run in flight, and wired them through both the operator's stream and the Pipelex one. With the Pipelex stream gone, the resolution stays and the second destination does not: the exception autocapture is one client again rather than two, `_track_to_pipelex` and the Pipelex span exporter are removed, and `_forget_posthog_capture_marks` — which existed only so the second stream could capture an error the first had already stamped — goes with them.
- **The Pipelex-owned telemetry stream is gone (Breaking)**: the runtime no longer reports to Pipelex's PostHog on the Gateway's behalf, so `GatewayApiKeyMissingError`, `GatewayDoNotTrackConflictError`, `GatewayTelemetryManagerInjectedError`, `TelemetryManagerAbstract.is_pipelex_telemetry_enabled` and the maintainer-only `[pipelex_gateway]` section of `telemetry.toml` (with its `is_pipelex_gateway_portkey_*` knobs) are removed; `TelemetryFactory.make_telemetry_manager` and `TelemetryManager` take only the user's `telemetry_config`. The user's own opt-in telemetry under `[custom_posthog]`, `[langfuse]` and `[[otlp]]` is untouched, and the telemetry ledger carries the entry that drops the retired section.
- **`pipelex login` is gone (Breaking)**: it wrote a `PIPELEX_GATEWAY_API_KEY` that nothing reads any more, and it also leaves `_CORE_COMMAND_ORDER` and the setup-command set that skips the readiness check — an order list naming a command the CLI does not register makes `pipelex --help` itself exit non-zero. The credential flow for the hosted service is rebuilt separately.
- **The `check-gateway-models` and `update-gateway-models` dev commands are gone**, with their `make ugm` / `make cgm` aliases and the `ticc` config-coverage target that ran the Gateway's Portkey configs.
- **`RemoteConfig` declares no fields of its own**: `posthog`, `backend_model_specs` and `aws_region` are no longer read, so the artifact's sections all arrive through `extra="allow"` and `GatewayConfig` drops its `aws_region`. `PipelexRoutingProfile` and `PipelexBackend.GATEWAY` are removed, `resolve_model_specs_section` no longer hands any backend a section it did not declare, and `is_pipelex_gateway_enabled` is gone — `enabled_managed_gateway_sections` is the one question the boot asks.
