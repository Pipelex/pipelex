...
- **Log lines carry the trace context of the span they were logged in**: the `json` sink writes `trace_id`, `span_id` and `trace_flags` in lowercase hex, the keys OpenTelemetry specifies for trace context in a JSON log that is not OTLP, on every line logged inside a valid span and none of them outside one, reserving the three names so a field spelling one rides under the `field_` prefix, and the `otlp` sink files each record under that span. The span is the Pipelex span active at the log call, a pipe's or an LLM call's, which the runtime holds in a context variable of its own from the span's start to its end whenever it traces a run, under the Pipelex Gateway's telemetry or AI span tracing on your own PostHog; outside one it is OpenTelemetry's current span, only read, so a line your code logs under its own span joins your trace. Pipelex never makes its spans current in the process's OpenTelemetry context, so a host's own instrumentation is never re-parented under them, and the spans' parents still travel in the job metadata. A boot line held until the sink arrives keeps the span it was logged in, and the opt-in boto3 Bedrock client runs its blocking call through `asyncio.to_thread`, which carries the context into the thread, as [Logging](tools/logging.md#the-trace-context) describes.
- **Named fields and a run-scoped context on the log calls**: the seven `log.*` methods take a keyword-only `fields={...}`, carried as attributes of the stdlib `LogRecord` and never rendered into the message, and `with log.context(request_id=..., pipeline_run_id=..., pipe_run_id=...)` stamps the run's identifiers onto every record emitted in scope, nested bindings merging and an absent identifier staying absent rather than reading `None`. `PipeRun.run` binds the request and pipeline identifiers from the job's metadata for a direct-mode run, and every pipe binds its own `pipe_run_id` around the whole of its run, its announcement and its failure included. A `dict` or `list` content is carried as the record's `data` attribute beside its console rendering, a JSON-ready snapshot of the call rather than the caller's live object, and an entry named like an attribute the record already owns, a record factory's included, or like one of the names set after the fields are attached — the formatter's, and the internal marker that tells a sink a record reached it already — is carried under a `field_` prefix instead of raising or steering delivery, whatever the order the names arrive in — a field spelling `data` beside structured content included, so the content keeps the name a sink reads it under and the field keeps its value beside it, as [Logging](tools/logging.md) describes.
- **Log sinks are a plugin capability selected by `[runtime.log] sink` (Breaking)**: where the records go is a registered sink token, `console` (the Rich handler, the default so the CLI keeps its rendering), `json` (one JSON object per line carrying `time`, `severity`, `logger`, `message`, the fields, the context identifiers and the exception, with no ANSI ever, for a server behind a log agent) or `otlp` (the OpenTelemetry logs signal, configured under `[runtime.log.otlp]`, so a collector receives the logs beside the spans), each of the two reserving the keys it writes itself so that a field named like one of them rides under a `field_` prefix rather than overwriting it, or a token an installed plugin registers through `add_log_sink`, and a token nobody provides stops the boot naming the registered ones, while `pipelex doctor` reports it in a row of its own and carries on through the console sink. `log_mode`, `poor_loggers`, `generic_poor_logger` and `is_console_logging_enabled` are gone with the rich-or-poor mode, and `set_log_mode`, `set_poor_log_formatter` and `LevelAndEmojiLogFormatter` with them, so a server that wants plain output selects `json` and a host that wants Pipelex silent sets `pipelex = "OFF"` under `[runtime.log.package_log_levels]`. **Migration:** run `pipelex migrate`; ledger entry `pipelex-config@5` deletes the four retired keys from `~/.pipelex/` and every project `.pipelex/` file, after which the defaults supply `sink = "console"`. A file that had `log_mode = "poor"` or `is_console_logging_enabled = false` chose a behaviour that deletion undoes and no migration may write back, so such a file sets `sink = "json"` or `pipelex = "OFF"` under `[runtime.log.package_log_levels]` by hand, as the entry's title says. [Log Sink Plugins](under-the-hood/log-sink-plugins.md) documents the seam.
- **A `gcp` log sink writing to Google Cloud Logging**: `sink = "gcp"` sends each record to Cloud Logging through the `google-cloud-logging` client library, which the new `gcp-logging` extra installs; selecting it without the extra stops the boot with the install hint and the `json` alternative named. Each record becomes one struct entry — the level mapped onto the Cloud Logging severity scale, the message, the logger, the exception and the fields as the JSON payload, the run-scoped identifiers as the entry's labels, which is what Cloud Logging indexes, and the run's own OpenTelemetry trace id, project-qualified, in the entry's `trace` field, so a line and the spans of its run are filed together. The entries leave through the library's background-thread transport, so no record costs an API round trip on the thread that logged it, with the export path rejected before the handler's lock so a refused batch is reported on stderr, rate-limited, rather than through the pipeline that refused it, and the teardown's flush carrying a deadline the library's own flush does not. `[runtime.log.gcp]` carries the log name, the project when it is not the ambient one, and a service-account credentials path for a process that should not use Application Default Credentials. **Most processes on Google Cloud want `json` instead**: Cloud Run, GKE and any platform running a logging agent over the container's stdout ingest what that sink writes and need no client at all, which [Logging Configuration](configuration/config-practical/logging-config.md) spells out.
- **Secrets are scrubbed and field values made injection-safe before any sink sees a record**: one processor, configured under `[runtime.log.redaction]` and on by default, replaces an `Authorization` header's credential whichever scheme carries it, an api-key or signature header value, an OAuth `code` parameter read through the delimiter that ends it so a percent-encoded code goes whole, a cookie header's every crumb, a secret entry written as a serialised entry or plainly as `name=value`, and an `sk_` / `plx_sk_` / `pk_` / `bl_` key spelled with a dash as with an underscore, with `[REDACTED]`, in the message, in the exception's rendered text and in every string a field carries. Each family reads the shape a secret is actually written in rather than running to the end of the line, so a sentence that merely says "cookie:" keeps its words and an identifier like `pk_customer_reference_index` is left alone, and the key family keeps the prefix that named the secret as every other family keeps its carrier. A field of the record, or a mapping entry at any depth, whose own name is a secret's (`authorization`, `password`, `api_key`, `access_token` and the like, read in any case and with a dash as an underscore) loses its value whatever it holds, judged by the name the call used so a field carried under the `field_` collision prefix is covered too, and a structured content is redacted by name before it is rendered, so the message says exactly what `data` holds even where the secret sat in a nested object or in a number, content JSON refuses and falls back to its `repr` included; a pydantic model or any other object is rendered before the walk exactly as the wire sinks would have rendered it after, and one that refuses to render becomes an `[UNRENDERABLE: …]` marker costing that value alone. Each control character in a field's string values becomes its printable escape so a caller-supplied string cannot forge a line, a field separator or a colour on a terminal; the message and the `data` attribute keep theirs, being the runtime's own rendering, and `data` is the runtime's name alone, a call that passes a field of that name having it carried as `field_data` and escaped like any other. It is installed by `log.install_sink` in front of whatever the sink appended, so every sink is behind it, and it edits the record the root logger shares rather than a copy, so a handler ordered after the sink is behind it too, and a boot that dies before its sink arrives runs it over the records held since `log.configure` before they reach the stdlib's last resort on stderr; `extra_patterns` adds the secret shapes only a deployment knows, and a pattern the `re` module refuses is a configuration error named at load, as is one that matches the empty string, which would replace every line the process writes. The `otlp` sink emits `exception.type` and `exception.stacktrace` and no `exception.message`, so what it exports is the scrubbed text, an exception carrying no traceback rendering as its own last line. A sink processor that raises is now reported on stderr by its type and its name, never by its text, instead of raising out of the log call or going to the stdlib's `handleError`, which prints the exception and the record's raw arguments; a record whose arguments do not fit its format string keeps the scrubbed format string and an `[ARGUMENTS WITHHELD: …]` notice for the same reason, and the redaction processor fails closed, stripping the record to a `[REDACTION FAILED: …]` notice before it reports and dropping the record outright where even that stripping could not complete, as [Logging](tools/logging.md#redaction) describes.

### Changed

- **Rich is the `cli` extra (Breaking)**: `rich` is no longer a core dependency, so install `pipelex[cli]` for the `pipelex` and `pipelex-agent` commands, the `console` log sink and the `rich` pretty-print mode (`uv tool install "pipelex[cli]"`). The runtime imports Rich only inside the code that renders, so a process installed without it boots and runs methods with `sink = "json"` and `pretty_print_mode` set to `"poor"` or `"silent"`, while a boot selecting the `console` sink or the `rich` mode without Rich stops naming the extra, and a `get_console()`, `rendered_pretty()`, model-listing, pipe-listing or directory-diff call raises `MissingDependencyError` naming it too. What this delivers is that no code path the runtime takes on the way to running a method needs Rich — not an environment with no Rich in it, since `typer` and `instructor` are core dependencies that both require it, so a plain `pip install pipelex` still installs Rich and `import pipelex` still loads it. Two consequences worth knowing: a `poor`-mode panel title no longer substitutes emoji codes, the substitution table being Rich's own, and a cost report whose console table cannot render still writes its CSV. [Rich Imports](contribute/rich-imports.md) documents the convention and the `make check-rich-imports` guard, which also refuses a deferred Rich import that no `require_rich(...)` precedes.
- **Loggers are named by module (Breaking)**: a record from `pipelex/pipe_operators/pipe_llm.py` is emitted on the `pipelex.pipe_operators.pipe_llm` logger instead of `pipelex`, so a handler keyed on the bare package name must key on the prefix, a filter an integration attached to the `pipelex` logger must move to its handler since Python never consults an ancestor logger's filters for a record propagating up from a module logger, and a `package_log_levels` key now works at module depth too, `pipelex-pipe_operators-pipe_llm = "DEBUG"` for instance. The stack walk that named the logger on every line is gone, replaced by one frame lookup, and the caller-info templates that name the module now render instead of raising.
- **`include_exception` carries the traceback as the record's `exc_info`**: instead of splicing `traceback.format_exc()` into the message, so the console renders one Rich traceback, the `json` sink an `exception` field and the `otlp` sink the `exception.*` attributes, and a call made outside an `except` block carries nothing where a `NoneType: None` line used to be appended.
- **Teardown removes only the handlers Pipelex installed**: `log.reset()` no longer strips every handler off the root logger nor calls `logging.shutdown()`, so a host's or a test's handler survives a Pipelex teardown; the sink's handler is flushed and closed, which is where a batching sink ships what it still holds.
...
- **S3 links name the bucket's own host (Breaking)**: the S3 storage provider now signs links, and reads and writes objects, on the bucket's regional host `<bucket>.s3.<region>.amazonaws.com` instead of the region's shared endpoint `s3.<region>.amazonaws.com`, so a content security policy can allow one bucket's links without allowing every bucket in the region. A legacy bucket name that cannot be a hostname, one with uppercase letters or an underscore, keeps the path-style form on the regional endpoint, for signed and unsigned links alike, where an unsigned link used to name a virtual host that could not serve it; the configuration still refuses a bucket name containing a dot. A self-hosted deployment whose egress rules allow S3 by hostname must allow `*.s3.<region>.amazonaws.com`.
- **Image and document inputs carry a `public_url` whatever form their url takes**: an input given as a `pipelex-storage://` reference now gets a link signed through the storage provider, replacing any link it carried, which may have expired, and an input given as an `http(s)` URL gets that URL as its `public_url` unless it names another. This holds for an image or document passed alone, in a list or in a structured field, so a template writing `{{ image.public_url }}` no longer renders `None` for one uploaded to storage or passed by URL, and a reference the storage provider refuses as a key is reported as an input error before the run starts.
- **A NUL byte in an input's path or storage reference is an input error**: a local file path, or a `pipelex-storage://` reference on local storage, holding a NUL byte used to escape as an internal error; it is now reported as an input error before the run starts.
- **Unsigned storage links percent-encode their key**: the S3 and GCS providers' unsigned links now encode a key holding a space, `+`, `#` or `?` as a signed link does, instead of naming a path the URL cuts short.
- **A cancelled pipe run ends its span**: the pipe's span was closed only when the run raised an `Exception`, so a run cancelled by a timeout, a disconnected client or a cancelled orchestration task never ended its span and no exporter received it. It now ends with an error status like any other failure.
- **A log call before `log.configure` no longer raises**: it goes to the stdlib's default handling at the stdlib's default level, so a library that logs before Pipelex boots, or a boot that logs while it configures, cannot crash on the log line itself. Between `log.configure` and the sink's arrival at boot, every record is held and replayed through the selected sink in order, so a boot's own lines are rendered by the sink the configuration chose, and a boot that dies before the sink arrives sends all of them to stderr rather than only a warning or worse: each one passed the level the configuration set, and a failed boot is when that trail is worth most.
- **A log call never raises on a value the record or JSON refuses**: a field named like an attribute the `LogRecord` class owns (`__class__`, `__dict__`, `getMessage`) is prefixed like any other collision instead of raising `TypeError` or shadowing the method every formatter calls, and a content `json` refuses, a circular reference or a mapping with a non-string key, is rendered as its `repr` with no `data` instead of raising `ValueError` or `TypeError` out of `log.info`. A non-finite float nested inside a pydantic model is spelled as text like one at the top level, so a single `NaN` costs that one float its type and no other value on the line its structure. On the `console` sink the same promise needed the stdlib's own guard put back: Rich overrides `emit` without it, so a message carrying a tag-shaped span — `list[int]` in a type complaint, or a bracketed path, which Rich reads as a closing tag — raised `MarkupError` out of the log call and replaced the failure being reported with itself. Such a line now costs at most its own rendering, and an error assembled from an exception is rendered verbatim rather than read as markup, so the brackets and the type names in it reach the terminal as they were written.
...
- **A sink object installed again after a reset builds a fresh handler, or refuses to**: the teardown closed the handler and the sink then handed that same closed handler back, so a sink whose `close()` really releases its target was re-installed dead, accepting every record and exporting none, with nothing raised to say so; the forwarded-record filter stacked up on it besides, one more at every install. A sink whose target is opened in `make_handler` now works across a configure/reset cycle, which is what the tests and an embedding host do. The `otlp` sink cannot, since it is constructed around its processor and its close shuts that processor down with the logger provider, so installing the same `OtlpLogSink` object a second time now raises instead of exporting nothing; the registered factory builds a new one at every boot, so a boot through the registry never meets it.
- **`pipelex doctor` no longer dies on a log sink that failed after it was installed**: the fallback to the console sink ran on any exception out of `log.install_sink`, on the assumption that a failed install leaves nothing installed. That holds for a failed handler build only — the sink is recorded before the records held since `log.configure` are replayed through it, so a handler that cannot render one of them and cannot report it either, which is what a closed stderr produces, leaves the sink installed and made the fallback raise "A log sink is already installed" out of the command. The fallback now runs only when nothing is installed; otherwise the row reports the failure and the installed sink is kept.
...
- **The README leads with the hosted route**: its quick start is the onboarding region included from `Pipelex/.github` — sign up at app.pipelex.com, then the Pipelex plugin in Claude Code or Codex or the Pipelex MCP in ChatGPT or Claude — and the runtime's own install, configuration and first run follow under their own heading, **Run it yourself**. The CV batch screening walkthrough moved from the README to the cookbook in the documentation. Its hero is the website's line, "Turn your expertise into an AI-powered App, MCP or API", with a paragraph on building a method with your coding agent and where it runs, and **What a method looks like** and **Why methods?** follow the install instead of preceding it. **Configure AI Access** no longer offers the Pipelex Gateway: it names your own provider keys or a local model.
- **The documentation site leads with the hosted route**: its getting-started opens on a new **Quick Start** page — sign up at app.pipelex.com, the Pipelex plugin in Claude Code or Codex, the Pipelex MCP in ChatGPT or Claude, then a webapp from the method-app template or the hosted API over HTTP, TypeScript or Python — and the runtime's own install moves to **Run It Yourself**. **Build with Claude Code** and its `npm install -g mthds` route are retired, and **Claude Code Skills Plugin** becomes **Pipelex Plugin**, which documents the Pipelex plugin instead of the MTHDS plugin; the old addresses redirect. The front page's buttons and the site's description lead with what Pipelex lets you do.
...
- **The previous multi-provider deck is parked in the package as a deck variant**: `pipelex/kit/deck_variants/multi_provider/` holds the four numbered deck files as they shipped before the default moved to Azure, so a project that wants the Anthropic, Google and OpenAI ladder back copies them over the kit's own deck directory instead of reconstructing it, and the directory's README says how. Nothing loads a variant — `init`, `update` and the deck manifest ignore it, and it sits outside `configs/` so no installer can pick it up — and a parity test holds every variant to the shipped deck's alias, preset and waterfall vocabulary and checks that each handle only the variant names is still declared by a kit backend file, so a parked deck goes red instead of rotting quietly.

### Changed

- **The shipped model deck resolves only to models the Pipelex Gateway serves from Azure (Breaking)**: every default alias and preset keeps its name, but the whole language ladder is now the GPT-5.6 range — the premium tier and `best-gpt` resolve to `gpt-5.6-sol`, the general and large-context tiers to `gpt-5.6-terra`, the small tiers to `gpt-5.6-luna` — and image generation's general and premium tiers to `gpt-image-2`; `$engineering-codebase-analysis` reaches its model through `@default-large-context-code`, the alias that names its job, instead of through the premium tier. Document extraction was already `azure-document-intelligence` and web search stays on Linkup, which Azure has no equivalent for. `pipelex update` refreshes the numbered deck files and backs up any you edited; a project that wants the previous models keeps them by naming them in `x_custom_llm_deck.toml`, which pipelex never touches.
- **The shipped deck declares `temperature = 1` throughout (Breaking)**: every model in the GPT-5.6 range fixes its temperature at 1, so every preset states that value instead of one the worker would override while warning on every call, and `[llm.choice_defaults].default_temperature` is 1 for the same reason — it is the temperature paired with an alias, a waterfall or a bare handle named outside a preset, the `for_text` and `for_object` choices every pipe without an explicit model takes among them. Presets are therefore distinguished by their model tier and their reasoning effort alone: `$writing-factual` and `$writing-creative` issue the same call, and `$engineering-structured` runs at temperature 1. A method that needs a temperature of its own writes an inline LLM settings table — `model = { model = "gpt-4o", temperature = 0.8 }` — since a bare handle is paired with the deck-wide default rather than with a value of the author's.
- **The shipped Gateway models reference lists the roster the Gateway actually serves**: `backends/pipelex_gateway_models.md` and its plain twin are generated from the served remote config, and they had not been regenerated since July — so they lacked the GPT-5.6 range this deck now resolves every tier to, while still advertising the GPT-4.1, o-series and GPT-5.1 to 5.2 generations that v0.61.0 retired. Both files are refreshed, in the kit and in `.pipelex/`, so the reference a user opens to check what their default deck runs on agrees with the deck.
- **CLI help and documentation examples name an alias the deck still defines**: `check-model`'s argument help, the model-reference and model-suggestion docstrings and the doc examples all used `@best-claude`, which this release removes, and now use `@best-gpt`. The "Configure AI Providers" guide also gains a section stating exactly what a fresh install resolves to — the GPT-5.6 ladder and GPT Image 2 for language and image generation, Azure Document Intelligence for extraction, and Linkup for search and for web-page extraction, the two places the deck leaves Azure.
- **The Elastic License 2.0 boundary is named by version rather than by date**: `docs/license.md` said the license applies to every version released after 2026-09-14, which left a reader holding a version to work out which side of that date it fell on. It now names the first Elastic version of each artifact — `pipelex` from v0.58.0, `pipelex-api` from v0.24.0, `@pipelex/mcp` from v0.15.0 — and spells out the last MIT version of each.
...
- **A failed publish can be recovered without damaging what already shipped**: `publish-pypi.yml` scopes the signature upload to `dist/*.sigstore.json` so it no longer clobbers the wheel and sdist attached just before it, turns `skip-existing` on for a re-run or a manual dispatch, and refuses to touch a release whose tag names a commit other than the one being published. A retry no longer rebuilds the dists either: the build job asks PyPI what this version published and recovers those bytes from the first place that holds them — the build an earlier attempt of this run stored, an artifact an earlier run stored for the same commit, or the files PyPI itself serves — building fresh only when PyPI answers that nothing was published under this version. Whatever it holds is then checked to be one wheel and one source tarball of the version being released and, when PyPI serves that version, byte-for-byte what PyPI publishes, so the release assets are the bytes PyPI serves rather than a rebuild that probably matches them. The release is now tagged at the commit it ships, named explicitly with `--target`, rather than at the default branch's head.
...
- **A generated image fetched from a remote URL is stored under the media type that URL served**: `GeneratedImageRawDetails` admits a bare URL with no mime type and no image format on the promise that the type is settled when the bytes are downloaded, and nothing kept that promise — the fetch discarded the response's `Content-Type` and the resolution fell through to the requested image format or to a hardcoded `image/jpeg`. The new `fetch_file_and_content_type_from_url_httpx` reports the declared type alongside the bytes, normalized and with its parameters stripped, and the factory stores under any `image/<subtype>` it names — not merely the formats `ImageFormat` can itself produce, since what this runtime generates is a different question from what a remote URL may serve, and a GIF or an AVIF stored as `image/jpeg` is the very error being removed. The storage key's extension follows that subtype so it keeps agreeing with the stored type. A type the caller declared still wins, and a served type that is not an image at all — the `text/html` of an error page dressed as a 200 — is dropped.
- **A remote image that fails to download degrades to its URL instead of failing the pipe**: `make_image_content` guarded its fetch with `except (httpx.HTTPStatusError, httpx.RequestError)`, but `fetch_file_from_url_httpx` converts every httpx failure into `RemoteFileFetchError`, so that clause caught nothing and a 404 or a timeout on a remote image propagated out and failed the whole run. The guard now names the exception the fetch actually raises, and the fallback it was written for — log the warning, keep the remote URL — runs.
- **A remote fetch honours a timeout again**: `fetch_file_from_url_httpx` passed its `request_timeout` straight to httpx, and httpx reads an explicit `timeout=None` — the parameter's own default — as *no timeout at all* rather than as its client default. Every caller that did not name one therefore had no timeout, so a hanging server blocked the caller indefinitely and the `TimeoutException` branch that raises a caller-facing `RemoteFileFetchError` was unreachable. The helper now passes httpx's `USE_CLIENT_DEFAULT` sentinel where no timeout was asked for.
