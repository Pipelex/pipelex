...
- **A failed publish can be recovered without damaging what already shipped**: `publish-pypi.yml` scopes the signature upload to `dist/*.sigstore.json` so it no longer clobbers the wheel and sdist attached just before it, turns `skip-existing` on for a re-run or a manual dispatch, and refuses to touch a release whose tag names a commit other than the one being published. A retry no longer rebuilds the dists either: the build job asks PyPI once what this version published, recovers those bytes from the first place that holds them — the build an earlier attempt of this run stored, then an artifact an earlier run stored for the same commit, then the files PyPI itself serves — and builds fresh only when PyPI answers that nothing was published under this version. Whatever it ends up holding is then checked, on every one of those paths, to be one wheel and one source tarball of the version being released and, when PyPI serves that version, to be byte-for-byte the files PyPI publishes; a divergence fails the job instead of reaching the release, so a build stored before another run published different bytes can no longer be attached and attested over them. That makes the release assets the bytes PyPI serves rather than a rebuild that probably matches them, which an unpinned build backend and an unpinned interpreter cannot promise, and it turns an expired artifact from a dead end into one more layer to fall through. The release is now tagged at the commit it ships, named explicitly with `--target`, rather than at the default branch's head.
