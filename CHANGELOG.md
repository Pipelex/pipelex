...
- **Secrets are scrubbed and field values made injection-safe before any sink sees a record**: one processor, configured under `[runtime.log.redaction]` and on by default, replaces a bearer token, an api-key or signature header value, an OAuth `code` parameter, a cookie header's whole value, a serialised secret entry and an `sk_` / `plx_sk_` / `pk_` / `bl_` key with `[REDACTED]`, in the message, in the exception's rendered text and in every string a field carries. A field of the record, or a mapping entry at any depth, whose own name is a secret's (`authorization`, `password`, `api_key`, `access_token` and the like, read in any case and with a dash as an underscore) loses its value whatever it holds, and a structured content is redacted by name before it is rendered, so the message says exactly what `data` holds even where the secret sat in a nested object or in a number; a pydantic model or any other object is rendered before the walk exactly as the wire sinks would have rendered it after, and one that refuses to render becomes an `[UNRENDERABLE: …]` marker costing that value alone. Each control character in a field's string values becomes its printable escape so a caller-supplied string cannot forge a line, a field separator or a colour on a terminal; the message and the `data` attribute keep theirs, being the runtime's own rendering. It is installed by `log.install_sink` in front of whatever the sink appended, so every sink is behind it, and it edits the record the root logger shares rather than a copy, so a handler ordered after the sink is behind it too; `extra_patterns` adds the secret shapes only a deployment knows, and a pattern the `re` module refuses is a configuration error named at load. The `otlp` sink emits `exception.type` and `exception.stacktrace` and no `exception.message`, so what it exports is the scrubbed text, an exception carrying no traceback rendering as its own last line. A sink processor that raises is now reported on stderr by its type and its name, never by its text, instead of raising out of the log call or going to the stdlib's `handleError`, which prints the exception and the record's raw arguments; a record whose arguments do not fit its format string keeps the scrubbed format string and an `[ARGUMENTS WITHHELD: …]` notice for the same reason, and the redaction processor fails closed, stripping the record to a `[REDACTION FAILED: …]` notice before it reports, as [Logging](tools/logging.md#redaction) describes.
