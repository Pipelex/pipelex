...
- **Native `Date` concept (calendar dates, as precise as the source):** A new built-in native concept `Date` (content class `DateContent`: a required `datetime.date` plus an optional `datetime.time`) for the dates that fill the documents Pipelex reads — issue dates, due dates, departures, dates of birth. The time is present only when the source states one (with its UTC offset kept verbatim), so `output = "Date"` never forces the model to fabricate a time: "delivery by March 15" yields a date with no time, a ticket's "7 Jul 2026, 15:40" keeps the time. Read it from a Python caller via `pipe_output.main_stuff_as_date`; it renders as ISO 8601 truncated to the stated precision. As a pipeline input, top-level TOML date/datetime literals map to `Date` directly (`departure = 2026-07-07T15:40:00+02:00`), the envelope forms `{"concept": "Date", "content": "2026-07-07"}` / a bare date object work, and a concept that `refines = "Date"` keeps its own generated subclass. Date string inputs are strict *extended* ISO 8601 — compact/basic (`20260707`), week-date, and numeric/epoch forms are rejected, and a string carrying a time is refused on the date-only field (put the time in `time`) so no stated time is ever silently dropped. Breaking: `Date` is now a reserved native concept code — a bundle declaring `[concept.Date]` errors.
- **Smart Inputs — signature-driven input shaping:** The inputs a caller provides are now interpreted **top-down against the pipe's declared input signature** instead of bottom-up from their shape alone. A bare string becomes the *declared* concept (a `legal.Question`, not a generic `native.Text`); a bare number satisfies a `Number`-refining input (a boolean never does — the `bool ⊑ int` trap is closed); a bare boolean a `YesNo`-refining one; an ISO date string a `Date`-refining one; a bare URL/path a declared `Image`/`Document` (a relative path resolves against the inputs file's directory, a leading `~` expands to home); a plain object validates against a structured concept; and a JSON/TOML list shapes element-wise into the declared item concept (a single value auto-wraps, an empty list is legal, a fixed-count `X[N]` is count-checked). A declared structured list additionally accepts a `.csv`/tabular path *by signature* — no envelope. The `{"concept", "content"}` envelope stays as an escape hatch and is now **compatibility-checked** against the declaration (an incompatible concept is a typed error; a compatible more-specific concept wins). Shaping runs on every surface — CLI file/inline inputs, the Python API, and the hosted runner — through one seam. Every shaping failure raises a typed error naming the input, the declared concept, what was provided, and the expected shape. Breaking: an input name the pipe does not declare is now a hard error listing the declared names (it was previously ignored silently), and a top-level `null` value is rejected — absence is expressed by omitting the key, not by `null`.

### Changed

- **TOML inputs rejection narrowed to bare time-of-day (Breaking):** Since TOML inputs shipped, every TOML date/datetime/time literal was rejected wholesale (`InputsDatetimeNotSupportedError`, "quote it as a string"). Now date and datetime literals map to the native `Date` concept and only a bare *time of day* (`opening = 09:00:00`) is rejected — a time alone has no date to attach to. The error class is renamed `InputsTimeOnlyNotSupportedError`.
...
- **`build inputs` / `pipelex-agent inputs` default to the light template (Breaking):** The generated inputs template is now the **light**, signature-driven shape (the values themselves, matching each input's declared concept) instead of the `{concept, content}` envelope — it is exactly what `pipelex run` accepts, so a generated-and-filled template runs as-is. The TOML template carries the declared concept for each key as a `# concept: ...` comment; pass the new `--explicit` flag (on both CLIs, composing with `--format json|toml`) to restore the ceremonial envelope form. The agent-CLI JSON success envelope (`success` / `pipe_code` / `inputs`) is unchanged — only the `inputs` payload flips to the light shape.
