...
- **The composition root now has the same layer seam as every other package: `pipelex/runtime_boot.py`.** `RuntimeBoot` stands up config, logging, secrets, telemetry, the class and func registries, the template sets, the model deck, storage, the content generator, the inference manager, the reporting delegate and the observers — and loads **zero interpreter modules** doing it (619 → 356 modules, 172 → **0** interpreter, measured). `pipelex.py` was the one module the layering work never reached: interpreter-layer by construction, therefore outside the declaration entirely, and an undeclared module is unpoliced rather than neutral — so nothing enforced which half of its boot sequence belonged to which layer, and there was no way to boot the runtime layer at all. This is also the first caller `RUNTIME_BUILTIN_PLUGINS` has ever had: a runtime-only boot discovers exactly the runtime half, leaving the two interpreter-touching built-ins absent. Declared in the hub-layering guard's `RUNTIME_LAYER_PACKAGES` as a module entry beside `pipelex.runtime_hub`, pinned by the import-closure test's `RUNTIME_LAYER_ENTRY_POINTS`, and pinned again by `tests/unit/pipelex/test_runtime_boot_closure.py`, which *boots* it in a subprocess rather than importing it — an import-time check cannot notice a boot step resolving out of an interpreter-contributed registry that is empty on that boot, and an in-process check would pass vacuously because both hubs are sticky singletons teardown never clears. Documented in [Hub layering convention](contribute/hub-layering.md#where-the-boot-splits).

- **`Concept` is pure data again, and the property is pinned.** `pipelex/core/concepts/concept.py` — a subclass of the MTHDS-protocol wire model `ConceptAbstract` — no longer reads the process-global class registry. `tests/unit/pipelex/core/concepts/test_concept_registry_boundary.py` asserts a golden set over an AST walk of `pipelex/core/concepts/**`: exactly two modules may touch the registry, and both are the **materialization write side** (`concept_factory` registering generated structure classes, `structure_generation/generator.py` looking up base classes to generate against). A golden set rather than a rule, for the reason the `cogt` boundary test gives — there is no predicate separating a sanctioned write-side read from a new read-side one, and no other gate can see the edge at all, since both the accessor and its users are runtime-layer. Documented in [Hub layering convention](contribute/hub-layering.md).

- **Renaming an error class is now a reviewable diff instead of a silent wire break.** `ErrorReport.error_type` is `type(self).__name__` with no indirection, and consumers outside this repo `switch` on that string — so a rename leaves their build green and their branch silently unmatched. Nothing in the repo flagged that, which is how `PipelexInterpreterError` → `MthdsParserError` reached four consumer sites unannounced. `tests/data/errors/error_identity.txt` now pins the `(error_type, title, type_uri)` triple of every discoverable `PipelexError` subclass, regenerated with `make generate-error-identity` (alias `make gei`, or `pipelex-dev generate-error-identity`) and gated by `tests/unit/pipelex/errors/test_error_identity_snapshot.py`. The rendering is pure — no Pipelex bootstrap — and the population comes from the same `iter_pipelex_error_subclasses()` helper the docs generator uses, so it cannot silently miss a deferred-import error module. Documented in [Error Model](under-the-hood/error-model.md).

- **The vendor-neutrality of `pipelex.cogt` is now pinned, not just documented.** `tests/unit/pipelex/cogt/test_cogt_dependency_boundaries.py` asserts two golden sets over an AST walk of `pipelex/cogt/**`: its third-party import roots (framework and infrastructure only — no inference-provider SDK), and the exact `cogt → pipelex.providers` statements, each recorded with its import form so promoting a deferred import to module level shows up too. These are the edges [Known inversions](contribute/hub-layering.md#known-inversions) documents in prose, and no other gate can see them — `pipelex.cogt` and `pipelex.providers` are both runtime-layer, so the hub-layering guard and the import-closure test are blind to an edge between them by construction. Golden sets rather than a rule, because these are accepted inversions: the point is to turn a sixth edge into a diff a reviewer reads. The img-gen neutrality test's coverage comment is corrected in the same change to claim only what its glob proves.

- **The subject-grants registry's sort order is now enforced, not just claimed.** `subject_grants.toml` is machine-written and rewritten sorted by key on every `make subject-grant`, and the convention doc has always said so — but nothing checked it, and an unsorted registry parses fine and grants exactly the same subjects, so a bulk path rewrite scrambled it silently and stayed scrambled across three commits. The keyword-only guard gains an `unsorted-grant` violation kind, riding the existing `make check-keyword-only` → `make agent-check` → CI gates with no new target. Order is read from the raw file text — TOML declares table order insignificant, so the parsed `dict` is no evidence of it — and the text scan is cross-checked against the parsed keys, with a disagreement raised as an explicit check error rather than a quietly narrowed scan. Registry order is a whole-file property, so it is a full-scan rule: the per-file `PostToolUse` hook cannot see it, exactly as with `dead-grant`. See [Keyword-only arguments](contribute/keyword-only-arguments.md).

- **`.test_durations` is gated against a bulk path rewrite.** The per-test timing map pytest-split uses to balance the CI shards is generated, committed, and touched by nobody, so a rewrite of test paths walks straight past it: the file still parses, the shards still run, and they silently unbalance against tests that can never match again. `tests/unit/repo/test_test_durations_paths.py` asserts every recorded test *file path* still exists on disk. The gate is deliberately on the file path, not the node id — a path rewrite breaks paths, while parametrization churn breaks node ids and is benign (pytest-split treats an unknown id as average duration), so pinning ids would need a full unfiltered collection plus a tolerance policy for legitimate drift.

### Changed

- **Breaking: `LLMPromptBlueprint` no longer has a `templating_style` field — the style is a parameter of `make_llm_prompt`.** The field *was* the run-time cache the determinism fix below removes, and it had exactly one writer in the tree (the `PipeLLM` operator) and no reader outside the two render sites; the factory never set it, and no test, doc, or schema referenced it, so it goes rather than lingering as a vestigial slot. `make_llm_prompt` and the private `_unravel_text` take a keyword-only `templating_style: TemplatingStyle | None = None`, so the existing call sites that don't style their prompts are unaffected. `TemplateBlueprint.templating_style` stays — it is a declared MTHDS field the compose and img-gen paths read — and it now takes precedence over the run-derived style, which is what the old write-back's guard already intended but the render call then ignored, passing the spec-level style regardless. Behavior on the LLM path is unchanged today, since `PipeLLMBlueprint.prompt` / `system_prompt` are plain strings with no nested template section and the factory therefore never sets a blueprint-level style; the ordering is now coherent rather than contradictory.

- **Breaking: `Pipelex` is now the interpreter-layer boot composed on top of `RuntimeBoot`.** Same address, same public class name, and a `make()` that keeps every existing parameter and its docstring (it gains `config_dir`, and `pipeline_manager` widens to the abstract type — both non-breaking) — it subclasses the runtime boot, imports it downward, and appends the `InterpreterHub`, the composed plugin manifests, the `PipeFuncExecutorRegistry` and its executor, the `LibraryManager`, the `PipelineManager`, the `PipeRegistryModels` registration, the `PipeRouter` and the `PipeRun`. Inheritance preserves every attribute address, so no consumer changes. Two breaking details: `Pipelex.__init__` takes keyword-only `config_dir` in place of the inert positional `config_dir_path` (nothing in the workspace constructed `Pipelex` positionally), and the dead `is_pipelex_service_enabled` attribute is gone — it was assigned `False` and never updated, because `setup()` only ever wrote a local of the same name, so it always answered `False` and nothing read it. The class-level accessors (`get_optional_instance`, `is_fully_booted`, `get_instance`, `teardown_if_needed`) now resolve **by subclass**, so `RuntimeBoot.is_fully_booted()` answers for a `Pipelex` too.

- **Breaking: class resolution left the `Concept` wire model for the concept provider, fixing two live bugs on the way.** `ConceptProviderAbstract` gains `get_structure_class(concept)`, implemented by `ConceptLibrary` against the active registry with a `StuffContent` subclass bound. Four members of `Concept` change or go:

    | was | is now |
    | --- | --- |
    | `Concept.are_concept_compatible(..., strict=)` | `Concept.are_compatible_by_declaration(...)` — the pure declaration tier — composed with the new pure `are_structure_classes_compatible` (`tools/typing/class_utils.py`) by `ConceptLibrary.is_compatible` |
    | `Concept.get_structure_class()` | `ConceptProviderAbstract.get_structure_class(concept=…)` |
    | `Concept.is_valid_structure_class(name)` | a private module-level helper in `concept_factory.py`, its only caller |
    | `Concept.search_for_nested_image_fields_in_structure_class()` | compose `get_structure_class` with the existing free `search_for_nested_image_fields(content_class=…)` |

    `Concept.render_concept_representation` now takes the already-resolved `structure_class`, and `StuffSpec.render_stuff_spec` / `InputStuffSpecs.build_inputs_template` / `render_inputs` take a `concept_provider` and resolve once. Three defects close with it. **Cross-package `refines` aliases now resolve at five call sites** that compared two `Concept` values directly (`pipe_llm` ×2, `template_image_analyzer` ×2, `template_document_analyzer`) and therefore had no `concept_resolver` — wrong compatibility verdicts at authoring time and on the PipeLLM text-vs-object run branch. **An unresolvable structure class raises `ConceptStructureClassNotFoundError`** (a `ConceptValueError`) instead of silently answering `False`: "unknown" and "incompatible" are different answers, and conflating them was a latent wrong-verdict bug. **`native.Anything` is answered rather than raised** — it deliberately has no content class, so `NativeConceptCode.is_structureless_concept` / `Concept.declares_a_structure_class` make that a declared property instead of a missing registration; verdicts are unchanged, the reasoning is now correct.

    One further tightening rides along, in the class tier rather than in `get_structure_class`: resolution now applies a `StuffContent` subclass bound (`get_required_subclass`) where the old path used an unbounded `get_class`. A name registered as something other than a `StuffContent` subclass used to reach `are_classes_equivalent` / `issubclass` and could answer `True`; it now raises `ConceptStructureClassNotFoundError`. Deliberate — a class that cannot hold stuff content is as unusable as a missing one — and unreachable through the loader, which refuses to build such a concept in the first place.

- **Fixed: five built-in vendor adapters were pulling the whole interpreter into the runtime layer, through the public exceptions aggregate.** `providers/{anthropic,bedrock,google,mistral}/*_list.py` imported `MissingDependencyError` from `pipelex.exceptions` — the all-errors aggregate, which re-exports every interpreter package's errors — instead of from its definition site, `pipelex.system.exceptions`. Each one therefore loaded 15 interpreter modules, inside a package (`pipelex.providers`) the layer declaration already claimed was clean. Both gates were blind to it: the hub-layering guard only tracks `interpreter_hub` reachability, and the aggregate never reaches it; the closure test's `providers.builtins` entry point does not cover these modules because the vendor plugins import them *inside a function*, hiding them from the static graph and from the aggregator's own closure alike. The imports now point at the definition site, per this repo's "direct full-path imports everywhere" rule, and `providers.anthropic.anthropic_list` joins `RUNTIME_LAYER_ENTRY_POINTS` so the fix cannot silently regress. `pipelex.test_extras` — measured clean, shipped, and imported at boot — is declared runtime-layer in the same change, leaving every top-level package accounted for.
- **Breaking: the four leaf models that kept `pipeline` and `pipe_run` out of the layer predicate moved to runtime-layer homes.** With them gone, the import-closure test names **every** interpreter package, and the property it pins loses its last qualification: *importing the Pipelex runtime loads zero interpreter modules* — previously "zero modules from the packages the predicate could name", because two documented interpreter homes had to be left out. Naming them was never blocked by a broken hub arrow; it was blocked by an address, and the remedy is the one `mthds_parsing` used — move the leaf, then widen the predicate. Excluding it would only have recorded the problem.

    | symbol | was | is now |
    | --- | --- | --- |
    | `SpecialPipelineId` | `pipelex.pipeline.pipeline_models` (module deleted) | `pipelex.system.job_metadata` |
    | `PipeRunMode` | `pipelex.pipe_run.pipe_run_mode` | `pipelex.system.pipe_run_mode` |
    | `PipeRunParamKey` | `pipelex.pipe_run.pipe_run_params` | `pipelex.system.pipe_run_param_key` |
    | `PipeRunError` | `pipelex.pipe_run.exceptions` | `pipelex.core.pipes.exceptions` |

    Each destination is decided by what the symbol *is*, not by a wish for one tidy address. `SpecialPipelineId` is the vocabulary of `pipeline_run_id`, which is `JobMetadata`'s field, so it lands beside it. `PipeRunMode` and `PipeRunParamKey` are run-scope directives rather than one of a method's values, which is what puts them in `system/` and not `core/`. `PipeRunError` deliberately breaks that pattern: the runtime layer *subclasses* it — `PipeRunInputsError` and `OptionalValueAbsentError` derive from it in `core.pipes.inputs` — so it joins the pipe-error family whose base it is, and lands in the error reference beside those two subclasses rather than one section away from them. `AsyncExecutionNotEnabledError`'s docstring, which claimed the module "lives in core" when it never did, is corrected in passing.

- **Breaking: the bundle-driven half of `graph_rendering` moved to `pipelex.pipeline`, and four measured-clean packages joined the declared runtime layer.** `pipelex.graph.graph_rendering.generate_graph_for_bundle` / `generate_view_for_bundle` are now `pipelex.pipeline.bundle_graph_rendering.*`; `GraphFormat` and `render_graph_from_spec` stay put. The split is along "do I need a loaded method?", which is the hub boundary's own question — producing a graph *spec* from a bundle requires dry-running it, rendering an existing spec does not. It matters because the old module reached `interpreter_hub` (and ~76 interpreter modules) through `pipeline.dry_run_pipeline` while every gate stayed green: the layer rule and the transitive rule both filter their candidates through `is_runtime_layer`, and `pipelex.graph` was undeclared, so the module sat outside the rules' domain entirely. `pipelex.graph`, `pipelex.tracing`, `pipelex.observer` and `pipelex.errors` are now in `RUNTIME_LAYER_PACKAGES` — each measured to load zero interpreter modules — which is what puts the evicted module's successor in the guard's domain. The generalized lesson, now recorded in [Known inversions](contribute/hub-layering.md#known-inversions): **an undeclared package is not neutral, it is unpoliced.** `pipelex.kit` (data files) and `pipelex.language` (has interpreter edges) stay out.
- **Breaking: `pipelex.hub` is gone, split into `pipelex.runtime_hub` and `pipelex.interpreter_hub`.** The split names the two layers it separates, on the textbook language-implementation division: the **runtime** is the machinery present at execution time whatever is loaded, the **interpreter** reads a method and executes it. There is no alias shim — a stale import fails loudly at import time rather than silently resolving to the wrong layer. `runtime_hub` holds process-scoped infrastructure (config, console, secrets, storage, telemetry, models, inference, content generation, reporting, run mode, tracing, the plugin registries, `get_class_registry`); `interpreter_hub` holds the loaded method's machinery (library manager and lookups, the current-library contextvar, library dirs, pipe router, pipe run and pipeline manager, the PipeFunc executor). `get_pipelex_hub` splits into `get_runtime_hub` / `get_interpreter_hub`, so every external call site must pick the container it actually meant. The rule is one-way and mechanically enforced (`make check-hub-layering`, alias `chl`, in `make agent-check` and CI): `interpreter_hub` may import `runtime_hub`, never the reverse, and no module anywhere may reference `pipelex.hub` — in an import *or* a string literal (dynamic import, mock patch target, config path). The payoff is measurable and pinned by a subprocess import-closure test: importing the Pipelex runtime now loads **zero** interpreter modules, down from 50 — you can embed the inference engine without loading a line of the MTHDS interpreter. See [Hub layering convention](contribute/hub-layering.md).
- **Breaking: misplaced types moved to the package that actually owns them.** `JobMetadata` / `JobCategory` / `UnitJobId` → `pipelex.system.job_metadata`, `JobMetadataError` → `pipelex.system.exceptions`, `TraceContext` → `pipelex.system.trace_context`, `DataInclusionConfig` → `pipelex.system.data_inclusion_config`, `TextFormat` → `pipelex.tools.templating.text_format`, `TemplatingStyle` / `TagStyle` → `pipelex.tools.templating.templating_style`, `TemplateCategory` → `pipelex.tools.jinja2.template_category`, and the neutral resolved-field layer → `pipelex.core.concepts.resolved_fields`. The TOML config shape is unchanged throughout (`[...graph_config.data_inclusion]` still parses); only the Python homes moved. The two pipe renderers regrouped into one `rendering/` package (`input_renderer` from `core.pipes.inputs`, `output_renderer` from `core.pipes.output`); they ship at `pipelex.pipe_machinery.rendering`, having left `core/` with the rest of the Pipe machinery — see the entry below.
- **Breaking: `core/`'s data model resolves concepts and pipes through injected providers instead of reaching into the hub.** `StuffFactory.make_stuff_from_stuff_content_or_data` / `make_from_blueprint` / `make_from_concept_ref`, `InputShaper.shape` / `resolve_input_kind`, `WorkingMemoryFactory.make_from_pipeline_inputs`, `InputStuffSpecsFactory.make_from_blueprint` / `make_from_string`, and `StuffSpecFactory.make_from_blueprint` all take a required `concept_provider`. A new read-side contract owns that surface — `ConceptProviderAbstract` (`pipelex.core.concepts.concept_provider_abstract`), which `ConceptLibraryAbstract` now extends, keeping library management in the interpreter layer. There is no pipe counterpart: nothing in core's runtime half resolves a pipe, so `get_required_pipe` stays on `PipeLibraryAbstract`. `pipelex.core`'s data-model packages join the enforced runtime layer as a result; the half that names a `Pipe` stays in the interpreter layer by construction.
- **Breaking: the pipe-kind registration manifest left `core/` — `CoreRegistryModels` no longer carries the pipe lists.** `PIPE_OPERATORS`, `PIPE_OPERATORS_FACTORY`, `PIPE_CONTROLLERS`, `PIPE_CONTROLLERS_FACTORY`, `PIPE_SIGNATURES` and `PIPE_SIGNATURES_FACTORY` now live on `PipeRegistryModels` in the new `pipelex.pipe_machinery.registry_models`; `pipelex.core.registry_models.CoreRegistryModels` keeps the value model (`STUFF`, `EXPERIMENTAL`, `FIELD_EXTRACTION`) only. Both manifests are registered side by side at boot, so no registered class changes and serialized payloads are untouched — only the import path of the pipe half moved. The point is the directory: a manifest listing every pipe class and every pipe factory made `core` read as a dependant of the pipe packages it sits below, and importing it pulled the whole interpreter in. `core.registry_models` now imports nothing from `pipe_operators` / `pipe_controllers` / `pipe_signature`, which drops core's inverted `core → pipe_*` import statements from 43 to 19. The new [Registration surface](contribute/registration-surface.md) page documents every place a new pipe kind has to be registered.
- **Breaking: the MTHDS parser left `core/` — `pipelex.core.interpreter` and `pipelex.core.bundles` are now `pipelex.mthds_parsing`, and `PipelexInterpreter` is `MthdsParser`.** `core/interpreter/interpreter.py` becomes `mthds_parsing/parser.py`; `bundle_elaborator`, `helpers`, `validation_error_categorizer`, `pipe_sorter` and `pipelex_bundle_blueprint` move across unchanged; the two `exceptions.py` modules merge into one. Parsing the MTHDS language is an MTHDS-owned concept, and `core/interpreter/` collided with the *interpreter layer* while meaning something much narrower. **`PipelexInterpreterError` is renamed `MthdsParserError`** — this is wire-visible, not merely an import break: the class name is the `error_type` value clients branch on, and its documentation `type_uri` moves from `errors/pipelex-interpreter-error/` to `errors/mthds-parser-error/` (a docs redirect keeps the retired URI resolving; a consumer switching on the string does **not** get one). Its RFC 7807 `title` moves with it, from `"Pipelex interpreter"` to `"MTHDS parser"` — declared rather than auto-derived, since deriving from the class name would render the standard's brand as "Mthds parser". Its error-reference page also moves macro-section, from Platform & tooling back to **Authoring & language**, where a `.mthds` that will not parse belongs. `BundleElaboratorError` keeps its name. `PipelexBundleBlueprintValidationErrorData` did **not** move with the parser: `pipeline/` and `libraries/` carry it into every runtime-layer import closure, so it now lives in `pipelex.core.exceptions` beside the two sibling structured error-data models — which is what lets `mthds_parsing` be named in the closure test's interpreter predicate with no exclusion.
- **Breaking: core's Pipe machinery moved to `pipelex.pipe_machinery`, and `pipelex.core` is now a single declared runtime-layer package.** `pipe_abstract`, `pipe_factory`, `pipe_blueprint`, `validation`, `template_guard_lint` and `rendering/` leave `pipelex.core.pipes` for `pipelex.pipe_machinery` (joining `registry_models`, which arrived there earlier); `handle_pipe_errors` follows its importers into `pipelex.mthds_parsing`. `PipeType` and `PipeCategory` travel with `pipe_blueprint`. What is left in `pipelex.core.pipes` is exactly its runtime half — `inputs/`, `stuff_spec/`, `pipe_output`, `variable_multiplicity`, `exceptions`. The layer question is **inbound, not outbound**: every module that moved has zero declared runtime-layer importers, while every module that stayed has at least one. The payoff is the declaration itself — the hub-layering guard's `RUNTIME_LAYER_PACKAGES` drops six `pipelex.core.*` entries for the single `pipelex.core`, and the import-closure test's interpreter predicate drops its per-module list *and* its one exclusion to become a plain top-level-package membership test. Both were enumerations working around modules filed in the wrong package. Measured: `pipelex.core` modules loading a non-zero number of interpreter modules 9 → **0**, and inverted `core → pipe_*` import statements 43 → **0**.
- **Breaking: the interpreter-touching built-in plugins moved to `pipelex.interpreter_plugins`, and `build_registrar` takes the plugin list as a parameter.** `DirectOrchestratorPlugin` and `PipeFuncPlugin` construct interpreter-layer objects (a `DirectOrchestrator`, a `DirectBundleValidator`, a `DirectPipeFuncExecutor`), so they no longer live under the runtime-layer `pipelex.plugins` package: they are now `pipelex.interpreter_plugins.direct.direct_plugin` and `pipelex.interpreter_plugins.pipe_func.pipe_func_plugin`. The runtime adapters' own manifest keeps them as `RUNTIME_BUILTIN_PLUGINS` / `RUNTIME_CORE_UNCONDITIONAL_PLUGIN_NAMES` (it ships at `pipelex.providers.builtins`, having moved with the adapters later in this same cycle — see the entry below); `pipelex.interpreter_plugins.builtins` composes both halves into `BUILTIN_PLUGINS` / `CORE_UNCONDITIONAL_PLUGIN_NAMES`. `build_registrar` now requires `builtin_plugins=` and `core_unconditional_plugin_names=` instead of importing them, which is what keeps `pipelex.plugins` importable without the interpreter. External plugins are unaffected — they are discovered through the `pipelex.plugins` entry point and sit in no declared layer. `PipelexPlugin` gains a documented invariant: **a plugin belongs to exactly one layer.**
- **Breaking: the built-in vendor adapters left `pipelex.plugins` for the new `pipelex.providers`.** `pipelex/plugins/` was two things under one name — the plugin *mechanism* and the built-in vendor *adapters* — and the name read as the second, which is what kept the one-way dependency between them invisible. The provider directories (`anthropic`, `azure_rest`, `bedrock`, `blackboxai`, `docling`, `fal`, `gateway`, `google`, `huggingface`, `linkup`, `mistral`, `openai`, `openrouter`, `portkey`, `pypdfium2`, `secrets`, `storage`) are now `pipelex.providers.<name>`, and their aggregate manifest moves with them to `pipelex.providers.builtins`. `pipelex.plugins` keeps the mechanism only: `contract`, `registrar`, `discovery`, `exceptions`, `model_handle`, `sdk_client_registry`, `sdk_client_manager`, `backend_extras_factory` and the capability registries. `providers/` beats `backends/` as a name because `pipelex/cogt/model_backends/` already exists and the code's own word for the non-inference adapters is "provider" (`secrets_provider_registry`, `storage_provider_registry`).

    Two things this does **not** change. **The plugin contract is untouched** — an out-of-tree plugin imports `pipelex.plugins.contract` and `pipelex.plugins.registrar`, both of which stay put, and it is still discovered through the `pipelex.plugins` entry-point group, which is a group identifier rather than a module path. **No layer moved**: both packages are declared runtime-layer, and `pipelex.providers` joins `RUNTIME_LAYER_PACKAGES` alongside `pipelex.plugins`. That addition is asserted by a test, because omitting it would have made the guard *quieter* rather than louder — its transitive rule filters candidates through `is_runtime_layer`, so an undeclared package is excluded from the rule's domain instead of reported by it.

    The break is in-tree only: measured across every sibling repo, **no external consumer imports the vendor paths**. In the error reference, the single "Provider plugins" heading splits into **Provider adapters** (a vendor SDK rejected the call) and **Plugin system** (a plugin failed to load or claimed a taken slot); both stay under *Inference & providers*, and no error class is renamed, so every `error_type` and `type_uri` is unchanged.

- **Breaking: the img-gen geometry mappings moved inward, out of the vendor adapters and into `pipelex/cogt/img_gen/`.** `pipelex.providers.google.google_img_gen_factory.GoogleImgGenFactory` is now `pipelex.cogt.img_gen.img_gen_gemini_mapping.ImgGenGeminiMapping`, and `pipelex.providers.openai.openai_img_gen_factory.OpenAIImgGenFactory` is now `pipelex.cogt.img_gen.img_gen_gpt_mapping.ImgGenGptMapping`. The wire-literal type aliases follow the family rather than the vendor (`ResolvedGoogleImageConfig` → `ResolvedGeminiImageConfig`, `GoogleAspectRatioType` → `GeminiAspectRatioType`, `GoogleImageSize` → `GeminiImageSize`, `OpenAIImageLegacySizeType` / `OpenAIImageModerationType` / `OpenAIImageInputFidelityType` → `GptImageLegacySizeType` / `GptImageModerationType` / `GptImageInputFidelityType`), as do the methods that had carried the vendor into the class name (`size_for_legacy_openai_image`, `moderation_for_openai_image`, `input_fidelity_for_openai_image` → `size_for_legacy_gpt_image`, `moderation_literal`, `input_fidelity_literal`). They were never vendor adapters: they map `AspectRatioTaxonomy` — a `cogt`-owned enum that names model families and no providers — onto provider wire values, and no family is served by a single adapter (the `openai` and `azure_openai` decks both ship GPT Image models; the gateway worker resolves Gemini geometry through the same mapping the native Google worker uses), so there was no provider key to dispatch on and no registry to dispatch through. `cogt/img_gen/img_gen_args_factory.py` now imports nothing from `pipelex.providers`, and the Google worker and the gateway factory import inward instead. One behavior-adjacent detail: `moderation_literal` returns `None` where it used to return the OpenAI SDK's `omit` sentinel — which is what removes the last SDK import from the mapping — and since the caller already dropped the key in that case, the emitted API arguments are identical. Neutrality is pinned by a test from both directions (no third-party import in the mapping source, no `pipelex.providers` module in its import closure), because both packages are runtime-layer and the hub-layering guard is blind to an edge between them by construction. Measured across every sibling repo: no external consumer imports either path.

### Fixed

- **Fixed: rendering an LLM prompt no longer mutates the pipe the library holds.** `PipeLibrary.get_required_pipe` returns the stored instance — nothing copies it — so two write-backs performed while rendering a prompt were mutating shared, long-lived state as a side effect of running a pipe. `PipeLLM._live_run_operator_pipe` cached the config-derived prompting style onto `self.llm_prompt_spec`, and `LLMPromptBlueprint._unravel_text` wrote that style into the caller's `TemplateBlueprint`. Both now travel as a run-scoped parameter. Three consequences close with it. A pipe's `model_dump()` — which feeds the graph registry via `_make_pipe_data_for_registry` and the crate payload that travels to a Temporal worker — is identical before and after its first run, so serialized output no longer depends on run order. A deck or config change made in-process now takes effect on the next run instead of being shadowed by the first run's cached value, and a model with no `InferenceModel` (the external-LLM-plugin case) no longer lets whichever style the first run happened to derive govern every later one. And the second write was pure pollution: the render call passed the spec-level style, never the blueprint field it had just written, so nothing on that path ever read it.

- **Fixed: an injected telemetry manager whose `teardown()` raises no longer aborts the failed-boot release suite.** `self.telemetry_manager` is whatever the telemetry factory produced, which includes a caller-supplied implementation — `telemetry_manager` is a public `make()` injection point typed only as `TelemetryManagerAbstract`. The built-in manager is written never to raise, but that guarantee does not extend to an injected one, and an unisolated call would skip every release after it (leaving logging configured and the class registry half-populated) and replace the boot error on the way out. The call is now isolated, logged, and followed by the rest of the suite.
- **Fixed: a teardown that fails partway no longer wedges the process permanently.** The releases at the end of `_teardown_runtime` — the hub config, the class-registry scoping, the `MetaSingleton` de-registration — are what a later boot in the same process depends on, and they sat in a bare sequence below calls made through injectable abstract types (`telemetry_manager`, `reporting_delegate`, `class_registry`, and one level up `pipeline_manager`, all public `make()` injection points). A raiser therefore skipped them and left the instance registered, with **no recovery**: the next `make()` dies on "already initialized" and `teardown_if_needed` resolves that same instance and re-enters the same raiser. Both the runtime release block and `Pipelex.teardown`'s three-phase sequence now use `try`/`finally` — no `except`, so a half-failed teardown still surfaces rather than looking successful. This is the same flaw the failed-boot path already isolated one method away; the normal path's consequence was the worse of the two, since it did not even de-register the singleton.

    The `finally` carries the **whole** un-poisoning set, not just the de-registration, and that distinction is the fix rather than a detail of it: releasing the singleton is what makes the next boot *succeed*, so anything poisoning left behind alongside it converts a loud failure into a silent one. `KajsonManager.teardown()`, `TemplateLoader.reset()` and `TemplateRegistry.clear()` therefore moved down beside it, exactly as `_release_after_failed_boot` already had them. `KajsonManager` is the one that bites — it is a singleton, so a surviving manager makes the next boot's `KajsonManager(class_registry=…)` hand back the old manager and silently discard the fresh registry, leaving `get_class_registry()` serving the previous boot's classes while that boot's own registrations land somewhere nothing resolves to, `is_ready` and all. The two release paths now guarantee the same poisoning set; the *dangling* set (`sdk_client_manager`, `reporting_delegate`, `func_registry`) still differs, which is the collapse tracked in `wip/boot-split/failed-boot-does-not-release-every-resource.md`. Pinned by `test_runtime_boot_teardown_resilience.py`, verified to fail when the move is reverted.

    The plugin-callback phase moved inside the same `try` on both teardown paths, for the reason `_release_after_failed_boot` already gave at its own call to it: `_teardown_plugin_callbacks` swallows each callback's ordinary `Exception`, but `except Exception` does not cover `BaseException`, so a callback that calls `sys.exit()` — or a `KeyboardInterrupt` landing mid-teardown — would otherwise skip the release block entirely.
- **Fixed: a second `make()` of the already-booted class is now refused instead of silently re-running `setup()`.** The exclusivity guard lived only in `__init__`, and `MetaSingleton.__call__` hands back the registered instance without re-running `__init__` — so a second `RuntimeBoot.make()` sailed past it and re-ran the entire setup on the live boot, returning the same object, while its docstring promised a `PipelexSetupError`. The two cross-class tests could not catch it, because a subclass is a different registry key and `__init__` does run there. The check now also runs *before* the construction in both `make()`s, single-sourced as `RuntimeBoot.raise_if_a_boot_already_holds_the_process_globals()`; the `__init__` site stays, since it is the only one that sees a direct construction.
- **Fixed: the singleton lookup no longer iterates a live process-global dict.** `MetaSingleton.get_subclass_instance` walked `instances` directly while its sibling `clear_subclass_instances` already copied. The dict grows at run time (`GraphTracerManager` registers itself during a pipeline run) and this lookup now sits on the boot hot path `ensure_pipelex_booted` reaches from concurrent activities — where a key added mid-iteration is a `RuntimeError`. It iterates a copy.
- **Fixed: a failed boot no longer leaves the telemetry manager singleton behind for the next boot to adopt.** `TelemetryManager` is an `ABCSingletonMeta` singleton, so discarding the boot instance did not release it — and because `MetaSingleton` short-circuits on an already-registered class, the next boot in the process got the *same object* with `__init__` never re-running, leaving `TelemetryManagerAbstract.get_instance()` resolving to a manager whose tracer provider had been shut down. Reachable in production through `ensure_pipelex_booted`'s per-call lazy boot, since the commonest boot failure (`models_manager.setup()` on a missing deck or credentials) fires after the telemetry factory. The failed-boot release path now calls the guarded `telemetry_manager.teardown()`, which flushes and clears the singleton.
- **Fixed: plugin teardown is now best-effort per callback, and can no longer replace the exception that killed a boot.** Two defects on one path. A raising callback used to propagate in place of the setup error, so a failed boot reported a teardown failure and never its cause; the release path's state cleanup now runs in a `finally` so it cannot be skipped either. And the callbacks are now attempted **individually**: previously one raising callback ended the LIFO loop, so with two plugins registered the second one's runtime was left live — catching around the loop rather than inside it looks equivalent but the loop has already exited. Errors are logged rather than propagated, on both the failed-boot and the normal teardown path, because no caller of a teardown can act on "plugin B failed to release" while every caller is harmed by B never being asked. **Breaking**: `teardown()` no longer propagates a plugin callback's exception.
- **Fixed: a failed boot no longer leaks a plugin runtime a boot-orchestrator already started.** `Pipelex.make()`'s failure handler released core's own process-global state but never ran the plugin-registered teardown callbacks, so a boot that died after the `TASK_MANAGER` slot thunk left whatever runtime that thunk stood up in place — and only the plugin knows what that is; our Temporal plugin's thunk registers its own process-global singletons and installs a sandbox predicate. The window is wider since the boot split (the thunk runs in the runtime half, so every interpreter-tail step now follows it, where three of them used to precede it), and the most reachable trigger is a plain config error: `pipe_func_config.execution_mode` naming an unregistered mode. The failure path now runs the plugin teardown callbacks first, mirroring the normal teardown's ordering, pinned by a test that asserts the thunk ran *and* the callback ran (so it cannot pass vacuously) and verified to fail without the fix.
- **Fixed: `config_dir` at boot is no longer silently ignored.** `Pipelex(config_dir_path=…)` stored the path and never read it again — `setup_config` was called without a `config_dir`, even though the hub method accepts one — so an embedder passing an explicit config dir got the ordinary project/global layering and no error. It is now wired through to `setup_config(config_dir=…)`, with a test that boots against a temp dir and asserts the scoped load is what landed.
- **Fixed: a second boot on top of an existing one now fails loud instead of serving a half-populated registry.** `set_runtime_hub`, `KajsonManager` and `log.configure` are all once-per-process, so a `Pipelex.make()` on top of a live `RuntimeBoot` (or vice versa) would quietly adopt the first boot's state. Each `make()` now asks the base class whether any boot holds the process globals and raises `PipelexSetupError` if one does.

- **Breaking: generated codegen artifacts were not lint-clean, so running your own formatter over them broke their stamp and got you accused of hand-editing.** A stamp's `content_hash` is a raw SHA-256 over the body bytes, and the emitters were not writing what a formatter wants — so the first `ruff check --fix` / `ruff format` (or `prettier --write`) over a generated tree rewrote the bytes and `pipelex codegen check` reported the file as `[hand-edited]`, which is the one thing the user did not do. It was silent until someone ran `codegen check`, and our own `pipelex-starter-python` template shipped a `piper/generated` lint exclusion to work around it — the tell that the emitter, not the consumer, was wrong.

    **Emitted bytes change for all three targets, so a committed projection reports drift until it is regenerated** — `pipelex codegen types --target <target> -o <output_dir> <method_dir>`, or `pipelex build structures <dir>` for the python-structures alias. Both Python targets now emit isort-grouped imports (merged per module, stdlib separated from third-party) and double-quoted `Literal` members; `python-structures` additionally moves to builtin generics (`list[str]`, `dict[str, int]`) and `X | None`, dropping its `typing.List` / `Dict` / `Optional` imports. `ts-zod` drops the Python two-blank-line idiom Prettier always collapses, and wraps the binder import only when it exceeds Prettier's print width. Ruff's `TC003` is the one rule that cannot be answered in the bytes — moving `from datetime import date` into an `if TYPE_CHECKING:` block would break the generated models, since pydantic resolves annotations at runtime — so consumers set `runtime-evaluated-base-classes` instead; **the lint exclusion on generated paths is no longer needed and should be dropped.** Documented in [Codegen Projections](under-the-hood/codegen-projections.md). This repo now sets that option too, which retired two hand-written `# noqa: TC001` / `# noqa: TC003` suppressions that were working around the same rule one file at a time.

    Two further emitter bugs surfaced while pinning the property. The imprecision caveat on an opaque concept was emitted as a literal `\n` escape rather than a paragraph break, so the docstring rendered as one line with visible escapes. And a description containing a double quote — `The "primary" thing` — was backslash-escaped into the docstring, which ruff's `D301` then auto-rewrites to an `r` prefix: byte-changing *and* text-corrupting, since the escapes start rendering literally.

    **A concept description is now rendered as an idiomatic docstring, which changes `__doc__`.** Summary on the first line, continuation lines indented to the class body, closing delimiter on its own line — the only shape `ruff format` and the pydocstyle rules both leave alone. Rendered any flatter, a multi-line description trips `D207`/`D209` and a description padded with edge whitespace trips `D210`, every one of them an automatically-applied fix that rewrites the bytes. So `__doc__` now carries the class-body indentation that every hand-written docstring carries; what holds exactly is `inspect.getdoc(cls) == inspect.cleandoc(description)`, the value consumers actually read. Exact bytes survive wherever they are consumed programmatically rather than as prose: `Field(description=...)` and the crate itself are unchanged. A description that cannot sit between `"""` at all now falls back to `'''` rather than backslash-escaping, which is what keeps it `D301`-clean.

    **Long lines are emitted already wrapped, with a trailing comma, because the consumer's `line-length` is theirs and not ours.** A description or choice list is authored text with no length bound, so a generated line can exceed any width — and the formatter would wrap the `Field(...)` call or the `Literal[...]` annotation, rewriting the bytes. Black and ruff both read a trailing comma as a deliberate choice to keep a construct exploded and will not rejoin it at *any* width, so anything past 88 columns — ruff's own default, and therefore the tightest width a consumer is likely to lint at — is emitted in that form. Short lines are untouched, so ordinary artifacts read as before. Linting Python at fewer than 88 columns can still rewrap short generated lines; that limit is documented rather than papered over.

    **Imports are now registered by whoever writes the name, never seeded up front.** Two ordinary crate shapes used fewer imports than the emitters assumed and were left with an unused one — an `F401`, which `ruff check --fix` deletes: a method declaring only structureless concepts (`Question` → `Answer`) emits no `Field(...)` at all, and one whose concepts all refine a native never reaches the runtime root base, leaving `StructuredContent` unused too.

    **`ts-zod` is emitted to match Prettier's defaults**, which now includes pre-wrapping the pieces a long concept code or choice list pushes over the 80-column print width: the `z.infer` type alias, the binder's `parse` / `serialize` signatures, and `z.enum([...])` (in place, or as a broken member chain when a `.optional()` / `.default(...)` is attached). A Prettier config that changes `printWidth`, `singleQuote` or `semi` will still rewrite the output; that assumption is now stated in the docs.

    A projection with **nothing to emit** was the same bug in miniature, and it needed no unusual method to hit: `python-structures` skips natives, so a method that declares no concepts of its own — a `Text -> Text` pipe — left that target with no class to write, and it emitted an import block nothing used (an unused-import finding `ruff check --fix` deletes) above a trailing blank-line run (`ruff format` collapses it). The same shape reached the other three artifacts from a bundle declaring only a domain, where the binder degenerated to `import {  } from "./types";`. An empty projection is now its header alone, which no formatter touches.

    Guarded by `tests/unit/pipelex/codegen/test_emitted_artifacts_are_lint_clean.py`, which emits a projection covering every `ResolvedTypeKind` — the fixture is asserted to reach every enum member, so a new kind nobody wires in fails loudly — and runs `ruff check` plus `ruff format --check` over it for each Python target, across a range of consumer `line-length` settings, plus the two crate shapes that use no seeded import. Only rules whose fix ruff never applies automatically are suppressed in the invocation, since only an applied fix can rewrite bytes. The TypeScript side asserts the config-independent invariants always — no collapsible blank runs, no code line past the print width — and runs `prettier --check` when prettier is on PATH, which CI has no toolchain for.

- **The runtime `StructureGenerator` emitted `Optional[X]` / `List` / `Dict` where the codegen emitters emit modern typing.** Its output is `exec()`d rather than written to disk, so no linter ever saw it, but the two paths render the same resolved-type tree and the emitter's own docstring justified its spelling by mirroring it. Both now agree on builtin generics, `X | None`, and double-quoted `Literal` members. One shape needed care: this module has no `from __future__ import annotations`, so a concept reference renders as a *quoted* forward reference, and `"Foo" | None` is a `TypeError` at runtime — `str` does not implement `|`. The union is folded inside the quotes (`"Foo | None"`) so the annotation stays one deferred expression for `model_rebuild`; a bare quoted ref is the only affected shape, as `list["Foo"] | None` is a real `GenericAlias`.

- **The hub-layering guard was only one hop deep, and four modules of the declared runtime-layer `pipelex.plugins` package breached the boundary through it.** `plugins/direct/direct_plugin.py`, `plugins/pipe_func/pipe_func_plugin.py` and their two aggregators (`builtins.py`, `discovery.py`) reached `pipelex.interpreter_hub` — and 57–67 interpreter modules — through `runtime_bridge`, `pipeline` and `pipe_operators`, while both `make check-hub-layering` and the import-closure test stayed green. The headline zero-interpreter-modules property was never damaged (nothing on the inference-layer or `runtime_hub` path reached those four), but the declaration claimed something the measurement contradicted. Fixed by relocating the two interpreter-touching plugins (above) rather than by declaring an exception, and the guard now has a **transitive rule**: it resolves the module-level import graph of `pipelex/` and reports any runtime-layer module that *reaches* the interpreter hub, with the shortest import chain and the line of its first hop. The built-in adapters' manifest also joins the import-closure test's entry points, since static analysis cannot see a dynamic import; it ships at `pipelex.providers.builtins`.
- **`hub.set_observer()` was documented as the way to register a custom observer, and never worked.** The value was written to the hub and never read — the live path is `Pipelex.make(observers={...})` → `MultiObserver` → `PipeRouter`, so anyone following that page got a silently ignored observer. The write-only state is deleted and both `docs/advanced/observer-provider-injection.md` and `docs/advanced/index.md` are rewritten to the mechanism that actually works, with a warning against hand-constructing a hub.
- **A duplicate `set_library_manager` pair in `Pipelex.setup` built, registered, then silently discarded a first `LibraryManager()`** when the caller passed none. Removed.
- **The validation feature page promised a check that does not exist and cannot exist.** "Structure validation — Verify concept compatibility between pipes, ensuring inputs and outputs match" read as an inter-step contract: step N's output concept matched against step N+1's declared input concept. `PipeSequence` steps do not chain that way — each resolves its inputs by name from working memory, sourced from the pipeline's inputs or any earlier step, so there is no consecutive-step contract to verify. [Validation & Dry Run](features/validation-dry-run.md) now describes the checks that actually run (per-pipe input concepts, controller output boundary, variable binding) and states the working-memory resolution model explicitly.
- **`pipelex build --help` advertised a `typescript` output format that does not exist.** The `output` group help read "(JSON, Python, or TypeScript)" while the accepted `--format` values are `json`, `python`, `schema` — passing `typescript` errored out. The group help now names the real formats; TypeScript remains what it always was, a downstream use of `--format schema` (JSON Schema → TypeScript interfaces or Zod schemas), which the per-subcommand help now states consistently across `pipe`, `bundle`, and `method`.
...
- **Mistral Workflows integration extracted into a dedicated package.** The optional `pipelex[mistralai-workflows]` extra and the `pipelex.plugins.mistralai_workflows.*` modules have been removed from `pipelex`. The Mistral Workflows integration now ships separately as its own package, `pipelex-mistralai-workflows`. The framework-agnostic runtime-bridge core (boundary types, `run_pipe_via_bridge`, `PipelexExecutionMode`, `ensure_pipelex_booted`) has been promoted from `pipelex.plugins.mistralai_workflows.*` to `pipelex.runtime_bridge.*` so any host runtime — not just Mistral Workflows — can embed Pipelex. No behavior changes; activities, boundary types, and execution modes are identical.
