...
- **Plugins declare their layer through their entry-point group (Breaking)**: The single `pipelex.plugins` entry-point group is replaced by two — `pipelex.plugins.kernel` and `pipelex.plugins.interpreter` — and `PLUGIN_API_VERSION` bumps to 4. A kernel-only boot queries the kernel group alone, so an interpreter-side plugin's module is never imported into a process that only runs kernel operations; that is what turns "this venue holds only the kernel" from a convention into a mechanism, since importing a plugin is how discovery used to drag the whole interpreter in. The declaration is enforced: a kernel-group plugin that registers an orchestrator, a bundle validator, a PipeFunc executor, or an interpreter-layer hub slot fails loud at register time. The restriction is one-directional — an interpreter-group plugin may still contribute kernel-tier capabilities such as an HTTP-error mapper. An installed plugin still published under the retired group is a startup error naming it and the group it should move to, never a silent nondiscovery. `pipelex plugins list` gains a Group column.
