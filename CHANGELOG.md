...
- **Inference-profile ref on the run payload (`PipeRunParams.inference_profile_ref`):** a neutral, optional reference (`owner_id` / `profile_id` / `fingerprint`) to an externally-stored inference configuration, for hosting layers that bind a run to a specific tenant configuration (BYOK). The ref travels in the serialized run payload — mirrored into the derived `CogtRunParams` carrier so it rides every cogt assignment across the activity boundary — and is written once at submission (new optional `inference_profile_ref` kwarg on `PipeRunParamsFactory.make_run_params`, or a `model_copy(update=...)` stamp). Core stores and transports the ref but never resolves it: no profile store, no resolver, no credentials. Default `None` keeps today's behavior (the executing process's boot configuration).
