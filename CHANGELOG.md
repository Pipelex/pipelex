...
- **MTHDS Test Corpus: the `operator.*` and `controller.*` tag axes.** The corpus tag vocabulary now covers every executable pipe kind alongside the native concepts, generated from `PipeType` and split into the two namespaces by that enum's own `category` property — one registry walk, so a pipe kind added later lands in the right namespace with nothing to update. Local names drop the `Pipe` prefix each code carries, since the namespace already says which half it is (`PipeLLM` becomes `operator.llm`), while `code` keeps the registry's spelling. The exhaustivity gate covers both new namespaces, and focused entries land with them: image generation, extraction, search, structuring, composition, and the sequence, batch and condition controllers. `PipeFunc` is the corpus's first recorded exclusion — it names a Python function that the runtime resolves against its function registry at validation time, so no portable entry can declare one.
- **The `feature.*` tag axis opens, with multi-file libraries as its first tag.** Language features are the one thing the corpus vocabulary cannot generate: a feature is a human-named cluster of behaviour rather than a registry entry — some span several blueprint fields, some none at all — and there is no feature registry to walk. So `feature.*` is hand-maintained in the generator, and a tag there carries a `description` where a generated tag carries `code`, since that sentence is the only place its meaning can live. Each feature tag is declared in the same change that lands its first covering entry, never ahead of it, so the vocabulary never advertises coverage the corpus does not have. The first is `feature.multi_file_library`, covered by a research-brief method built top-down across several files with each pipe forward-declared as a signature before it is written — migrated out of the e2e tree, which now reads it from the corpus.

- **Two more `feature.*` axes: optionals and smart inputs.** A noticeboard line whose house-style hint is optional — guarded in the prompt so an absent value renders nothing and is recorded as absent — and a claims summary whose caller passes bare values (a string, a number, a dict, a list) to be shaped top-down against the declared signature. Both migrated out of the e2e tree, which now reads them from the corpus. Re-themed on the way in, per the corpus rule that an entry's subject matter is ordinary-world and never the machinery it exercises; the tests that asserted on the old qualified concept refs moved with them.

- **`feature.structured_output` joins the vocabulary**, covered by the entry that was already its minimal exemplar — a shop's opening hours read off the door into typed fields — rather than by a near-duplicate written beside it. `feature.dependencies`, the remaining name on the v1 list, is deliberately **not** declared: a method depending on another package needs a `METHODS.toml` manifest and a second package directory reached by a relative path, which is a different shape from the one-directory entry the contract pins. Declaring the tag before an entry can cover it would advertise coverage the corpus does not have.

- **A nested structured-output corpus entry**: a driver's round sheet filed into a shape the corpus had no exemplar for — a list of structured objects, each carrying its own list and an optional field. Written fresh rather than migrated: pipelex's own structure-shape matrices are named for the shapes they test (`ConceptWithNestedStructures`, `ConceptWithOptionals`), which is load-bearing for the tests that read them and precisely what the corpus's subject-matter rule forbids, so re-theming them would have made both sides worse.

- **The corpus is proven to ship, against a wheel that is actually built**: wheel consumers reach the corpus through `importlib.resources`, so a corpus that stopped being packaged would be invisible in this repo and would break every consumer at once, on the release. A packaging test builds the real wheel and looks inside it, deriving what it expects from the corpus tree rather than from a list somebody has to maintain — a new entry, a new file kind or a new namespace is covered the moment it lands. It pins both halves of the split at once: the generated `vocabulary.toml` and every entry file ship, and the generator that writes them stays out. No packaging configuration change was needed, which is what the test now holds true rather than assumes.
- **Image-payload corpus entries**, so a consumer that needs an `Image` crossing a pipe boundary has one: a sequence that writes a scene and draws it, and a round trip that produces a photograph and reads it back to caption it. Both are `composite`, both sit at the `inference` tier, and — like every other entry — neither names a model. That is a contract property rather than a preference: presets are resolved by the validation engine, so an entry pinning one would fail validation outright on any consumer whose deck does not define it.

### Changed

- **The image round-trip bundle moved into the corpus** and its integration test re-sources from there. An `Image` produced by one pipe and consumed by the next is a language-level shape, so the corpus owns it; the local copy under `tests/integration/pipelex/pipes/pipelines/` is deleted.
