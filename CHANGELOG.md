...
- **Breaking: where a run's bytes go is now told to the runtime, not derived from who is running (`JobMetadata.storage_scope`)**: Storage keys were built from `user_id` — `{user_id}/results/{run_id}/` for delivery, `{primary_id}/{secondary_id}/…` for generated content, a flat top-level `normalized/` for inputs pulled off a `data:` URL. That ties *where the bytes live* to *who uploaded them*, which is fine until two people share the work: the second one asks for a file the first one attached and is refused, because the only thing standing between them is a string comparison on the first path segment. A host that serves teams cannot express "these two people may read the same file" in that layout at all.
  ...
  **`uri_format` loses `{secondary_id}`** (supported placeholders are now `{primary_id}`, `{hash}`, `{extension}`, with `{primary_id}` carrying the whole scope), and the shipped default becomes `{primary_id}/{hash}.{extension}`. A configuration still naming `{secondary_id}` is refused **at boot**, by name, with the supported set listed — the free-string value domain is not something the migration ledger can prove stale, so this one is a loud failure rather than a repair. Edit `uri_format` in your `.pipelex/pipelex.toml` if you customized it.
...
- **The configuration root is reshaped along the layers of the runtime (Breaking)**: `pipelex.toml` had no scheme at the top — a grab-bag (`[pipelex]`), a codename (`[cogt]`), the plugin system's own table (`[plugins]`), a rename-hint table (`[migration]`) and a stray `session_id`. It now has one, and it is the split the code already had: `[runtime]` for process-scoped infrastructure (storage, secrets, logging, cloud credentials, reporting, tracing, observation, the plugin denylist), `[inference]` for the model-calling seam (the model deck, LLM, image generation, extraction, plus the default templating style and the shape of dry-run mocks), `[interpreter]` for library-scoped method machinery (MTHDS parsing, pipe runs, pipe functions, pipeline execution, scanning, the builder), and `[kit]` for the dev tooling. The redundant `_config` suffix is dropped from every key at every depth, so `[pipelex.log_config.rich_log_config]` is `[runtime.log.rich_log]` and `[cogt.llm_config.instructor_config]` is `[inference.llm.instructor]`. Class names keep their suffix — the redundancy was in the key, not the type. Every setting keeps its value; only its address changes, and the packaged defaults are now written in order so no root table is split across the file. **Migration:** run `pipelex migrate` after upgrading — the change ships as ledger entry `pipelex-config@3`, which rewrites `~/.pipelex/` and every project `.pipelex/` file in place. Three things are dropped rather than moved: `[migration.migration_maps.*]` (renames are ledger entries now, so the old-to-new name hints in validation errors go away, `.mthds` hints included), `session_id` (process state on the config object that nothing in `pipelex` read), and `plugins.boot_orchestrator`, which becomes a boot argument only — pass `--orchestrator` or `Pipelex.setup(boot_orchestrator=...)`, and read it back at run time from the `runtime_hub` accessor rather than off the config. Code reading the config tree moves with the keys: `get_config().pipelex.storage_config` is `get_config().runtime.storage`, and so on.
- **The migration ledger contract is published, and a config-model change now owes it a review**: `docs/migration-ledger.md` was written ahead of the engine it specifies and shipped built-but-unlisted; the commands it describes exist, so it joins the documentation navigation beside the other contributor contracts and loses its status banner. Alongside it, the `config-docs` drift contract's review list gains the migration ledgers and the two modules holding the four validators that narrow a configuration value domain — a narrowing expressed in a validator is invisible to the fingerprint projection the coverage gate reads, so no derived check can ever demand the ledger entry it owes, and a review obligation is the only place that can land. The migration goldens are deliberately not in that list: they are generator output, and their staleness is already `check-migration-schemas`' business.
- **Writing a migration entry is a skill, and a release checks one was written**: `.claude/skills/add-migration/` is the only thing that writes a ledger — it derives the entry from what the coverage gate refused rather than from a reading of the diff, bumps the surface's schema version, regenerates the goldens, and adds the changelog bullet. `/release` gains step 3b before the version bump: `make check-migration-schemas` does not run in `make agent-check`, so an unaccounted schema change would otherwise reach a release unseen; the step blocks on it, sends a failure to `add-migration`, and confirms a `**Migration:**` changelog bullet and a correct `introduced_in` for every new breaking entry.
- **A stale telemetry configuration is migrated instead of thrown away**: `telemetry.toml` failing to load used to get one answer everywhere — *the format has changed, run `pipelex init telemetry`* — written for the flat pre-`[custom_posthog]` file and then shown for every other way a file can fail. That command writes a fresh file, so on the very case it named it discarded the PostHog key, the Langfuse credentials and the OTLP exporters that the shipped `telemetry-config@2` entry carries forward. The telemetry loader now raises through the same reporting path as every other configuration surface, so its error carries the structured `migration` block and the human CLI, the agent envelope and `pipelex doctor` all say whether the file is *old*, *wrong*, or reported on by a migration that would not repair it — rather than each holding a hardcoded remedy. `TelemetryConfigError` is a `PipelexConfigError`, which is where its `error_domain = config` now comes from — its entry in the agent CLI's domain table is gone as redundant. The `doctor` telemetry row carries a structured `finding` (`healthy`, `not_found`, `unparseable`, `out_of_date`, `invalid`) that every caller branches on, and `out_of_date` is a promise the migration would rewrite that file, so a file it would only report on is `invalid` and keeps the dry run's pointer; writing a fresh file is offered for a missing file only, and regeneration is described as what it is, a way to start over that discards the file. The telemetry models stayed in `telemetry_config.py` and the loader moved into a new `telemetry_loader.py` beside it, because the loader's failure path reaches the migration registry and the registry imports every configuration model.
- **The fingerprint records the bounds a file actually meets, and the enumerated spellings where an operation can reach them**: A bound in a field's own metadata *binds* — pydantic applies it on top of whatever a union member declares — so the two sources are now merged separately and intersected instead of pooled and widened; a field whose own `Field(le=6)` was the real ceiling used to be recorded as `le=100` because a member said so, and a later tightening of that ceiling then read as a change to an already-looser one. Enumerated members beneath an open mapping are now recorded only on the wildcard record, whose own value is the enumerated one: recording them on the container as well made the coverage gate demand a remap of a table value, a demand no operation could satisfy. Every other container is still descended into, since a `list[enum]` has no child record — and there the gate now names `unsafe` as the only remedy rather than crediting a remap that would skip on every run. One golden moved with it: the duplicated member set on `runtime.log.package_log_levels`.
- **The narrowing relation stopped crying wolf on four shapes**: A container whose argument widened (`list[int]` → `list[int | str]`) is read structurally instead of as one opaque string; `int` is absorbed by `float`, which accepts every integer in strict validation as well as lax; `enum` and `literal` absorb each other, because they are two renderings of one thing and what moved between two member *sets* is reported by the enumerated-spelling half; and over an integral type `gt=n` and `ge=n+1` are folded to the same bound, so swapping one spelling for the other stops reading as a tightening. Each was a false breaking verdict on a schema no user's file can tell apart, and the remedy the gate named — bump the version, write an `unsafe` entry — was work with no beneficiary.
- **`make up-migration-schemas` left `make up`, and refuses to erase a removal**: Every other regenerator in that aggregate rewrites a derived artifact from a live source, so running them out of habit is harmless; the head fingerprint golden is the proof obligation the coverage gate compares against, and rewriting it after deleting a field erases the very removal the gate exists to catch. It is now run on purpose, and it stops before overwriting a head golden recording a path, an enumerated spelling or a value domain the models no longer have. The refusal names both readings — a real removal wants a bump and an entry; a golden that merely predates a change to the fingerprint format wants the new `make umigf` — because the code cannot tell them apart.
- **The fix applier moves and renames a table whole even when a descendant is written in several places**: A `[a.b]` … `[a.c]` … `[a.b.d]` layout stores `a.b` as several chunks under one key, and tomlkit's own header-cache invalidation walks the merged dict facade — one chunk per key — so renaming or moving `a` used to leave the later chunks rendering under the old name: the file came out split between the old name and the new, with an `applied` verdict. The applier now refreshes every chunk beneath a renamed or moved table, so the file re-parses to the same value under the new path, in the original order. This reaches every caller of the applier, the `.mthds` fix loop included.
- **Templating style is now an authoring decision (Breaking)**: How a pipe's inputs are tagged into a prompt is declared on the pipe — `templating_style` on `PipeLLM`, either a bare tag-style string (`"xml"`, `"ticks"`, `"square_brackets"`, `"no_tag"`) or a full `{ tag_style, text_format }` table — with a single runtime default for everything that declares nothing (`[inference.templating].default_templating_style`, shipped as `xml`). It is no longer derived from model metadata, so the same method renders the same prompt shape on every model. **Every prompt rendered for an OpenAI-family model changes shape, from back-tick fences to XML tags.** `PipelexKernel.llm_text` and `llm_object` take the same optional argument. **Migration:** the change left two shapes behind on machines that already had configuration files, and both are now accounted for. Ledger entry `pipelex-config@2` carries `[pipelex.prompting_config]` forward — the default style travels to `default_templating_style` and lands on today's `[inference.templating]` once the reshape entry runs after it, while the per-target `prompting_styles` map is dropped, since a style is no longer chosen per model; the backup beside the file is where the old map remains readable. It is inserted *below* the configuration reshape, which addresses the post-rename spelling and is therefore renumbered from `pipelex-config@2` to `pipelex-config@3`. The per-model half of the same change is `inference-backend@2`, above. Running `pipelex migrate` once applies whichever of them your files need.
...
- **`templating_style` tables reject unknown keys (Breaking)**: `TemplatingStyle` and the rich template table of `PipeCompose` (`[pipe.name.template]`, i.e. `TemplateBlueprint`) are now `extra="forbid"` like every other blueprint. A misspelled optional key — `{ tag_style = "xml", text_formt = "markdown" }`, or `templating_stile` / `extra_contxt` on a template table — used to be dropped silently, yielding a prompt rendered under a shape the author never asked for; it is a validation error now. The MTHDS JSON Schema gains `additionalProperties: false` on both definitions.
- **`pipelex run --dry-run` no longer requires credentials**: A dry run makes no inference call, so it now boots keyless (every run forced to DRY) with real model specs — the same boot `pipelex-agent run --dry-run` already used. Until now `pipelex run --dry-run` booted as if for a live run and failed outright on a machine without an API key, while the agent CLI succeeded; the two CLIs now agree. Anyone who relied on `pipelex run --dry-run` as a credentials check should use a live run for that.
- **Unknown per-model backend keys must be header-shaped (Breaking)**: A key in a per-model table of a backend TOML that is not a model-spec field is sent to the provider as an HTTP request header — that mechanism is now documented, and it is gated on shape: the key must contain a hyphen (`x-portkey-provider`, `anthropic-beta`). An unknown key without one (`max_tokns`, or a field deleted from the blueprint) used to go out on the wire silently while the real setting stayed unset; it now fails the backend load, in strict and lenient boots alike, with an error naming the key, the model and the file. A hyphenated spelling of a real field (`max-tokens`) is rejected too, naming the field it resembles, and so is a header-shaped key whose value is not a string (`x-foo = 3`) — a header value must be a quoted string, and is never stringified onto the wire. A key that clears the shape gate must also be usable as a header on the wire: the name may contain only letters, digits and RFC 7230 token punctuation, so a quoted `"x-foo bar"` is refused, and the value must be printable ASCII on a single line with no leading or trailing whitespace, so a line break or an invisible trailing space is refused — each of those used to load fine and have the HTTP client reject the request on the first call to that model. In the remotely served Pipelex Gateway config the same rule prunes such keys instead of failing, as version skew. `endpoint_path` — the provider-side route the Gateway image models are called by — is a declared model-spec field rather than a header entry.
...
- **A bracket in a path no longer breaks the health report**: `pipelex doctor` printed almost every dynamic value through Rich's markup parser, so a directory named `[dev]` vanished from the output and a message quoting a `[/b]` — which a configuration validation error quotes back from the user's own file — raised a markup error and took the whole report down, worst exactly when the report is what the user came for. Every value the report interpolates is now escaped: the four row messages, the configuration paths, backend names, environment-variable names, deck filenames, the backend error line, and the text of every exception the `--fix` pass reports.

- **`pipelex doctor --fix` no longer offers to overwrite a telemetry configuration it could migrate**: The fix machinery decided what it could repair by searching the row's *message* for `"format has changed"` or `"invalid configuration"`, and on a match offered to write a fresh `telemetry.toml` — discarding every setting in it. It now branches on a structured finding, so an out-of-date file is answered with `pipelex migrate` and an invalid one is left to a person. Rewording the row can no longer switch the repair path off in silence, which the string search would have done without a test failing.
- **The doctor's telemetry check strips the reserved `[meta]` table the way boot does**: A `telemetry.toml` carrying the migration machinery's reserved `[meta]` table boots perfectly well and was reported invalid by the health check, which validated the raw document.
- **A boot that fails on the main configuration reports it instead of crashing**: `RuntimeBoot` and the doctor both caught `pydantic.ValidationError` around `setup_config`, and the main configuration does not raise one — `ConfigRoot` translates pydantic's error into `ConfigValidationError` before it leaves — so the arm never fired for the one configuration everything depends on, and a machine with a bad `pipelex.toml` got a bare traceback rather than the field-level translation the arm existed to produce. Both sites now catch either shape a refusal takes and share one helper, so the same refusal produces the same message and the same structured block wherever it is caught.
- **The agent CLI no longer prints two error envelopes on one stream**: `agent_error` leaves through `typer.Exit`, which is a `RuntimeError` and not a `SystemExit`, so a command boundary re-raising only `SystemExit` caught the error it had just reported and reported it again. Two JSON documents on stderr is not JSON, and a machine consumer parsing it got "Extra data" instead of the error. `pipelex-agent models` and `pipelex-agent check-model` were the two sites.
- **Renaming a table written in dotted form no longer swallows the keys after it**: `k.x = 1` and a `[a.k]` header say the same thing to a TOML parser and are two different things to a TOML document, and which one a key is is a flag the parser sets rather than a property of its name. Rebuilding the key from its name alone — what tomlkit's own re-key primitive does — lost that flag, so a renamed dotted key came back out as a block header, and a block header absorbs every scalar that follows it in the same table: a neighbouring `m = 3` silently became a key of the renamed table, with an `applied` verdict and no diagnostic. Renamed at an inner segment the whole chain re-rendered at the document root, taking the subtree out of the table it lived in. The applier now renames the document's body entries where they sit, carrying the flag forward and covering every chunk of a key written on several dotted lines — which also stops a cosmetic blank line being injected after any table it renames. This reaches every caller of the applier, the `.mthds` fix loop included.
- **A migration no longer replaces a symlinked configuration file with a regular one**: a file symlinked out of a dotfiles directory is now read, backed up and replaced at the end of the link, the way the `.mthds` fix loop already resolves its own targets — the link survives and the real file is the one that gets migrated. Replacing the link path instead put a regular file where the link was and left the real file unmigrated.
- **A migration run no longer takes away a backup it did not make**: the backup stamp resolves to the second, so two runs of the same file can address the same name. A run now reserves the name atomically and leaves whatever is already there — an older copy of the file, which is to say the original — instead of overwriting it, and a run whose write is refused deletes only the copy it made itself. A copy kept because a write could not be confirmed is renamed out of the `.bak.` rotation into `.rescue.`, so the next successful run cannot prune away the very file the report told the user to go and get — and when that copy cannot be moved, because another run made it or the rescue name is taken, the report says it is still in the rotation and asks the user to take it now instead of promising it will keep; and the backup's directory entry is `fsync`-ed before the target is replaced, so "back up first, replace second" holds across a power loss and not only a process exit.
- **A backend file that is both out of date and wrong now says both, and stops offering to reset itself**: The boot builds its own message for the two libraries that refuse after the main configuration is already up — the inference backend library and the model deck — and it named no migration surface for either, which was right while neither had a ledger. Now one does, so a backend file the ledger only half explains never reached boot tolerance's warning and was reported on its unknown key alone, while the very same staleness in `pipelex.toml` produced the full migration block. The component now carries its own surface, and the classes that mean *the backend library's files are not loadable* are likewise named once — both stated beside each other rather than at each `except` clause, since a clause naming one class of a family is a hole rather than an error, and the scan is only reachable if the boot catches what the loader really raises. The refusal reaching that message need not carry a pydantic error at all, which is the second shape a backend file refuses in: a per-model key rejected by name for not being header-shaped arrives with no cause, and the surface is asked about the file either way. And the refusal's own sentence is now kept whole rather than reduced to the pydantic analysis behind it: the loader says *which model, which backend, which file* before it quotes the analysis, and a message cut down to `max_tokens: Input should be a valid integer` sent the reader to grep a directory of files that all have a `max_tokens` — the model deck's message, which names the deck's paths the same way, keeps them now too. In the same change, a message carrying a migration block stops appending *or run `pipelex init config` to regenerate them, which resets all config files to their default values*: the block has just promised that `pipelex migrate` keeps every value the file holds, and whichever remedy the reader followed, the other sentence made it the wrong one. The model deck, which has no ledger and gets no block, keeps that sentence — for it, it is the only answer there is.
- **A bad value in `backends.toml` is reported as a configuration error, naming the backend and the file**: A backend table whose own fields fail the blueprint — `endpoint = 42` — left the library loader as pydantic's bare `ValidationError`, which no clause in the boot named, so it reached the user as a traceback that named a field and neither the table it sat in nor the file. It is now raised as the library's own validation error, said with the backend and the file in front of the analysis, and handled by the boot like every other refusal of that library. The clause that used to raise that class wrapped a TOML read that never raises a pydantic error, and is gone.
- **`pipelex_gateway` with a non-boolean `enabled` no longer disappears from the deck**: The boot decides whether to fetch the Pipelex Gateway's model specs by reading `backends.toml` itself, and it read `enabled` as *is the literal `true`* while the library loader read the raw value's truthiness — so `enabled = 1` fetched no specs for a backend the loader then loaded, which a strict boot refused as *"remote model specs were not provided"* and a lenient one silently dropped from the deck. Both readers now read the value the same way.
- **A blocked configuration file now says which state it is in**: `FileBlockedReason` gained `unreadable` and `state_uncertain`, so an unreadable file is no longer reported as *unwritable* and a write whose outcome the transaction could not describe is no longer reported as one that never happened. The wording of that last case described an unreachable path — for the single-file commit a migration performs, a failed replacement re-raises its own error, so the only transaction failure that reaches a plan is the one raised after the target was replaced.

### Removed

- **`prompting_target` (Breaking)**: Gone from `LLMSetting`, `InferenceModelSpec`, `InferenceModelSpecBlueprint` and every backend TOML, along with the `PromptingTarget` enum and the per-target style map (`[pipelex.prompting_config]` and its `prompting_styles` sub-table, replaced by `[inference.templating]`). **Migration:** delete every `prompting_target` line from your own `.pipelex/inference/backends/*.toml` — backend files are read strictly, so a surviving one fails the boot. Check `portkey.toml` in particular: the key appears in `[defaults]` *and* on individual model entries, and a surviving per-model one is rejected by name as an unknown, non-header key.
