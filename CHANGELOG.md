...
- **A failed publish can be recovered without damaging what already shipped**: `publish-pypi.yml` scopes the signature upload to `dist/*.sigstore.json` so it no longer clobbers the wheel and sdist attached just before it, turns `skip-existing` on for a re-run or a manual dispatch, reuses the build an earlier attempt of the same run stored instead of rebuilding it, and refuses to touch a release whose tag names a commit other than the one being published. The release is now tagged at the commit it ships, named explicitly with `--target`, rather than at the default branch's head.
