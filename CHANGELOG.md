...
- **Service terms are asked for any managed gateway backend, not for `pipelex_gateway` by name**: the boot puts an installation behind service-terms acceptance as soon as any Pipelex-managed gateway backend is enabled, and the init flows now ask that same question. Selecting only `pipelex_manifold` — through the interactive picker, through `pipelex-agent init --config`, or by enabling it by hand — prompts for the terms and records the acceptance, and `pipelex init agreement` reaches the prompt on such an installation instead of reporting that no acceptance is required. Acceptance stays a single global flag, so one answer covers every managed backend; declining now disables all of them rather than only the Portkey-cloud one.
