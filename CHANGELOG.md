...
- **A publish that fails after reaching PyPI can be recovered, and a recovery cannot damage what already shipped**: four defects in `publish-pypi.yml`, all of them found in `kajson` first and fixed there. The signature upload named `dist/**`, which the runner's bash expands as `dist/*` for want of `globstar`, so it also matched the wheel and sdist attached the step before — and `--clobber` deletes an asset before replacing it, meaning a transient failure there stripped the release of the very distributions that earlier step exists to secure. The publish step carried no `skip-existing`, so any retry replayed an upload of an immutable version, died, and skipped the `github-release` job that `needs` it, leaving the release unsigned with no way to finish it. A retry also rebuilt the distributions instead of reusing the ones already published, and with an unpinned build backend the same commit can build differently, so GitHub would serve — and Sigstore attest — bytes that differ from PyPI's. And `skip-existing` on its own cannot tell a genuine recovery from a retry of a run that failed *because* an earlier commit had published that version, where skipping would carry a newer commit's build into `--clobber` over the tagged release. The fixes, in order: scope the signature upload to `dist/*.sigstore.json`; turn `skip-existing` on only for a re-run or a manual dispatch; reuse a build an earlier attempt of the same run stored, failing outright when it has expired; and refuse to touch a release whose tag names a different commit, reading "no such tag" as the recovery path it is and any unanswerable call as a reason to stop. The release is now also tagged at the commit it ships, named explicitly with `--target`: `gh release create` otherwise tags the default branch's head, which on a publish from a `pre-release/v*` merge or a `release/vX.Y.Z` dispatch is not the commit that was built at all, and which any unrelated merge landing in between can move.
