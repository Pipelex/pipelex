...
- **A failed publish can be recovered without damaging what already shipped**: `publish-pypi.yml` scopes the signature upload to `dist/*.sigstore.json` so it no longer clobbers the wheel and sdist attached just before it, turns `skip-existing` on for a re-run or a manual dispatch, and refuses to touch a release whose tag names a commit other than the one being published. A retry no longer rebuilds the dists either: the build job looks for the bytes this version published in three places before it builds anything — the build an earlier attempt of this run stored, then an artifact an earlier run stored for the same commit, then the files PyPI itself serves, each one verified against the sha256 digest PyPI publishes beside it — and builds fresh only once PyPI has answered that nothing was published under this version. That makes the release assets the bytes PyPI serves rather than a rebuild that probably matches them, which an unpinned build backend and an unpinned interpreter cannot promise, and it turns an expired artifact from a dead end into one more layer to fall through. The release is now tagged at the commit it ships, named explicitly with `--target`, rather than at the default branch's head.
