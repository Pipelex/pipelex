...
- **Every preset in the shipped deck declares `temperature = 1` (Breaking)**: the whole GPT-5.6 range and `gpt-6-astra` fix their temperature at 1, so each preset states that value instead of one the worker would override while warning on every call. Presets are therefore distinguished by their model tier and their reasoning effort alone: `$writing-factual` and `$writing-creative` issue the same call, and `$engineering-structured` runs at temperature 1. A method that needs a temperature of its own should name a model that accepts one directly, such as `gpt-4o`.
