...
- **Template preprocessor: inline `@<ident>` only raises when `<ident>` is a declared input of the surrounding pipe.** The strict line-bounded `@` rule introduced earlier in this release was hostile to coding agents authoring HTML/CSS templates — `<style>@media (...) { ... }</style>`, `@font-face { ... }`, and the like raised at load time and required `@@` escapes everywhere. The validator is now gated by the pipe's declared `inputs`: an inline `@<ident>` raises only when the candidate's root identifier matches a declared input (a real typo). Other inline `@` candidates — CSS at-rules (`@media`, `@import`, `@keyframes`, `@font-face`, `@property`, …), Python/Java decorators (`@deprecated`, `@Override`), and any other shape that doesn't collide with an input — pass through silently. The line-bounded rewriter (alone-on-line `@var` → `{{ var|tag("var") }}`) is unchanged and remains inputs-agnostic. `preprocess_template(template, *, declared_inputs=...)` gained a keyword-only `declared_inputs: set[str] | None` parameter (default `None` = lenient, no validation); a new pair of helpers `validate_template_sigils(template, declared_inputs)` and `rewrite_template_sigils(template)` exposes the validate and rewrite steps separately. `render_template` (runtime) now calls `rewrite_template_sigils` directly — templates are already validated at load time, so the render path doesn't need the `declared_inputs` context. All pipe blueprints (`PipeLLMBlueprint`, `PipeComposeBlueprint`, `PipeSearchBlueprint`, `PipeImgGenBlueprint`, `PipeComposeFactory`) and template analyzers (`TemplateDocumentAnalyzer`, `TemplateImageAnalyzer`) thread their declared inputs into the validator; the base `TemplateBlueprint` and `ConstructBlueprint`-discovery validators remain lenient (no inputs context at that layer — the surrounding pipe's validator catches typos). `@@`/`$$` escapes still work as the explicit literal-character opt-out.
