...
- **A submission-time reachability probe over remote inputs**: before a live run starts, every http(s) URL a `Document` or `Image` input carries gets one HEAD (a ranged one-byte GET where HEAD is rejected), all in parallel, under `[interpreter.pipeline_execution.input_reachability]` (`is_enabled`, `timeout_seconds`). A link that is unambiguously dead — the host does not resolve or refuses, or answers 404/410 — refuses the run with the caller-facing `PipelineInputUnreachableError` before anything is spent; any other answer, a bot wall's 403 or a slow host included, passes with a warning and the real fetch decides. It runs in the process that accepts the request, never as workflow code, and only over the caller's own inputs.
