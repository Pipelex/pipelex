...
- **The corpus's invalid axis: an `error.*` tag namespace and the entries that cover it.** The MTHDS Test Corpus covered the valid language surface; the faults it can report were untagged and unexercised. `vocabulary.toml` now carries an `error.*` namespace generated from `VALIDATION_ERROR_TYPES`, and the corpus carries a focused invalid entry for every tag in it that a `.mthds` bundle can actually produce — each one authored to trigger exactly one error, and gated on doing so. The namespace needed no rule of its own despite its registry mixing two spellings: the existing normalization leaves an already-snake_case stage code untouched and turns `DryRunError` into `error.dry_run_error` exactly as it turns `YesNo` into `native.yes_no`. Five codes are excluded, each with the measurement behind it rather than an argument from the call graph — `circular_dependency_error` and `unknown_concept` are unreachable through bundle validation (the first is raised only by the builder's spec-to-blueprint conversion, the second is shadowed by the concept-reference resolution that reports the same fault as `unresolved_concept`), `optional_force_redundant` is advisory-only and rides the `warnings` array, and the two `unknown_*` fallbacks name the absence of a diagnosis rather than a language fault.
- **The exhaustivity gate now requires an invalid entry to agree with itself.** Such an entry states its fault twice — as `expected_error`, the wire string, and as an `error.*` tag in `covers`, the normalized form — and nothing checked that the two matched, so an entry could advertise coverage of a fault it does not produce. The check joins through the vocabulary's own `code` field rather than re-normalizing, so it carries no second copy of the normalization rule. The same claim is gated from the other side too: a valid entry may not carry an `error.*` tag, since its bundle produces no diagnostic and the exhaustivity arm would otherwise count a claim nothing honours.
...
- **The same bundle produced different diagnostics depending on how it reached the validator**, and the library-directory route — the one the CLI and every local tool take — was the lossy one. Validating in-memory contents (what the HTTP `/validate` layer does) reported a pipe or concept fault as a categorized item with its `error_type` and locators; validating the identical bundle from a file resolved against a library directory reported one untyped residual carrying only a message. Two arms in `LibraryManager._load_mthds_files_into_library` were destroying the structured data on the way out: a pydantic `ValidationError` was rewrapped as a bare `LibraryError`, which routes to the error cascade's structured-forwarding arm rather than its categorizing arm, so nothing ever categorized it; and a `ConceptLibraryError` was re-raised message-only, dropping the per-reference items it now carries. In practice `unresolved_concept`, `optional_input_unguarded` and `optional_output_required` were simply unreachable from a file on disk. Both arms now preserve what they were discarding, and a new parity test validates the same bundle through both entry shapes and requires the two to agree.
