...
- **Storage config validation now covers every provider and requires a literal `{hash}` placeholder in `uri_format`.** GCP's `lazy_validate` accepted any uri_format containing the bare substring `hash` (e.g. `assets/myhash/file`), which has no substitution slot and would render the same URI for every stored object — silent storage-wide overwrites; the local and in-memory configs had no validation at all, leaving the same misconfiguration uncaught. The per-provider checks and the operator-facing error message now live on a shared config base (so the providers can no longer drift apart), the storage provider factory validates local/in-memory configs the same way it already did S3/GCP, and `StorageProviderConfig.storage_path` dispatches on the configured method instead of blaming a missing local config when the method is S3/GCP.
