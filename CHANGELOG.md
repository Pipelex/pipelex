...
- **Minimum `mthds` version**: The floor moves to `mthds>=0.11.0`, the releases carrying that split and the descriptor's coherence rules — a top-level field whose `required` contradicts its presence marker, one pairing `gating: true` with an optional marker, and a wire node spelling absence as an explicit `null` are now rejected at the parse, which gates this engine's emission at derivation time rather than on the wire. Everyone downstream of Pipelex inherits that floor.
