...
- **Breaking: `pipelex.hub` is gone, split into `pipelex.service_hub` and `pipelex.method_hub`.** There is no alias shim — a stale import fails loudly at import time rather than silently resolving to the wrong layer. `service_hub` holds process-scoped infrastructure (config, console, secrets, storage, telemetry, models, inference, content generation, reporting, run mode, tracing, the plugin registries, `get_class_registry`); `method_hub` holds the loaded method's machinery (library manager and lookups, the current-library contextvar, library dirs, pipe router, pipe run and pipeline manager, the PipeFunc executor). `get_pipelex_hub` splits into `get_service_hub` / `get_method_hub`, so every external call site must pick the container it actually meant. The rule is one-way and mechanically enforced (`make check-hub-layering`, alias `chl`, in `make agent-check` and CI): `method_hub` may import `service_hub`, never the reverse, and no module anywhere may reference `pipelex.hub` — in an import *or* a string literal (dynamic import, mock patch target, config path). The payoff is measurable and pinned by a subprocess import-closure test: importing the inference layer now loads **zero** interpreter modules, down from 50. See [Hub layering convention](contribute/hub-layering.md).
