...
- **`MTHDS_PROTOCOL_VERSION` is now a re-export of the SDK's `PROTOCOL_VERSION`** (`mthds.protocol.protocol`) instead of a hardcoded duplicate — the SDK constant is the single source of truth for the protocol version. The re-export survives only until the hosted API imports the SDK constant directly.
