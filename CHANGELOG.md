...
- **Unit tests for inference plumbing, core runtime, and tools/config internals.** Offline coverage for the layers around inference and execution that can break independently of any provider: the structured-output ↔ instructor mode mapping, backend-credentials error messages, model-deck reference checks (presets/aliases/waterfalls/handles with fuzzy suggestions), the per-provider image-gen argument factory (Flux/Qwen/SDXL/GPT-image taxonomies), the LLM and image-gen worker routing factories (SDK dispatch, client caching, missing-dependency errors), gateway completions request-shaping and extract-output parsing (Azure/Mistral/Deepseek/Linkup), and the Mistral factory (messages, OCR extract output, document prep/upload). Core runtime: the local observer JSONL sink, the output renderer's Anything-concept resolution, the bundle-level graph rendering dispatch, the builder bundle-spec validation/`to_blueprint()`/pretty rendering and inputs operations, and the pipeline runner's error paths and MTHDS protocol surfaces (`validate`/`models`/`version`). Tools: the TOML config-sync engine (structure/comment preservation, dry-run, idempotency), document/image format enums, PIL image conversion, and the storage config validators.
...
- **Keyword-only arguments — enforcement guard + Wave 1 leaf packages (breaking for affected signatures).** Non-subject function parameters across the `pipelex/` runtime are being made keyword-only so call sites are self-documenting (`copy_file(src, target_path=dst, overwrite=False)` over `copy_file(src, dst, False)`). The convention is mechanically enforced by a new `pipelex-dev check-keyword-only` AST guard wired into the `make check` / `make agent-check` family, shipping red-to-green against a committed baseline that shrinks as each wave lands. This entry covers Wave 1 — the low-risk leaf packages `tools/`, `errors/`, `reporting/`, `observer/`, `tracing/` — whose helper signatures now place a bare `*` after the subject (every argument after the first must be named at the call site). Jinja2 filter functions (`@pass_context` / `@pass_environment` / `@pass_eval_context`) are a new framework-entrypoint carve-out: the Jinja2 engine invokes them positionally from template syntax, so their arguments stay positional. The public surface (`pipelex.py`, `hub.py`) is deliberately untouched here and lands in the final, most careful wave.
- **Keyword-only arguments — Wave 2 domain core (breaking for affected signatures).** The domain-core packages `core/`, `language/`, `kit/`, and `libraries/` now follow the keyword-only convention: every function/method places a bare `*` after its subject, so all later arguments must be named at the call site. This is mostly a call-site refactor — adding the `*` is the easy part; the value is that callers tree-wide now read self-documentingly (`generate_representation(concept_ref, structure_class=Channel)`, `update_cursor_rules(repo_root, kit_index=..., agent_set=...)`). Because several kept-keyword-only changes land on base classes and protocols (`PipeAbstract._validate_before_run` / `_validate_after_run` / `_register_execution_data`, `StuffContent.rendered_markdown` / `rendered_markdown_async`, `LibraryManagerAbstract` loaders, `PipeFactoryProtocol`, the `StructureGenerator` codegen surface), their `@override` implementations across `pipe_operators/`, `pipe_controllers/`, and `pipe_signature/` adopt the matching `*` for type-checker parity — those packages' own non-override signatures are still converted in their later waves. The five `render_with_images` content implementations (`ImageContent`, `ListContent`, `StructuredContent`, `TextAndImagesContent`, `StuffArtefact`) now match the already-keyword-only `ImageRenderable` protocol on `text_format`.
- **Keyword-only arguments — Wave 3 inference layer (breaking for affected signatures).** The inference-layer packages `cogt/` and `plugins/` now follow the keyword-only convention: every function/method places a bare `*` after its subject, so all later arguments must be named at the call site (`build_mock_objects(item_class, count=...)`, `model_cost_per_token(costs, cost_category=...)`, `suggest_model_alternatives(model_deck, model_type=..., name=..., kind=...)`). As in earlier waves the bulk is call-site keywordization tree-wide; the cascade is heaviest where the now-keyword-only `ContentGeneratorProtocol`, `LLMWorkerAbstract` / `ImgGenWorkerAbstract` / `SearchWorkerAbstract`, `PluginFactoryAbstract`, and the `OpenAICompletionsFactory` / `OpenAIResponsesFactory` base `make_extras` force their `@override` implementations across `plugins/` and the Temporal in-workflow content generator to adopt the matching `*`. Three callback surfaces are carve-outs: the dry-mock `_ReportLLMJobFunc` callback type became a keyword-only `Protocol` (a `Callable[[...]]` cannot express keyword-only parameters) so its callbacks stay keyword-only end to end, while `SchemaToModelFactory._restricted_import` — installed as the exec sandbox's `__import__` and therefore invoked positionally by the interpreter — and `DryRunFactory._main_pipe_from_pipe_dict` — registered as a polyfactory `PostGenerated` callback that the library invokes positionally as `fn(name, values)` — both stay positional and are marked `# kw-only: ignore` (the same runtime-entrypoint category as the Jinja2 filter carve-out).
- **Keyword-only arguments — Wave 4 execution path (breaking for affected signatures).** The execution-path packages `pipe_operators/`, `pipe_controllers/`, `pipe_run/`, `pipeline/`, `graph/`, `runtime_bridge/`, and `pipe_signature/` now follow the keyword-only convention: every function/method places a bare `*` after its subject, so all later arguments must be named at the call site (`make_llm_prompt(blueprint, working_memory=...)`, `run_pipe_via_bridge(pipe_input, pipe_run_mode=..., dry=...)`, `make_from_graphspec(graph_spec, graph_config=..., title=...)`, `copy_for_child(node_id, next_sequence=...)`). As in earlier waves the bulk is call-site keywordization tree-wide (including callers in `tests/` and `temporal/`); the override cascade is heaviest where the now-keyword-only `PipeController._live_run_controller_pipe` / `_dry_run_controller_pipe`, `PipeOperator._live_run_operator_pipe` / `_dry_run_operator_pipe`, the `GraphTracerProtocol` surface, and the `PipeRouterProtocol` / `PipeRunProtocol` force their `@override` implementations across the pipe-operator and pipe-controller subclasses to adopt the matching `*`. No new framework carve-outs were needed; the one nested closure handed to `functools.partial` (`PipeBatch._run_branch`) keeps its subject positional and now passes its remaining argument by keyword through the partial.
- **Keyword-only arguments — Wave 5 framework-sensitive packages & public API (breaking, including the public surface).** The final wave converts the framework-sensitive packages `builder/`, `temporal/`, `system/`, `cli/`, and the top-level public surface (`pipelex.py`, `hub.py`) to the keyword-only convention, bringing the whole `pipelex/` source tree to zero known violations — the guard's burn-down baseline is now empty. **Public API — breaking for downstream callers:** `Pipelex.make()`, `Pipelex.setup()`, and `PipelexHub.setup_config()` keep their first parameter positional (`integration_mode` / `config_cls`) but every later argument is now keyword-only — a caller passing a second-or-later argument positionally (e.g. `Pipelex.make(IntegrationMode.PYTHON, False)`) must switch to keyword form (`Pipelex.make(needs_inference=False)`); the common no-arg and all-keyword calls (`Pipelex.make()`, `Pipelex.make(needs_inference=False)`) are unaffected. Downstream consumers to check: `pipelex-api`, `pipelex-worker`, `n8n-nodes-pipelex`, cookbook, starter. The framework entrypoints the guard already carves out (Typer commands, Temporal `@activity.defn` / `@workflow.*` handlers, pytest fixtures) are untouched. Additional runtime-callback surfaces stay positional and are marked `# kw-only: ignore` because a framework or the interpreter invokes them positionally where the type checker is blind: the Temporal codec aiohttp route handler (`_apply`), the `sys.excepthook` handler (`DualClientExceptionCapture._exception_handler`), the two PostHog `on_error` callbacks (`TelemetryManager._handle_transmission_error` / `_handle_pipelex_transmission_error`), and the pytest hookimpl `pytest_collection_modifyitems`; the lone `threading.Thread` target (`serve_until_callback`) instead stays convention-compliant by passing its bound argument through `kwargs=` at the call site.
- **Keyword-only arguments — fully enforced; migration scaffolding removed.** With the whole `pipelex/` source tree converted (Waves 1–5), the convention is now permanently enforced rather than burned down: the migration baseline file and the `pipelex-dev check-keyword-only --regen-baseline` flag are removed, and the guard hard-blocks on *any* violation. It runs in `make agent-check`, in the `make check` aggregate, and in a dedicated `lint-keyword-only` CI job (gated by the required `Lint (all)` status check), so no non-compliant signature can merge. The convention is promoted to a standing rule documented at `docs/contribute/keyword-only-arguments.md` and summarized in the contributor `CLAUDE.md` / `AGENTS.md`. A genuinely justified exception still uses the inline `# kw-only: ignore` escape hatch.
- **Keyword-only arguments — taste pass on subject-positional signatures (breaking for affected signatures).** A follow-up review of the Exception 1 "the subject may stay positional" permission found signatures where the leading positional was not genuinely the object of the function, so it had been kept positional by habit rather than by meaning. Those are now fully keyword-only (`def f(*, a, b)`): co-equal or mutually-exclusive inputs (`make_variable_multiplicity`, `make_qualified_structure_class_name`, `CostRegistry.compute_total_cost`, `BoundingBox.make_from_two_corners`); an inverted subject where the real object was a later argument (`report_validation_error`); the domain-namespace-first factories (`ConceptFactory.make_concept_ref_with_domain` / `make_from_blueprint`, `PipeFactory.make_pipe_ref_with_domain` / `make_from_blueprint`, `InputStuffSpecsFactory` / `StuffSpecFactory.make_from_*`); the name/key-first setters (`WorkingMemory.set_stuff` / `add_new_stuff`, `MultiObserver.add_observer`, `TemplateLoader.register_set`, `TemplateRegistry.register`); and the source/target & symmetric-comparison pairs that also carry trailing options (`copy_file`, `mirror_dir`, `sync_toml_values`, `copy_directory_structure`, `has_diff_dirs`, `make_diff_dirs_pretty`, `is_multiplicity_compatible`, `are_concept_compatible`, `contracts_match`, `_display_diff`, the graph tracer's `add_edge` / `_add_generated_edge`). A clean symmetric pair with no trailing options instead joins the Exception 2 symmetric-tuple allowlist and stays fully positional (`are_classes_equivalent(class_1, class_2)`). The convention doc's Exception 2 section now states the operating rule (clean pair → allowlist; pair-with-options → all-keyword).
- **Keyword-only arguments — second subject-correctness pass (breaking for affected signatures).** A deeper review of the Exception 1 permission (via an AST detector that flags functions whose name's head noun — the verb's object — sits in a keyword-only parameter rather than the positional subject) found more signatures where a discriminator, destination, or flag had been left in the subject slot while the real object was keyword-only. Fixes split two ways. **Reversed to the correct subject:** `parse_pipe_spec(spec_data, *, pipe_type)` (was `(pipe_type, *, spec_data)` — the data is parsed, not the type; now matches `parse_concept_spec(spec_data)`), `hydrate_content(raw_content, *, concept)`, `categorize_blueprint_validation_error(error, *, blueprint_dict)`, `_row_to_dict(data_row, *, header)`, and the writers `write_manifest(manifest, *, deck_dir)` / `save_bytes_to_binary_file(byte_data, *, file_path)` (data-first, matching `save_text_to_path(text, *, path)`). **Made fully keyword-only** (no single genuine subject): `write_rows`, `display_health_report`, `customize_backends_config`, `ConfigLoader.load_config`, `LLMAssignment.make_from_prompt`, `StructureGenerator.generate_from_structure_blueprint`, and the IO/SDK-receiver helpers `setup_telemetry`, `display_selected_backends`, `prompt_primary_backend`, `MistralFactory.upload_file_to_mistral_for_ocr`. A deeper sweep of the same pass continued the cleanup. **Further reversed to the correct subject:** `StuffFactory.combine_stuffs(stuff_contents, *, concept, name)` (the contents are combined; `concept` is the target type), `MigrationConfig.text_in_renaming_keys` / `_values(text, *, category)` (the text is processed; category is a selector), `ModelManager.build_deck(model_deck_blueprint, *, enabled_backends)` (built from the blueprint; backends filter it). **Further made fully keyword-only:** the whole `ImgGenParamSupport.check_*` family (the value is what's checked, `rules` is the criteria), `validate_pipe_in_bundle` (+ its agent-CLI `_core`), the entire `ContentGeneratorProtocol` generator family (`make_llm_text` / `make_object` / `make_object_list` / `make_single_image` / `make_image_list` / `make_templated_text` / `make_render_page_views` / `make_extract_pages` / `make_search_structured`, plus both implementations — `job_metadata` was already passed by keyword everywhere because the `@update_job_metadata` decorator requires it in kwargs), the LLM worker hooks `_after_text_job` / `_after_object_job`, `DeliveryExecutor._notify_webhook`, `execute_delivery`, and `load_template`. All are internal except `parse_pipe_spec`, which the hosted runner API (`pipelex-api`) calls — that consumer is updated in lockstep.
- **Canonical protocol validation report (breaking).** `PipelexValidationReport` is reworked into the canonical typed shape shared by every Pipelex backend and moved to `pipelex/pipeline/validation_report.py`: `bundle_blueprint` (the batch's primary blueprint — first declaring `main_pipe`, else first; replaces the untyped single-or-list `blueprint` dump), `pipe_io_contracts` keyed by namespaced `pipe_ref` with typed `PipeIOContract` entries (per-pipe input JSON Schemas + output concept/multiplicity — new module `pipelex/pipeline/pipe_io_contracts.py`, ported from the hosted API's route-local builder), `validated_pipes` (per-pipe sweep outcomes, the same entries as the agent-CLI envelope), plus the existing `pending_signatures` / `is_runnable` verdict. The report is assembled in exactly one place — `build_validation_report` — that every backend calls, and primary-blueprint selection is one shared rule (`select_primary_blueprint` in `pipelex/pipeline/blueprint_selection.py`, also folded into the Temporal validate activity).
- **`validated_pipes` entry key `pipe_code` → `pipe_ref` (breaking).** The entry has always carried the namespaced `pipe_ref` (`domain.code`) under a key named for the wrong identity; the key now says what the value is. Applies to the agent-CLI / builder validate envelopes and the protocol report.
- **Protocol `validate` now produces a best-effort `graph_spec` — the local runtime catches up with the hosted route.** When the validated batch declares a `main_pipe`, `PipelexMTHDSProtocol.validate` dry-runs it in-process via `dry_run_pipe_in_process` against the still-open validation library and ships the resulting `GraphSpec` on the report — the same single-library-load pattern and best-effort degrade contract as the Temporal validate activity, enforced by ONE shared implementation (`best_effort_graph_spec`) both backends call: a graph-arm domain failure (`PipelexError`, pydantic `ValidationError`, polyfactory `FactoryException`) yields `graph_spec=None` with validation still successful; no declared `main_pipe` means no graph. To make that wiring importable, `dry_run_pipe_in_process` moved to its own module `pipelex/pipe_run/dry_run_in_process.py`: its old home `dry_run_pipeline.py` imports the protocol runner, so sharing a module would have been an import cycle.
- **The Temporal validate activity result carries the full worker-side artifact set.** `DryValidateResult` gains `pending_signatures` (the runnability verdict crosses the boundary at last) and `pipe_io_contracts` (typed `PipeIOContract` entries computed activity-side against the worker's loaded library — the API side never needs to re-acquire a library to render schemas). Both fields are required on the wire: a version-skewed worker that doesn't emit them fails loudly at deserialization instead of defaulting to a silently wrong runnability verdict. Breaking for the wire model; Temporal validate has never shipped to prod.
- **`MTHDS_PROTOCOL_VERSION` is deleted (breaking).** The hardcoded duplicate of the protocol version is gone from `pipelex/pipeline/runner.py`; the SDK's `PROTOCOL_VERSION` (`mthds.protocol.protocol`) is the single source of truth and consumers (including the hosted API) import it directly.
- **Additive multi-file library construction.** Merge-time rules now support building a same-domain `.mthds` library as separate, additive files (forward-declared headers + separate definitions), enabling parallel top-down method construction:
  - A `PipeSignature` and a concrete pipe with the same code reconcile — the concrete pipe satisfies the signature instead of raising a duplicate-code error. `PipeSignature` works like a forward declaration ("header"), its concrete pipe like the definition; their `inputs`/`output` contracts must match. Contracts are compared by *normalized concept identity*, so a header's bare `Brief` reconciles with a definition's domain-qualified `thisdomain.Brief` (and native `Text` with `native.Text`); multiplicity is compared structurally. Two concrete pipes with the same code, or two declarations with genuinely different contracts, remain errors.
  ...
  - Domain `description` / `system_prompt` now merge order-independently across same-domain files: a membership-only file that omits them defers to whichever file declares them, instead of warning and racing on filesystem load order. The root's header therefore always wins over siblings that carry only `domain = "..."`. Two files declaring *different* non-empty values still keep the first and warn — a genuine double-declaration to resolve.
  - **`pending_signatures` + `is_runnable` on the validate result.** A successful `validate bundle` (notably a lenient `--allow-signatures` run) now reports the library-wide set of pipes still declared as `PipeSignature` — the unimplemented headers left to satisfy. Surfaced as a `pending_signatures` array plus an `is_runnable` boolean on the agent-CLI / builder validate JSON envelope, and in markdown as an explicit runnability verdict: a complete bundle states it is runnable, while a bundle with pending signatures states it is not yet runnable immediately above the verbatim "Pending signatures" section — so a top-down build (and the LLM driving it) can see exactly what remains without inferring completeness from an absent section. The protocol-level `validate` (`PipelexMTHDSProtocol`) reports the same verdict: `pending_signatures` + `is_runnable` are now Pipelex extension fields on `PipelexValidationReport`, so a build driving the runtime through the MTHDS Protocol sees it too. (The hosted runner's `/validate` builds its own response envelope and needs its own change to surface the verdict — it does not inherit these fields.)
- **`PipeSignature` evicted from the executable pipe taxonomy.** A signature is a contract, not a way of running, so it is no longer a `PipeType` / `PipeCategory` member: `PipeType.PIPE_SIGNATURE` and `PipeCategory.PIPE_SIGNATURE` are **removed** (breaking — any code matching on `PipeCategory.PIPE_SIGNATURE` must drop that arm). It stays a `PipeAbstract` subclass (the dry-run-only shim): a signature now carries `type = "PipeSignature"` with `pipe_category = None`, and `is_signature` is a class fact (base `False`, overridden to `True` on `PipeSignature` / `PipeSignatureBlueprint`) rather than an enum read. `signature_for` is retained for tooling; setting `signature_for = "PipeSignature"` is still rejected, now structurally (it is not a valid `PipeType`) rather than by a dedicated validator.

### Fixed

- **The protocol model deck no longer loses aliases to cross-category collisions.** `PipelexModelDeck.aliases`/`waterfalls` were flat maps built by `update()`-ing the per-category maps together — the same alias name in several categories (e.g. `default-small` for both llm and extract) silently kept only the last category's entry, dropping aliases from the unfiltered deck. Both extensions are now keyed by model category (`aliases: {category: {alias: model}}`), preserving every entry; the protocol's flat `models` list is unchanged (each entry already carries its category as `type`). Breaking for the deck's extension shape.
- **JSON-Schema rendering failures on the validate surfaces are now structured errors.** A pydantic schema-generation failure while building `pipe_io_contracts` (an unschematizable structure-class field) surfaced as a raw third-party exception — which the Temporal error boundary would not convert, so Temporal retried a deterministic failure and the caller got an opaque infra error instead of a validation-shaped one. `build_pipe_io_contracts` now converts it to the new `PipeIOContractError` (with the offending pipe/input/concept named), non-retryable at the validate workflow tier like `ValidateBundleError`.
- **Protocol `validate`'s library teardown can no longer mask the real error.** If the body failed mid-window and the validation library's teardown then also raised, the teardown error replaced the in-flight one — the caller's failure named teardown instead of the actual problem. The wrapper now suppresses a teardown raise while a body error is propagating (and still raises it when the body succeeded), and captures the validation library id once — right after `validate_bundle` leaves it current — so the graph arm and the teardown always target the same library. Mirrors the Temporal validate activity's existing guards.
- **Empty `mthds_contents` rejected with a structured error.** `validate_bundle` (and therefore protocol `validate`) accepted an empty `mthds_contents` list — it slipped past the provided-params check and crashed the canonical report's primary-blueprint selection with a raw `IndexError`. It now raises a `ValidateBundleError` at entry, and `select_primary_blueprint` itself guards the empty batch with the same structured error. The main-pipe selection rule is also now genuinely single-implementation: the hand-rolled first-declaring-`main_pipe` loops in `execution_seams`, `dry_run_pipeline`, and `inputs_ops` were folded into `select_primary_blueprint`.
...
- **Concepts-only loading validates concept references.** Loading a bundle with concepts only (the `pipelex structures` path / `load_concepts_only`) skipped the cross-file concept-reference check that the full load runs, so a structure field pointing at an undeclared concept (`concept_ref` / `item_concept_ref`) was silently accepted, leaving an invalid schema in the library. It is now validated on the concepts-only path too, matching the full load. The concepts-only library-directory loader batches all sibling files into one validation pass (like the directory loader already did), so a structure field referencing a concept declared in a sibling resolves regardless of file load order.
...
- **OpenAI image moderation mapping was inverted.** `is_moderated = true` sent OpenAI's gpt-image `moderation="low"` (the LESS restrictive setting) and `false` sent `"auto"` (standard filtering) — the exact opposite of the documented "enable content moderation" intent. The mapping now matches the flag: enabled → `"auto"`, disabled → `"low"`. Alongside the mapping fix, `ImgGenSetting.is_moderated` now defaults to `None` ("no explicit choice") instead of `False`: workers omit the moderation/safety-checker parameter so the provider's own default applies (OpenAI's documented default is `auto`). Without this, model-deck alias/handle resolution — which builds settings without the field — would have started sending the less restrictive `low` by default once the mapping was corrected. For FAL models this also means `enable_safety_checker` is no longer explicitly disabled by default; the provider default applies there too.
- **Storage config validation now covers every provider and requires a real `{hash}` replacement field in `uri_format`.** GCP's `lazy_validate` accepted any uri_format containing the bare substring `hash` (e.g. `assets/myhash/file`), which has no substitution slot and would render the same URI for every stored object — silent storage-wide overwrites; the local and in-memory configs had no validation at all, leaving the same misconfiguration uncaught. The check parses the format string: every placeholder must be a plain `{name}` (escaped `{{hash}}`, format specs like `{hash:.0}` or `{primary_id:.0}`, conversions, and attribute/index access are all rejected — on any field they render constant, colliding, or oversized keys, so the rule is enforced uniformly rather than only on `{hash}`), placeholders outside the supported set (`{primary_id}`, `{secondary_id}`, `{hash}`, `{extension}`) are rejected at config time instead of crashing with `KeyError` at content-store time, a `{hash}` placeholder is required, a test rendering catches anything parsing can't, and bucket configs must set a positive `signed_urls_lifespan_seconds` (or `"disabled"`) so signed URLs can't be born dead. Operator note: this validation runs when the storage provider is constructed at Pipelex boot — a misconfigured `uri_format` (including for the default local method) now fails fast at startup rather than at the first content store. The per-provider checks and the operator-facing error message now live on a shared config base (so the providers can no longer drift apart), the storage provider factory validates local/in-memory configs the same way it already did S3/GCP, and `StorageProviderConfig.storage_path` dispatches on the configured method instead of blaming a missing local config when the method is S3/GCP.
- **Mistral chat requests now send the system message before the user message.** `MistralFactory.make_simple_messages` appended the system message after the user message, contradicting its own docstring and the OpenAI-typed sibling method.
- **String concept values in bundle specs are now constructible and load correctly.** Validating a string against the `ConceptSpec | str` union crashed in a `mode="before"` validator that assumed dict input, and `to_blueprint()` mapped the string into the blueprint's `structure` field, where the concept loader rejects anything that isn't a registered structure class name. A string concept value now validates cleanly and passes through to the blueprint as the concept's description.
- **LocalObserver JSONL records always carry the true lifecycle event name.** A payload carrying its own `event_type` key could silently overwrite the event name in the written record, breaking event-type-based filtering for JSONL consumers; the event name now wins the merge.
...
- **Tests under `tests/**/build/` directories were silently skipped.** pytest's default `norecursedirs` includes `build`, so the whole `tests/unit/pipelex/cli/commands/build/` directory (mirroring the source layout) was never collected in full runs — including a pre-existing cross-package refines regression test. The pytest config now overrides `norecursedirs` to the defaults minus `build`, and sets `testpaths = ["tests"]` so bare `pytest` runs never scan outside the test tree.
- **`Anything`-output option numbering is now deterministic.** The output renderer iterated a pipe-dependency set when collecting a `PipeCondition`'s possible outputs, so the user-facing `output_option_N` / `schema_option_N` numbering could vary between runs; dependencies are now sorted by pipe code.
...
- **`--dry-run` now honors the configured backend — the mock moved from the operators to the cogt leaf.** `run_mode` rides a new `CogtRunParams` carrier (derived from `PipeRunParams.run_mode`, stamped on every cogt assignment, serialized across the Temporal wire), so a DRY run on a Temporal backend dispatches the REAL `act_llm_gen_*` / extract / img-gen / search / render activities and the leaf mocks **inside** the activity — testing dispatch, scheduling, serialization, routing, and cross-worker propagation at zero AI cost, with no API keys and no storage IO (the img/extract DRY branches sit above the store step). `ContentGeneratorDry` is deleted: operators no longer swap generators (the base operator's dry path simply reuses the live path), and the in-process validation scopes pass an inline `ContentGenerator`. Object mocks are now schema-built on every backend — identical mock on direct and Temporal; classes with exotic format constraints should declare `examples`/`mock_format` (a re-validation failure surfaces as a typed `DryRunObjectFidelityError`, and a deterministic mock-build failure as the new non-retryable `DryRunMockBuildError`). The acceptance gate is Tier 17 of the `temporal-e2e-validate` skill (GREEN on all leaf families + keyless worker, RED-proven) plus a Mode-1 pytest.
- **Unified dry run — `--mock-inference` removed (breaking).** There is now exactly one non-live run mode: `--dry-run`. The mock-inference mode is retired; its capability — non-zero synthetic usage so the cost report renders, used to validate cross-worker cost-report assembly cheaply — survives as an **internal** `is_mock_usage` sub-flag of DRY on `CogtRunParams` (replacing `is_mock_inference`; setting it on a LIVE run is a validation error). It is exposed on the Python surface (`PipelexMTHDSProtocol` / `execute` / `prepare_pipe_job` via `is_mock_usage=...`) and as a hidden test-only CLI trigger, deliberately undocumented. Dry-run leaf coverage was already uniform across every operator, so `MockInferenceUnsupportedError` and its img-gen/extract/search guards are deleted; `MockInferenceObjectFidelityError` is renamed `DryRunObjectFidelityError` (mock-built objects are DRY-only now); the `mock_inference` usage sentinel is renamed `mock_usage`.
...
- **Temporal runs without graph events skip the guaranteed-empty trace-flush activity.** Post-fix, the workflow's in-memory trace buffer is populated only by inline (workflow-thread) emissions — and with the unified dry run mocking inside the leaf activities, the only inline source left is graph events when graph tracing is on. Every usage emission (LIVE, DRY, or mock-usage) happens activity-side through the per-process event log, so scheduling `act_flush_trace_events` with an empty payload was a wasted activity round-trip per workflow in the fan-out (history events, task-queue dispatch, worker slot — including every child workflow spawned by controllers). The schedule is now gated payload-purely on `emit_graph_events`, so the decision replays identically and the command stream stays a pure function of the payload.
- **The "runner-side usage emission engaged" log is now INFO and states the actual contract.** Since activity-side usage emissions always go through the per-process activity event log (by design, co-located or split deployment alike), the old per-process WARNING — which claimed the fallback was specific to split deployments — fired on every standard deployment with a false explanation. Renamed to `log_once_runner_fallback_engaged`, logged once per process at INFO with the writer id.
...
- **The temporalio extra is no longer imported on every pipelex boot.** `reporting_manager.py` (imported by `pipelex.pipelex` on every `Pipelex.make()` / CLI invocation) eagerly imported `temporalio.activity` at module level to detect in-activity usage emissions — putting the entire temporalio import (Rust bridge, protobuf, ~130ms measured) on every boot's critical path wherever `pipelex[temporal]` is installed, including processes that never touch Temporal, and silently breaking the runtime bridge's documented lazy-import contract. The gate now sniffs `sys.modules` instead: a process that never imported `temporalio.activity` cannot be inside a Temporal activity (the activity context is set by temporalio's own machinery, which requires the module). Worker processes are unaffected. A subprocess boot-cost guard pins the lazy-import contract.

- **Per-run Temporal worker-local state is keyed by run id, not workflow id.** The per-workflow library (`wf_{run_id}`), the graph tracer key, and the report-delegate event-log context key now use `workflow.info().run_id` — replay-stable but unique per run — instead of the workflow id. Workflow ids are reused across workflow-level `retry_policy` attempts, Temporal reset, and resubmission of the same `pipeline_run_id` (workflow ids derive deterministically from it), so workflow-id keying let a closed predecessor run's late eviction cleanup tear down a live successor run's library, tracer, and usage context on the same worker — failing the successor inline. With run-id keying such cross-run collisions are structurally impossible; `open_fresh_library` remains the same-run self-heal. `LibraryManager.teardown` also now forgets a library entry even if the library's own teardown raises (pop-first + `finally`), so a raising teardown can never turn into permanent worker-local poison, and the stale-entry healing path is tolerant of concurrent teardown.
- **An evicted Temporal workflow no longer poisons same-worker reruns through a leaked per-workflow library.** `WfPipeRouter`'s worker-local cleanup (per-workflow library + crate fingerprint, report-delegate event-log context) sat in the `finally` block AFTER the awaited `act_flush_trace_events` activity. An eviction-time interruption raised at that await (`_WorkflowBeingEvictedError` is a `BaseException`) escaped the `except Exception` around the flush and aborted the rest of the `finally`, leaking the library and its crate fingerprint in the worker-local `LibraryManager` under the deterministic `wf_{workflow_id}`. On a same-worker replay or re-run of that workflow id, `open_library` returned the stale library, a fresh `ClassRegistry` was attached, and `load_from_crate` was fingerprint-skipped — so the crate's dynamic classes never landed in the new registry and inline hydration failed where history recorded success: command-stream divergence, and with dynamic-class results on a single-worker queue, a persistent silent hang. Two complementary fixes: `WfPipeRouter` now opens its per-workflow library via the new `LibraryManager.open_fresh_library`, which force-tears-down any pre-existing library under the id (a leftover there can only be an interrupted predecessor), and the workflow `finally` now runs all worker-local cleanup BEFORE the awaited flush activity, which moved last — an eviction can only skip the flush itself, never the teardown. Guarded by an integration test that installs the leaked predecessor state and re-runs the workflow under the same id.
- **Costs-only Temporal runs no longer hang on a routine sticky-cache eviction.** In costs-only tracing mode (`emit_usage_events=True, emit_graph_events=False`), the workflow's in-memory trace buffer was populated exclusively by co-located activity completions writing cross-thread through the process-global `ReportingManager`, and the `act_flush_trace_events` schedule was gated on that buffer being non-empty. Activities don't re-execute on replay, so after a sticky-cache eviction the recreated buffer stayed empty, the recorded flush command was never re-emitted, and Temporal raised `[TMPRL1100]` — a silently hung workflow, on a single worker, with no config drift. Two changes make the command stream a pure function of payload + history: the flush-activity schedule is now gated purely on the payload field `emit_graph_events` (see the Changed entry above; runs without graph events skip the guaranteed-empty flush entirely) — and activity-side usage emissions always go through the per-process runner-fallback event log — the same path used when activities run on a remote worker — instead of the workflow's in-sandbox buffer, even when the activity is co-located with the workflow worker. Guarded by a replay regression test that records a costs-only history and replays it through `temporalio.worker.Replayer`.
- **Per-workflow tracing setup no longer depends on worker-local leak state.** `GraphTracerManager.open_tracer` raised `ValueError` when a stale tracer key was left behind by a prior interrupted execution on the same worker (e.g. a workflow evicted before its cleanup ran), and `WfPipeRouter` papered over that with a best-effort `except Exception` that silently disabled tracing — dropping the recorded flush command on replay and diverging the trace context carried by all downstream commands. `open_tracer` now pops and replaces a stale tracer (tearing it down, with a WARNING), and the catch-all around tracing setup is gone: the setup block is pure in-memory state, so any failure is a real bug that surfaces deterministically.
...
- **`dry_run_pipe_in_process` — graph-producing dry-run that never dispatches.** New entry in `pipelex/pipe_run/dry_run_pipeline.py`: dry-runs a pipe against an already-open library fully in-process and returns its `GraphSpec`, even under a Temporal-enabled hub. Three ContextVar scopes pin the whole run in-process: `scoped_event_log` (in-memory trace transport), `scoped_pipe_router` (nested controller sub-pipes resolve a local router instead of the `TemporalPipeRouter`), and the new `pipelex.hub.scoped_content_generator` (inference leaves resolve an inline dry generator instead of `ContentGeneratorInWorkflow` — holds regardless of whether the DRY mock lives at the pipe level or the leaf). This is the graph half of running `/validate` as one in-process Temporal activity.

- **`scoped_event_log` — a shared, scope-pinned trace-event transport.** New `pipelex.hub.scoped_event_log(event_log)` context manager (with `get_event_log_override()` accessor, mirroring `scoped_pipe_router`): both the write side (tracer emission in `pipeline_run_setup`) and the read side (`assemble_tracing`) now prefer the scoped instance over building their own backend via `make_event_log`, so a run can trace into ONE shared `InMemoryEventLog` — no NDJSON file, no DynamoDB round-trip — and still assemble its `GraphSpec` from it. A set override implies tracing-enabled (honored even when `tracing_config.is_enabled` is off); the scoped instance's `close()` must be idempotent or a no-op (the write-side tracer closes its transport at teardown — `InMemoryEventLog`'s is a no-op). ContextVar-scoped, so concurrent runs with separate scopes stay isolated. This is the foundation for running the `/validate` graph dry-run fully in-process and in-memory inside a Temporal activity.

### Coordination

- **pipelex-worker lockstep** — the Temporal payload models cross into `pipelex-worker` via the `pipelex` pin; the worker must ship the same `pipelex` version as `pipelex-api` when the MTHDS Protocol unification deploys.
