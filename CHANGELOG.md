...
- **`mthds` pin → 0.4.0** — the restructured SDK (`mthds.protocol` + `mthds.runners.api`). The agent CLI's API run path uses `MthdsAPIClient` (`mthds.runners.api.client`) directly; domain shapes import from `mthds.protocol.*`, Dict wire models from `mthds.runners.api.models`.
