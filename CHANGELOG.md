...
- **Fixed: a failed boot no longer leaks a plugin runtime a boot-orchestrator already started.** `Pipelex.make()`'s failure handler released process-global *state* but never ran the plugin-registered teardown callbacks, so a boot that died after the `TASK_MANAGER` slot thunk left a live runtime running — for a Temporal worker, threads and a client connection. The window is wider since the boot split (the thunk runs in the runtime half, so every interpreter-tail step now follows it, where three of them used to precede it), and the most reachable trigger is a plain config error: `pipe_func_config.execution_mode` naming an unregistered mode. The failure path now runs the plugin teardown callbacks first, mirroring the normal teardown's ordering, pinned by a test that asserts the thunk ran *and* the callback ran (so it cannot pass vacuously) and verified to fail without the fix.
