...
- **An Output Field Named `json`, `copy`, `schema` or `construct` No Longer Breaks Distributed Runs:** when a structured result crosses a worker boundary, the output class is rebuilt from its JSON schema — and the rebuilder cannot give a field a name that shadows a Pydantic attribute, so it renames it and records the real name as an alias. The result was then serialized under the renamed key, and validating it back against your class failed with `Field required` on a field you had supplied. The conversion now serializes by alias, so the keys match the schema your class published. Field names that needed no rename are unaffected.
...
- **Structured Web Search Through Linkup Returned An Envelope No Output Class Could Accept:** the Linkup backend asked for sources alongside the structured payload, so it returned `{data, sources}` instead of the payload itself — which then failed validation against the output structure class, making structured search unusable on that backend. It now returns the structured payload alone, matching the gateway backend. The output structure's schema still reaches the provider in full; it just crosses as JSON rather than as a class, so the provider SDK cannot pre-validate the response and make your validators run twice.
