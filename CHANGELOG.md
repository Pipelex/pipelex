...
- **Unknown per-model backend keys must be header-shaped (Breaking)**: A key in a per-model table of a backend TOML that is not a model-spec field is sent to the provider as an HTTP request header — that mechanism is now documented, and it is gated on shape: the key must contain a hyphen (`x-portkey-provider`, `anthropic-beta`). An unknown key without one (`max_tokns`, or a field deleted from the blueprint) used to go out on the wire silently while the real setting stayed unset; it now fails the backend load, in strict and lenient boots alike, with an error naming the key, the model and the file. A hyphenated spelling of a real field (`max-tokens`) is rejected too, naming the field it resembles. In the remotely served Pipelex Gateway config the same rule prunes such a key instead of failing, as version skew. `endpoint_path` — the provider-side route the Gateway image models are called by — is a declared model-spec field rather than a header entry.
