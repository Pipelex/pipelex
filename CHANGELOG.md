...
- **Live conformance check against the standard's native-concepts page**: A new test holds the pinned native-concept set (`pinned_blueprints.py`) to the standard's own `native-concepts.md`, read from the sibling `mthds` checkout — definition for definition and field for field, section and field order included. The dedicated `MTHDS standard conformance` CI workflow checks the standard out beside this repo (unpinned, at its default branch) and runs the check on every pull request, so the day the standard moves a definition this repo goes red instead of a downstream port. A red with no pinned-set change in the PR means the standard moved, not that the PR broke something; the remedy is a dedicated catch-up change.

### Changed

- **`Concept[1]` is the single form everywhere, at run time too (Breaking)**: The language says a count of one is not a list — `Concept[1]` declares the same slot as `Concept`, with the count written out — and the contract, the schema render and the input-form descriptor all said so, but the runtime's memory shaper framed a `[1]` slot as a one-item list and the output resolution generated one. A consumer that rendered a form from the descriptor and submitted what it said was therefore refused by the shaper. The count now collapses to the single form at the two parsers and the one factory that build a multiplicity, so the value `1` no longer exists downstream: a `[1]` input takes the value itself and refuses a list exactly as a bare declaration does, a `[1]` output produces one object rather than a `ListContent` of one, a header's `Brief` and a definition's `Brief[1]` are one contract instead of a merge mismatch, and a presence marker beside a count of one (`Concept[1]?`) is accepted on inputs as it already was on outputs. A sequence declaring a plural output whose last step yields a count of one no longer validates — a count of one is the single form, and the single form does not fulfill a list expectation — so `is_multiplicity_compatible` answers `False` where it answered `True`. A run-time `output_multiplicity=1` override now means the single form rather than a one-item list, and a re-rendered ref is canonicalized — `Concept[1]` comes back as `Concept`, the other spelling of the same slot. The rule holds across the layers that restate it: the builder's spec validator accepts `Concept[1]?` inputs exactly as the blueprint does, an image-generation pipe declaring `Image[]` honors a run-time count of one instead of refusing it as unresolved, and the static absence-taint pass classifies a one-count step result as single the way the run path resolves it. A zero count is now refused where it is written — `Concept[0]` inputs fail blueprint and spec validation with "multiplicity must be at least 1", as outputs always did — instead of surfacing later as an uncaught parse error. See [Understanding multiplicity](building-methods/pipes/understanding-multiplicity.md).
- **`native.Html`'s `css_class` is optional (Breaking)**: Matching MTHDS v0.9.0, `HtmlContent.css_class` is now `str | None = None` and the pinned blueprint no longer marks the field required — an `Html` value is its markup, and the class is presentation a producer states when it has one. Composed HTML no longer fabricates an empty `css_class` to satisfy the model, JSON rendering omits an unset class instead of emitting an empty string, and structured output no longer forces a generating LLM to invent presentation data nobody asked for.
- **`native.Date` and `native.Html` are `object` nodes in the input-form descriptor (Breaking for descriptor consumers)**: The derivation moves both onto the standard's object arm, deriving `fields` from the pinned definitions like the other structured natives — a `Date` slot is an object with a required `date` and an optional `time` child, an `Html` slot an object with a required `inner_html` and an optional `css_class` — instead of the former `date` and `prose` scalars, which reported shapes the slots' own schemas rejected.
- **Input-form descriptor conformance fixes (Breaking for descriptor consumers)**: A description-only concept's `refines` now carries the authored chain alone (absent when nothing was authored) instead of a fabricated `native.Text` link the spec forbids reconstructing — text-valuedness still reaches the wire as `kind: "prose"`. And a list's `item` node no longer carries a `name` member: the spec states an item has no authored name, and the index labels entries.
...
- **The input-form descriptor's item layer is a type of its own (Breaking for importers)**: The standard now splits the field union in two instead of carrying `name` as an optional slot on one shared base, and `pipelex/pipeline/input_form.py` re-exports both layers. `InputFormField` stays the named union, whose per-kind models (`TextField`, `ObjectField`, …) now require `name: str` rather than admitting `None`; the new `InputFormItem` union is their nameless counterpart (`TextItem`, `ObjectItem`, …), which is what a list's `item` holds and the only shape that position accepts. Two re-exported base classes were renamed with it: `InputFormFieldBase` is now `InputFormItemBase`, and `TextValuedFieldBase` is now `TextValuedItemBase`. **The wire is unchanged** — this engine already stopped emitting `name` on an item; what moved is that the rule is now the shape rather than a convention, so an item built with a name fails at construction and a consumer reading `field.name` gets a `str` with nothing to narrow.
- **Agent-rule refinement on `TypeAdapter`** (`pipelex/kit/agent_rules/python_standards.md`): the flat ban becomes a scoped one — don't design shapes that need a `TypeAdapter`, but validating into a top-level union or container owned by an external contract (a standard's wire model, a log stream of one-of-many event types) is its sanctioned use, as a module-level singleton. The input-form deriver's list-item conversion is exactly that case and keeps its adapter.
- **`mthds` is pinned exactly (Breaking for anyone who also depends on `mthds`)**: The dependency moves from a floor to `mthds==0.11.0`, the release carrying that split and the descriptor's coherence rules — a top-level field whose `required` contradicts its presence marker, one pairing `gating: true` with an optional marker, and a wire node spelling absence as an explicit `null` are now rejected at the parse, which gates this engine's emission at derivation time rather than on the wire. Everyone downstream of Pipelex inherits that exact version: a consumer that also depends on `mthds` must move in step with Pipelex rather than take an `mthds` release on its own.
