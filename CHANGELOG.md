...
- **Secrets are scrubbed and field values made injection-safe before any sink sees a record**: one processor, configured under `[runtime.log.redaction]` and on by default, replaces a bearer token, an api-key or signature header value, an OAuth `code` parameter, a cookie, a JSON secret entry and an `sk_` / `plx_sk_` / `pk_` / `bl_` key with `[REDACTED]`, in the message and in every string a field carries, and turns each control character in a field's string values into its printable escape so a caller-supplied string cannot forge a line, a field separator or a colour on a terminal. It is installed by `log.install_sink` in front of whatever the sink appended, so every sink is behind it, and it edits the record the root logger shares rather than a copy, so a handler ordered after the sink is behind it too; `extra_patterns` adds the secret shapes only a deployment knows, and a pattern the `re` module refuses is a configuration error named at load. A sink processor that raises now goes to the handler's `handleError` instead of raising out of the log call, as [Logging](tools/logging.md#redaction) describes.
