...
- **`backends_override.toml` and `routing_profiles_override.toml`**: a personal override layer for the two inference documents, the one `pipelex.toml` and `telemetry.toml` already had. Each sits beside its base file under `.pipelex/inference/`, is git-ignored and never shipped by the kit, and carries only the keys it sets: `[pipelex_gateway] enabled = false` in one and `active = "all_anthropic"` in the other is a complete pair. The base resolves as before — the project's file if it has one, otherwise the global one — and the overrides deep-merge over it, the global `~/.pipelex/inference/` one first and the project's one last, so a single edit under the home directory moves every project on the machine onto another backend, a project override still wins where one project must differ, and deleting the two files restores the shipped default. That order is deliberately not `pipelex.toml`'s, where a project base beats a global override: every project carries a tracked `backends.toml`, so a project base that won would defeat the file's purpose. Every reader sees the merged document — the boot, the gate that decides whether to fetch gateway model specs, `pipelex show backends`, and the doctor's file, credentials and models rows, including under `--global`, where the sequence is pinned to that directory and its own override. `pipelex init` keeps reading and writing the base file, because its checkboxes must pre-fill from the file they will write. The two names are in `GIT_IGNORED_CONFIG_FILES`, so the kit mirror, `make check-config-sync` and the global bootstrap copy look past them, and the migration ledger never touches them. See [Personal overrides](configuration/config-technical/inference-backend-config.md#personal-overrides).

- **`make test-ts-gates`** (alias `make ttg`): the TypeScript emission gates, provisioned and made mandatory. The `ts-zod` target promises its bytes are already what prettier would write — a codegen stamp is a hash over those bytes, so anything less has a consumer's first `prettier --write` report an untouched artifact as hand-edited. Two tests actually read the emission rather than measuring its lines: `prettier --check` over it, which is also the only thing that parses it, and running the emitted schema under a real zod against the runtime's own payload. Both were written to skip when node was absent, which in a Python repo meant they skipped in CI and on every developer machine, so the emitter's whole content contract was guarded by nothing. This target installs a pinned prettier and zod into the gitignored `.ts-toolchain/` and runs the codegen suite with `PIPELEX_REQUIRE_TS_GATES=1`, under which a missing binary is a failure instead of a skip; the new `Tests (ts emission gates)` CI job runs the same target behind a node step and blocks through the `Tests (all)` aggregate. An ordinary run on a machine with no node still skips, and now says which command would provision the toolchain. Moving the pinned prettier version is an emitter change rather than a dependency bump: the emitter models that formatter's exact break, quote and object-literal behaviour, so the gate is expected to red and the emitter to be taught what moved. See [TypeScript Emission Gates](contribute/typescript-emission-gates.md).

- **`.worktreeinclude`**: the gitignored files a fresh worktree of this repo needs — `.env`, the personal `.pipelex/` overrides (including the two inference ones) and `.pipelex-dev/test_profiles_override.toml` — are now declared in a tracked file at the repo root instead of living in one contributor's shell script. Claude Code reads it when it creates a worktree with git, and the workspace's `wt` tool reads it for the durable ones, so a fresh worktree of `pipelex` arrives configured either way. The file holds only the names of untracked files this repo's `.gitignore` already discloses, never their contents; it was previously gitignored here, which made the mechanism personal and untracked before anyone had used it.

- **`build_output_form` — the output-form descriptor, and a payload schema on the output contract.** The validate report now carries a third artifact, `output_form`, beside `pipe_io_contracts` and `input_form`, and the output contract carries a `json_schema` where it carried none. Both are the MTHDS change the standard landed in the same version, and both close the one real asymmetry between a pipe's two halves: an input had a presentation view and a payload schema; an output had neither, so a tool registry had no return type and a result renderer had to infer a payload's shape by inspecting it.

  **The builder is a projection, not a new derivation.** `InputFormDeriver.derive_concept` — the public method the input derivation already calls for every nested concept-typed field of every input — is what describes an output, because an output is a concept reference exactly like an input is. It stamps no slot facts (`presence` and `gating` are facts of an input slot, and an output is not one), and it iterates the same `result.pipes` as the two builders beside it, so all three artifacts share one key set by construction rather than by intent.

  **The plural wrap is the one place the builder does real work.** Plurality is not on the concept: `concept_ref` is the element with the multiplicity suffix stripped on both sides of the contract, so `derive_concept` describes a concept alone and cannot know it. `build_output_form` reads `pipe.output.multiplicity` and wraps a plural output as a `list` node whose `item` is the element — skipping it would describe one item where a run returns many, silently.

  **The output schema is the PAYLOAD's, which is where it departs from its input twin.** An input's schema states what a caller *sends*, so a plural slot's is a bare array. An output's states what *comes back*, which is a `StuffContent` — so the single arm goes through the same `render_stuff_spec` call the inputs do (a `native.Text` input and a `native.Text` output carry byte-identical schemas), and the plural arm is `ListContent[structure_class]`, asked of pydantic rather than hand-built: a hand-built envelope is a second declaration of a runtime type and stops tracking it the moment it changes. On the fixed arm the element array carries `minItems`/`maxItems` equal to `item_count`, since `ListContent` states no bounds of its own and a `Concept[N]` output would otherwise be indistinguishable from a `Concept[]` one by schema alone.

  `generate-projection-corpus` emits `output_form.json` alongside its two siblings, and a new corpus bundle covers outputs specifically — flat and nested structures, both plural arms, an optional output, a wrapping native scalar and a file-bearing one. It exists because every bundle before it resolved to `Text`: a corpus whose every pipe outputs one native scalar can only capture one output descriptor and one output schema.

  Requires `mthds` 0.12.0.

- **`pipelex-dev generate-projection-corpus`**: The generator of the shared fixture corpus that pins the two client-side inputs-template projections — the TypeScript one in `mthds` and its Python twin — against each other. Each client SDK now renders a pipe's fill-in template from the input-form descriptor it already has instead of asking a server for one, so the two renderings have to agree exactly, TOML `# concept:` comment lines included. The command pairs each pipe's descriptor with the expected template in both shapes (compact and explicit) and both formats (JSON and TOML), and records where that expectation deliberately differs from this engine's own inputs-template renderer: the expectation is authored from a reference projection that walks the descriptor, where the engine reflects the runtime content classes. Each class of difference is declared rather than discovered — an undeclared one fails the run, and so does a declared one that no longer occurs, so an engine fix retires its entry deliberately. A new corpus bundle covers what the existing probe bundles did not: a text field merely named `url` beside a real file position, optional file fields nested inside a structure, and both a fixed `[N]` and a variable `[]` slot over a structured concept. See [Projection Fixture Corpus](contribute/generate-projection-corpus.md).
- **The projection corpus round-trips every template it pins through the input shaper**: `pipelex-dev generate-projection-corpus` now hands each projected template straight back to the runtime — both shapes, assembled exactly as an entry-pipe run assembles a caller's inputs — because a fill-in template exists to be filled in and run, and nothing checked that before. Twice the corpus pinned slots the runtime rejects outright and a human review round caught it rather than the capture; the second time the divergence gate absorbed the broken sites into a declared class and exited zero, since "differs from the engine" and "the engine refuses this value" are different questions. The verdict follows the same declared-never-discovered discipline as the divergence record, through an `EXPECTED_UNSHAPEABLE` registry mapping a pipe and shape to the item tracking the gap: an undeclared refusal fails the command naming the pipe, shape and error, a declared entry whose template has started shaping fails it too so a fix retires its declaration deliberately, and a declared one is stated in the manifest as an `unshapeable` entry carrying the contract-stable error class and its ledger item — so a known-open gap is recorded rather than blocking the capture. See [Projection Fixture Corpus](contribute/generate-projection-corpus.md).
- **Include a method by address — fetch-on-miss**: a bundle referencing another method by address (`github.com/...->domain.pipe`) that misses the installed methods now fetches the package by reference — honoring an `@<tag>` pin, through the same grammar, bounds, and tags-only rules as a direct fetch, with the same hosted-parity warning for Python structure classes locally and a hard refusal on sandbox-hosted deployments — installs it into `~/.mthds/methods/` (atomic staging write, `.git/` stripped, fetch provenance recorded beside the manifest in a `.provenance.json` sidecar: address, tag, and the resolved commit SHA), and loads it so resolution proceeds. Once installed, the method is reused and the network is never touched again — including for a reference pinning a different tag, which warns and uses the installed copy. The former silent-miss warning in the library manager is now a real diagnostic: a miss that cannot be bridged (fetch disabled via the new `[interpreter.methods] fetch_on_miss` config or the `PIPELEX_METHODS_FETCH_ON_MISS` environment variable, an unfetchable address, or a failed fetch) raises an error naming the address and the remedy, never a silent pass. The install target is guarded by identity: two packages sharing a bare directory name but coming from different addresses collide with a loud error naming both addresses (never a silent load of the wrong package, never an overwrite), a concurrent install of the same package is tolerated (each attempt stages under a unique dot-prefixed directory, and losing the final rename race to a matching occupant uses the winner's copy), and a fetched package containing a symlink is refused outright, since a link's target would bypass the content scans. Installed-method lookup by full address is now case-insensitive, matching GitHub's owner/repository semantics and the fetch path's manifest-identity matching, and discovery skips dot-prefixed directories. See [Packages → Include by Address](building-methods/packages.md) and [Methods Configuration](configuration/config-practical/methods-config.md).
- **Pipelex Manifold, a second Pipelex-operated inference backend (private beta)**: `pipelex_manifold` joins `pipelex_gateway` in `backends.toml`, declared with `endpoint = "${PIPELEX_MANIFOLD_ENDPOINT}"` and `api_key = "${PIPELEX_MANIFOLD_API_KEY}"`, and a matching `all_pipelex_manifold` routing profile ships beside `all_pipelex_gateway`. It is **shipped disabled**, because a backend's `enabled` flag is read before any `${…}` variable is resolved — the boot has to know which managed backends are live in order to know which sections of the published configuration to read — so an installation that has not joined the beta would otherwise warn on every boot about a section it never asked for. The two services share model handles and differ only in the wire id and sdk each names, so decks, aliases and presets need no edit to move between them: switching profile is the whole change. A new provider package supplies the sdk set the manifold catalog names (`manifold_completions`, `manifold_responses`, `manifold_img_gen`, `manifold_extract`, `manifold_search`), with Claude travelling over the shared Anthropic driver rather than a manifold-specific one. Its endpoint rule is normative and has no fallback: the backend declares the service **origin with no `/v1`**, each client appends what it expects beneath it, and an endpoint that resolves to nothing disables the backend rather than defaulting to any other vendor's URL. Extract and search reach the service on its own routes rather than dressed as chat completions, and translate the counts those routes report into the token units the cost model prices, so the same work reports the same cost figure under either service. The service token travels on one header of the dialect's own, `x-pipelex-api-key`, on every protocol: the OpenAI-substrate factories and the native client set it directly, and the shared Anthropic driver reads it from the backend's `auth_header` field.
- **The gateway's request-shape refusals are classified**: the Pipelex inference gateway bounds what a request may weigh and how deeply it may nest, and refuses anything over those bounds itself — before authentication, before a provider sees the request. Those refusals now have a class in `classify_inference_error` instead of falling through the generic status ladder, so a caller who sent something too large reads *the request was too large for the inference gateway* rather than *the provider rejected the request*: a body over its cap and a storage object over its cap are caller-fixable size errors, a body nesting past the depth limit asks for a flatter structure, and a body whose declared size the gateway cannot read is an operator's problem, not the caller's. None is ever retried — an identical retry earns an identical refusal. The refusal is matched on the gateway's own error code and not on the reporting provider, because the same request reaches the gateway through the Portkey substrate, through plain `httpx` on the native extract and search routes, and through the shared Anthropic driver that Claude travels on. The same refusal can arrive under two codes, because the gateway renders it in the vocabulary of the route it came in on — its own `pig-0N` family on the LLM routes, its frozen `pipelex_*` contract codes on the native extract and search routes — so a file over its cap is recognized under both spellings and gives the caller the same remedy either way. Reading those contract codes at all required inverting one precedence: the native-route envelope puts a generic `invalid_request_error` in `error.type` beside the real code in `error.code`, so the two Pipelex-service Extract hops now read `code` first, which vendor hops do not (Anthropic's error section carries a `type` and no `code`). `ClassificationResult` gains a `gateway_request_limit` field and `ProviderErrorMetadata` the matching read-only property. Recognizing the code at all first required fixing where it is read on the Portkey substrate: that SDK's exception factory replaces the response payload with the refusal's message string, so `provider_error_code` came back empty for every error it raised — the code is now recovered from the response the SDK had already buffered. See [Error Model](under-the-hood/error-model.md).
- **The gateway's "cannot resolve this reference" refusals are classified**: a request may name a file rather than carry it — a `pipelex-storage://` key the Pipelex inference gateway resolves for the caller, or a document URL it fetches on their behalf — and when it cannot turn that reference into bytes it refuses the request itself. Every one of those refusals arrives on HTTP 400, so they all fell through the status ladder and told the caller to review the prompt: a mistyped storage key, an object this deployment cannot read, a URL the SSRF guard will not fetch from, a media type the pipeline does not accept — all one generic *the provider rejected the request*. They now have a class of their own beside the request-shape limits, and the members group by remedy rather than by wire code, so two codes share advice only when the caller's next move is the same. Three consequences are worth stating: a deployment that serves no storage references at all is an operator's problem and says so instead of asking the caller to fix an input that cannot help; a host the gateway refuses to fetch from is stated as the deliberate security policy it is, never as a fault to work around; and the LLM routes' single fail-closed slot for "cannot resolve" defers to the gateway's own message, which names which cause applied, rather than guessing. A URI whose scheme the route does not read is part of the family too, which is the refusal an `http://` document URL actually reaches. Like the request limits, none of them is ever retried — the gateway refused before a provider saw the request — and recognition keys on the gateway's own error code rather than on the reporting provider, since the same refusal reaches the runtime through the Portkey substrate, through plain `httpx` on the native extract and search routes, and through the shared Anthropic driver that Claude travels on. `ClassificationResult` gains a `gateway_unresolved_reference` field and `ProviderErrorMetadata` the matching read-only property. See [Error Model](under-the-hood/error-model.md).
- **The gateway's routing refusals are classified**: before a request can reach a provider the Pipelex inference gateway has to decide *which* provider — it reads the model out of the request, looks it up in its own routing table, and hands the call to the integration that serves it. When that resolution fails there is nowhere to send the request, so the gateway refuses it itself, and every one of those refusals arrives on HTTP 400. They therefore fell through the status ladder and read as a provider rejecting the caller's content: someone who named a model the deployment does not serve was told *the provider rejected the request — review the prompt, parameters, and inputs*, and received an `LLMCompletionError` rather than the `LLMModelNotFoundError` that case has always produced when the model deck itself cannot find a model. They now have a class of their own beside the request-shape limits and the unresolvable references, and it is `CONFIGURATION` throughout — nothing about the prompt, the parameters or the inputs causes any of them, and no edit to those avoids one. A model the gateway does not serve is a missing model and now renders as one, carrying the model handle so the pipe layer raises the same availability error it raises for a deck that cannot find a model. A model whose integration is switched off for want of a credential is the gateway operator's problem, not the caller's, so it says contact support rather than sending someone to rotate their own perfectly valid key or to go shopping for a different model. A model served over another protocol, or by an integration that does not serve the capability asked of it, is neither of those: the model exists and is served, so the advice says to change the model or the model deck and never claims the model was not found. Two of the gateway's routing codes stay deliberately outside the family, since no client that talks to one of our gateways produces either today — one refuses client-side routing forms our clients do not send, the other a path the gateway does not mount. Like the two families beside it none of these is ever retried, and recognition keys on the gateway's own error code rather than on the reporting provider. **Breaking for HTTP consumers**: moving the family from `CONTENT` to `CONFIGURATION` moves its domain from `INPUT` to `CONFIG`, so all four now answer **500** rather than 422 on any surface built on `ErrorReport.http_status` — the same answer a missing model has always drawn from the status ladder's 404 arm, and the right one in kind, but it puts these in server-error alerting and hands outer clients a status they retry. `ClassificationResult` gains a `gateway_routing_refusal` field and `ProviderErrorMetadata` the matching read-only property. See [Error Model](under-the-hood/error-model.md).
- **Per-model local overrides for any Pipelex-managed backend**: `backends/pipelex_manifold.toml` works exactly as `backends/pipelex_gateway.toml` does — the override file is found by the backend's own name, and neither file can reach the other's models. The permitted keys stay `sdk` and `structure_method`, deliberately identical across both: the model id is what the service resolves a request against, so overriding it locally turns a catalog problem into a single-machine one.

### Changed

- **The inference loaders take a path sequence, and the routing loader raises what the boot catches** (breaking). `InferenceBackendLibrary.load` takes `backends_library_paths`, `load_active_routing_profile` takes `routing_profile_library_paths`, and `ModelManager.setup` takes both, each a sequence whose first path is the base file and whose rest are the override files in merge order — built by the new `ConfigLoader.backends_file_paths()` / `routing_profiles_file_paths()` and read by the new `load_toml_from_base_and_overrides`. `is_pipelex_gateway_enabled` takes a `config_dir` instead of a file path, so a caller pins a directory rather than building a path by hand. The routing loader used to raise `ModelManagerError` for a missing file and `RoutingProfileLibraryError` for a profile naming a disabled backend, while the boot caught `RoutingProfileLibraryNotFoundError` and `RoutingProfileDisabledBackendError` — two clauses that were never reached, so a profile activated before its backend was enabled surfaced as a raw internal error. The loader now raises those two classes, keeps `RoutingProfileLibraryError` for an invalid document, a file that does not parse or an `active` naming no profile, and the boot turns all three into a `PipelexSetupError` that keeps the loader's account of what to fix; the doctor's Models row reports that third class too, where it used to surface as an unexpected error. The backend library refuses a file that does not parse and a backend whose value is not a table (`acme = false` where `[acme] enabled = false` was meant) as `InferenceBackendLibraryValidationError`, and `is_pipelex_gateway_enabled` — which the boot reads before the library loads — raises that same class for a parse error, so the boot's own clause names the file instead of a raw `TomlError` escaping before any clause is reached. When an override was merged, both loaders log which files the document was read from. `pipelex init` decides whether the gateway is enabled on the merged document, so a gateway switched on only by an override gets the terms prompt, and when declining the terms disables the gateway in the base while an override still pins it on, it warns and names the file. `ConfigLoader.backends_file_paths()` / `routing_profiles_file_paths()` take their base from the `backends_file_path` / `routing_profiles_file_path` properties, so a fixture that patches a property still redirects every reader; the integration fixtures that pin a routing profile now patch the sequence method instead, which also keeps a developer's personal override from re-routing the backend under test.
- **The `.pipelex/.gitignore` pipelex writes lists the personal overrides.** Beside the migration-backup rule, a fresh configuration directory now gets one line per personal override file — `pipelex_override.toml`, `pipelex_temporary_override.toml`, `telemetry_override.toml`, `inference/backends_override.toml` and `inference/routing_profiles_override.toml` — so a developer's own choices never reach a new project's `git status`. The file is still written only when absent and never rewritten; a project set up earlier adds the names to its own `.gitignore`.
- **A Pipelex-managed inference backend is now anything that declares a model-specs section, not one hard-coded name (Breaking)**: The runtime used to know exactly one managed gateway, by name, in a dozen places. Naming a `model_specs_section` in a backend declaration is now what makes a backend managed, more than one can be live at once, and the single published configuration is fetched once and sliced into one per-backend view rather than merged — a handle one service serves and the other does not is a legitimate configuration, not a conflict. The deck-integrity check runs once per live managed service, and asks its section only about the handles the active routing profile actually routes there, because a union across services would accept a handle routed somewhere that cannot serve it while the same whole-deck check run twice would demand every handle appear under both. Whether the deck already resolved a handle stays a whole-waterfall question, since a waterfall's fallbacks may cross backends and the runtime walks the list until one resolves — restricting that half too would refuse a mixed-profile deck the runtime serves perfectly well. `pipelex_gateway` keeps reading its historical section without declaring it, since `backends.toml` is the user's own file and an upgrade never rewrites it. A managed backend whose `${…}` variables do not resolve, or whose declared section is absent from the published configuration, is **disabled with a warning naming it**; every bring-your-own-key backend keeps failing the boot loudly, which is the point of scoping the tolerance rather than granting it globally — a typo'd `ANTHROPIC_API_KEY` must not become a silently missing backend. Breaking: `is_gateway_backend` is gone, the backend loader takes `managed_gateway_configs` instead of a single gateway config, and `GatewayUnknownModelError` now names the backend it came from, reusing the attribution slot `CogtError` already carries.
- **Service terms are asked for any managed gateway backend, not for `pipelex_gateway` by name**: the boot puts an installation behind service-terms acceptance as soon as any Pipelex-managed gateway backend is enabled, and the init flows now ask that same question. Selecting only `pipelex_manifold` — through the interactive picker, through `pipelex-agent init --config`, or by enabling it by hand — prompts for the terms and records the acceptance, and `pipelex init agreement` reaches the prompt on such an installation instead of reporting that no acceptance is required. Acceptance stays a single global flag, so one answer covers every managed backend; declining now disables all of them rather than only the Portkey-cloud one. The offline cache priming step at the end of `pipelex init` asks the same question, so a manifold-only installation gets its offline fallback at init rather than at its first online boot.
- **Remote config pin moves to `pipelex_remote_config_13.json`**: version 13 is the first published artifact carrying `manifold_model_specs` beside `backend_model_specs`, so it is the first one a runtime reading the manifold section can be pointed at by default. Until now the beta reached that section only by overriding `PIPELEX_REMOTE_CONFIG_URL` at a locally served workbench artifact. The bump is inert for the Portkey-cloud path: version 13's `backend_model_specs`, `posthog` and `aws_region` compare identical to version 12's, so an installation with `[pipelex_manifold]` disabled reads exactly what it read before. Earlier releases stay pinned to `_12`, which is frozen.
- **Inference errors now declare an `error_domain`, derived from the category the worker already assigned** (breaking): the whole `CogtError` family used to report no domain at all, so every inference failure — a content-policy refusal included — was rendered as HTTP 500 by any consumer reading `ErrorReport.http_status`. `InferenceErrorCategory` gains an `error_domain` property and `CogtError.to_error_report()` derives from it, which means a **content-classified failure now answers 422 instead of 500**: a provider content-policy refusal, a malformed prompt image, a bad prompt parameter and their siblings are all properties of material the caller submitted. The widest reach of that mapping is worth stating plainly, because it goes past the classes that declare `CONTENT` on themselves: **every provider HTTP 400 classifies as `CONTENT`**, as does a builtin `ValueError` / `RuntimeError` / `FileNotFoundError` out of a local file extractor, so a context-length overflow, a parameter a model rejects, and an engine-side request-construction fault all answer 422 now and leave the 5xx rate. The `UserAction` on those was already `CHANGE_INPUT`, so the status is only catching up with the advice the report was already giving. The other mappings change no status and only make the report truthful about why — `CONFIGURATION` is `config`, `TRANSIENT` / `CAPACITY` / `AMBIGUOUS` are `runtime`, and `UNKNOWN` deliberately asserts no domain, since classification itself failed there and an absent domain already renders 500. Deriving rather than declaring per class is what keeps `error_domain` and `error_category` from contradicting each other on the wire; a leaf that declares its own domain still wins, and the provider-429 passthrough is still checked before the domain. The effect reaches the HTTP boundary through every wrapping layer: `PipelineExecutionError`'s generic `RUNTIME` was documented as a floor applied only when the cause chain surfaced no domain, and until now the cause chain surfaced none for any inference failure, so the floor fired every time. Consumers that route on the domain rather than the status see the same correction — a validate hook classifying `config` / `runtime` as a warning and everything else as a block now warns on a genuine backend or model-deck misconfiguration surfacing through dry-run inference, where before an undomained error fell through to the default block and told the agent to edit a `.mthds` file that was not at fault. On the agent CLI this makes three `AGENT_ERROR_DOMAINS` entries unreachable, since a report that carries its own domain is read before that fallback lookup: `ModelDeckPresetValidatonError` and `GatewayUnknownModelError` derive the `config` the dict already stated and are simply removed, while `ModelChoiceNotFoundError` now declares `error_domain = input` on the class itself — a mistyped model reference is the caller's to fix, on the hosted API as much as in a local file, even though its category is `CONFIGURATION` from the provider call's point of view, and an operator-side deck fault surfaces as `ModelDeckPresetValidatonError` instead. The drift test that bars a self-describing class from those dicts gained the arm that covers derivation, so the next such entry cannot go dead unnoticed. See [Error Model](under-the-hood/error-model.md).
- **The shared projection corpus covers the open-shaped natives the two engine fixes unblocked**: `scaffold_bundle.mthds` gains a `scaffold_anything_slot` pipe carrying `native.Anything` and a `scaffold_dynamic_prompt` pipe putting a `native.Dynamic` slot at an LLM prompt position under both markers — the latter a bundle that did not validate at all while the prompt analyzers read a dynamic concept's universal compatibility as identity. The `Anything` slot is captured knowing the runtime refuses it: its template is the empty object every `unknown` node renders, and at an `Anything` position the input shaper accepts a bare string alone and returns it as a `native.Text` stuff, so both shapes of its pipe are declared in `EXPECTED_UNSHAPEABLE` against the item tracking the shaper fix rather than left to fail the round-trip gate. That is the gap being stated, not widened — the descriptor states `Anything` as `unknown` exactly as it states `Dynamic` and `Composite`, so a projection reading only the descriptor has nothing else to render there, and closing it deletes those entries and regenerates. It gets a pipe to itself deliberately: an `EXPECTED_UNSHAPEABLE` key is a whole template and the shaper stops at the first slot it refuses, so declaring it inside `scaffold_open_natives` would have suspended the round-trip check for the pinned and structureless natives beside it — including the only site where `unknown-empty-object` is measured. **`output_bundle.mthds` also joins the documented bundle list**, in the command block, in the CI corpus list and in the agent rules: it has been part of the committed capture since the output-form descriptor landed, but every regeneration recipe still named three bundles, so following one would have dropped its pipes from the capture. Re-committing the capture byte-identically in `mthds-js` and `mthds-python` is tracked separately, since a capture has to come from a merged `dev` and the two mirrors must land together. See [Projection Fixture Corpus](contribute/generate-projection-corpus.md).
- **`kajson` version**: The pin moves to `kajson==0.7.1`, the release making `ClassRegistry.register_classes_dict({})` a no-op instead of raising `IndexError` — its logging branch for the single-class case indexed into an empty collection. This engine seeds a fresh per-run or per-library registry from a snapshot of the process-global one, so a snapshot that happens to be empty used to crash the seeding rather than register nothing; both seeding sites (distributed-run rehydration and the scoped-registry test fixture) are now safe unconditionally. The release changes nothing in the encoder or decoder, so the serialized wire format is untouched in both directions. Everyone downstream inherits that exact version.

### Removed

- **`InferenceBackendLibrary.check_backend_credentials` and `CredentialsValidationReport`.** The method had no caller, and it read the base `backends.toml` alone — a reader that would have quietly ignored the new override files. The doctor's credentials row, which does the same job over the merged document, is the one that remains.

- **`DocumentFormat`**: the `pipelex/tools/misc/document_utils.py` module and the `DocumentFormat` enum it held are gone. Nothing in the engine ever imported them — the only reference outside the module was its own unit test — and the MIME-type and file-extension knowledge they duplicated already lives in `pipelex/tools/misc/filetype_utils.py`, which is what the runtime actually calls. (Breaking for anyone importing the symbol from outside: use `filetype_utils` instead.)

### Fixed

- **`pipelex doctor` no longer reports a false backend-configuration error for `pipelex_gateway` on a stock install**: the Backend Files row probed each backend by loading the whole library strictly and with no gateway config, which the loader treats as a caller's omission — so the shipped defaults, which enable the gateway and ship its override file, were reported as a malformed file with a message addressed to a library caller, and the Models row returned on that verdict before its own check ever ran. The row now loads leniently, which is exactly the split it was missing: a backend whose gateway specs or credentials are unavailable is skipped and left to the Models and Credentials rows, while a malformed file stays fatal. The false error also hid real ones — the gateway is the first table in `backends.toml`, so every probe died there before reading any other file, and a malformed `anthropic.toml` was reported valid. Attribution was string-based too: a probe charged a backend with any error whose text contained its name, and the loader's unknown-key explanation names `portkey` in passing, so that same anthropic error was reported against portkey as well. The loader now stamps `backend_name` on every error it raises about one backend, and both doctor rows charge a failure to the backend it declares — the Models row did the same string match into the same reports, and leniency reaches it more often now that the Backend Files row stops returning early, so an unknown-key error about any backend also condemned `portkey`, whose name rides in the loader's own advice text as the example header `x-portkey-provider`. As a side effect the error report of such an error now carries that backend as its `provider`, where it carried none.
- **A class-registry read before boot no longer starves the boot's global registry**: `KajsonManager.get_class_registry()` instantiates the kajson singleton around an empty registry when none exists, and the boot's own `KajsonManager(class_registry=…)` then handed that pre-boot manager back and silently discarded the boot's registry — the core models landed where nothing resolved, while the boot still reported ready. Any pre-boot manager is now dropped before the boot constructs its own. Reachable from any pre-boot caller of the scoped accessor's global fallback, and from a library opened on a bare `LibraryManager`.
- **A library's generated structure classes no longer escape into another request**: structure classes materialized from a concept's `structure` table are registered under a name key (`<domain>__<Concept>`), and a `Library` that carried no class registry of its own resolved through to the one process-global registry — so every load in a process shared a single slot per concept ref. Two bundles naming the same concept contaminated each other: a later bundle declaring `Summary` with only a description silently reused an earlier bundle's structured `Summary` (in its I/O contracts, in its input form, and as the class its `PipeLLM` structured output against), and two loads that interleaved at an `await` overwrote each other's registration, so whichever landed last was what both rendered schemas from. In a server handling several bundles — the validate route, the dry-validate activity, every crate route — that crossed request boundaries. `LibraryManager.open_library` now attaches a fresh `ClassRegistry`, pre-seeded from the process-global one, to every library it creates, so a load's generated classes live and die with the library that asked for them and the global registry keeps only what boot put there; isolation is structural rather than something a caller opts into. Host code should note the consequence: a structure class registered while a library is current lands in that library's registry, so a Python class every load must see has to be registered before any library is opened. `pipelex run --save-csv` now takes its row model off the produced rows rather than re-resolving the class by name after the run library is torn down. See [Runtime bridge and transport → Per-call scoping](under-the-hood/runtime-bridge-and-transport.md).
- **A dict-default key carrying a double quote made the generated `types.ts` unstable under prettier**: an object-literal key that is not a plain identifier was quoted with `json.dumps`, so a key like `marked "urgent"` — reachable from `default_value = { 'marked "urgent"' = 3 }`, ordinary TOML on a `type = "dict"` field — came out double-quoted with escapes and prettier rewrote it to the escape-free `'marked "urgent"'` on the consumer's first format run, breaking the stamp. Keys now go through the same fewer-escapes quote rule as every other emitted literal, which the value side already used. This was found and deliberately deferred during the ts-zod optionality work, on the grounds that a real gate over the emission would catch its whole family at once rather than one more hand-written assertion; the newly mandatory prettier gate reddened on it immediately.

- **The ts-zod projection could not parse the engine's own wire payload** (breaking for generated TypeScript — regenerate): a non-required concept field was projected as `.optional()`, which means `T | undefined` in Zod and rejects an explicit `null`. The runtime serializes an unset optional field as exactly that — the generated runtime class annotates every non-required field `X | None` and the transport dump keeps nulls on purpose (no `exclude_none`, spelled out at the composer's dump sites) — so `Schema.parse` failed on a successful run's output with one issue per nulled field. The wire genuinely carries both spellings of "unset", a key a partial payload omitted and a key the runtime nulled, and the projection now describes both: `.nullish()` for a non-required field, and `.nullable().default(…)` where it carries a default, since a producer may null a defaulted field explicitly. There is no `.transform()` folding `null` into `undefined` — the binder uses one schema for both `parse` and `serialize`, so a transform would desynchronize the pair. Parsed optional fields therefore type as `T | null | undefined` instead of `T | undefined` (`??` covers both), and a recursive concept's explicit `export type` carries the same widening, because it is checked against the schema's inferred output. The two Python targets already accepted both spellings; this is a parity fix. Because `.nullable()` costs width on every defaulted field, the emitter's line-breaking now models prettier's real rule — a member chain of two or more calls breaks entirely, whatever the expression — rather than treating `z.enum([…])` as the only shape that can overflow; a defaulted `z.string()` overflows on nothing but a long field name and default literal. Prettier then re-measures each call on its new line and leaves flat any that now fits, so an ordinary three-choice enum carrying a default — which reaches the break on its `.nullable()` width alone — keeps its members on one line rather than being exploded into a shape the formatter folds straight back. See [Codegen Projections](under-the-hood/codegen-projections.md).
- **A string carrying a double quote made every generated artifact unstable under its formatter**: choices and string defaults were always emitted double-quoted with escapes (`"marked \"urgent\""`), but ruff and prettier apply the same normalization — double quotes by default, single where that strictly reduces the escape count — so both rewrote the literal to `'marked "urgent"'` on a consumer's first format run, at any line width, and `pipelex codegen check` then reported an untouched file as hand-edited. All three targets now emit the quote style their formatter keeps, verified shape by shape against ruff and prettier including the tied and backslash-adjacent cases.
- **A choice containing a comma or a bracket emitted generated code that does not parse**: an exploded choice list was split on every comma, so an ordinary authored choice like `"blocked, awaiting the owner"` became two unterminated string literals — in the TypeScript target and in both Python targets alike, since each split its own exploded form the same naive way. Every split that computes an emitted line break now walks the string literals, in both emitters and in both quote styles, the single-quoted spelling included: a choice carrying a `"` is spelled `'…'` because that is the style prettier keeps, so a walker watching only `"` cut it at its own comma. The same applies to the TypeScript member-chain split, which located a break by counting brackets and counted the ones *inside* a literal too — a choice is prose whose brackets need not balance (`a) strongly agree` is an ordinary answer option), and an unmatched closer walked the depth to zero mid-string and cut the chain through the middle of it. The TypeScript half is the one nothing was catching: every broken form is a run of *short* lines, so the print-width guard is blind to them and the only test that parses the emitted TypeScript skips wherever there is no node toolchain. An always-on invariant now asserts that no emitted line ends inside an unterminated string literal, which needs no toolchain and covers the whole class.
- **A dict-valued default made the generated `types.ts` unstable under prettier**: the default was rendered as a JSON object (`{"alpha": "first"}`), and prettier rewrites a TypeScript object literal at any line width — it pads the braces and drops the quotes from every key that is a plain identifier. One such default was therefore enough for a consumer's formatter run to change the artifact's bytes and have `pipelex codegen check` report an untouched file as hand-edited, which is precisely the drift the emitted formatting exists to prevent. Defaults now render as TypeScript literals, quoting only the keys prettier itself keeps quoted.
- **An unknown or ambiguous entry `pipe_code` is reported as the caller's mistake, not a server fault**: the two entry-shaped lookups — the ones a CLI argument and an API request field land in — raised the bare `PipeNotFoundError` / `PipeLibraryError`, which declare no `error_domain`, so `ErrorReport.http_status` fell through to 500. A caller typo in the `pipe_code` of a run request therefore came back as an internal error, logged with a traceback and looking retryable to every SDK, for a mistake only the caller could fix — and the same typo already answered 422 on the build routes, which catch those classes explicitly, so one input produced two verdicts depending on the route. Two dedicated classes now carry `ErrorDomain.INPUT` and a `CHANGE_INPUT` action: `EntryPipeNotFoundError` when the entry lookup resolves nothing, `EntryPipeAmbiguousError` when a bare code is declared in several domains. Both subclass the errors they replace, so every existing catcher still matches, and every presentation derives from the domain at once — 422 on the API, the classification the agent CLI reports, the hosted status. Both are caller-facing copy that survives STRICT disclosure, since the message names only the caller's own `pipe_code` and the qualified `pipe_ref`s of bundles they loaded themselves — which is the only thing that makes an ambiguity actionable. In-body resolution is deliberately untouched: `get_optional_pipe` / `get_required_pipe` keep raising the undomained classes, because a ref written inside a bundle is not a caller's input mistake. The one crossing case is translated rather than left to leak: an ambiguous `alias->bare_code` is detected one frame down, inside the alias-scoped search, and the entry lookup re-raises it as `EntryPipeAmbiguousError` keeping the original as its cause. The agent CLI's `validate` arms stopped hardcoding `PipeNotFoundError` as the reported `error_type` and now name the class that actually failed, which is where the domain and hint on that envelope come from.
- **A typo'd `--pipe` / `pipe_ref` slice selector is reported as the caller's mistake too**: bundle validation narrowed to one pipe (`pipelex-agent validate bundle --pipe`, its `validate method --pipe` twin, and the `pipe_ref` field the build routes forward) raised the undomained `PipeNotFoundError` when the selector named nothing in the bundle, so the same caller typo that answers 422 through the entry lookups came back as a 500 through this one — the divergence the entry-pipe classification above was meant to remove, left in place one module over. The selector is entry-shaped input by the same definition (a code a human typed at an entry point), so the miss now raises `EntryPipeNotFoundError` with its `INPUT` domain and `CHANGE_INPUT` action, keeping the raise site's own bundle-scoped wording, which names only the codes the caller supplied. It is a subclass of the class it replaces, so every existing catcher still matches; the hand-written 422 mapping the public runner API keeps for this case becomes redundant once a release carries the change.
- **A `native.Anything` input no longer crashes the pipe-IO contract builder**: the builder resolved a structure class for every input, and `native.Anything` is the one concept the standard says has none — so a legal bundle declaring an `Anything` input validated cleanly and then died assembling the report, which the hosted validate route rendered as HTTP 500 with advice that could not be followed ("include the module" for a class that deliberately does not exist). The render chain now takes a structureless arm before any class lookup: on the wire, an `Anything` input publishes the permissive schema — no constraint keywords, only the identity annotations every rendered input schema carries (`title` = concept ref, `description` = the authored description) — which is semantically the empty schema "any JSON value", while sparing consumers a bare `{}` they would have to special-case; in a fill-in template it renders the empty mapping `{}`, the same value the client-side projections already chose for unknown nodes. Multiplicity wraps exactly as for class-backed concepts. The same arm heals the input shaper's expected-shape hint and `pipelex-dev trace-input-semantics`, which crashed on the same lookup. See [Pipe IO Contracts](under-the-hood/pipe-io-contracts.md).
- **A `native.JSON` input produced a fill-in template the runtime refused**: the input-form deriver grouped `native.JSON` with the three natives the standard calls structureless — `Dynamic`, `Anything` and `Composite` — so its descriptor node was `unknown`, which every inputs-template projection renders as the empty object. But `JSON` is not structureless: its pinned blueprint carries a required `json_obj`, and `JSONContent` refuses a payload without it, so the template a caller was handed for a `JSON` input could never be filled in and submitted. It now expands through the same pinned-structure arm as `Date`, `Html`, `Page`, `TextAndImages` and `SearchResult`, rendering `{"json_obj": {}}`, which the input shaper takes back. The engine's own `_pinned_structure` already drew the line in exactly this place — it returns a structure for `JSON` and `None` for the other three — so the deriver is now consistent with it, and the empty object stays the honest rendering for the natives that genuinely withhold their shape. The shared projection corpus gained a pipe covering all three of the natives it can reach at a slot position, which is where the defect had been sitting unmeasured. See [Projection Fixture Corpus](contribute/generate-projection-corpus.md).
- **A `native.Dynamic` input referenced in a `PipeLLM` prompt no longer fails dry-run validation demanding `ImageContent`**: the prompt analyzers decided whether a variable is a direct image (or document) by asking `is_compatible`, and `Dynamic`'s universal compatibility short-circuits that to `True` — so the analyzer recorded a direct image reference for a slot whose content the caller decides, and every legal bundle mentioning a `Dynamic` input in a prompt was refused at validation. A dynamic root concept now gets no static classification in either analyzer — the question is "is this definitely an X", which universal compatibility cannot answer — matching the guard shape the codebase already used in `PipeLLM.validate_output_with_library` and the input shaper. A `Dynamic` prompt variable renders as text, as it already did in `PipeCompose` templates; authors who want image or document attachment declare `Image`/`Document` (or a refining concept), and `| with_images` on a `Dynamic` variable stays refused. The document analyzer carried the identical latent bug, previously masked by the image misclassification, and gets the same guard. The same analyzer serves `PipeImgGen`'s positive and negative prompts, so the rule lands there too. One live behaviour is given up deliberately and is worth knowing: a `Dynamic` slot does not force `DynamicContent` — the input shaper hands the raw value to the bottom-up factory, so a caller passing an `ImageContent` into a `Dynamic` slot produced a `native.Image` stuff, which the direct reference then attached to the prompt. Attachment is now decided from the declared concept alone, so that image renders as text instead; declare the slot `Image` to keep it reaching the model. See [PipeLLM](building-methods/pipes/pipe-operators/PipeLLM.md).
- **A bad pipeline-input url survives STRICT disclosure as caller-facing copy**: `PipelineInputContentError` declared `caller_facing_message = True`, which is the name of the *report field*, not of the class-level flag that sets it (`_authors_caller_facing_message`). The declaration therefore did nothing, and the message a caller who simply omitted a url needs to see — the one listing the accepted schemes — was replaced by the generic internal-error placeholder on any surface serializing in STRICT mode, exactly the redaction the declaration was written to prevent. Repairing the flag also revealed why the class could not carry it as it stood: its other raise site reports an unreadable local path, and that message names the resolved path and the `OSError` subclass that rejected it, which would have made STRICT answer "does this server-side path exist, and may the runner read it?" for any authenticated caller of a deployment leaving `is_upload_local_content_enabled` on. The family is therefore split: `PipelineInputUrlMissingError` carries the blank-url message and is caller-facing, while `PipelineInputContentError` keeps the unreadable-path message and stays redacted. Both are pinned in the class-level-metadata sweep, in both directions.

- **`HtmlContent`'s JSON rendering emitted a member the standard does not define, and omitted the one it requires.** `rendered_json` renamed `inner_html` to `html`, so a `native.Html` result serialized as `{"html": …}` while the concept — pinned by the standard, projected into the output contract's schema, and described by the output-form descriptor — states `inner_html`, required. A consumer reading the contract found the required member absent and an undefined one in its place. The override is deleted: the JSON rendering is now the model's own dump, member for member, like every other content class.

  Found by a real run, and only findable by one: the payload is what a run produced, so no projection of a declaration could have disagreed with itself. It became a *bug* rather than a quirk the moment the output contract started carrying a schema — before that, nothing published the shape the payload was failing to match.

- **The projection corpus's divergence gate sees every engine/projection difference**: `pipelex-dev generate-projection-corpus` states that every difference between the reference projection and the engine's own renderer lands in a declared class, and that an undeclared one fails the command — but the walk missed differences in one direction and absorbed regressions into existing classes in the other, so a projection defect could pass generation green and be committed into `mthds-js` and `mthds-python` as contract bytes. The dict walk iterated only the projected keys, so a field the projection stopped rendering was never compared at all; it is now recorded under an id deliberately absent from the declared reasons, which makes the record refuse rather than pass. The file-leaf arm returned on the expansion without ever comparing the URL both sides carry, where plain recursion would have handed a regressed placeholder to the url-named-text class that absorbs it; the shared keys are now compared strictly. And `fixed-count-honoured` fired on any list longer than the engine's single element, which cannot tell an honoured `Concept[N]` from a wrong count or from a variable `[]` slot wrongly duplicated; it now consults the slot's declared `item_count`, taken from the descriptor. Separately, a pipe declaring no inputs used to abort the whole capture — an empty input form is valid and the projection renders it as `{}`, but the engine's renderer refuses one — and is now captured from the projection alone, with no engine half and nothing compared. Re-measuring the same corpus produces bytes identical to the committed capture, with the same classes at the same counts. See [Projection Fixture Corpus](contribute/generate-projection-corpus.md).
- **`--help` works again on every `method` subcommand**: `pipelex validate method`, `pipelex run method`, the three `pipelex build ... method` commands and their agent-CLI twins all crashed with a rich `MarkupError` traceback instead of printing their help. The method-address example in each command's `NAME` argument help — `github.com/owner/repo[/name][@tag]` — reaches rich's markup parser through typer, which read `[/name]` as a closing tag with no opening one. The brackets are escaped at each of the sites, so the example renders as written and the help exits zero.
- **Class-backed concepts reflect field by field in the input-form descriptor**: a concept declaring `structure = "SomeClass"` used to collapse to `kind: "unknown"` the moment one field annotation could not be mapped, taking every sibling field with it — and a nested non-native pydantic model was reported as an opaque payload rather than walked into. Every `document` and `image` position beneath such a concept disappeared from the descriptor, so a consumer preparing inputs from it passed a local file path at one of those positions through un-uploaded and the run failed on the unresolved path. Reflection is now partial: each annotation is mapped on its own, an unmappable one is `unknown` at that field alone, a nested model is an `object` the reflection walks into, a `list[X]` reports its element node, and a class that holds itself is cut at the revisit instead of recursing. A `RootModel` is reported as its root annotation's node wherever it appears — as a field of a reflected class, and as the registered class backing the concept itself — because the value it accepts is the root value, so the descriptor no longer states an object over a synthetic `root` key that contradicts the `json_schema` derived beside it. The native consistency probe keeps its faithful-or-absent rule — it is compared against a normative pinned blueprint, where a plausible-looking partial answer would be the failure — and the two reflections now share one annotation vocabulary (`pipelex/core/concepts/annotation_shapes.py`) instead of one calling the other. See [Input-Form Descriptor](under-the-hood/input-form-descriptor.md).
- **Generated-image storage key extension follows the actual mime type**: a generated image could be stored under a `.png` key while its bytes and reported `mime_type` were `image/jpeg`, because `GeneratedContentFactory` picked the key's extension from the *requested* image format while `mime_type` came from the provider's *actual* answer — and a provider may return a different format than requested. The factory now resolves the effective mime type once (provider-reported, then base64-extracted, then the requested format, then the `image/jpeg` default) and derives the storage-key extension from that same value, on every path (stored bytes and fetched remote content alike), so the key and the `mime_type` can no longer diverge and anything inferring a type from the extension sees the truth.
- **The agent-rules templates carry the `generate-projection-corpus` entry, and the sync check now runs in CI**: the command's documentation was written straight into the generated repo-root `CLAUDE.md` instead of into the template it is generated from, so the installed rules and the kit disagreed and `make check-rules` — and with it the whole `make check` aggregate — was red on `dev` and on every branch cut from it. The entry now lives in `pipelex/kit/agent_rules/commands.md` and in its Codex twin `codex_commands.md`, which also gives `AGENTS.md` the command it was missing altogether. The check ran in no workflow, which is how a hand-edited generated file merged in the first place; it is now the `lint-agent-rules` job and part of the required `Lint (all)` aggregate, so an installed target that has drifted from its template fails the pull request.

## [v0.55.0] - 2026-08-29

### Added

- **Run a method by address**: every command that resolves a method target (`pipelex run method`, `pipelex validate method`, the `pipelex build` method commands, and their agent-CLI twins) now accepts a method reference `<address>[@<tag>]` — `github.com/<owner>/<repo>[/<selector>]`, optionally pinned at a git tag (a `@<name>` that names a branch rather than a tag is refused, so only tags pin a revision) — alongside installed names, local paths, and full GitHub URLs. The new `pipelex/methods/` package is the single implementation the CLI, the API, and tests all share: the `MethodRef` grammar parser, fetch-at-tag (wiring the `mthds` `clone_at_version` machinery beside `clone_default_branch`, with the fetched commit SHA always resolved and reported as provenance), package location inside the clone by **manifest identity** (scan for `METHODS.toml`, match the full address, case-insensitively; a miss or ambiguity is a loud error listing the packages the clone contains), env-tunable fetched-package ceilings (file count and total bytes, plus a bounded manifest scan — a size cap per `METHODS.toml` read and a cap on manifests considered per repository), and the reusable structures-refusal check — an alias-aware AST scan for `StructuredContent` subclasses (bases reached through `import ... as` aliases and attribute access are caught alongside the literal name) producing the rule-naming error (*hosted execution accepts MTHDS concepts and sandboxed PipeFuncs, not in-process Python*), which the CLI surfaces as a hosted-parity warning and a hosted runner can apply as a hard refusal. A fetched target's default outputs (run results, graphs, generated files) anchor in the caller's working directory — the fetched clone itself is ephemeral and deleted at process exit — `build runner` additionally copies the fetched package beside the generated script so the runner stays self-contained, and a method-reference failure reaches the agent CLI as its structured error envelope, like every other agent-CLI error. See [Run a Method by Address](tools/cli/run-by-address.md). (Breaking: bare `github.com/...` targets no longer resolve as local paths, and packages inside a fetched repository are located by manifest identity rather than directory path — `/tree/<branch>/<dir>` deep links now resolve through the manifest's `address`/`name`.)
...
- **Telemetry config is migratable:** `telemetry.toml` is now a migration surface. Upgrading from the legacy flat format to the `[custom_posthog]` format is handled by `pipelex migrate`, preserving user API keys and exporters instead of forcing a destructive `pipelex init telemetry` reset. The loader now raises through the same reporting path as every other configuration surface, so the human CLI, the agent envelope and `pipelex doctor` all say whether the file is _old_, _wrong_, or reported on by a migration that would not repair it, rather than each holding a hardcoded remedy. **Migration:** run `pipelex migrate` — ledger entry `telemetry-config@2` lifts a flat first-generation file into `[custom_posthog]`, keeping every value that has a home in the current shape and saying which settings it drops and why.
...
- **A blocked configuration file says which state it is in:** `FileBlockedReason` gained `unreadable` and `state_uncertain`, so an unreadable file is no longer reported as _unwritable_ and a write whose outcome the transaction could not describe is no longer reported as one that never happened.

### Removed

- **`prompting_target` (Breaking):** Removed from `LLMSetting`, `InferenceModelSpec`, `InferenceModelSpecBlueprint`, and all shipped backend TOML files, along with the `PromptingTarget` enum and the per-target style map. Delete `prompting_target` from your local `.pipelex/inference/backends/*.toml` files, or the strict boot loader will reject them — or run `pipelex migrate`, which does it for you. Check `portkey.toml` in particular: the key appears in `[defaults]` _and_ on individual model entries, and a surviving per-model one is rejected by name as an unknown, non-header key.
...
- **Build-time crate qualification**: Introduced a standalone `crate_qualification` pass that fully qualifies in-body references to their owner domain at library build time, ensuring the normalizer and the live library strictly agree on reference resolution. This supersedes v0.43.0's _Cross-domain pipe refs in normalized crates_: the two readers still agree, but on owner-domain qualification rather than on a crate-wide search.

### Changed

- **Strict in-body pipe reference resolution (Breaking)**: Bare in-body pipe references (sequence steps, parallel branches, condition outcomes, `batch_over` targets) now resolve _only_ within their own domain. To reference a pipe in another domain, use the fully qualified form (e.g. `sales.generate_tagline`). This enforces `[exports]` visibility rules that were previously bypassable, and lets multiple domains safely reuse the same pipe code without library load failures.
...
- **Documentation**: New [Pipelex Kernel](under-the-hood/pipelex-kernel.md) page, with [Hub layering convention](contribute/hub-layering.md), the plugin pages and the under-the-hood execution pages updated for the kernel rename, the entry-point group split and the batch-branch hook.
...
- **Registry Sort Enforcement:** The keyword-only guard now enforces the sort order of the `subject_grants.toml` registry (`unsorted-grant` violation) to prevent silent breakage during bulk rewrites. See [Keyword-only arguments](contribute/keyword-only-arguments.md).
...
- **Registration Surface Docs:** New [Registration surface](contribute/registration-surface.md) page documents every place a new pipe kind has to be registered.
...
- **`pipelex resolve` and the `pipelex codegen` family:** `pipelex resolve` assembles a bundle closure and emits the **normalized library crate** — a flat, fully-qualified, natives-expanded, self-contained, fingerprinted snapshot of a resolved library — as JSON or TOML. On top of the crate:
  - `pipelex codegen types --target ts-zod|python-pydantic|python-structures` projects the crate's concept set into typed artifacts. The `ts-zod` target emits a pure `types.ts` (imports only `zod`, with wire-native snake_case keys so a schema validates a wire payload directly) plus a `binder.ts` exposing a typed `parse`/`serialize` pair per concept; `python-pydantic` emits self-contained `BaseModel`s; `python-structures` emits runtime `StructuredContent` classes.
...
- **[BREAKING] `validated_pipes` key `pipe_code` → `pipe_ref`**: the entry has always carried the namespaced `pipe_ref` (`domain.code`) under a key named for the wrong identity; the key now says what the value is.
...
- **`PipelexRunner` → `PipelexMTHDSProtocol`** — the runner class now implements the MTHDS Protocol (`mthds.protocol.protocol.MTHDSProtocol`, mthds 0.4.1). Method renames: `execute_pipeline` → `execute`, `start_pipeline` → `start` (still `NotImplementedError` locally). `execute`/`start` carry the protocol's basic args plus a generic `extra` passthrough — server-specific args (a client-supplied run id, callbacks, a stored `method_id`) ride `extra`, never named params. Response classes: `PipelexPipelineExecuteResponse` → `PipelexRunResultExecute` (subclasses `RunResultExecute` — `pipeline_run_id` + `pipe_output`), `PipelexPipelineStartResponse` → `PipelexRunResultStart` (subclasses `RunResultStart` — `pipeline_run_id` only). `state` / `created_at` / `finished_at` / `main_stuff_name` / `workflow_id` are pipelex extension fields on the protocol's base responses; the run identifier keeps the name `pipeline_run_id` everywhere.
...
- **Protocol `validate` / `models` / `version` on `PipelexMTHDSProtocol`** — `validate` wraps `validate_bundle` (blueprints + per-pipe structures into the protocol `ValidationReport`); `models` wraps the builder's `list_models` into a `ModelDeck`; `version` reports `protocol_version` 0.6.0 with the installed pipelex version.
...
- **Distributed tracing** — pluggable event-log backends (NDJSON, DynamoDB, in-memory, buffering) emit `TraceEvent`s that reassemble into a complete `GraphSpec`. Enabled by default; works in single-process mode too.
...
- **`EventLogProtocol` with pluggable backends**: `NdjsonEventLog`, `DynamoDBEventLog`, `InMemoryEventLog`, `BufferingEventLog`. Emits `TraceEvent`s as pipes start/finish across processes. Configured via `[pipelex.tracing_config]` (`backend = "ndjson" | "dynamodb"`). Tracing is **enabled by default** — single-process runs also produce a trace.
...
- **Removed duplicate `ContentGenerationError` class**: the unused `pipelex.cogt.content_generation.exceptions.ContentGenerationError` was deleted (had zero external imports). Its subclasses re-parented: `NeitherUrlNorDataError` → `PipelexError`, `UnsafeSchemaError` → new `SecurityError`.
