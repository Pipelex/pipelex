...
- **The Pipelex Gateway routes on the model id in the request body, and the runtime stops trying to steer it.** Which backend serves a model is now the gateway's decision, read from the model id every request already carries; the client says what it wants, not where to get it. Everything the runtime used to send to route a call is gone: the `x-portkey-provider` fallback header the extras factory invented, the `with_options(config=...)` selection in the extract and search workers, and the per-model `x-portkey-config` the catalog carried. The gateway refuses a client that tries to route anyway, so a stale caller fails closed with a named error rather than reaching a backend it chose for itself.

  Image generation follows the same rule, which is what lets it drop a whole vendored REST path. `GatewayImgGenWorker` used to post a hand-built request to an `endpoint_path` the catalog spelled out per model, splice `/images/generations` into `/images/edits` by string replacement when the job had input images, and assemble the multipart body itself because `portkey_ai`'s async client silently drops `files`. It now calls the SDK's own `images.generate` and `images.edit` with `model` in the body. The route is the method, and the multipart field names come from the SDK: a list under `image` is serialized as `image[]` parts and a single file as the bare `image` field — the distinction that decides whether an edit of several images is an edit of several images or a plausible edit of the first.

- **`img_gen.default-small` is `gpt-image-1-mini` again.** It had been pointed at a Gemini model for one step, while the Azure `gpt-image-*` family was out of the gateway catalog and an all-gateway routing profile could not resolve it. The family is served again, so the alias goes back to what the presets that use it — `gen-image-fast`, `gen-image-testing`, `synthesize-photo` — were written for.
...
- **`[inference.gateway_test.config_id_substitutions]` is gone, and with it `GatewayDeck` and `GatewayDeckError`.** The table mapped one gateway config id onto another so that a model could be pointed at a different gateway configuration by hand. Breaking: there are no gateway config ids any more, so a table keyed by one has nothing to key on. Configuration files carrying the section are repaired by `pipelex migrate` — the main configuration surface moves to schema version 4, whose single operation deletes it. To send a model somewhere else, change what the gateway serves for that model.
