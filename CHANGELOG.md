...
- **Category-aware error boundary on Temporal activities.** Every in-scope Temporal activity is now decorated with `@convert_pipelex_errors` (new module `pipelex/temporal/tprl/activity_error_boundary.py`), which converts a `PipelexError` raised inside the activity into a `TemporalError` at the activity boundary. This packs the structured `ErrorReport` into `ApplicationError.details` and derives `non_retryable` from the error's `InferenceErrorCategory` — so the workflow-side `TemporalError.from_app_error` keeps `error_category`, `user_action`, `model` and `provider` instead of landing in its `error_report is None` fallback, and a non-retryable `CogtError` (`CONFIGURATION`, `CONTENT`, `CAPACITY`) is no longer retried. This is the Temporal-side twin of the non-Temporal `PipeRouter` transient-retry path. `act_assemble_graph` is deliberately left unwired — it is best-effort observability that swallows every failure and degrades to `None`. As part of this, `TemporalError._log_critical` / `_log_error` now select `activity_log` vs `workflow_log` based on `activity.in_activity()`, since `workflow.logger` raises `_NotInWorkflowEventLoopError` outside a workflow event loop.

### Changed

- **Agent CLI `run` / `validate` / `init` default to markdown output (breaking).** These commands previously always emitted JSON; they now accept `--format markdown|json` and default to markdown, matching `models` / `check-model` / `doctor`. Use `--format json` for the structured payload. Errors follow the same option — markdown to stderr by default, JSON with `--format json`. The `inputs` / `concept` / `pipe` / `fmt` / `lint` commands are unaffected (always JSON / raw passthrough).
...
- **Transient-retry now actually fires for LLM pipes.** `PipeRouter`'s application-level retry of `TRANSIENT` inference failures was effectively dead for the most common case — an LLM call. `PipeLLM` and `PipeStructure` catch the worker's `LLMCompletionError` (a `CogtError`) and re-raise it as a plain `PipeRunError` before it reaches the router, and the router's `except PipeRunError` branch did not retry — it wrapped into `PipeRouterError` and gave up. So a transient LLM failure (rate limit, timeout, brief outage) was never retried in-process even though `max_transient_retries` defaults to `3`. The router now unifies its retry decision: a `PipeRunError` whose `__cause__` is a retryable `CogtError` is retried with the same exponential backoff as a raw `CogtError`, and on exhaustion still wraps into `PipeRouterError` so the pipe location context is preserved.
- **Two worker error-classification miscategorizations corrected.** `LinkupNoResultError` (a search or fetch that returned nothing) had no explicit branch in either Linkup worker's `_classify_linkup_error` and fell through to the `TRANSIENT` catch-all — so the `PipeRouter` transient-retry loop would burn its full retry budget and backoff sleeps on a query that cannot succeed by retrying. It is now classified `CONTENT` + `CHANGE_INPUT` in both the search and the extract worker. **Behavior change:** a no-result search is now non-retryable (`CONTENT.is_retryable` is `False`) — the router stops retrying a query that returns nothing on every attempt. Separately, the `FileNotFoundError` branch in the Docling and pypdfium2 extract workers was classified `CONFIGURATION` — inconsistent with its sibling branches (`CONTENT` + `CHANGE_INPUT`) and with the rest of the codebase, where `CONFIGURATION` is reserved for setup problems. A missing input file is a content problem; it is now `CONTENT`. This second change does not alter retry behavior — `CONFIGURATION` and `CONTENT` are both non-retryable.
