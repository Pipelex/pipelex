...
- **Breaking: the built-in vendor adapters left `pipelex.plugins` for the new `pipelex.providers`.** `pipelex/plugins/` was two things under one name — the plugin *mechanism* and the built-in vendor *adapters* — and the name read as the second, which is what kept the one-way dependency between them invisible. The seventeen vendor directories (`anthropic`, `azure_rest`, `bedrock`, `blackboxai`, `docling`, `fal`, `gateway`, `google`, `huggingface`, `linkup`, `mistral`, `openai`, `openrouter`, `portkey`, `pypdfium2`, `secrets`, `storage`) are now `pipelex.providers.<vendor>`, and their aggregate manifest moves with them to `pipelex.providers.builtins`. `pipelex.plugins` keeps the mechanism only: `contract`, `registrar`, `discovery`, `exceptions`, `model_handle`, `sdk_client_registry`, `sdk_client_manager`, `backend_extras_factory` and the seven capability registries. `providers/` beats `backends/` as a name because `pipelex/cogt/model_backends/` already exists and the code's own word for the non-inference adapters is "provider" (`secrets_provider_registry`, `storage_provider_registry`).
