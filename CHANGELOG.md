...
- **Structured `validation_errors` on the error wire**: `ErrorReport` gains a typed `validation_errors: list[ValidationErrorItem] | None` field. `ValidateBundleError.to_error_report()` now flattens its per-error data (blueprint validation, pipe-factory, and pipe/concept validation — the latter including pipe/concept *instantiation* errors) onto it via a single shared builder (`pipelex/pipeline/validation_errors.py`), so a bundle-validation 422 problem document carries machine-mappable per-error diagnostics — `category`, `message`, identity fields, and a `source` (declaring file) for cross-file mapping — instead of only a `detail` string. The same builder feeds the agent CLI's `validation_errors` JSON array, so the CLI and API structured shapes cannot drift. The list is surfaced under STRICT disclosure (`_STRICT_KEPT_FIELDS`) because it describes the caller's own submitted bundle, not server internals. (The non-categorized `dry_run_error_message` / `signature_check_error` channels are intentionally not projected as structured items — they ride the human-readable `detail`.)
...
- **Per-content `source` on the in-memory validate path**: `PipelexInterpreter.make_pipelex_bundle_blueprint(mthds_name=...)` and `validate_bundle(mthds_names=...)` let a caller attach a logical name to each in-memory bundle string, threaded into `blueprint.source`. A nameless `mthds_contents` submission previously produced `source=None`, breaking cross-file diagnostics; a host (e.g. an HTTP API) can now pass per-item names so the structured `validation_errors` carry a real owning file. The on-disk CLI path is unchanged (it already records real file paths).
...
- **Offline Temporal entry-point unit tests**: Coverage for operational surfaces that previously only failed inside a live cluster — the worker CLI, the codec HTTP server (encode/decode, CORS, error→status mapping), and the server connection logic (API-key resolution, TLS, codec selection).
...
- **Protocol `validate` now produces a best-effort `graph_spec`**: when the batch declares a `main_pipe`, the local runtime dry-runs it in-process and ships the resulting `GraphSpec`, via one shared implementation (`best_effort_graph_spec`) the Temporal validate activity also calls; a graph-arm domain failure degrades to `graph_spec=None` with validation still successful. (`dry_run_pipe_in_process` moved to `pipelex/pipe_run/dry_run_in_process.py` to break an import cycle.)
- **[BREAKING] Temporal validate activity carries the full worker-side artifact set**: `DryValidateResult` gains required `pending_signatures` and `pipe_io_contracts` fields computed worker-side, so a version-skewed worker fails loudly at deserialization instead of defaulting to a silently wrong verdict. (Temporal validate has never shipped to prod.)
...
- **JSON-Schema rendering failures on the validate surfaces are now structured errors**: a pydantic schema-generation failure while building `pipe_io_contracts` now raises the new non-retryable `PipeIOContractError` (naming the offending pipe/input/concept) instead of a raw exception that the Temporal boundary would retry forever.
- **Protocol `validate` teardown can no longer mask the real error**: a raising library teardown is suppressed while a body error is propagating, and the validation library id is captured once so the graph arm and the teardown always target the same library (mirrors the Temporal validate activity's guards).
...
- **Temporal worker startup no longer logs a misleading project name**: starting for the current project (no explicit argument) now omits the name instead of logging `'None'`.
