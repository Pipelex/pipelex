...
- **Named fields and a run-scoped context on the log calls**: the seven `log.*` methods take a keyword-only `fields={...}`, carried as attributes of the stdlib `LogRecord` and never rendered into the message, and `with log.context(request_id=..., pipeline_run_id=..., pipe_run_id=...)` stamps the run's identifiers onto every record emitted in scope, nested bindings merging and an absent identifier staying absent rather than reading `None`. `PipeRun.run` binds the request and pipeline identifiers from the job's metadata for a direct-mode run, and every pipe binds its own `pipe_run_id` around its execution. A `dict` or `list` content is carried as the record's `data` attribute beside its console rendering, JSON-ready for a list of models too, and an entry named like an attribute the record already owns, a record factory's included, is carried under a `field_` prefix instead of raising. See [Logging](tools/logging.md).
