...
- **Tracing — `TraceEvent` gains a `writer_id` field** (default `"primary"`) so standalone activity workers can emit into the same backend partition as workflow workers without colliding on `(workflow_id, sequence)`. Emitters stamp `writer_id` at construction time (no copy-on-emit).
- **NDJSON read-side keys updated for multi-writer correctness**: dedup key is now `(workflow_id, writer_id, type, sequence)`; sort key is `(workflow_id, sequence, writer_id)` — sequence stays primary so the runner's `UsageReportEvent` does not sort before the router's `PipeStartEvent`. The file-handle cache key now includes `writer_id`, preventing two writers for the same `(pipeline_run_id, workflow_id)` from sharing a stale handle.
- **DynamoDB sort key format updated to `EVENT#{workflow_id}#{writer_id}#{sequence:010d}`**. Existing rows are unaffected (legacy emissions write `writer_id="primary"`).
- **`ReportingManager._get_registry` replaced with a strict/or-create split**: `_get_registry_strict` (raises on miss, used by `_report_*_job` so runner processes silently skip the registry add when `open_registry` was never called) and `_get_or_create_registry` (creates on miss, used by `inject_tokens_usages`, the console cost-report path, and `generate_report` for never-opened runs). Removes the silent orphan-registry accumulation on activity workers.
- **`ContentGeneratorDry` now invokes `report_inference_job`** with a synthetic `LLMTokensUsage`, so dry-run mode produces real `UsageReportEvent`s observable through the same backend path as live runs.
...
- **Activities on standalone worker processes no longer drop `UsageReportEvent`s silently.** `ReportingManager._emit_usage_event` previously returned silently when `_event_log_contexts` had no entry for the workflow's `lookup_key` — the permanent state on a runner process that never executed `WfPipeRouter.run()`. The runner now falls back to a process-local event log built from `tracing_config` (cached per process via a `threading.Lock`-guarded factory in `pipelex.tracing.activity_event_log`), keyed by a stable per-process `writer_id = f"act_{pid}_{uuid8}"`. A single one-shot warning per process records that the fallback engaged. Specific exceptions (`OSError`, `MissingDependencyError`, `PipelexConfigError`, `botocore.ClientError`) are caught and logged at WARNING — never silently dropped, never re-raised into the inference path.
