...
- **Secrets provider is now a plugin seam:** The secrets backend (`env`) is selected at boot from a `SecretsProviderRegistry` keyed by the new open `secrets_config.method` token (default `"env"`), populated by the always-on built-in `SecretsPlugin`. A third party can now ship a `pipelex-secrets-<backend>` package (Vault, AWS Secrets Manager, …) that advertises itself under the `pipelex.plugins` entry-point group and is selected via `secrets_config.method` — the same mechanism the storage seam uses. Out-of-the-box behavior is unchanged: `env` stays the default. An unknown method fails loudly at boot with `UnknownSecretsMethodError` (listing the registered methods), not at config parse. See the new `docs/under-the-hood/secrets-provider-plugins.md`.
...
- **Absent results are first-class on the wire and every surface (optionals Step E):** A run whose declared `?` output resolves absent is a success everywhere. `main_stuff_name` on `PipelexPipeRunOutput` and the `/execute` response now names the declared output slot even when it resolved absent — consumers branch on the absence record carried in the serialized working memory (`absences`), never on transport. The absence ledger round-trips cross-process (`hydrate_working_memory` reconstructs it, so a recorded absence no longer degrades to a hard miss after transit). The CLI run surfaces render an explicit absence document instead of crashing: `pipelex run` prints the absence and `--save-main-stuff` writes the `"absent": true` artifact (json + md, no viewer), the agent CLI's `run` returns the absence document as its result payload, and OTel/Langfuse output capture serializes the absence for both inputs and outputs.
- **IO contracts carry optionality (`optional` on `pipe_io_contracts`):** `PipeInputContract` and `PipeOutputContract` gain an `optional: bool` — `true` for `?`-declared inputs (may be omitted; the pipe handles absence) and `?`-declared outputs (may resolve absent).
- **Advisory `warnings` on the validation report + the useless-`!` lint:** The validation report gains a general `warnings` array — same item shape as `validation_errors`, but advisory: it never flips `is_valid`. Its first occupant is `optional_force_redundant`: a `!` (force) input whose slot is guaranteed present in every analyzed flow — the assertion can never fire, so the marker is dead weight. The lint aggregates across flows before warning (a `!` that is meaningful in one flow is never flagged because another flow guarantees the slot). Every whole-bundle validate surface carries it: the protocol report, the agent-CLI validate envelopes (rendered as a "Warnings" section in markdown), the human `pipelex validate` (yellow advisory lines — never the exit code), and the builder validate operations.
- **Execution graphs show skips and optional data edges:** `GraphSpec` nodes gain the `skipped` status with a `skip_reason` (a lifted pipe ends its node as skipped — "why did my workflow produce nothing?" is now answerable from the graph), and data edges fed by a declared-optional (`?`) output carry `optional: true`. A skipped pipe with a plural output still registers the real empty list it wrote, so downstream data edges resolve like any produced value. Both flow through the in-process tracer and the distributed event-replay assembler (new `pipe_end_skipped` trace event).
- **Static optionality validation — the absence-taint pass (breaking):** Validation now proves the absence safety theorem at author time: every maybe-absent slot must reach an explicit sink. A taint pass over each controller's dataflow computes per-slot presence (`guaranteed` / `maybe-absent`) and rejects escapes with new typed validation errors: `optional_not_handled` (a maybe-absent slot ends a sequence — or feeds a condition through a mapped `?`-output pipe — without the controller declaring a `?` output; the message names the absence origin, the propagation path, and the three fixes), `optional_output_required` (**breaking**: a `continue`-reachable `PipeCondition` must declare its output optional, e.g. `output = "Constraint?"` — `continue` resolves the output absent, so a plain output would hide the no-output path), and `optional_branch_required_field` (a required structure field of a `PipeParallel` output fed by a maybe-absent branch — make the field non-required or sink the absence upstream). Plural slots are never tainted: a lifted plural output is the empty list and a batched step compacts, so taint stops at `[]`.
- **Template guard-lint for optional inputs (`optional_input_unguarded`):** Every template reference to a declared-optional (`?`) input must be guarded — `@?var`, a `{% if var %}...{% endif %}` block, or an inline presence conditional (`... if var is defined else ...`). An unguarded reference (including a plain `@var` sigil on an optional input, a deep `{{ var.attr }}`, or use in an `{% else %}` arm) fails validation with the precise fix. Applies to `PipeLLM` prompts and system prompts, `PipeCompose` templates, and `PipeCondition` expressions. In the same motion, `@?` finally means what it says: a variable declared optional on a pipe is no longer presence-required by the controller miss-gates — the pipe runs and its guarded templates take the absent arm.
- **Liftable-pipe inventory on the valid report (`liftable_pipes`):** The validation report now lists every pipe that may be skipped (lifted) when an optional slot resolves absent — the liftable pipe, the controller it lifts within, the trigger slots, and the absence source. This is the build-time visibility that makes implicit lifting acceptable: authors see "may be skipped when X is absent" at `/validate` time, not just in the run report.
...
- **Breaking: dropped Python 3.10 support — `requires-python` is now `>=3.11,<3.15`.** The 3.10 compatibility bridges are gone: `StrEnum`, `Self`, and `Traversable` are now imported directly from the stdlib (`enum` / `typing` / `importlib.resources.abc`) at every call site, the `pipelex.types` re-export module that existed only to paper over 3.10 is deleted, and the `backports.strenum` conditional dependency (and its type stub) is dropped. CI test/lint matrices, the `package-check` `requires-python` floor gate, and the contributor docs now start at 3.11. Install on Python 3.11 or newer.
...
- **`PipeCondition` `continue` now resolves the output as absent (breaking):** The `continue` special outcome no longer passes the current main stuff through (and no longer errors when there is none). It records a declared-absent `AbsenceRecord` for the condition's declared output — with the evaluated expression as the reason — and returns success, memory otherwise unchanged. Dry-run parity: an all-special-outcomes condition records the same absence instead of raising when `continue` is reachable; a fail-only condition (every outcome and the default map to `fail`) still raises in dry run, since every live path can only fail. Migration: the previous value stays in the working memory under its own name, so downstream pipes consume it explicitly by that name (declared `?` when it may be absent); a coalescing operator is the planned ergonomic replacement for pass-through.
