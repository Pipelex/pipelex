...
- **Highlight — Structured validation errors**: validation failures now return a typed, per-error `validation_errors[]` on the error wire instead of a bare `detail` string, with the same shape across the HTTP API and the agent CLI.
...
- **Highlight — Keyword-only argument enforcement**: a mechanical guard enforces the keyword-only argument convention across `pipelex/`, end to end — local autofix through the CI gate.
