...
- **The shared projection corpus covers the open-shaped natives the two engine fixes unblocked**: `scaffold_bundle.mthds` gains `anything_in = "Anything"` in `scaffold_open_natives`, so all three natives that declare no pinned structure now sit at slot positions beside `native.JSON`, and a new `scaffold_dynamic_prompt` pipe puts a `native.Dynamic` slot at an LLM prompt position under both markers — a bundle that did not validate at all while the prompt analyzers read a dynamic concept's universal compatibility as identity. The `Anything` slot is captured knowing the runtime refuses it: its template is the empty object every `unknown` node renders, and at an `Anything` position the input shaper accepts a bare string alone and returns it as a `native.Text` stuff, so both shapes of that pipe are declared in `EXPECTED_UNSHAPEABLE` against the item tracking the shaper fix rather than left to fail the round-trip gate. That is the gap being stated, not widened — the descriptor states `Anything` as `unknown` exactly as it states `Dynamic` and `Composite`, so a projection reading only the descriptor has nothing else to render there, and closing it deletes the two entries and regenerates. The capture is re-committed byte-identically in `mthds-js` and `mthds-python`. See [Projection Fixture Corpus](contribute/generate-projection-corpus.md).
