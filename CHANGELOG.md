...
- **One ISO 8601 contract for the `Date` and `Time` natives**: `pipelex.core.stuffs.iso_temporal` now owns what those natives accept — the *extended* forms only (`2026-07-07`, `15:40:00+02:00`), rejecting the basic and week/ordinal spellings that `fromisoformat` would otherwise restate in a calendar the source never used. Both the content models and `StuffContentFactory` parse through it — including the time half of an authored `2026-07-07T15:40:00`, which is split out rather than handed to `datetime.fromisoformat` — so a model-generated value and an authored one are held to the same contract instead of two. It also rejects the end-of-day `24:00` form, which names the *next* day's midnight: Python 3.14 reads it as this day's midnight where 3.11-3.13 raise, so a `Time` (or a `Date`'s time) could silently land a full day early on part of the supported interpreter range.
