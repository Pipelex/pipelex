...
- **S3 links name the bucket's own host (Breaking)**: the S3 storage provider now signs links, and reads and writes objects, on the bucket's regional host `<bucket>.s3.<region>.amazonaws.com` instead of the region's shared endpoint `s3.<region>.amazonaws.com`, so a content security policy can allow one bucket's links without allowing every bucket in the region. A bucket name containing a dot keeps the path-style form on the regional endpoint, for signed and unsigned links alike, where an unsigned link used to name a virtual host whose certificate cannot match it. A self-hosted deployment whose egress rules allow S3 by hostname must allow `*.s3.<region>.amazonaws.com`.
- **Every image and document input carries a `public_url`**: an input given as a `pipelex-storage://` reference now gets a link signed through the storage provider, replacing any link it carried, which may have expired, and an input given as an `http(s)` URL gets that URL as its `public_url` unless it names another. A template writing `{{ image.public_url }}` no longer renders `None` for an input uploaded to storage or passed by URL, and a reference the storage provider refuses as a key is reported as an input error before the run starts.
