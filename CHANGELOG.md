...
- **Breaking: misplaced types moved to the package that actually owns them.** `JobMetadata` / `JobCategory` / `UnitJobId` → `pipelex.system.job_metadata`, `JobMetadataError` → `pipelex.system.exceptions`, `TraceContext` → `pipelex.system.trace_context`, `DataInclusionConfig` → `pipelex.system.data_inclusion_config`, `TextFormat` / `TemplatingStyle` / `TagStyle` → `pipelex.tools.templating`, `TemplateCategory` → `pipelex.tools.jinja2.template_category`, and the neutral resolved-field layer → `pipelex.core.concepts.resolved_fields`. The TOML config shape is unchanged throughout (`[...graph_config.data_inclusion]` still parses); only the Python homes moved. The two pipe renderers regrouped into `pipelex.core.pipes.rendering` (`input_renderer` from `core.pipes.inputs`, `output_renderer` from `core.pipes.output`).
- **Breaking: `core/`'s data model resolves concepts and pipes through injected providers instead of reaching into the hub.** `StuffFactory.make_stuff_from_stuff_content_or_data` / `make_from_blueprint` / `make_from_concept_ref`, `InputShaper.shape` / `resolve_input_kind`, `WorkingMemoryFactory.make_from_pipeline_inputs`, `InputStuffSpecsFactory.make_from_blueprint` / `make_from_string`, and `StuffSpecFactory.make_from_blueprint` all take a required `concept_provider`. Two new read-side contracts own that surface — `ConceptProviderAbstract` (`pipelex.core.concepts.concept_provider_abstract`) and `PipeProviderAbstract` (`pipelex.core.pipes.pipe_provider_abstract`) — which `ConceptLibraryAbstract` / `PipeLibraryAbstract` now extend, keeping library management in the interpreter layer. `pipelex.core`'s data-model packages join the enforced runtime layer as a result; the half that names a `Pipe` stays in the interpreter layer by construction.
