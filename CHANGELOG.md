...
- **Strict input validation (Breaking):** Undeclared input names are now a hard error (previously silently ignored), and top-level `null` values are rejected — absence must be expressed by omitting the key.
