...
- **An Output Field Named `json`, `copy`, `schema` or `construct` No Longer Breaks The Worker-Boundary Round Trip:** when a structured result crosses a worker boundary, the output class is rebuilt from its JSON schema — and the rebuilder cannot give a field a name that shadows a Pydantic attribute, so it renames it and records the real name as an alias. The result was then serialized under the renamed key, and validating it back against your class failed with `Field required` on a field you had supplied. The shared conversion — and the dry-run boundary mock, which dumps for the same wire — now serializes by alias, so the keys match the schema your class published. Field names that needed no rename are unaffected. Our distributed-execution plugin still carries its own copy of the conversion until it adopts the shared helper, so distributed runs pick this fix up with that plugin's next release.
- **Structured Web Search No Longer Loses Your Class's Invariants Either:** `PipeSearch` with a structured (non-text) output was the one path the structured-output fix below left behind — in-process it still handed the search provider a class rebuilt from JSON schema, dropping the custom validators, `json_schema_extra` hints and output-structure description the rebuild cannot express. The caller's live class now travels down to the search leaf and is used as-is, exactly as on the LLM object paths. The rebuild remains for the case it exists for: a distributed worker holding only the serialized assignment, where the class genuinely cannot travel and the raw result dict stays on the wire.
- **Structured Web Search Returned An Envelope No Output Class Could Accept:** both search backends handed the leaf `{data, sources}` instead of the structured payload itself — the direct Linkup backend by asking for sources alongside the payload, the gateway backend because its relay does the same — which then failed validation against the output structure class, making structured search unusable on either backend. The direct backend now asks for the payload alone (a structured result has nowhere to put sources), and the gateway backend unwraps the envelope its relay returns, so both hand the leaf the bare payload. A response shape neither backend can hand on now surfaces as a classified search error — a new `LinkupSearchResponseError` on the direct backend, the existing `GatewaySearchResponseError` on the gateway — instead of an opaque validation failure against your output structure class. On the direct backend the output structure's schema still reaches the provider in full; it just crosses as JSON rather than as a class, so the provider SDK cannot pre-validate the response and make your validators run twice. Neither backend *trusts* its own request to have decided the response shape: both recognise the envelope structurally, because not asking for sources is a request rather than a guarantee, and an output structure whose fields all carry defaults would validate an unexpected envelope *silently* into an all-defaults object — wrong answers with no error. Recognising rather than demanding it also means the two sides never have to deploy together.
- **A Web Search You Were Billed For Now Always Reaches Your Cost Report:** a structured search whose response shape the backend rejected raised after the provider had already answered and after its usage was recorded, and the search worker only reported a job on the success path — so the call was billed and then vanished from the run's report. Completion and reporting now happen on every way out. A failure that never reached the provider recorded no tokens, so it reports as the zero-cost attempt it was.
- **A Malformed Gateway Response Body Is Now A Classified Search Error:** a relay answering with something that is not JSON raised a bare `JSONDecodeError` — neither a Pipelex error nor annotated with the model — slipping past the classification the rest of that path provides. Both gateway search paths now surface it as `GatewaySearchResponseError`. Relatedly, a search that simply found nothing to fill your output structure no longer reports itself as a malformed response advising you to change model: it raises the new `LinkupSearchEmptyResultError` / `GatewaySearchEmptyResultError` and points at the query instead.
...
- **One Home For Converting A Leaf Result Into Your Class:** the conversion that turns a content-generation leaf's result into the caller's own class — with its `isinstance` short-circuit and its typed dry-run fidelity error — now lives in `pipelex.cogt.content_generation.object_revalidation` instead of being copied per backend. The helper takes `dump_mode="json"` for a payload boundary, so a distributed-execution plugin can call it instead of carrying its own copy that can drift.
- **Dry-Run Object Mocks Are Built From Your Real Class:** a consequence of the structured-output fix above, called out separately because it can require action. The dry-run leaf mock — for `PipeLLM` outputs and now for `PipeSearch`'s structured output too — now builds from your output class rather than a schema rebuild of it, so constraints the rebuild used to erase are enforced at mock-build time. A field whose type polyfactory cannot satisfy — an opaque custom type with a `__get_pydantic_core_schema__`, or a bare generic `ListContent` — can now fail `pipelex validate` with `DryRunMockBuildError` where it previously passed. Declare `examples` or `mock_format` on the offending field to fix it. The failure is loud and names the class and the remedy; it is not a silent behavior change. (Breaking)
- **`pipelex validate` Now Rejects A Structured-Search Output Your Provider Could Never Be Given:** every structured leaf describes your output class to the provider as JSON Schema, so a class Pydantic cannot describe — an `arbitrary_types_allowed` field, or an opaque type carrying only `__get_pydantic_core_schema__` — cannot drive one. The mock factory will happily build such a class, so a dry run used to pass and the live run then died inside the worker on a raw Pydantic error with no model attribution and no remedy. It now fails validation as `OutputStructureSchemaError`, naming the class. Replace the offending field's type, or give that type a `__get_pydantic_json_schema__`. (Breaking)
