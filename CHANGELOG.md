...
**Every operator's execution semantics is now importable without the interpreter.** The new public `pipelex.kernel` subpackage extracts what a step *does* — deck resolution, prompt assembly, generation, memory write-back — into plain functions covering every operator, all servable on a kernel-only boot with zero `.mthds` loaded and pinned by a permanent boot-contract test. The `PipelexKernel` facade gives programmatic callers ergonomic entry points with typed result envelopes. The layer below the interpreter takes the kernel's name, and plugins now declare which side they belong to through their entry-point group, so an interpreter-side plugin is never imported into a kernel-only process. Distributed execution sharpens alongside: a `run_batch_branch` router hook lets a backend isolate each batch branch, and the fan-out bound is frozen onto the run to close a replay hazard. Parity fixes make crate normalization, the `python-structures` projection, and input shaping agree with the runtime. Breaking changes are import-path moves, an error-class unification, a new required `build_registrar` parameter, and stricter failure classification — see below.

### Added

- **Pipelex kernel (`pipelex.kernel`)**: New public subpackage holding operator-execution semantics as importable functions — deck resolution, prompt assembly, generation, memory write-back — no longer reachable only through a fully booted interpreter with a loaded library. Every operator is covered (LLM and structuring, extraction, image generation, search, template composition, Python function calls), every call is servable on a kernel-only boot with zero `.mthds` loaded, and a permanent boot-contract test pins it by invoking each one on such a boot and checking that no interpreter module was loaded. Two arms stay with the interpreter because their inputs are language or deployment artifacts rather than execution semantics: `PipeCompose`'s construct mode (driven by an MTHDS blueprint) and `PipeFunc`'s pluggable executor (which selects *where* a function runs). Documented in [Pipelex Kernel](under-the-hood/pipelex-kernel.md), with a drift contract keeping the doc tracking the kernel surface.
- **`PipelexKernel` facade with typed result envelopes**: A facade class managing per-run state (`JobMetadata`, `CogtRunParams`) with ergonomic entry points such as `llm_text` and `llm_object`. Every kernel operation returns a strongly-typed envelope (`LlmTextResult`, `LlmObjectResult`, `ExtractResult`, `ImgGenResult`, `SearchResult`, `ComposeResult`, `FuncResult`) carrying the produced content, the updated memory, and tracer intermediates. `pipelex.kernel.memory_ops` standardizes the memory boundary with `shape_inputs`, `store_result`, and typed extraction helpers (`extract_main_content`, `extract_named_content`, and their `_as_list` variants).
...
- **Exceptions-aggregate layering gate**: A new AST guard bans kernel-layer packages from importing the all-errors aggregate `pipelex.exceptions`, closing the blind spot that let a single convenience import pull the interpreter into the kernel layer. `pipelex.kernel` is declared in the hub-layering guard's kernel-layer packages from its first commit.
...
- **"Runtime layer" is now "kernel layer" (Breaking)**: The layer below the method interpreter — the machinery present at execution time whatever is loaded — takes the name of the package that defines it. `pipelex.providers.builtins` exports `KERNEL_BUILTIN_PLUGINS` and `KERNEL_CORE_UNCONDITIONAL_PLUGIN_NAMES`; the hub-layering guard declares `KERNEL_LAYER_PACKAGES` and answers `is_kernel_layer`; the layer's prose, docs and closure tests follow. "Runtime" was overloaded three ways in one tree — the whole engine, the orchestration venue, and this layer — so `runtime_bridge`, which means the venue, keeps the word, while `runtime_hub` and `runtime_boot` still carry the layer's former name and are renamed when the layer moves into its own package.
- **Plugins declare their layer through their entry-point group (Breaking)**: The single `pipelex.plugins` entry-point group is replaced by two — `pipelex.plugins.kernel` and `pipelex.plugins.interpreter` — and `PLUGIN_API_VERSION` bumps to 4. A kernel-only boot queries the kernel group alone, so an interpreter-side plugin's module is never imported into a process that only runs kernel operations — previously, merely installing such a plugin dragged the whole interpreter into every boot, because discovery loaded it unconditionally. The declaration is enforced: a kernel-group plugin that registers an orchestrator, a bundle validator, a PipeFunc executor, or one of the three interpreter-layer hub slots fails loud at register time. The restriction is one-directional — an interpreter-group plugin may still contribute kernel-tier capabilities such as an HTTP-error mapper. An installed plugin still published under the retired group is a startup error naming it and the group it should move to, never a silent nondiscovery. Declaring the *same* plugin under both groups is likewise a startup error: a boot reading both would otherwise load it twice and run its `register` twice, which mostly surfaces as a duplicate-contribution error naming the same plugin as both parties to the conflict, and — for a plugin contributing only a teardown callback, the one menu method with neither a key nor a tier guard — does not surface at all. `pipelex plugins list` gains a Group column. Scope worth stating: this governs which plugins are *discovered* and what they may *register*, not what a plugin's module imports — a kernel-group plugin that imports the interpreter at module scope still pulls it in, which only an import-closure rule would prevent.
- **`build_registrar` gained a required `entry_point_groups` parameter (Breaking)**: Every caller must now say which entry-point groups to read; there is no default. This affects any host embedding Pipelex that resolves plugin contributions itself — notably one reading `get_http_error_mappers()` to wire transport-fault handling — and it fails at import rather than at first use when the call sits at module scope. Pass `ENTRY_POINT_GROUPS` from `pipelex.interpreter_plugins.builtins` for a full boot, or `KERNEL_ENTRY_POINT_GROUPS` from `pipelex.providers.builtins` for a kernel-only one.
...
- **Keyless-boot forced-DRY rule has one home**: `pipelex.runtime_hub.resolve_run_mode_for_boot` now owns the coercion that a keyless boot (`needs_inference=False`) applies to a requested run mode, and both run-params factories call it. The pipe tier's behavior is unchanged; the kernel tier's `PipelexKernel.make` previously minted a LIVE `CogtRunParams` on an offline boot and now gets the same forced DRY and the same warning as a pipe run.
...
- **One ISO 8601 contract for the `Date` and `Time` natives (Breaking)**: `pipelex.core.stuffs.iso_temporal` now owns what those natives accept — the *extended* forms only (`2026-07-07`, `15:40:00+02:00`), rejecting the basic and week/ordinal spellings that `fromisoformat` would otherwise restate in a calendar the source never used. Both the content models and `StuffContentFactory` parse through it — including the time half of an authored `2026-07-07T15:40:00`, which is split out rather than handed to `datetime.fromisoformat` — so a model-generated value and an authored one are held to the same contract instead of two. It also rejects the end-of-day `24:00` form, which names the *next* day's midnight: Python 3.14 reads it as this day's midnight where 3.11-3.13 raise, so a `Time` (or a `Date`'s time) could silently land a full day early on part of the supported interpreter range. Breaking for authored inputs that used a non-extended spelling: `"15:40:00+0200"`, `"2026-07-07T154000"`, `"24:00:00"`, and surrounding whitespace are now refused where they previously parsed.
