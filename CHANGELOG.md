...
- **Every span and event a run produces is attributed to that run's caller**: `PostHogSpanExporter` resolves the identity per span and `track_event` per event, both through one `TelemetryIdentity` value, instead of stamping the single `distinct_id` bound when the telemetry manager was built. A run's `user_id` becomes the capture's `distinct_id` and its `analytics_groups` ride PostHog's groups facet; the span sites write both onto the span as `pipelex.run.user_id` and `pipelex.run.analytics_groups`, so every OpenTelemetry exporter sees the same fact and Langfuse finally receives `langfuse.user.id`. The configured `user_id` and the Gateway-key hash stay as the fallback for everything that names no caller of its own: an event outside any run, and a run whose `user_id` is one of the local placeholders — `local` for a run on your own machine, `dry-run-no-user` for a dry-run sweep — which are the same strings everywhere and so are declined as identities rather than merging every deployment onto one person. The groups ride a fallback capture too, so a host may group its runs without naming their callers. `mode = "anonymous"` identifies nobody, a run's own caller and the configured `user_id` included. Pipelex's own Gateway stream is one shared project across every deployment, so it receives neither: a run's caller reaches it only as a one-way digest taken inside the Gateway-key hash, which keeps two callers of one deployment apart while making a collision between two deployments impossible, and the host's `analytics_groups` are not forwarded to it at all. Neither the user id nor the groups ever become an event property or a span name. A host that serves many callers from one process therefore sees each caller's generations on their own timeline rather than everything under one identity per deployment.
