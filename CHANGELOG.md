...
- **MTHDS intent hints land in the engine (H2).** A `hints` table — flat string-to-string, one defined key `intent` with the closed vocabulary `prose` / `label` / `rating` / `quantity` — can now be authored at three sites: on a concept, on a structure field, and on a pipe input slot via the expanded form `name = { concept = "...", hints = { ... } }` (the slot table is closed: unknown keys are rejected, and a hint-free table collapses to the plain string form). Hints are strictly non-normative: they travel through the library crate (a concept's effective hints are assembled during normalization along the refinement chain, nearer declaration winning; field and slot hints travel as authored) and surface on the input-form descriptor, where every node's `hints` slot carries its final site-over-concept merge and an applicable `intent` feeds the node's `kind` — while execution, validation verdicts, and pipe contracts never read them, and a method authoring no hints keeps its crate fingerprint byte for byte. Validation gains an advisory lint (`hint_unknown_key`, `hint_unknown_intent`, `hint_inapplicable_intent` on the report's `warnings` array — warn only, never reject, well-formed unknown content preserved). Documented in `docs/building-methods/concepts/intent-hints.md` and `docs/under-the-hood/input-form-descriptor.md`.

- **The validation report carries an input-form descriptor per pipe.** `PipelexValidationReport.input_form` maps every `pipe_ref` in `pipe_io_contracts` to an ordered list of field descriptors — the MTHDS input-form descriptor (workspace spec `docs/specs/mthds-input-form-descriptor.md`) — derived by `build_input_form` from the loaded pipes and the authored blueprints, never from the emitted JSON Schema. A renderer gets the field kind by concept chain, the namespaced `concept_ref` and `refines` list on every concept node, the three-valued `presence`, a stated `gating` fact, fixed `[N]` counts, authored defaults beside `required`, and one-member `choices` lists, with `unknown` as the honest escape hatch. Class-backed concepts are reflected through the new `reflect_structure_class`, constraints included. Breaking: `build_validation_report` now requires `input_form`, so every backend assembles it or fails loudly. The `trace-input-semantics` harness captures the descriptors as `hop5_input_form.json`. Documented in `docs/under-the-hood/input-form-descriptor.md`.

- **Emitted JSON Schemas carry the concept's identity and authored description.** The SCHEMA render (pipe input contracts, `build output --format schema`, the trace harness) now injects the concept ref as the schema's top-level `title` — replacing the mangled generated-class name — and the concept's authored description as its top-level `description`, uniformly for generated, class-backed, and native concepts. A fixed-count input or output (`Concept[N]`) now emits `minItems`/`maxItems` = N on its array wrap, with real multiplicity threaded through the render seam instead of a boolean; a variable-length list (`Concept[]`) stays unbounded. Native content classes (`TextContent`, `ImageContent`, `DocumentContent`, `PageContent`, `NumberContent`, `HtmlContent`, `YesNoContent`) gained docstrings matching their pinned native descriptions, so provider-side schemas built straight from the classes carry them too.
...
- **`pipe_io_contracts` learns presence and real multiplicity (breaking protocol reshape).** Input contracts now carry `presence` (`plain` / `optional` / `force`) — the declared marker verbatim — replacing the two-valued `optional`, so lint and graph surfaces can see where `!` assertions live without a presentation view. Input and output contracts both carry a three-valued `multiplicity` (`single` / `variable` / `fixed`) with `item_count: int` present exactly when `fixed`, retiring the old ruling that deliberately reported a fixed count as `variable` (the input contract previously had no multiplicity field at all). The output contract keeps its two-valued `optional` (`!` is rejected on outputs). Spec-first: the MTHDS Protocol spec's `pipe_io_contracts` row and optionals section moved in the same change, and the reshaped conformance assertions are gated until the release cascade. Documented in `docs/under-the-hood/pipe-io-contracts.md` (the per-pipe contracts a valid report carries) and `docs/building-methods/pipes/understanding-multiplicity.md`.

- **A structure field can no longer declare both `required = true` and a `default_value` (breaking).** The two are contradictory instructions — a default's meaning is "applied when the caller omits the field", which makes absence legal — and the generator used to resolve the conflict silently by dropping the required-ness through parameter ordering. The pair is now rejected at blueprint validation — and at the builder's spec layer, so an authoring agent fails before emitting TOML the parser would reject — with a message naming both remedies, and the generator branch that tiebroke it is an explicit invariant that raises if ever reached. Relatedly, a pydantic default on a hand-written structure class now counts as an authored fact: the input-form descriptor reports such a field as `required: false` with its `default_value`, instead of claiming it required and hiding the default.

- **Unknown keys in a structure-field table are now rejected (breaking).** A typo'd or hopeful key on a concept structure field (`minimum`, `unit`, a misspelled `hint`) used to be silently dropped at parse while the bundle validated green — silently discarded authored intent. `ConceptStructureBlueprint` now forbids unknown keys, making the field table's keys strict exactly like an input slot table's; hint *content* stays lenient (unknown hint keys still warn and are preserved). The MTHDS JSON Schema regenerates accordingly (structure-field objects carry `additionalProperties: false`), and hint-free crates keep byte-identical fingerprints. Documented in `docs/building-methods/concepts/inline-structures.md`, which now states the closed key set and points a constraint at a Python class, where pydantic `Field(...)` metadata is read as an authored fact.
