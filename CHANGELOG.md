...
- **Pipelex kernel (`pipelex.kernel`)**: A new public subpackage holding operator-execution semantics as importable functions, so what a step *does* — deck resolution, templating-style derivation, prompt assembly, generation, memory write-back — is no longer reachable only through a fully booted interpreter with a loaded library. Every operator is covered: LLM and structuring, extraction, image generation, search, template composition, and Python function calls. Every one of those calls is servable on a runtime-only boot with zero `.mthds` loaded, and a permanent test pins it by calling each of them on such a boot and then checking that no interpreter module was loaded. Two arms stay with the interpreter, because their inputs are language or deployment artifacts rather than execution semantics — `PipeCompose`'s construct mode, which is driven by an MTHDS blueprint, and `PipeFunc`'s pluggable executor, which selects *where* a function runs.

### Changed

- **A bare pipe reference resolves inside its own domain (Breaking)**: An in-body pipe reference — a sequence step, a parallel branch, a condition outcome, a `batch_over` target — written without a domain now means the referring pipe's own domain, and is never searched for elsewhere. Previously `PipeLibrary` resolved a bare code by scanning every domain, so a pipe could reach a pipe in a domain nobody named; because the MTHDS visibility rules deliberately exempt bare references from the `[exports]` check (they were never supposed to leave home), that search made `[exports]` unenforceable through the one reference form exempt from checking it. Referencing another domain now requires writing that domain out, and validation names the reference it actually tried, explains the rule, and suggests the qualified spelling when some other domain declares the code. Across every `.mthds` tree in this workspace, two references need that one-word fix.
- **Bare pipe codes you type yourself keep working**: `pipelex run`/`show`/`which`, the builder operations, and the pipe code on an API request go through a named entry affordance (`get_optional_entry_pipe` / `get_required_entry_pipe`, exposed on `interpreter_hub`) that still matches a bare code across domains — you are pointing at a pipe, not writing a reference — and raises naming the candidates when two domains declare it. It deliberately ignores `[exports]`, since package visibility governs what one method may reference from inside another, not what a human may invoke. Installed dependency packages are excluded from that search, so adding a dependency cannot make one of your own codes ambiguous.
- **Two domains may reuse a pipe code again**: The standard blesses this as a no-conflict case, and the crate-wide search had turned it into a load failure the moment anything referenced the code bare. Reusing a code across domains is a working configuration once more.
