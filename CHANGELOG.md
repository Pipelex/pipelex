...
- **`PipeSignature` — contract-only pipes for top-down pipeline design.** A new pipe type (`type = "PipeSignature"`) declares a pipe's `inputs`, `output`, and `description` without an implementation, so an author or agent can sketch a complete pipeline before committing to the operator that will eventually do the work. Signatures slot into `PipeBlueprintUnion` and `PipeSpecUnion`, are produced by the new `PipeSignatureFactory`, and ship with their own runtime (`PipeSignatureRuntime` in `pipelex/pipe_signature/`). At dry-run time a signature mints a mock `Stuff` matching its declared output (multiplicity included); at live-run time it raises `PipeSignatureNotExecutableError`. Multiplicity is supported on inputs (`Doc[]`, `Image[3]`) just like every other pipe. Optional `signature_for` field hints the downstream pipe type the signature stands in for (tooling-only; cannot itself be `PipeSignature`); optional `pipe_dependencies` lists the pipes the signature claims to depend on (metadata for tooling, ignored at runtime). New docs page: [Signature Pipes](https://docs.pipelex.com/latest/building-methods/pipes/signature-pipes/). Cross-link: `wip/signature-based-validation.md`.
- **`--allow-signatures` flag on `pipelex validate`.** Both `pipelex validate pipe` and `pipelex validate bundle` accept `--allow-signatures` (default off) to dry-run pipelines that still contain `PipeSignature` placeholders. Without the flag, strict validation refuses any pipeline whose dependency graph reaches a signature, raising `SignaturesNotAllowedError` and reporting every reachable signature plus the controller chain (`signature_refs` and `dep_paths`) so the user knows exactly which pipes are still stubs. The agent CLI (`pipelex-agent validate`) defaults to lenient — its primary use case is sketching pipelines mid-authoring.
- **Graph walk for signature detection.** `PipeAbstract.collect_signature_refs(pipe_lookup=...)` and `collect_signature_paths(pipe_lookup=...)` walk a pipe's dependency graph (via the new default `pipe_dependencies()` method on `PipeAbstract` and per-controller overrides), aggregating signature pipe_refs and the controller chains that reach them. Used by the strict-mode pre-check inside `dry_run_pipe`/`dry_run_pipes` and by `validate_bundle` to surface a single `SignaturesNotAllowedError` listing every offender across the bundle.

### Changed

- **`dry_run_pipe`, `dry_run_pipes`, `validate_bundle`, and `validate_bundles_from_directory` now accept `allow_signatures: bool = False`.** Strict by default. The same flag is threaded through `pipelex/cli/commands/validate/_validate_core.py` (the CLI entry point) and `pipelex/cli/agent_cli/commands/validate/_validate_core.py` (the agent CLI, which defaults to lenient). `ValidateBundleError` gained a `signature_check_error: SignaturesNotAllowedError | None` field so callers (and the CLI error renderer) can show the signature dep chain when validation fails on a signature.
