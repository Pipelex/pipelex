...
- **Provider HTTP 404s now raise dedicated `LLMModelNotFoundError` / `ImgGenModelNotFoundError`.** A model-or-deployment-not-found 404 from any LLM or image-gen provider now raises the dedicated `*ModelNotFoundError` (a `ModelNotFoundError` subclass, `CONFIGURATION` category) instead of a generic `LLMCompletionError` / `ImgGenGenerationError`. Because these are siblings of the generic errors — not subclasses — a 404 propagates past the operator's `except LLMCompletionError` / `except ImgGenGenerationError` to `except ModelNotFoundError` in `PipeOperator._live_run_pipe`, which re-raises `PipeOperatorModelAvailabilityError` carrying the unavailable `model_handle`. Per-provider LLM SDK-error classification also moved into extracted `classify_*_sdk_error()` free functions under `pipelex/plugins/*/*_error_classification.py`.
...
- **Gateway LLM propagation-race 404s are no longer misclassified as permanent.** A freshly-routed Pipelex Gateway LLM deployment can briefly return a 404 ("specified deployment could not be found") before propagation completes. Gateway LLM models run on the OpenAI workers, so that 404 was classified `CONFIGURATION` / non-retryable like a genuine unknown-model 404 — and never retried. New thin `GatewayCompletionsLLMWorker` / `GatewayResponsesLLMWorker` subclasses now demote that specific 404 to a `TRANSIENT`/retryable error, matching the gateway image-gen worker; the shared OpenAI classifier and plain (non-gateway) OpenAI usage are unchanged.
