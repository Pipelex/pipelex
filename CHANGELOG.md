...
- **Secrets are scrubbed and field values made injection-safe before any sink sees a record**: one processor, configured under `[runtime.log.redaction]` and on by default, replaces a bearer token, an api-key or signature header value, an OAuth `code` parameter, a cookie header's whole value, a JSON secret entry and an `sk_` / `plx_sk_` / `pk_` / `bl_` key with `[REDACTED]`, in the message, in the exception's rendered text and in every string a field carries, and a mapping entry whose key names a secret (`authorization`, `password`, `api_key`, `access_token` and the like) loses its value whatever it holds, a pydantic model or any other object being rendered before the walk exactly as the wire sinks would have rendered it after. Each control character in a field's string values becomes its printable escape so a caller-supplied string cannot forge a line, a field separator or a colour on a terminal; the message and the `data` attribute keep theirs, being the runtime's own rendering. It is installed by `log.install_sink` in front of whatever the sink appended, so every sink is behind it, and it edits the record the root logger shares rather than a copy, so a handler ordered after the sink is behind it too; `extra_patterns` adds the secret shapes only a deployment knows, and a pattern the `re` module refuses is a configuration error named at load. The `otlp` sink emits `exception.type` and `exception.stacktrace` and no `exception.message`, so what it exports is the scrubbed text. A sink processor that raises now goes to the handler's `handleError` instead of raising out of the log call, and the redaction processor fails closed, stripping the record to a `[REDACTION FAILED: …]` notice before it reports, as [Logging](tools/logging.md#redaction) describes.
