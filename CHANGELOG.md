...
- **Exceptions-aggregate layering gate**: A new AST guard bans runtime-layer packages from importing the all-errors aggregate `pipelex.exceptions`, closing the blind spot that let a single convenience import pull the interpreter into the runtime layer. `pipelex.kernel` is declared in the hub-layering guard's runtime-layer packages from its first commit.
