...
- **`LLMWorkerInternalAbstract` folded into `LLMWorkerAbstract` (breaking):** The LLM worker family now matches the other three (img_gen, extract, search): the abstract base holds the `InferenceModelSpec` (`__init__(inference_model, reporting_delegate=None)`) and owns the whole job lifecycle — `llm_job_before_start` (started_at stamp + usage-report init), spec-driven capability checks (vision, documents), constraint application, and spec-based OTel naming. Previously those lived on the intermediate `LLMWorkerInternalAbstract` (a remnant of the pre-plugin-system "external plugin" era), so an out-of-tree worker subclassing the documented `LLMWorkerAbstract` silently inherited a template method with a hole in it: forgetting to call `llm_job_before_start` crashed reporting on a `None` duration. That state is now unrepresentable — a plugin worker implements `_gen_text`/`_gen_object` and nothing else. `LLMWorkerInternalAbstract` is gone; subclasses now extend `LLMWorkerAbstract` directly. The prompt/response debug dumps moved from the worker base to the `llm_generate` funnel (the dump helpers in `llm_utils` are now config-gated internally), keeping the worker ABC free of `pipelex.config` (the hub imports the ABC at module level, and config imports the hub — a config import from the ABC would be an import cycle).
- **Legacy `set_llm_worker_from_external_plugin` removed (breaking):** This pre-plugin-system path on the inference manager registered a spec-less worker class by handle — the old meaning of "external plugin". The real seam (a `pipelex.plugins` entry point registering `(family, sdk)` backends) fully replaces it.
- **Docs — cookbook inference-plugins page rewritten for the plugin system:** `docs/cookbook/using-inference-plugins.md` now documents the real cookbook example (an installable `pipelex.plugins` entry-point package serving the zero-key `hello-1` model) instead of the retired pre-plugin-system "plugin = config entry" walkthrough that referenced a model handle defined nowhere. Also fixed the stale claim that model config lives in `.pipelex/pipelex.toml` (it lives in `.pipelex/inference/`), and corrected the lookup-miss error named in `docs/under-the-hood/inference-backend-plugins.md` (`InferenceBackendNotFoundError`, not `NotImplementedError`).
...
- **Provider error-body reader tolerates unread httpx streaming responses:** `_parse_response_text_body` (shared by the HuggingFace, FAL, Azure REST, and Mistral error classifiers) no longer crashes with `httpx.ResponseNotRead` when an SDK error carries a streaming `httpx.Response` whose body was never buffered (possible with `huggingface_hub` 1.x async streaming errors); it now degrades to a body-less metadata payload, keeping status code, request id, and retry-after intact.
