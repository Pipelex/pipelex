...
- **Validation items carry the positions and declared concepts the parser knew**: a TOML syntax error's item carries the 1-based `line` and `column` the parser stopped at, as new optional item fields, and an `unresolved_concept` item carries `declared_concepts`, the bare codes of the concepts declared in its domain, so an editor can anchor the diagnostic and an author sees what the reference could have named.
