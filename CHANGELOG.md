...
- **A sink object installed again after a reset builds a fresh handler**: the teardown closed the handler and the sink then handed that same closed handler back, so a sink whose `close()` really releases its target — the `otlp` sink shuts its logger provider down — was re-installed dead, accepting every record and exporting none, with nothing raised to say so; the forwarded-record filter stacked up on it besides, one more at every install. A sink whose target is opened in `make_handler` now works across a configure/reset cycle, which is what the tests and an embedding host do.
