...
- **The agent CLI's API runner identifies itself as `pipelex-cli`**: `pipelex-agent --runner api` now builds its `MthdsAPIClient` in one place, `pipelex/cli/cli_api_client.py`, passing `app_info` named `pipelex-cli` with the installed `pipelex` version, so every request it sends carries the `User-Agent` `pipelex-cli/<version> mthds-python/<version> python/<version> (<os>; <arch>)`. The hosted API can then attribute a run to the CLI rather than to a bare SDK call. A program using `pipelex` as a library builds its own client and is never labelled `pipelex-cli`.
...
- **`mthds` is pinned to 0.16.0**: the release that gives `MthdsAPIClient` its `app_info` option and the `User-Agent` it builds from it, which the agent CLI's API runner now uses to name itself.
