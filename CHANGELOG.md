...
- **S3 links name the bucket's own host (Breaking)**: the S3 storage provider now signs links, and reads and writes objects, on the bucket's regional host `<bucket>.s3.<region>.amazonaws.com` instead of the region's shared endpoint `s3.<region>.amazonaws.com`, so a content security policy can allow one bucket's links without allowing every bucket in the region. A bucket name that cannot be a hostname, a dotted one or a legacy one with uppercase letters or an underscore, keeps the path-style form on the regional endpoint, for signed and unsigned links alike, where an unsigned link used to name a virtual host that could not serve it. A self-hosted deployment whose egress rules allow S3 by hostname must allow `*.s3.<region>.amazonaws.com`.
