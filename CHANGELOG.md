...
- **Concept codes in inputs resolve through an entry affordance, and `search_domain_codes` is gone (Breaking)**: A bare concept code a caller supplies in an input envelope (`{"concept": "Memo", ...}`) now prefers the entry pipe's own domain, then falls back to a unique library-wide match; a code two domains declare raises an error naming the candidates instead of silently depending on load order, and installed dependency packages are excluded from the search so adding one cannot make a host code ambiguous. When the entry pipe itself comes from a dependency package, the preference is package-scoped (`alias->domain`) and the rest of that package is searched before the host crate, so a multi-domain dependency's concepts stay reachable from any of its entry pipes; dotted refs obey the same scope-wins precedence, and `alias->Concept` explicitly searches the named package. The `search_domain_codes: list[str]` parameter is removed from the public surface (`PipelexMTHDSProtocol`, `pipeline_run_setup`, `prepare_pipe_job`, `shape_inputs`, `WorkingMemoryFactory.make_from_pipeline_inputs`, `StuffFactory.make_stuff_from_stuff_content_or_data` — replaced by `search_scope: str | None` derived from the entry pipe), and `ConceptProviderAbstract.get_required_concept_from_concept_ref_or_code` is replaced by `get_required_entry_concept`. This also removes latent defects in the old multi-domain walk: it died on the first domain that lacked the code, and the miss escaped as `ConceptLibraryError` — a class the input-shaping handlers do not catch; every entry-lookup refusal — invalid string, miss, ambiguity — is now `ConceptLibraryConceptNotFoundError`.
- **`pipelex fix` rename collisions are scoped per-domain**: The fix loop's cross-file collision guard dropped a `strip-namespace` rename whenever ANY loaded bundle declared the same bare code — but the library keys pipes by `domain.code`, so only a same-domain sibling can actually collide. Two same-named pipes in different domains now rename independently; a same-domain duplicate still bails loudly.
