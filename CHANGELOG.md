...
- **Costs-only LIVE Temporal runs skip the guaranteed-empty trace-flush activity.** Post-fix, the workflow's in-memory trace buffer is populated only by inline (workflow-thread) emissions: graph events when graph tracing is on, and dry-run inline usage reporting. In costs-only LIVE mode neither exists — every LIVE usage emission happens activity-side through the per-process event log — so scheduling `act_flush_trace_events` with an empty payload was a wasted activity round-trip per workflow in the fan-out (history events, task-queue dispatch, worker slot — including every child workflow spawned by controllers). The schedule is now gated payload-purely on `emit_graph_events or DRY run mode`, so the decision replays identically and the command stream stays a pure function of the payload.
