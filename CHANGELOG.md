...
- **Canonical protocol validation report (breaking).** `PipelexValidationReport` is reworked into the canonical typed shape shared by every Pipelex backend and moved to `pipelex/pipeline/validation_report.py`: `bundle_blueprint` (the batch's primary blueprint — first declaring `main_pipe`, else first; replaces the untyped single-or-list `blueprint` dump), `pipe_structures` keyed by namespaced `pipe_ref` with typed `PipeIOContract` entries (per-pipe input JSON Schemas + output concept/multiplicity — new module `pipelex/pipeline/pipe_structures.py`, ported from the hosted API's route-local builder), `validated_pipes` (per-pipe sweep outcomes, the same entries as the agent-CLI envelope), plus the existing `pending_signatures` / `is_runnable` verdict. The report is assembled in exactly one place — `build_validation_report` — that every backend calls, and primary-blueprint selection is one shared rule (`select_primary_blueprint` in `validate_bundle.py`, also folded into the Temporal validate activity).
...
- **JSON-Schema rendering failures on the validate surfaces are now structured errors.** A pydantic schema-generation failure while building `pipe_structures` (an unschematizable structure-class field) surfaced as a raw third-party exception — which the Temporal error boundary would not convert, so Temporal retried a deterministic failure and the caller got an opaque infra error instead of a validation-shaped one. `build_pipe_structures` now converts it to the new `PipeStructuresError` (with the offending pipe/input/concept named), non-retryable at the validate workflow tier like `ValidateBundleError`.
