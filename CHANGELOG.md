...
- **The gateway's request-shape refusals are classified**: the Pipelex inference gateway bounds what a request may weigh and how deeply it may nest, and refuses anything over those bounds itself — before authentication, before a provider sees the request. Those refusals now have a class in `classify_inference_error` instead of falling through the generic status ladder, so a caller who sent something too large reads *the request was too large for the inference gateway* rather than *the provider rejected the request*: a body over its cap and a storage object over its cap are caller-fixable size errors, a body nesting past the depth limit asks for a flatter structure, and a body whose declared size the gateway cannot read is an operator's problem, not the caller's. None is ever retried — an identical retry earns an identical refusal. The refusal is matched on the gateway's own error code and not on the reporting provider, because the same request reaches the gateway through the Portkey substrate, through plain `httpx` on the native extract and search routes, and through the shared Anthropic driver that Claude travels on. `ClassificationResult` gains a `gateway_request_limit` field and `ProviderErrorMetadata` the matching read-only property. Recognizing the code at all first required fixing where it is read on the Portkey substrate: that SDK's exception factory replaces the response payload with the refusal's message string, so `provider_error_code` came back empty for every error it raised — the code is now recovered from the response the SDK had already buffered. See [Error Model](under-the-hood/error-model.md).
