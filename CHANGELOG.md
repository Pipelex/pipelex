...
- **Fixed: a failing plugin teardown callback can no longer replace the exception that killed a boot.** The failed-boot release path runs plugin-registered callbacks — unbounded third-party code — and a raising one propagated in place of the setup error, so the caller saw a teardown failure and never the cause. The releases now run in a `finally` (so a raising callback cannot leave the process un-bootable) and the callback error is logged rather than propagated, which is the one place on that path where a blind catch is warranted: everything else in it is our own code.
