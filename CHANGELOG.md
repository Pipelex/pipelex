...
- **A failing dry run is one located item per failing pipe (Breaking)**: validation reports a dry-run failure as one `dry_run` item per failing pipe, with the error type `DryRunError` and the `pipe_code`, `domain_code` and `source` of the innermost pipe that failed and is not allowed to fail (the `source` only when that pipe belongs to the content under validation, never a file of the host's own library directories), where it used to report one item holding a Python repr of the sweep's records, once more for every controller the failure passed through. The item keeps the failure's own message only when that message is caller-facing and otherwise names the failure's title, so a configuration or storage failure met during a dry run no longer reaches a hosted caller through the verdict; `pipelex validate` lists the items under `Dry Run Errors:`, `pipelex-agent validate pipe` and `validate pipe --all` answer a failing dry run with the invalid-verdict envelope and its items instead of a `DryRunError` envelope with none, and the agent `inputs` commands drop their `dry_run_error` key. In Python, `ValidateBundleError` takes `dry_run_failures` in place of `dry_run_error_message`, and `DryRunError` carries `failures`.
