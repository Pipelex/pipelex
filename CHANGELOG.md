...
- **A Pipelex-managed inference backend is now anything that declares a model-specs section, not one hard-coded name (Breaking)**: The runtime used to know exactly one managed gateway, by name, in a dozen places. Naming a `model_specs_section` in a backend declaration is now what makes a backend managed, more than one can be live at once, and the single published configuration is fetched once and sliced into one per-backend view rather than merged — a handle one service serves and the other does not is a legitimate configuration, not a conflict. The deck-integrity check runs once per live managed service, restricted to the handles the active routing profile actually routes there, because a union across services would accept a handle routed somewhere that cannot serve it while the same whole-deck check run twice would demand every handle appear under both. `pipelex_gateway` keeps reading its historical section without declaring it, since `backends.toml` is the user's own file and an upgrade never rewrites it. A managed backend whose `${…}` variables do not resolve, or whose declared section is absent from the published configuration, is **disabled with a warning naming it**; every bring-your-own-key backend keeps failing the boot loudly, which is the point of scoping the tolerance rather than granting it globally — a typo'd `ANTHROPIC_API_KEY` must not become a silently missing backend. Breaking: `is_gateway_backend` is gone, the backend loader takes `managed_gateway_configs` instead of a single gateway config, and `GatewayUnknownModelError` now names the backend it came from, reusing the attribution slot `CogtError` already carries.
- **Every whole-bundle validate channel now carries the same advisory warnings**: The `warnings` array used to be assembled site by site, and the sites disagreed — the protocol validation report carried the optionality and intent-hint lints, while the agent CLI, the builder operations and the bare CLI each carried the optionality lint alone, so which advisories an author saw depended on which command they typed. One composition point now builds all three families in one fixed order and every whole-bundle channel calls it — the protocol report, the agent CLI, the builder's per-bundle operations, and the bare CLI's `validate bundle` and `validate --all` — which brings the intent-hint lints to the CLI and builder surfaces for the first time. Two surfaces stay out and say so: single-pipe `validate <PIPE_CODE>`, which makes no bundle-wide claim, and the builder's `validate_all`, which carries no `warnings` key at all. Riding along is the bound those lints needed before reaching a channel invoked casually: a site naming many undefined hint keys reports the first few and then how many more there were, and a long authored key or value is elided in the message rather than interpolated whole. Breaking: `LibraryManagerAbstract` gained an abstract `get_accumulated_blueprints`, which the collector reads to find the entry pipes, so an alternative manager injected through `Pipelex.setup(library_manager=...)` implements it or fails loudly at construction — the same rule the report's shared assembly follows, and for the same reason: a silent `[]` would report zero entry pipes and look clean.

### Fixed

- **An empty `[concept.X.structure]` table is no longer described as prose**: The input-form deriver decided a concept was structured by testing its merged structure table for *truthiness*, while the engine's own `ConceptFactory` branches on `structure is not None`. A concept authoring an empty structure table — legal MTHDS, backed at run time by a field-less structured model whose schema is `{"type": "object", "properties": {}}` — therefore fell through to the prose branch, and the descriptor claimed `kind: "prose"` with a `refines` link to `native.Text` that nobody authored, contradicting the schema sitting beside it. The deriver now makes the engine's distinction, so such a slot is an `object` with an empty `fields` list; a concept refining an empty-structure base is decided by the dict one link up the chain. This also makes the vacuous-presence lint's field-less wording reachable from a real bundle rather than only from a hand-built descriptor.
- **The bare CLI's `validate bundle` no longer swallows its advisory warnings behind the signature gate**: The strict pending-signature gate raised `typer.Exit(1)` before the advisory echo ran, so a bundle holding any unimplemented `PipeSignature` placeholder printed none of its warnings — and a half-implemented method is exactly the state an author is in while building. The echo now runs before the gate, the same ordering `validate --all` has always used and states in its own comment.
- **A field-less structure class is described as an empty object, not as `unknown`**: `reflect_structure_class` answers `None` for two different situations — a registered class declaring no field, and a field whose annotation it could not map — and the input-form deriver treated both as opaque, reporting `kind: "unknown"`. Zero fields is a faithful reading of that class rather than a failure to read it, so a class-backed concept whose class declares nothing is now an `object` with an empty `fields` list, exactly like the authored empty structure table it mirrors; a genuinely unmappable annotation still reports `unknown`. The shared reflection helper keeps its `None`, because its other consumer — the native consistency probe — reads that as "structureless by design".
