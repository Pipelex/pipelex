...
- **A run's results carry the I/O artifacts that describe its data:** beside `graphspec.json`, and gated by the same `graphspec_json` inclusion flag, a run now writes `pipe_io_contracts.json`, `input_form.json` and `output_form.json`, the validation report's own artifacts under the standard's names, keyed by namespaced `pipe_ref` for every pipe of the library the run executed against (its own pipes, not a dependency package's aliased entries), which is the validated bundle plus whatever `library_dirs` loaded beside it. A graph viewer shows a data node's value only when it holds the contracts and the output form, and until now only `/validate` produced them, so every consumer of a run's graph either took the no-data floor or paired the graph with a validation of some other bundle text. They are built once at the end of the run, inside its library window, and ride on `PipeOutput.pipe_io_artifacts` beside `graph_spec` (a build failure of any kind lands on `pipe_io_artifacts_error`, never failing the run nor skipping its delivery); the SPI payload carries them as `pipe_io_artifacts_dump`, ready for the synchronous `/execute` response to relay once the API reads the field. `pipelex run`, `pipelex-agent run` (which also names the three paths in its `graph_files`, and settles its graph inclusion before the run so the companions are always built with its graphspec) and the direct-mode delivery executor all write them; a graphspec written into a reused directory removes, with a warning, an earlier run's companions when the new run carries none; a run hosted on our Temporal plugin writes `graphspec.json` without its siblings until the plugin builds them in the crate's window; the fixture corpus is unchanged, since the corpus generator now renders through the same function. See [Execution Graph Tracing](under-the-hood/execution-graph-tracing.md#outputs).
...
- **The Commands section of the agent rules is a compact list, so the generated `CLAUDE.md` costs every session a third of what it did.** `pipelex/kit/agent_rules/commands.md` drops its long-form headings and code blocks for one bullet per gate, each naming the target, its alias, when to run it and where the full rationale lives; `make rules` regenerates `CLAUDE.md` from it. It also now says the one thing a session could not learn from the file itself: that `CLAUDE.md` and `AGENTS.md` are generated, that `make rules` is how they move, and that `make check-rules` is a CI gate `make agent-check` does not run. The repo's Claude Code permission allowlist pre-approves the targets the rules recommend (`atd`, `ttg`, `cko`, `sgr`, the drift and rules targets, the generator aliases), so following the rules does not prompt.
