...
- **A choice containing a comma emitted generated code that does not parse**: an exploded choice list was split on every comma, so an ordinary authored choice like `"blocked, awaiting the owner"` became two unterminated string literals — in the TypeScript target and in both Python targets alike, since each split its own exploded form the same naive way. The split now walks the string literals, in both emitters. The TypeScript half is the one nothing was catching: the broken form is a run of *short* lines, so the print-width guard is blind to it and no always-on test parses the emitted TypeScript.
