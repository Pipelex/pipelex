...
- **Validation items carry the positions and declared concepts the parser knew**: a TOML syntax error's item carries the 1-based `line` and `column` the parser stopped at, as new optional item fields, and an `unresolved_concept` item carries `declared_concepts`, the bare codes of the concepts the validated bundle declares in its domain, so an editor can anchor the diagnostic and an author sees what the reference could have named.
...
- **A failing dry run is one located item per failing pipe (Breaking)**: validation reports a dry-run failure as one `dry_run` item per failing pipe, with the error type `DryRunError` and the `pipe_code`, `domain_code` and `source` of the innermost pipe that failed and is not allowed to fail (when submitted content is validated, the `source` only for that content's own pipes, never a file of the host's own library directories), where it used to report one item holding a Python repr of the sweep's records, once more for every controller the failure passed through. The item keeps the failure's own message only when that message is caller-facing and otherwise names the failure's title, so a configuration or storage failure met during a dry run no longer reaches a hosted caller through the verdict; `pipelex validate` lists the items under `Dry Run Errors:`, `pipelex-agent validate pipe` and `validate pipe --all` answer a failing dry run with the invalid-verdict envelope and its items instead of a `DryRunError` envelope with none, and the agent `inputs` commands drop their `dry_run_error` key. In Python, `ValidateBundleError` takes `dry_run_failures` in place of `dry_run_error_message`, and `DryRunError` carries `failures`.

### Fixed

- **Every validation error reaches the reader**: an error the blueprint categorizer does not know, such as a misspelled field, is its own `blueprint_validation` item without an `error_type`, carrying its `source`, `pipe_code` and `field_path`, where it used to vanish whenever another item existed and surface only once that one was fixed; and `format_pydantic_validation_error` renders every error it is given, listing the kinds without a heading of their own under "Other validation errors" whatever else is present.
