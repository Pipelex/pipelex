...
- **Architecture docs:** Added design documents `wip/graph/graphspec-source-map-enrichment.md` and `wip/graph/validate-explicit-graph-target.md` covering the new graph validation features.
- **Environment config:** Added an autogenerated `.codex/environments/environment.toml` to standardize the `pipelex` environment setup.
