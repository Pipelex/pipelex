...
- **Log lines carry the trace context of the span they were logged in**: the `json` sink writes `trace_id`, `span_id` and `trace_flags` in lowercase hex, the keys OpenTelemetry specifies for trace context in a JSON log that is not OTLP, on every line logged inside a valid span and none of them outside one, reserving the three names so a field spelling one rides under the `field_` prefix, and the `otlp` sink files each record under that span. A pipe's span and an LLM call's span are now OpenTelemetry's current span from their start to their end whenever Pipelex traces a run, under the Pipelex Gateway's telemetry or AI span tracing on your own PostHog, so a line logged during the run names the pipe or the LLM call it came from, a boot line held until the sink arrives keeping the span it was logged in, and anything else reading the current span in that process, a library's own instrumentation or an error tracker, now joins the run's trace rather than the caller's, while the spans' parents still travel in the job metadata, as [Logging](tools/logging.md#the-trace-context) describes.
