...
- **`@@` and `$$` template escapes.** Authors can now opt out of sigil interpolation per occurrence: `@@var` renders as the literal string `@var` (no `{{ ... }}`), and `$$var` renders as the literal `$var`. The escapes are non-overlapping left-to-right: `@@@var` → `@{{ var|tag("var") }}` (one literal `@` plus one interpolated var), `@@@@var` → `@@var` (two literal `@`s). Use these for any case the new heuristic cannot recognize automatically — most commonly `@@font-face` and `@@namespace` inside CSS `<style>` blocks. Cross-link: `wip/template-preprocessor-css-collision.md`.

### Fixed

- **Template preprocessor no longer rewrites CSS at-rules and email addresses as variables.** The `@var` / `@?var` / `$var` regexes in `pipelex/cogt/templating/template_preprocessor.py` were too permissive: CSS at-rules inside `<style>` blocks (`@media (...)`, `@import "..."`, `@import url(...)`, `@keyframes spin {...}`, `@layer reset {...}`, `@container (...)`, `@page {...}`, `@supports (...)`, `@charset "..."`), email addresses (`user@example.com`), and code constructs (jQuery `$("...")`, Bash `$(...)`, shell `${...}`) were all silently rewritten into Jinja2 `{{ var|tag("var") }}` form, breaking HTML rendering and other templates. The preprocessor now uses a single-pass regex with: a `(?<!\w)` lookbehind on the `@` / `@?` arm (blocking emails — the `$` arm intentionally omits the lookbehind so prose like `Q$quarter` still interpolates), a `(?!\s*[({"'])` lookahead that blocks direct openers after the captured identifier, and a `(?!\s*\w+\s*(?:[(]|\{(?!\{)))` lookahead that blocks one-word-then-`(`/`{` shapes (`@keyframes spin {`, `@import url(...)`, `@layer reset {`) while explicitly allowing `{{` so Jinja constructs in the same template aren't misclassified. The single pass ensures the lookahead sees only original-template characters — never `{` braces from a previous substitution pass. Residual cases the heuristic cannot disambiguate require the new explicit `@@` escape: at-rules whose name contains a hyphen (`@font-face`, `@counter-style`, `@color-profile`, `@view-transition`, `@font-feature-values`) — the greedy identifier `[a-zA-Z0-9_.]+` truncates at the hyphen; at-rules whose argument starts with `-`/`--` (`@property --my-color {...}`, `@color-profile --my-profile {...}`) — the inner-word arm can't reach past the dash; at-rules with a multi-word prelude before the brace (`@counter-style my-counter {...}`, `@namespace svg "..."`, `@font-feature-values Font Name {...}`) — the inner-word arm only matches one word. All require `@@` (e.g. `@@property --my-color {...}`). Cross-link: `wip/template-preprocessor-css-collision.md`.
