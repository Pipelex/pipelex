...
- **`PipeOperatorModelChoiceError` is the located unknown-model refusal (Breaking)**: it is now raised whenever a pipe naming an unknown model is built, with a message naming the pipe and the field; it is `input`-domained and caller-facing, carries `domain_code`, `field_name`, `suggestions` and `source`, and its `model_choice` is the reference as written. The agent CLI's hint for it points at `check-model` rather than `doctor`.
- **A run refuses an invalid bundle with the validation verdict (Breaking)**: a run loads its bundle before any pipe runs, and a bundle that load refuses now raises `ValidateBundleError` with the same located `validation_errors` validating it gives, which a hosted run route answers as an HTTP 422 that keeps them under STRICT disclosure, instead of the raw class of whichever check refused it (a 500 for a misspelled concept or a wiring mismatch, a 422 without items for a TOML fault, a `runtime` `PipeExecutionError` for a check inside a pipe's validator, a `PipeOperatorModelChoiceError` for an unknown model); an unknown entry pipe still raises its `PipeNotFoundError`. `pipelex run` prints the grouped panel `pipelex validate` prints, `pipelex-agent run` and any other agent error envelope caused by such a verdict carry its `validation_errors`, and the runner's new `library_dirs_are_callers` option, which both CLIs set, gives a refusal in the caller's own library directories the same verdict while a host's directories still load untranslated.
- **A pipe factory's refusal is a located validation item**: `PipeLLMFactoryError`, `PipeComposeFactoryError`, `PipeExtractFactoryError`, `PipeConditionFactoryError` and `PipeParallelFactoryError` are now `input`-domained and caller-facing, so a `PipeExtract` whose input is neither an image nor a document validates to a `pipe_validation` item on that pipe and its file, whose message names the input and says to redeclare it, where it used to escape every validator as no verdict; a `PipeLLM` whose prompt or inherited system prompt does not parse is refused with the line of that prompt it fails at and no longer quotes any of the prompt, which may be a host's; the builder's `validate_ops.validate_pipe` and `validate_all` also answer a refusal raised while they load libraries or dry-run a pipe with the `ValidateBundleError` verdict instead of the raw error. `pipelex-agent validate pipe --all` now keeps the file on an item raised while its libraries load, where it used to lose it because the refusal was translated only after the failed library was torn down.
...
- **A validation verdict names no path on the host**: an item located inside a package the method depends on by address, and a duplicate declaration inside that package, now name the bundle by the package's address and its path inside the package (`github.com/acme/harbour-methods/tides/notices/tide_notices.mthds`) instead of the directory the host installed it in, on every surface; and when submitted content is validated or run beside a host's own library directories, an item located in one of their files carries no `source` and a message that named such a file, or the module one of their Python files was imported as, reads `<host library file>`, so STRICT disclosure no longer hands a hosted caller a path on the server. A bundle validated from a file keeps the full paths of its own library directories, and so does content validated against directories that are the caller's: `validate_bundle` and `validate_bundles_in_process` take `library_dirs_are_callers`, which `PipelexMTHDSProtocol.validate` now passes as its `execute` does.

## [v0.66.0] - 2026-09-25

### Highlights

**Logs are structured, scrubbed and sent where you choose.** Every log call can carry named fields and the run's identifiers, one configuration key selects the sink (the Rich console, JSON lines for a log agent, OpenTelemetry, Google Cloud Logging or a plugin's own), secrets are redacted before any sink sees a record, and a line logged inside a span carries its trace context. Rich moves behind the `cli` extra, so the runtime no longer needs it on the way to running a method.

### Added

- **Named fields and a run-scoped context on the log calls**: the seven `log.*` methods take a keyword-only `fields={...}`, carried as attributes of the stdlib `LogRecord` and never rendered into the message, and `with log.context(request_id=..., pipeline_run_id=..., pipe_run_id=...)` stamps the run's identifiers onto every record emitted in scope, nested bindings merging and an absent identifier staying absent rather than reading `None`. A direct-mode run binds its request and pipeline identifiers from the job's metadata, and every pipe binds its own `pipe_run_id` around the whole of its run. A `dict` or `list` content is also carried as the record's `data` attribute, a JSON-ready snapshot of the call, and a field named like an attribute the record already owns, like a name Pipelex sets on it, `data` included, or like a key a sink writes itself rides under a `field_` prefix instead of raising or overwriting it, as [Logging](tools/logging.md) describes.
- **Log sinks are a plugin capability selected by `[runtime.log] sink` (Breaking)**: where the records go is a registered sink token: `console` (the Rich handler, the default), `json` (one JSON object per line carrying `time`, `severity`, `logger`, `message`, the fields, the context identifiers and the exception, with no ANSI ever, for a server behind a log agent), `otlp` (the OpenTelemetry logs signal, configured under `[runtime.log.otlp]`), or a token an installed plugin registers through `add_log_sink`. A token nobody provides stops the boot naming the registered ones, and `pipelex doctor` reports it in a row of its own. `log_mode`, `poor_loggers`, `generic_poor_logger` and `is_console_logging_enabled` are gone with the rich-or-poor mode, and `set_log_mode`, `set_poor_log_formatter` and `LevelAndEmojiLogFormatter` with them, so a server that wants plain output selects `json` and a host that wants Pipelex silent sets `pipelex = "OFF"` under `[runtime.log.package_log_levels]`. **Migration:** run `pipelex migrate`; ledger entry `pipelex-config@5` deletes the four retired keys from `~/.pipelex/` and every project `.pipelex/` file, after which the defaults supply `sink = "console"`. A file that had `log_mode = "poor"` or `is_console_logging_enabled = false` chose a behaviour that no migration may write back, so it then sets `sink = "json"` or `pipelex = "OFF"` under `[runtime.log.package_log_levels]` by hand, as the entry's title says. [Log Sink Plugins](under-the-hood/log-sink-plugins.md) documents the seam.
- **A `gcp` log sink writing to Google Cloud Logging**: `sink = "gcp"` sends each record to Cloud Logging through the `google-cloud-logging` client library, which the new `gcp-logging` extra installs, and selecting it without the extra stops the boot naming the extra. Each record becomes one structured entry, with the level mapped onto Cloud Logging's severities, the fields in its JSON payload, the run's identifiers as its labels and the run's trace id in its `trace` field, so a line is filed with the spans of its run; the entries leave through the library's background transport, and `[runtime.log.gcp]` carries the log name, the project and an optional service-account credentials path. **Most processes on Google Cloud want `json` instead**: Cloud Run, GKE and any platform whose logging agent reads the container's stdout ingest what that sink writes with no client at all, as [Logging Configuration](configuration/config-practical/logging-config.md) spells out.
- **Log lines carry the trace context of the span they were logged in**: inside a valid span the `json` sink writes `trace_id`, `span_id` and `trace_flags` in lowercase hex, the keys OpenTelemetry specifies for trace context in a JSON log that is not OTLP, and the `otlp` sink files each record under that span. The span is the Pipelex span active at the log call, a pipe's or an LLM call's, whenever the runtime traces a run, and otherwise OpenTelemetry's current span, only read, so a line your code logs under its own span joins your trace. Pipelex never makes its own spans current in the process's OpenTelemetry context, so a host's instrumentation is never re-parented under them, as [Logging](tools/logging.md#the-trace-context) describes.
- **Secrets are scrubbed and field values made injection-safe before any sink sees a record**: one processor, on by default and configured under `[runtime.log.redaction]`, replaces with `[REDACTED]` the credential of an `Authorization` header, an api-key or signature header, an OAuth `code` parameter, every cookie crumb, a `name=value` secret entry and an `sk_` / `plx_sk_` / `pk_` / `bl_` key, in the message, in the rendered exception and in every string a field carries, and the whole value of any field or mapping entry, at any depth and in a structured content too, whose name is a secret's (`authorization`, `password`, `api_key`, `access_token` and the like). Control characters in a field's string values become printable escapes, so a caller-supplied string cannot forge a line or a terminal colour, and `extra_patterns` adds the secret shapes only a deployment knows, a pattern the `re` module refuses or one matching the empty string being a configuration error at load. The processor runs ahead of every sink, and of a handler the host adds after it, and fails closed: a record it cannot redact is reduced to a `[REDACTION FAILED: …]` notice or dropped, and a sink processor that raises is reported on stderr by its type and name, never by its text, as [Logging](tools/logging.md#redaction) describes.

### Changed

- **Rich is the `cli` extra (Breaking)**: `rich` is no longer a core dependency, so install `pipelex[cli]` for the `pipelex` and `pipelex-agent` commands, the `console` log sink and the `rich` pretty-print mode (`uv tool install "pipelex[cli]"`). The runtime imports Rich only inside the code that renders, so a method runs with `sink = "json"` and `pretty_print_mode` set to `"poor"` or `"silent"` without it, while a boot selecting the `console` sink or the `rich` mode without Rich stops naming the extra, and `get_console()`, `rendered_pretty()`, the model and pipe listings and the directory diff raise `MissingDependencyError` naming it too. `PrettyPrintable`, `TextType` and `StyleType` in `pipelex/tools/misc/pretty.py` now exist only for type checking, so importing them at runtime raises `ImportError`. A plain `pip install pipelex` still installs Rich and `import pipelex` still loads it, since the core dependencies `typer` and `instructor` both require it, and a `poor`-mode panel title no longer substitutes emoji codes; [Rich Imports](contribute/rich-imports.md) documents the convention and its `make check-rich-imports` guard.
- **Loggers are named by module (Breaking)**: a record from `pipelex/pipe_operators/pipe_llm.py` is emitted on the `pipelex.pipe_operators.pipe_llm` logger instead of `pipelex`, so a handler keyed on the bare package name must key on the prefix, and a filter an integration attached to the `pipelex` logger must move to its handler, since Python never consults an ancestor logger's filters for a record propagating up from a module logger. A `package_log_levels` key now works at module depth too, `pipelex-pipe_operators-pipe_llm = "DEBUG"` for instance, and the caller-info templates that name the module now render instead of raising.
- **`include_exception` carries the traceback as the record's `exc_info`**: instead of splicing `traceback.format_exc()` into the message, so the console renders one Rich traceback, the `json` sink an `exception` field and the `otlp` sink the `exception.type` and `exception.stacktrace` attributes, and a call made outside an `except` block carries nothing where a `NoneType: None` line used to be appended.
- **Teardown removes only the handlers Pipelex installed**: `log.reset()` no longer strips every handler off the root logger nor calls `logging.shutdown()`, so a host's or a test's handler survives a Pipelex teardown; the sink's handler is flushed and closed, which is where a batching sink ships what it still holds. A sink installed again after the reset builds a fresh handler, except an `OtlpLogSink` object, whose close shuts its processor down, so installing the same one a second time raises; the registered factory builds a new one at every boot.
...
- **A log call before `log.configure` no longer raises**: it goes to the stdlib's default handling at the stdlib's default level, so a library that logs before Pipelex boots, or a boot that logs while it configures, cannot crash on the log line itself. Between `log.configure` and the sink's arrival, the boot's records are held and replayed through the selected sink in order, and a boot that dies before its sink arrives sends all of them to stderr.
- **A log call never raises on a content JSON refuses or on a bracket Rich reads as markup**: a content `json` refuses, a circular reference or a mapping with a non-string key, is rendered as its `repr` with no `data` instead of raising `ValueError` or `TypeError` out of `log.info`. On the `console` sink, a message carrying a tag-shaped span, `list[int]` in a type complaint or a bracketed path, no longer raises `MarkupError` out of the log call, and an error assembled from an exception is rendered verbatim rather than read as markup.
