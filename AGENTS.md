...
One scoped exception to "every function return must be typed": a pydantic `@model_serializer` / `@field_serializer` return must stay **unannotated** unless the annotation is the exact type the model publishes. Pydantic turns a serializer's return annotation into the model's serialization JSON Schema and prefers it over the schema it would generate, so a `-> dict[str, Any]` publishes the whole model as an opaque `{"type": "object", "additionalProperties": true}` — no properties, no `required`, no closed shape — for every consumer that generates response schemas, which FastAPI always does for its responses. A precise annotation is fine and desirable (`-> str` on a model that serializes to its raw string); a `dict`/`Any` one erases the model, and nothing in a normal test suite notices because validation-mode schemas and `model_dump()` both stay correct.
...
Don't reach for `TypeAdapter` on a shape you designed — needing it there means the top-level shape was chosen wrong: wrap it in a named `BaseModel`, which brings `model_validate` and the rest for free. Its one legitimate use is validating into a top-level type that is not a model class and not yours to wrap — a discriminated union or container owned by an external contract (a standard's wire model, a log stream of one-of-many event types); build that adapter once at module level and reuse it, and never use a `TypeAdapter` as a runtime substitute for `cast` on a value that is already typed. And don't convert an existing serialized `BaseModel` to a dataclass just because dataclasses are serialization-safe too: a model that relies on discriminated unions or validators must stay a `BaseModel`. When a serialized dataclass/model field references a type from an optional dependency, alias it to `Any` at runtime so it validates as `Any` without importing the extra (e.g. `if TYPE_CHECKING: from temporalio.common import RetryPolicy` / `else: RetryPolicy = Any`) — a bare forward ref instead raises `PydanticUserError` on construction.
